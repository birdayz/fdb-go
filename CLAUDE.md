...
- **Integer overflow — Java throws, Go silently wraps (silent data drift).** fdb-relational 4.11.1.0 inherits Java `long`'s overflow semantics indirectly: arithmetic that exceeds int64 range raises `ArithmeticException: long overflow`. Go's embedded engine uses Go's int64 which wraps two's-complement on overflow without raising. `SELECT v + 1 FROM t WHERE v = 9223372036854775807` (MAX_INT64) → Java errors, Go returns `-9223372036854775808`. Surfaced nightshift-61 by `integer_overflow_add` cross-engine probe; scenario was dropped, gotcha documented. Aligning Go would require swapping plain `+` / `-` / `*` for overflow-checking variants (or `math/bits` helpers) at every arithmetic site in `pkg/relational/core/functions/arith.go` AND propagating the resulting error class. Tracked as a deferred TODO. Both engines agree on stored-value round-trips and MIN/MAX/COUNT over boundary values without intermediate arithmetic; only the arithmetic operators diverge.
