...
pkg/recordlayer/query/plan/plans/   # RecordQueryPlan (physical) hierarchy
    ...
    rule_*.go                       # 31 logical-rewrite rules (FilterMerge,
                                    ...
                                    #   PushFilterThroughTypeFilter,
                                    #   PushFilterThroughSort,
                                    #   PushFilterThroughUnion,
                                    #   PushFilterThroughIntersection,
                                    #   PushFilterThroughProjection,
                                    #   PushTypeFilterBelowFilter,
                                    #   PullFilterAboveDistinct,
                                    #   PullFilterAboveSort,
                                    #   PullFilterAboveProjection,
                                    #   PullCommonFilterAboveUnion,
                                    #   PullCommonFilterAboveIntersection,
                                    #   DistinctMerge,
                                    #   DistinctOverSortElim,
                                    #   DistinctOverUnionDedup,
                                    #   TypeFilterMerge, TypeFilterRedundant
                                    #   OverScan, UnionMerge,
                                    #   IntersectionMerge, NoOpFilter,
                                    #   ProjectionMerge, ProjectionElim,
                                    #   SortMerge, SortDedupKeys,
                                    #   SortConstantKeysElim,
                                    #   UnsortedSortElim,
                                    #   UnionSingletonElim,
...
pkg/relational/core/query/plangen/  # LogicalOperator → RelationalExpression
                                    ...
                                    #   Project / Sort / Insert / Update /
                                    #   Delete. Project / Sort / Update each
                                    #   accept simple scalar forms via
                                    #   lowerSimpleScalarText: bare column,
                                    #   int / float / bool / NULL / single-
                                    #   quoted string. Returns ErrUnsupported
                                    #   for arithmetic / function calls /
                                    #   qualified refs / exponent-form
                                    #   numerics / apostrophe-escape strings,
                                    #   and for LogicalLimit / Aggregate /
                                    #   Join / CTE / DDL — all gated on
                                    #   text→Value parsing or specific
                                    #   Expression ports.
...
| `FuzzCompareValues` | core SQL three-valued compare — boundedness / antisymmetry / reflexivity / transitivity across NULL × int64 × float64 × string × bool × []byte type triples | Clean (134M execs/90s w/ transitivity, nightshift-57) |
| `FuzzCastValue` | SQL CAST(v AS T) across 14 target types × 5 source types (nil / int64 / float64 / string / bool) — no-panic + NULL pass-through + value-OR-error invariants | Clean (60M execs/60s, nightshift-57) |
| `FuzzSemanticEquals_Properties` | Cascades B1 RelationalExpression SemanticEquals — reflexivity, symmetry, hash consistency on TRUE-equality across random tree pairs (Scan / Filter / Distinct / Union shapes) | Clean (13.3M execs/10s, dayshift-58) |
| `FuzzAliasMap_BijectionInvariant` | Cascades B1 AliasMap — bijection invariant after arbitrary Compose chains: forward[s]=t ⇒ reverse[t]=s on every binding | Clean (9.2M execs/10s, dayshift-58) |
| `FuzzGetCorrelatedToOfPredicate` | Cascades B1 predicate-side correlation walker — nil-safety / empty-on-constant-leaves / soundness (no fabricated aliases) across random Constant/ValuePredicate/And/Or/Not trees | Clean (15.4M execs/10s, dayshift-58) |
| `FuzzFixpointApply` | Cascades B6 rule-engine driver — termination, idempotence-at-convergence, initial-member preservation across random expression trees + random rule subsets | Clean (47.1M execs/180s with 31-rule default set incl. 6-rule PushFilterThroughX family + 4-rule PullFilterAboveX family + 2-rule PullCommonFilterAbove(Union/Intersection) + PushTypeFilterBelowFilter + DistinctOverUnionDedup + DistinctOverSortElim + SortDedupKeys + SortConstantKeysElim + FilterDedupPredicates; sub-Reference descent in FixpointApply; Union + Intersection shapes in seed corpus; SemanticEquals fallback in Reference.Insert unblocks rules that wrap fresh inner References, dayshift-58) |
| `FuzzWalk_Termination` | Cascades B1 expressions.Walk + Size — terminates within 1024 visits on random tree shapes; Walk visit count == Size return value | Clean (17.5M execs/10s, dayshift-58) |
...
10. **Emergent behaviour over special-case checks** — when porting Java semantics, prefer to match the underlying architectural property that produces the behaviour, not a downstream observable. Java rejects `ORDER BY` non-natural-col via the *absence* of `ImplementSortRule` in Cascades — when no rule generates a satisfying physical plan, `CascadesPlanner.resultOrFail` throws `UnableToPlanException`. The Go fix (nightshift-60) removed the `sort.SliceStable` fallback and gated each scan-strategy branch on natural-order satisfiability; rejection emerges from "no scan satisfies", structurally mirroring "no rule fires". A bolted-on `if !pkOrderingSatisfies { throw }` would be brittle (diverges the moment Java's rules change); the structural fix is robust because both engines reject for the same architectural reason.

## Java↔Go conformance gotchas (fdb-relational)

These are the integration constraints that bit us hard in swingshift-52. Add to this list when you find more. Items are grouped by theme; new entries should slot into the matching subsection.

### Setup / connection

JDBC URL plumbing, schema label propagation, harness-call lifecycle, and FDB API-version timing.
...
CREATE TABLE syntax, identifier handling, JDBC type-name reporting, reserved keywords, literal forms, and the INFORMATION_SCHEMA gap.
...
- **`plan` is reserved.** `CREATE TABLE t (... plan STRING ...)` raises `RelationalException: syntax error` at the `plan` token — `plan` is a reserved keyword in fdb-relational's lexer. Joins the existing list of reserved column names (`blob`, etc.). Use `tier`, `plan_type`, `plan_name`, etc. instead. Surfaced swingshift-56 by lifting `secondary_index_pushdown.yaml` (which used `plan` as a column name in its testdata — that yamsql file is Go-only, never runs against Java upstream).
...
- **Floating-point exponent literal requires uppercase `E` AND a decimal point.** Per `RelationalLexer.g4#EXPONENT_NUM_PART`, scientific notation is `1.5E10` — `1.5e10` (lowercase e) and `1E200` (no decimal point) are both rejected. fdb-relational's lexer treats the integer-prefixed form as `<integer-literal>` followed by `E…` which then triggers `NumberFormatException` when Java's `Long.parseLong("1E200")` runs. Use `1.0E200`. The Go embedded engine accepts all four forms; cross-engine corpus entries must use the strict `<digits>.<digits>E<exp>` form. Surfaced swingshift-56.
...
### Parser bugs (visitor NPEs / syntax errors)

Forms the grammar accepts but the visitor / lexer mishandles, or shapes that aren't in the grammar at all. Cross-engine corpus must skip these entirely until upstream fixes them.
...
- **Explicit `CROSS JOIN` syntax — RESOLVED nightshift-61.** fdb-relational 4.11.1.0 NPEs on `SELECT ... FROM a CROSS JOIN b` (`NullPointerException: Cannot invoke ... InnerJoinContext.expression()` — the parser visitor unconditionally calls `accept(...)` on the ON-clause expression, null for CROSS JOIN). Go's embedded engine previously accepted CROSS JOIN at the InnerJoinContext arm with a nil ON expression. Aligned to Java by rejecting CROSS JOIN at parse time in `pkg/relational/core/embedded/select_parser.go#extractJoinClause` — when `j.CROSS() != nil`, returns `ErrCodeUnsupportedOperation`. Same architectural reason in both engines: visitor doesn't handle CROSS JOIN. Workaround: comma-join `FROM a, b`. Per project conformance principle: doesn't work in Java → doesn't work in Go.
- **Scalar subqueries `(SELECT ...)` are not supported by fdb-relational's parser.** Forms like `SELECT (SELECT MAX(v) FROM t) FROM t WHERE id = 1`, `WHERE v = (SELECT MAX(v) FROM t)`, `SELECT v - (SELECT MIN(v) FROM t) FROM t` all raise `RelationalException: syntax error` with the caret pointing at the inner `SELECT`. The 4.11.1.0 grammar's `expressionAtom` rule doesn't include the subquery alternative. Cross-engine corpus must skip `scalar_subquery.yaml` entirely. Go's embedded engine implements scalar subqueries; this is a one-sided gap. Surfaced swingshift-56.
- **`col IN (SELECT ...)` parser-NPEs in fdb-relational 4.11.1.0.** Both `WHERE v IN (SELECT v FROM b)` and `WHERE v NOT IN (SELECT v FROM b)` raise `NullPointerException: Cannot invoke "RelationalParser$ExpressionsContext.expression()" because "expressionsContext" is null` from the IN-predicate visitor — the grammar accepts the form (separate from the `expressionAtom` scalar-subquery hole) but the visitor unconditionally walks `ExpressionsContext` which is null when the IN list came from a subquery. Joins the existing parser-NPE bug list (`CROSS JOIN`, scalar subqueries' expressionAtom, `COUNT(DISTINCT)`). Go's embedded engine implements IN-subquery correctly; cross-engine corpus must skip `subquery_in.yaml` entirely. Surfaced nightshift-57.
- **`COUNT(DISTINCT col)` (and SUM/AVG/MIN/MAX with DISTINCT) — RESOLVED nightshift-61.** fdb-relational 4.11.1.0's parser visitor NPEs on every aggregate with DISTINCT (`AggregateWindowedFunctionContext.ALL().getText()` is called unconditionally; ALL is null when DISTINCT is present). Go previously implemented all DISTINCT aggregate variants. Aligned to Java by rejecting at execution time in `aggregate.go` and `select_query_full.go`: when `ac.aggDistinct` is true on any aggCol, return `ErrCodeUnsupportedOperation` ("DISTINCT aggregate %s is not supported"). Same architectural reason in both engines: visitor doesn't handle the DISTINCT case. Workaround: pre-aggregate via a derived table or rewrite. Per project conformance principle: doesn't work in Java → doesn't work in Go.
- **`WHERE (boolean_expr)` with bare parentheses is rejected.** `WHERE (b = true)` — even when `b` is BOOLEAN — fails with `RelationalException: expected BooleanValue but got RecordConstructorValue`. The parser treats `(...)` as a record/tuple constructor unless it appears inside a context that forces predicate parsing (e.g. `NOT (b = true)` works because `NOT` requires a boolean operand). Drop redundant parens or wrap with `NOT NOT (...)` if you genuinely want them.
- **Simple-CASE form (`CASE expr WHEN val THEN ...`) is silently broken in fdb-relational 4.11.1.0.** The grammar (`RelationalParser.g4#caseExpressionFunctionCall`) accepts the form, but `BaseVisitor.visitCaseExpressionFunctionCall` is `return visitChildren(ctx);` — it never implements simple-CASE evaluation. Searched-CASE (`CASE WHEN cond THEN ...`) works correctly because `visitCaseFunctionCall` properly delegates to `expressionVisitor.visitCaseFunctionCall`. Result: simple-CASE silently returns the WRONG value (typically the ELSE branch, or junk from the un-aggregated child traversal) without raising any error. **This is silent data drift — a query that compiles, runs, and returns wrong results.** Always rewrite simple-CASE as searched-CASE: `CASE v WHEN 1 THEN 'a' WHEN 2 THEN 'b' ELSE 'c' END` → `CASE WHEN v = 1 THEN 'a' WHEN v = 2 THEN 'b' ELSE 'c' END`. Surfaced nightshift-61.

### Planner-blocked features

Forms the parser accepts but the Cascades planner / type-checker / function registry refuses. Most are tracked by upstream RFC-022 (Batch B) or are missing function-registry entries.
...
- **`MIN(s)` / `MAX(s)` over non-numeric columns — RESOLVED nightshift-61.** `MIN(name)` over a STRING column raises `VerifyException: unable to encapsulate aggregate operation due to type mismatch(es)` in fdb-relational 4.11.1.0; the function registry only installs numeric MIN / MAX overloads. Go's embedded engine previously accepted any-comparable input via `functions.CompareValues`. Aligned to Java by adding `requireMinMaxNumeric` runtime gate at all 4 MIN/MAX accumulation sites (`pkg/relational/core/embedded/aggregate.go` + `select_query_full.go`, regular and DISTINCT paths). Non-numeric input now returns `ErrCodeUnsupportedOperation`. Lexicographic min/max over strings or bytes needs a `SELECT name FROM t ORDER BY name LIMIT 1` rewrite (which itself isn't supported either — LIMIT N is JDBC-only). Per project conformance principle: doesn't work in Java → doesn't work in Go.
...
- **Bit-shift operators `<<` / `>>` — RESOLVED nightshift-61.** fdb-relational 4.11.1.0's lexer/parser accept `<<` and `>>`, but the function registry has no evaluator — the planner returns `RelationalException: Unsupported operator <<`. The Go embedded engine previously evaluated shifts (10x speedup over Java would be irrelevant if it diverges silently); aligned to Java by removing the `<<` / `>>` cases from `pkg/relational/core/functions/arith.go` `ApplyBitOp`. They now fall through to the default `ErrCodeUnsupportedOperation` arm. Same architectural reason: function registry has no evaluator. Bitwise AND / OR / XOR (`& | ^`) still work in both engines. Per project conformance principle: doesn't work in Java → doesn't work in Go.
- **`WITH RECURSIVE name AS (non-recursive-body)` is rejected by fdb-relational** with `RelationalException: condition is not met!`. SQL spec + Postgres permit a non-self-referencing body under `WITH RECURSIVE` (RECURSIVE is a scope enabler, not a requirement); fdb-relational requires an actual UNION ALL self-reference. Use plain `WITH name AS (...)` for non-recursive bodies. Surfaced swingshift-56.
- **`IS TRUE` / `IS FALSE` — RETRACTED nightshift-61.** Earlier shifts believed fdb-relational rejected these; the cross-engine harness's `is_true_rejected` corpus entry surfaced that Java actually ACCEPTS `WHERE flag IS TRUE` and `flag IS FALSE` (and the `IS NOT *` variants), returning the expected three-valued-logic-collapsed results. Both engines accept the form. No alignment needed; pre-existing positive `is_true` / `is_false` corpus entries cover this.
- **Bare `NULL` literal in arithmetic operands is rejected by the planner.** `SELECT a + NULL`, `SELECT NULL / 5`, etc. fail with `VerifyException: unable to encapsulate arithmetic operation due to type mismatch(es)` — fdb-relational requires a typed NULL, e.g. `CAST(NULL AS BIGINT)`, before it picks an arithmetic operator overload. The Go-side engine handles bare NULL by SQL three-valued semantics (NULL absorbs operand). For cross-engine yamsql harness use the typed-cast form, otherwise drop the test.
- **`SELECT <expr>` without `FROM` is unsupported by the planner.** `SELECT -10 / 3` (constant-only projection) raises `UnableToPlanException`. fdb-relational requires every SELECT to have a FROM clause, even when the projection is independent of the row. Workaround: SELECT through a single-row table or cross-engine-omit the test.
- **`WITH ... AS (...) SELECT ... ORDER BY ...` is rejected.** When a CTE precedes the SELECT, fdb-relational raises `RelationalException: order by is not supported in subquery` for any ORDER BY on the outer query — the planner appears to scope the outer ORDER BY into the CTE's subquery context. Drop ORDER BY when using CTEs (or move the sort outside via a wrapping query, if that worked, but typically it doesn't).
- **`SELECT ... ORDER BY col UNION ALL SELECT ... ORDER BY col` is rejected.** Inner-SELECT ORDER BY on either side of a UNION raises `UnableToPlanException` (different rejection path from the CTE case above — Cascades-emergent rather than parser-side). Both engines must drop inner-SELECT ORDER BY when feeding a UNION. Surfaced nightshift-60.
- **`NULLIF(a, b)` is not registered in fdb-relational's function table.** Returns `RelationalException: Unsupported operator NULLIF`. COALESCE works (it's one of the few registered scalar functions). The Go embedded engine implements both. Cross-engine corpus entries can use COALESCE freely; rewrite NULLIF as `CASE WHEN a = b THEN NULL ELSE a END`.
- **Bare-bool-operand inside WHERE is rejected by Java (one-sided).** `WHERE b AND TRUE` / `WHERE b OR FALSE` / `WHERE NOT b` over a BOOLEAN column raise `VerifyException` in fdb-relational 4.11.1.0, even though the same shapes in projection (above) are accepted. Java treats the operand of AND/OR/NOT inside WHERE as needing a strictly-typed BooleanValue, not a coerced FieldValue. Go's embedded engine accepts both contexts uniformly (operand-of-boolean-op is value-context regardless of where the enclosing expression sits). The asymmetry is fdb-relational-side; cross-engine corpus drops these specific WHERE-bare-bool-operand forms. Surfaced nightshift-57.
- **`ORDER BY` on a column outside the natural-order continuation raises `UnableToPlanException`.** Cascades planner can only sort by a column that is already naturally ordered by the chosen scan plan. Specifically: a range on a leading PK col + ORDER BY a trailing PK col (`WHERE k > 'a' ORDER BY g` on PK (k, g)), an `ORDER BY` on a non-PK col without a satisfying index (`ORDER BY v` where v is not an index leading col), and `WHERE region = 'us' AND bucket > 1 ORDER BY tag` on PK (region, bucket, tag) all fail. Single-col equality + ORDER BY trailing PK col works (`WHERE region = 'us' AND bucket = 1 ORDER BY tag`) because the natural order is preserved. Surfaced swingshift-56.
- **Only `UNION ALL` is supported; `UNION` (distinct) is rejected.** fdb-relational 4.11.1.0 raises `RelationalException: only UNION ALL is supported` for any plain `SELECT ... UNION SELECT ...` — the planner has no de-duplication operator wired into the union path. Workaround: `UNION ALL` followed by an explicit application-side dedup; SQL-level dedup is not portable. Surfaced swingshift-56.
- **Scalar-function-wrapping-aggregate fails Java planner.** `SELECT COALESCE(SUM(a), 0) FROM t`, `SELECT CASE WHEN COUNT(*) > 3 THEN 'many' ELSE 'few' END FROM t`, etc. raise `IllegalStateException: unable to eval an aggregation function with eval()`. fdb-relational 4.11.1.0's planner has post-aggregation evaluation only for plain arithmetic over aggregates (`SUM(a) + SUM(b)`, `AVG(a) + 1`); scalar functions wrapping the aggregate output are not implemented. The inverse direction (aggregate-of-expression: `SUM(CASE WHEN ...)`, `COUNT(qty * price)`) works fine. Go's embedded engine handles both directions correctly. Surfaced swingshift-56.
- **`ORDER BY <arithmetic-expression>` raises `UnableToPlanException`.** Even single-col `ORDER BY a + b` (no aggregate, no function call) fails Cascades planning. The planner can only sort by columns that are already naturally ordered by the chosen scan plan — an arithmetic expression isn't naturally ordered by any FDB key, so the planner needs a sort rule it doesn't have. Same Cascades-natural-order gotcha as the other ORDER BY entries. Surfaced swingshift-56 (separate from the originally-documented "ORDER BY non-PK col without index" form).

### Behavioural divergences

Both engines accept the SQL but produce different runtime results, error messages, or types. Watch for silent data drift; cross-engine corpus must drop or rewrite divergent forms. Includes RESOLVED items kept here for context.
...
- **`GREATEST(NULL)` / `LEAST(NULL)` (single-NULL-arg) raises `VerifyException` in Java.** The variadic-of-NULL path needs at least one typed argument to determine the result type; bare `GREATEST(NULL)` has no anchor for type resolution. Multi-arg `GREATEST(1, NULL, 3)` works fine because the non-NULL operand pins the type. Cross-engine corpus drops the single-NULL form. Surfaced swingshift-56.
...
- **Integer overflow — RETRACTED nightshift-61 (already aligned).** Initially documented as silent drift (Java throws ArithmeticException, Go silently wraps); a closer read of `pkg/relational/core/functions/arith.go` showed that Go's `ApplyMathOp` ALREADY overflow-checks via `AddInt64Checked` / `SubInt64Checked` / `MulInt64Checked` — Go raises `ErrCodeNumericValueOutOfRange` (22003) on overflow with message "integer overflow on N + N". Both engines reject; only the error class / message text differs. This was always aligned; the misread came from the cross-engine probe failing on Java's side first (which the harness reports), masking that Go also errors. Per-engine error-message divergence is a CrossEngineErrorContains substring concern, not a behavioural divergence.
- **`CAST(<double> AS BIGINT)` — RETRACTED nightshift-61 (already aligned).** Initially probed as silent drift after `cast_chain` test #4 reported "Java returned 2, expected 1". Misread: my test EXPECTED was 1 (truncation), but Go ACTUALLY returns 2 — `pkg/relational/core/functions/cast.go`'s `CastValue.DOUBLE_TO_LONG` arm uses `math.Floor(n + 0.5)` which is exactly Java's `Math.round` semantics. Both engines round 1.9 → 2. Restored the scenario with the correct expected values.
...
- **`SELECT <agg> FROM t WHERE <none-match> HAVING <agg-pred>` — RESOLVED nightshift-61.** SQL spec: aggregate over empty WHERE-result still produces a single grouping with COUNT=0/SUM=NULL etc., then HAVING filters. fdb-relational 4.11.1.0 instead treats the empty input as "no grouping at all" — HAVING never fires, 0 rows. Go's embedded engine previously followed SQL spec (single grouping), diverging silently from Java when HAVING evaluated TRUE on the synthetic empty group. Aligned to Java in `aggregate.go` and `select_query_full.go` by gating the synthetic-empty-group materialisation on `sq.havingExpr == nil` — when HAVING is set, the synthetic group is skipped and the query returns 0 rows. The HAVING-absent path (`SELECT COUNT(*) FROM empty_t` → 1 row [[0]]) still emits the synthetic group, so SQL-spec aggregate-over-empty semantics survive when HAVING isn't in play. Per project conformance principle: doesn't work in Java → doesn't work in Go.
- **Bare-bool-column-as-operand-in-projection — RESOLVED nightshift-57.** `SELECT b AND TRUE`, `SELECT NOT b`, `SELECT b OR FALSE` etc. over a BOOLEAN column now match Java (Kleene 3VL). Fix threaded `allowBareField bool` through `evalExprPredicateTri` / `evalComparisonPredicateTri` (`pkg/relational/core/embedded/eval_predicate.go`): operands of AND/OR/NOT/XOR (any context) and projection-level `evalExpr` (`eval_proto.go`) pass `true` so a bare `FullColumnNameExpressionAtomContext` evaluates as a value via IsTruthy. Top-level WHERE / HAVING entry still passes `false`, preserving Java's `WHERE flag` rejection ("expected BooleanValue but got FieldValue"). Five corpus entries (`SELECT b AND TRUE`, `SELECT b AND FALSE`, `SELECT b OR TRUE`, `SELECT b OR FALSE`, `SELECT NOT b`) now pass strict cross-engine equivalence.
- **Equality + contradicting range yields the equality row in Java (correctness divergence vs SQL spec).** `SELECT id FROM t WHERE id = 2 AND id > 5` returns `[2]` from fdb-relational 4.11.1.0; the equality pushdown narrows to id=2 and the range residual `id > 5` is silently dropped from the post-filter. Possibly an intentional optimization (assume `C` satisfies `C > C2`) but it produces wrong results when the user query has a contradiction. SQL spec + Go engine correctly return empty. Cross-engine corpus must drop the contradictory-equality-and-range form. When fdb-relational eventually aligns with SQL spec, the form can be re-added. Surfaced swingshift-56.
- **`ORDER BY` non-natural-order rejection — RESOLVED nightshift-60 (single-table SELECT path).** Pre-fix Go's embedded engine had a hard-coded `sort.SliceStable` fallback at `select_query_full.go:1152` whenever `naturalOrderSatisfies` failed, silently materialising and sorting any ORDER BY. Java's Cascades planner has no `ImplementSortRule` — only `RemoveSortRule` + `PushRequestedOrderingThroughSortRule`. With no rule generating a `RecordQuerySortPlan` in the SQL stack, queries whose ORDER BY isn't satisfied by an inner plan reach `CascadesPlanner.resultOrFail` and throw `UnableToPlanException`. The Go fix mirrors Java structurally: removed the fallback; gated EVERY PK / secondary-index pushdown branch (equality, range, composite-range, composite-prefix, IN-list, single-value secondary-IN-list) on natural-order satisfiability via `pkOrderingSatisfiesOrderBy` / `indexBranchSatisfiesOrderBy` so they decline (let the chain fall through) when their emission order can't satisfy ORDER BY; sorted the IN-list values internally so the lazy-chain emits in PK-sorted order; added `tryIndexScanForOrdering` (full secondary-index scan as the last branch before the full-PK fallback, picks an index whose `(idxCols, pkCols)` satisfies the requested ordering — matches Java's RemoveSortRule firing when an index scan's Ordering satisfies). The post-scan rejection check fires when `!satisfiable && !isAggregate && !sq.distinct`. Aggregate / DISTINCT exempt (DISTINCT is a Go-only feature with its own pipeline; aggregate produces a small post-aggregation result set). **At-most-1-row scans (PK equality, single-value IN-list) are NOT exempted** — Java's RemoveSortRule checks the Ordering property explicitly, and an equality match has `Ordering=()` which doesn't satisfy a non-empty requested ordering. The PK equality / IN-list branches gate on `pkOrderingSatisfiesOrderBy` and decline when ORDER BY targets a non-PK col, letting the chain fall through to `tryIndexScanForOrdering` (or rejection if no index satisfies). Test schemas updated to add indexes where ORDER BY targets a non-PK col. **Remaining**: JOIN (`pkg/relational/core/embedded/join.go:561`), CTE (`cte_scan.go:257`), UNION (`union.go:177`) sort sites still have in-memory sort fallbacks; not Java-conformant for those query shapes. **JOIN is more nuanced than SELECT**: Java's Cascades planner picks the JOIN-side outer based on cost, so `FROM A, B WHERE A.pk = B.fk ORDER BY B.pk` succeeds in Java by running B as outer (emits in B.pk order) with per-row A lookups. A static "left source PK only" gate (prototyped + reverted nightshift-60) rejects too many queries Java accepts. The correct Go-side fix needs JOIN-side reordering at plan-time, or routing the query through Cascades — defer to C2 QueryExecutor. ErrCode 0AF01 (`ErrCodeUnsupportedSort`).
- **Unary minus on a column reference is rejected by fdb-relational's parser.** `SELECT -col FROM t` raises `RelationalException: syntax error` at the `-` — only numeric-literal unary minus parses (e.g., `SELECT -10 FROM t`). The Go embedded engine accepts `-col`. For cross-engine portability use `0 - col` (no parens, since bare-paren `(0 - col)` triggers the next gotcha). Surfaced nightshift-60 by `arithmetic_compound` test #7.
- **Bare-paren `(expr)` around a single scalar in projection** is parsed by fdb-relational as a single-element record/tuple constructor, returning `ImmutableRowStruct` rather than the scalar value. Same parser quirk as the existing `WHERE (b = true)` rejection — extends to projection context. The pattern only triggers when the parens contain a single scalar with no outer operator forcing scalar context: `(a + b) * 2` works because the outer `* 2` forces arithmetic context, but `SELECT (0 - col) FROM t` returns ImmutableRowStruct. Use bare `0 - col` in projection (drop the parens). Surfaced nightshift-60.
- **Arithmetic operator-precedence diverges from SQL standard.** SQL standard binds `*`, `/`, `%` tighter than `+`, `-`, so `a + b * 2` parses as `a + (b * 2)`. fdb-relational 4.11.1.0's parser instead binds them at the same precedence left-to-right, so the same input parses as `(a + b) * 2` — surfaced nightshift-60 by `SELECT a + b * 2 FROM t WHERE id = 1` over `(a, b) = (10, 3)` returning 26 (Java) vs 16 (SQL spec / Go). Same parser pattern as the OR/AND divergence below. The Go embedded engine follows the SQL standard. Cross-engine corpus entries must use explicit parens for any mix of additive and multiplicative operators; implicit-precedence forms are not portable.
- **`OR`/`AND` operator-precedence diverges from SQL standard.** SQL standard binds `AND` tighter than `OR`, so `WHERE a OR b AND c` parses as `a OR (b AND c)`. fdb-relational 4.11.1.0's parser instead binds them at the same precedence left-to-right, so the same input parses as `(a OR b) AND c` — surfaced nightshift-60 by `SELECT id FROM t WHERE a = 1 OR b = 0 AND c = 1` returning [2,4] (Java) vs [1,2,4] (SQL spec / Go). The Go embedded engine follows the SQL standard. Cross-engine corpus entries must use explicit parens for any OR/AND mix; implicit-precedence forms are not portable. Same-precedence concern likely applies to NOT relative to comparison operators — use `(NOT (col = x))` not `NOT col = x` in cross-engine corpus.
- **`SUM(CASE WHEN ... THEN <int-literal> ELSE <int-literal> END)` reports column type INTEGER in Java, BIGINT in Go.** Surfaced dayshift-62 by the `case_in_aggregate` cross-engine probe. Java's Cascades planner types each CASE branch from its integer-literal (Java's `INT` for `1` / `0` since they fit in 32 bits), and the SUM-overload that matches keeps the INTEGER type without widening. Go's `inferConstantJDBCType` types every bare integer literal as BIGINT (per the existing "bare numeric literals are typed BIGINT" gotcha — fine for arithmetic context but loose for CASE branches), and Go's `aggregateResultJDBCType` for SUM with `aggExpr != nil` recursively-infers the inner expression's type and inherits the BIGINT. Net divergence: Java reports INTEGER, Go reports BIGINT, despite both engines computing the same row values. Two layers to align: (a) change Go's literal-typing for integer literals to INTEGER when the value fits in int32 (then BIGINT widening happens explicitly via CAST or arithmetic context), AND (b) keep SUM-overload result typing inheriting from the operand. Defer to a dedicated alignment shift; documented as TODO. Workaround for cross-engine corpus: pin both branches with `CAST(... AS BIGINT)` so the CASE → BIGINT and SUM result → BIGINT in both engines (see corpus entry `case_in_aggregate_bigint_cast`).
- **`SUM(BIGINT) / COUNT(*)` integer-division — RESOLVED nightshift-57.** Go's SUM accumulator now preserves int64 when every observed value is integral, matching Java's `SUM(BIGINT)→BIGINT` typing. `SELECT SUM(qty) / COUNT(*) FROM t` over a BIGINT `qty` integer-divides on both engines (10/3=3). Implementation: `mapGroupState` / `groupState` (`pkg/relational/core/embedded/aggregate.go`, `select_query_full.go`) carry parallel `sumsI []int64` and `sumNonInt []bool` per slot; `sumNonInt` starts as the zero value (false = "still int-only") and only flips to true the first time a non-int64 runtime value is observed. On emit, integer-only groups return the int64 accumulator; mixed/float groups fall back to the float64 accumulator. Overflow on the int64 accumulator wraps silently — same as Java's `long` accumulator. AVG remains float-typed (Java aligns; AVG of int promotes to DOUBLE). Surfaced swingshift-56, fixed nightshift-57.
...
- **Java's `IndexTypes.COUNT` index expression must be `GroupingKeyExpression(field("price"), 0)`, NOT `field("price").groupBy(empty())`.** The `(expr, groupedCount)` constructor splits `expr` into a leading grouping part (first `expr.columns - groupedCount` columns) and a trailing grouped/aggregated part. For COUNT-grouped-by-X you want X to be the GROUPING part (since count is implicitly +1, nothing is aggregated), so `groupedCount=0`. The intuitive `field("X").groupBy(empty())` puts `field("X")` in the grouped slot and Java rejects with `InvalidExpressionException: index type does not support non-group fields; use COUNT_NOT_NULL`. For ungrouped SUM (sum all values, no grouping), the symmetric idiom is `GroupingKeyExpression(field("price"), 1)` — `groupedCount=1` means the one column is the aggregated part (sum the value), nothing is the grouping part. Matches `sum_index_conformance.java` and the other index-type test files.

### Cross-language SchemaTemplateCatalog wire-format gotchas

A2 closed dayshift-54. The four divergences peeled and all fixed:
...
- **Java's `RecordLayerSchemaTemplate.toRecordMetadata()` sets the builder version to the SchemaTemplate's version (CATALOG_TEMPLATE_VERSION = 1) before adding indexes; subsequent addIndex calls bump from 1.** Go's `BuildCatalogMetaData()` was starting at 0 (default), giving an off-by-one final version (3 vs Java's 4). Fix: explicit `SetVersion(1)` in `BuildCatalogMetaData()` before the addIndex calls.
- **Java's `FileDescriptorSerializer` emits relative type-name references in field descriptors** (e.g. `setTypeName("T")` — no leading dot, no package). Go's `protodesc.NewFile` resolves the reference relative to the field's enclosing message scope first and doesn't fall back to file-scope lookup; result: `descriptor not found: RecordTypeUnion.T`. The `protodesc.FileOptions{AllowUnresolvable:true}` option does NOT silence this. Fix: pre-process the FileDescriptorProto via `absolutizeFieldTypeNames` (in `metadata_proto.go`) to rewrite every `field.type_name` to absolute form (`"T"` → `".T"` or `".pkg.T"`) before passing to `protodesc.NewFile`. Java's `Descriptors.FileDescriptor.buildFrom` apparently does this internally.
- **Java's fdb-relational uses `RecordTypeUnion` (not `UnionDescriptor`) for the union message** and emits union fields as `TypeName_N` (per `FileDescriptorSerializer.java:107` — `typeDescriptor + "_" + fieldCounter`). Go's `findUnionDescriptorName` previously only knew `UnionDescriptor`, and `setRecordsWithUnionName` only handled fields named `_TypeName` (RecordLayer convention). Fix: `findUnionDescriptorName` now also recognises `RecordTypeUnion`; `setRecordsWithUnionName` derives the record type name from the field's TYPE reference for fields not following the leading-underscore convention.
...
## Conformance status (updated 2026-04-28)
