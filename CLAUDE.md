...
- a PER-SHELL quirk emptying the input to the search itself: an UNQUOTED `grep --include=*.go` behaves differently per shell, so the diagnosis must never be ported without re-measuring. bash passes the unmatched pattern through literally and grep runs normally; a shell that rejects an unmatched glob (zsh here, fish reportedly) never runs grep at all, and a pipeline capturing only stdout then reports 0 hits, which "proves" the symbol does not exist anywhere. Quote the pattern, and identify the shell by running `echo $0` rather than trusting the environment banner. This is not one quirk but a family — an unexpanded glob, a pattern whose metacharacters the shell ate before `grep` saw them, a path that does not exist in *this* worktree, a `--include` filter matching no file, and a search rooted at a directory the tree does not have. Five distinct causes, all hit inside a single PR's verification, and every one renders as the same confident `0`. Any zero-hit sweep used as evidence of absence must first be shown to be a well-formed command — run it against a term you know is present, in the same invocation, and report that positive control alongside the zero. A zero without its control is not a measurement.
