...
- **`ORDER BY` non-natural-order rejection — RESOLVED nightshift-60 (single-table SELECT path).** Pre-fix Go's embedded engine had a hard-coded `sort.SliceStable` fallback at `select_query_full.go:1152` whenever `naturalOrderSatisfies` failed, silently materialising and sorting any ORDER BY. Java's Cascades planner has no `ImplementSortRule` — only `RemoveSortRule` + `PushRequestedOrderingThroughSortRule`. With no rule generating a `RecordQuerySortPlan` in the SQL stack, queries whose ORDER BY isn't satisfied by an inner plan reach `CascadesPlanner.resultOrFail` and throw `UnableToPlanException`. The Go fix mirrors Java structurally: removed the fallback; gated the secondary-index pushdown branches and PK/composite IN-list branches on natural-order satisfiability so they decline (let the chain fall through) when their emission order can't satisfy ORDER BY; sorted the IN-list values internally so the lazy-chain emits in PK-sorted order; added `tryIndexScanForOrdering` (full secondary-index scan to satisfy ORDER BY when no WHERE pushdown matched but an index's natural order satisfies); the post-scan rejection check fires only if all the above failed AND scan isn't aggregate / DISTINCT / at-most-1-row. The rejection is structural ("no scan satisfies"), matching Java's emergent rejection ("no rule fires"). Test schemas updated to add indexes where ORDER BY targets a non-PK col. **Remaining**: JOIN (`pkg/relational/core/embedded/join.go:561`), CTE (`cte_scan.go:257`), UNION (`union.go:177`) sort sites still have in-memory sort fallbacks; not Java-conformant for those query shapes. Address when those paths get the same treatment, ideally via the C2 QueryExecutor port. ErrCode 0AF01 (`ErrCodeUnsupportedSort`).
