...
- **`CAST(<double> AS BIGINT)` — Java rounds, Go truncates toward zero (silent data drift).** Surfaced nightshift-61 by `cast_chain` cross-engine probe: `SELECT CAST(1.9 AS BIGINT)` returns 2 in Java (round-half-up or banker's rounding) and 1 in Go (`int64(float64Value)` Go semantics, truncation toward zero). SQL standard leaves DOUBLE→INTEGER coercion implementation-defined, but the engines disagree. Aligning Go to match Java requires changing `CastValue` in `pkg/relational/core/functions/cast.go` to round when target is integer and source is float. Probe scenario was dropped; gotcha documented. Defer.
