...
- **`COUNT(DISTINCT col)` NPEs in fdb-relational 4.11.1.0.** `SELECT COUNT(DISTINCT col) FROM t` raises `NullPointerException: Cannot invoke ... AggregateWindowedFunctionContext.ALL().getText()` — the parser visitor unconditionally calls `.getText()` on the ALL token, which is null when DISTINCT is present. Joins the two other parser-visitor NPE bugs (`CROSS JOIN` ON clause, scalar subquery's expressionAtom). Go's embedded engine handles `COUNT(DISTINCT)` correctly; this is a one-sided gap. The grammar accepts the form but the visitor doesn't. Surfaced swingshift-56.
