...
- **Equality + contradicting range yields the equality row in Java (planner bug).** `SELECT id FROM t WHERE id = 2 AND id > 5` returns `[2]` from fdb-relational 4.11.1.0; the equality pushdown narrows to id=2 and the range residual `id > 5` is silently dropped from the post-filter. SQL spec + Go engine correctly return empty. Cross-engine corpus must drop the contradictory-equality-and-range form. Surfaced swingshift-56.
