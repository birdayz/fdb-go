...
- **Bare `WHERE flag` on a BOOLEAN column is rejected by fdb-relational** with `RelationalException: expected BooleanValue but got FieldValue`. Boolean columns must be compared explicitly: `WHERE flag = TRUE`. The grammar accepts `WHERE flag` but the planner rejects FieldValue-as-predicate downstream.
