...
- **Bare-bool-column-as-operand-in-projection diverges (Go stricter than Java).** `SELECT b AND TRUE FROM t`, `SELECT NOT b FROM t`, `SELECT b OR FALSE FROM t` — Go's embedded engine rejects with `0A000: expected BooleanValue but got FieldValue: bare column reference cannot be used as a predicate`, while Java accepts and returns the three-valued result. Surfaced by Track A3 cross-engine harness (dayshift-55). Workaround on Go side: rewrite as `SELECT (b = TRUE) AND TRUE`, `SELECT NOT (b = TRUE)`, etc. Go-engine fix tracked as separate work — the projection path's bool-vs-FieldValue check is over-zealous; Java treats a bare bool column as a value in projection context.
