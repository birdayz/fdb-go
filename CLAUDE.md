...
- **`SUM(BIGINT) / COUNT(*)` integer-divides in Java, float-divides in Go.** `SELECT SUM(qty) / COUNT(*) FROM t` where qty is BIGINT: Java returns the integer-divided result (`SUM=10` / `COUNT=3` → `3`); Go's SUM accumulator is always float64, so the result is `3.333...`. Documented in the yamsql comment as a Go-side divergence pending an int-preserving SUM for int-only inputs (fix would either: detect that all inputs are BIGINT and accumulate as int64, then promote on overflow; or align with Java's integer-division semantics for BIGINT/BIGINT). Cross-engine corpus drops the form. Surfaced swingshift-56.
