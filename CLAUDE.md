...
- **`SELECT <agg> FROM t WHERE <none-match> HAVING <agg-pred>` diverges.** SQL spec (and Go embedded engine): aggregate over empty WHERE-result still produces a single grouping with COUNT=0/SUM=NULL etc., then HAVING filters that single grouping. fdb-relational instead treats the empty WHERE result as having no grouping at all, so HAVING never fires and the query returns 0 rows. Bare `SELECT COUNT(*) FROM t HAVING COUNT(*) > 0` (no WHERE) still works the same on both — the divergence is specific to WHERE+HAVING with all rows filtered out.
