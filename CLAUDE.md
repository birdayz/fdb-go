...
- **Explicit `CROSS JOIN` syntax NPEs in fdb-relational 4.11.1.0.** `SELECT ... FROM a CROSS JOIN b` raises `NullPointerException: Cannot invoke ... InnerJoinContext.expression()` — the parser visitor unconditionally calls `accept(...)` on the ON-clause expression, which is null for CROSS JOIN. Use comma-join (`FROM a, b`) for cartesian products instead. Surfaced swingshift-56.
