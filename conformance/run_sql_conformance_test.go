...
func divergenceHolds(div *plandiff.Divergence, javaResult, goResult plandiff.RunResult) (bool, string) {
	// Infra guard, same reasoning entryConforms carries for the non-annotated
	// path: a transport failure or a server-side deadline says nothing about
	// whether the pinned divergence still holds. Without this the annotated
	// path reports a starved server as "Go's behaviour no longer matches the
	// pinned divergence" — a diagnosis pointing at the engine when the cause is
	// the host. Still a failure, just an honestly labelled one.
	if infra, detail := javaInfraFailure(javaResult.Err); infra {
		return false, detail
	}
