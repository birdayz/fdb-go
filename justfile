...
binding-stress runs="100" ops="1000":
    ...
    _binding_stress_run {{runs}} {{ops}} | tee "$REPORT"
    echo "Report: $REPORT"
    tail -5 "$REPORT" | grep -q "0 fail" || exit 1
...
binding-stress-duration duration ops="1000":
    ...
    print(s)
    ")
    echo "Running binding-stress for ${SECS}s ({{ops}} ops/seed)..."
    _binding_stress_timed "$SECS" {{ops}} | tee "$REPORT"
    echo "Report: $REPORT"
    tail -5 "$REPORT" | grep -q "0 fail" || exit 1

[private]
_binding_stress_run runs ops:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    for seed in $(seq 1 {{runs}}); do
        ...
        if [ $((seed % 10)) -eq 0 ]; then
            echo "--- $seed/{{runs}} (pass=$PASS fail=$FAIL dead=$DEAD) ---"
        fi
    done
    docker rm -f fdb-stress 2>/dev/null >/dev/null
    echo ""
    ...
    if [ -n "$FAILURES" ]; then echo "Failures:$FAILURES"; fi
    echo "========================================="

[private]
_binding_stress_timed secs ops:
    #!/usr/bin/env bash
    set -euo pipefail
    bazelisk build //cmd/fdb-stacktester:fdb-stacktester 2>&1 | tail -1
    STACKTESTER=$(realpath bazel-bin/cmd/fdb-stacktester/fdb-stacktester_/fdb-stacktester)
    ...
    echo "binding-stress: timed run, {{ops}} ops/seed, deadline=$(date -Iseconds -d @$DEADLINE)"
    echo "Started: $(date -Iseconds)"
    echo ""
