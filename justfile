...
# A result-row change is almost always a bug; a plan change is a query-engine change (Graefe
# reviews it). Capture is deterministic over SimFDB, so a rerun with no engine change is a no-op.
golden-update:
