...
// LIMIT prefix, grouped aggregates, multi-value IN + UNION ALL (concatCursor, now resuming), comma
// joins, and a spread of richer cursor shapes — explicit equi-JOIN, sort-over-join, correlated
// EXISTS (semi-join), correlated scalar subquery (FlatMap), HAVING (filtered streaming aggregate),
// and a nested derived table — so each operator's mid-stream resume is exercised under the tiny
// scanned-rows budget, not just the flat scan/filter path. Nothing is quarantined: bare
// DISTINCT rejoined the sweep once the executor carried its dedup set through the continuation.
func queries(rng *rand.Rand) []query {
	...
	return []query{
		...
		// Bare DISTINCT was quarantined here while the hash-distinct operator lost its dedup
		// state across a resume and re-emitted duplicates. The executor now rides the seen-set
		// through the continuation (executeHashDistinct / gen.DistinctHashContinuation), so the
		// shape is back in the sweep and pinned by TestDistinctContinuationDedups.
		{"SELECT DISTINCT cat FROM t", false},
		{"SELECT a.id, b.id FROM t a, t2 b WHERE b.ref = a.id", false},
		{"SELECT a.id, b.id FROM t a, t2 b WHERE b.ref > a.id", false},
		// Richer cursor shapes, each probing a different operator's mid-stream resume under a tiny
		// scanned-rows budget (all verified paged==unpaged before landing — see the shape probe):
		{"SELECT a.id, b.id FROM t a JOIN t2 b ON b.ref = a.id", false},                                            // explicit equi-JOIN cursor
		{"SELECT a.id, b.id FROM t a JOIN t2 b ON b.ref = a.id ORDER BY a.id, b.id", true},                         // sort over a join
		{"SELECT id FROM t WHERE EXISTS (SELECT 1 FROM t2 b WHERE b.ref = t.id)", false},                           // correlated semi-join
		{fmt.Sprintf("SELECT id, (SELECT COUNT(*) FROM t2 b WHERE b.ref = t.id) FROM t WHERE cat = %d", k), false}, // correlated scalar subquery (FlatMap)
		{"SELECT cat, COUNT(*) FROM t GROUP BY cat HAVING COUNT(*) > 1", false},                                    // filtered streaming aggregate
		{fmt.Sprintf("SELECT id FROM (SELECT id, val FROM t WHERE val > %d) s WHERE s.val < 100", k), false},       // nested derived table
		// The 3-way comma join (t a, t2 b, t2 c WHERE …) is probe-verified clean but ~72 rows/run;
		// left out of the per-seed loop as disproportionately expensive to resume at limit=1.
	}
}
