...
// TestUnreadableCapSurvivesBatchBoundary pins that a range scan whose first batch stops SHORT of
// a pending versionstamped op still raises accessed_unreadable on the NEXT batch.
//
// The dimension this covers is the batch boundary, and it is invisible to any single-batch test.
// The cap is per-FETCH — the client keeps ri.begin/ri.end at the requested bounds and lets
// rywCache.getRange re-derive the cap on every call (fdb/range_result.go:309 passes them
// unmodified). If the iterator instead wrote the capped end back into its own scan bounds, the
// unreadable key would land exactly ON the next window's exclusive end, where unreadableScanCap
// skips it: batch two would find no cap, return no rows, and report clean exhaustion. The scan
// would then have walked up to an unreadable position and reported success, which is the one
// answer no real client gives.
func TestUnreadableCapSurvivesBatchBoundary(t *testing.T) {
	...
	for it.Advance() {
		kv, err := it.Get()
		if err != nil {
			t.Fatalf("Get during batch 1: %v", err)
		}
		got = append(got, string(kv.Key))
	}
	if len(got) != 2 || got[0] != "a" || got[1] != "b" {
		t.Fatalf("rows before the cap = %v, want [a b] — the first batch must return the rows "+
			"it resolved strictly inside the truncated window", got)
	}
	// Advance() returned false; the reason must be the 1036, not exhaustion.
	_, err := it.Get()
	if err == nil {
		t.Fatal("the scan reached the unreadable key and reported clean exhaustion: " +
			"accessed_unreadable(1036) was swallowed at the batch boundary")
	}
	var fe fdb.Error
	if !errors.As(err, &fe) || fe.Code != 1036 {
		t.Fatalf("error after the cap = %v, want accessed_unreadable (1036)", err)
	}
}
