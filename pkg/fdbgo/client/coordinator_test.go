...
func TestCoordinatorBootstrap(t *testing.T) {
	...
	loc, locErr := db.db.locCache.locate(db.db, ctx, []byte("test_key"), NoTenantID, types.SpanContext{}, false)
	if locErr != nil {
	...
	version, _, _, err := db.db.grvBatchers[grvBatcherDefault].getReadVersion(db.db, ctx, grvPriorityDefault, types.SpanContext{}, nil, false, false)
	if err != nil {
	...
	if len(dbInfo.CommitProxies) > 0 && cErr == nil {
		...
		rv2, _, _, _ := db.db.grvBatchers[grvBatcherDefault].getReadVersion(db.db, ctx, grvPriorityDefault, types.SpanContext{}, nil, false, false)
		...
		sharedRV, _, _, _ := db.db.grvBatchers[grvBatcherDefault].getReadVersion(db.db, ctx, grvPriorityDefault, types.SpanContext{}, nil, false, false)
