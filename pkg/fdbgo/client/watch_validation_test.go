...
func TestWatchSetup_ChargesSlotAtRegistrationOrder(t *testing.T) {
	...
	if _, _, _, _, _, err := tx1.WatchSetup(ctx, []byte(t.Name()+"_k1")); err != nil {
	...
	if _, _, _, _, _, err := tx2.WatchSetup(ctx, []byte(t.Name()+"_k2")); fdbCodeOf(err) != ErrTooManyWatches {
	...
	if _, _, _, _, _, err := tx3.WatchSetup(ctx, []byte(t.Name()+"_k3")); err != nil {
...
func TestWatchSetup_CancelledTxnDoesNotLeakSlot(t *testing.T) {
	...
	tx1 := db.CreateTransaction()
	tx1.Cancel()
	if _, _, _, _, _, err := tx1.WatchSetup(ctx, []byte(t.Name()+"_k1")); fdbCodeOf(err) != 1025 {
...
func TestWatchSetup_CancellationOutranksWatchCap(t *testing.T) {
	...
	tx.Cancel()
	if _, _, _, _, _, err := tx.WatchSetup(ctx, []byte(t.Name()+"_k")); fdbCodeOf(err) != 1025 {
...
func TestWatchSetup_FailedSetupDoesNotPoisonNextWatch(t *testing.T) {
	...
	ccancel()
	if _, _, _, _, _, err := tx.WatchSetup(cctx, []byte(t.Name()+"_k1")); err == nil {
	...
	_, _, _, watchCtx2, _, err := tx.WatchSetup(ctx, []byte(t.Name()+"_k2"))
	if err != nil {
...
func TestWatchSetup_TerminalErrorsOutrankCap(t *testing.T) {
	...
	ccancel()
	tx1 := db.CreateTransaction()
	if _, _, _, _, _, err := tx1.WatchSetup(cctx, []byte(t.Name()+"_k1")); !errors.Is(err, context.Canceled) {
	...
	if _, _, _, _, _, err := tx2.WatchSetup(ctx, []byte(t.Name()+"_k2")); fdbCodeOf(err) != 1031 {
...
func TestWatchSetup_CancellationOutranksKeyValidation(t *testing.T) {
	...
	tx := db.CreateTransaction()
	tx.Cancel()
	if _, _, _, _, _, err := tx.WatchSetup(ctx, []byte("\xff\x05")); fdbCodeOf(err) != 1025 {
...
func TestWatchSetup_RejectsSystemAndOversizedKeys(t *testing.T) {
	...
	t.Run("system_key_2004", func(t *testing.T) {
		...
		_, _, _, _, _, err := tx.WatchSetup(context.Background(), []byte("\xff\x05"))
	...
	t.Run("oversized_key_2102", func(t *testing.T) {
		...
		_, _, _, _, _, err := tx.WatchSetup(context.Background(), big)
		var fe *wire.FDBError
