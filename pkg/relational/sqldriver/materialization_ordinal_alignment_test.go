...
// Sentinel for the materialization read path being fully ordinal.
//
// RecordLayerResultSet.columnValue reads SOLELY from the aligned positional
// output row and returns a loud (XX000) error if a row carries no aligned
// positional row. So driving each battery shape end-to-end — Scan-ing every
// column of every row — proves that shape's final output row is fully ordinal:
// any column that lacked an aligned positional slot would surface the
// correct-or-loud error here.
//
// The battery spans the shapes whose ColumnDef label and ExplainValue slot name
// can disagree, stressing positionalAligned's ordinal-alignment path: scans,
// INNER/LEFT/3-way joins, CTE joins, unions, DISTINCT, ORDER BY, LIMIT,
