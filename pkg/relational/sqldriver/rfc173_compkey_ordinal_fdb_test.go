...
// RFC-173 regression pin for the INTERSECTION/MERGE-SORT COMPARISON-KEY extractor
// edge. The multi-aggregate intersection plan (RFC-081 MultiIntersection, built by
// tryMultiAggregateIntersection over two+ aggregate indexes sharing a group column)
// merges its child aggregate-index scans on a comparison key = the GROUP column.
// That key was extracted by evaluating the key value against the child row's NAME-
// keyed Datum map (multiIntersectionCompKeyFunc). Now it evaluates against the
...
// any executing SQL on the supported surface — UNION DISTINCT / INTERSECT are 0AF00 /
// 42601 reach gaps, ordered UNION ALL plans as InMemorySort-over-Union (not a merge-sort
// union), and the primary-key intersection plan is generated FINAL but never cost-
// selected. They are routed for the atomic-cap correctness contract but carry no
// executing-SQL pin (there is nothing to run). This test pins the one reachable site.
