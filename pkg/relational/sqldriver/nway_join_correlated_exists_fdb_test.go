...
// A multi-way (>=3) INNER join under a correlated WHERE EXISTS resolves
// positionally while preserving the index SARG. The mechanism
...
// so these certs compare row SETS, no ORDER BY.
//
...
func TestFDB_NWayJoinCorrelatedExists(t *testing.T) {
	t.Parallel()
	if clusterFilePath == "" {
	...
	// idSet runs q and returns the sorted result ids (row-SET comparison — this
	// scope excludes ORDER BY; see the header).
	...
	// rollback must truncate it (else the executor pre-evaluates it twice — a
	// side-effect leak). Rows pin the combined shape; e.eid=501 >
	...
	t.Run("plan_keeps_sarg_not_cross_product", func(t *testing.T) {
		...
		if !strings.Contains(explain, "[=]") {
			t.Fatalf("N-way join lost the SARG'd index probe (collapsed to a cross product?):\n%s", explain)
		}
		if strings.Contains(explain, "NestedLoopJoin(INNER, NestedLoopJoin(INNER, Scan(P), Scan(Q)), Scan(R))") {
			t.Fatalf("N-way join regressed to the materialized P×Q×R cross product:\n%s", explain)
		}
	})
}
