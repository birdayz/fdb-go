...
// INNER contains a JOIN. The inner's first table shares the SQL alias the
// seed's inner leg is keyed by, which used to collide the typed
...
// the reason join-inners used to be gated to a name-model plan. A fresh-unique
// inner correlation id dissolves the collision, so join-inners ordinalize:
// same rows, no tripwire.
