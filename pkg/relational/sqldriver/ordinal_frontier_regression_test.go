...
// column resolution on the non-join frontier must NOT break the
// provided-ordering rebase. A column's identity is ordinal, not name-keyed, so
// if the ordering pull-up (pullUpOrderingFromSelectChild) stopped agreeing,
// index-ordering match would fail, RemoveSortRule would stop firing, and a
// redundant InMemorySort would reappear over an already index-ordered
// single-table scan — a plan-property regression a row-content-only check
// would be blind to. This pin asserts the sort stays
...
// TestFDB_GroupByHavingOrderBy pins GROUP BY / HAVING / ORDER BY
// over a single-table frontier under ordinal column resolution. Grouping
// keys, the HAVING predicate, and the ORDER BY key all
// ride the generic FieldValue path that resolves by ordinal against the
