...
// TestFDB_RFC173S4_4051CleanLift is the certificate for the RFC-173 S4 retirement
// of the correlated-scalar enclosure producer (translateProjectWithCorrelatedScalar
// no longer forces `inInnerCluster` on its OUTER, and routes its INNER through
// translateSubqueryRef so the never-merged scalar subquery roots a FRESH cluster —
// the same primitive the existential inner uses).
//
// The former single `= true` (under a defer) conflated two architecturally distinct
// positions:
//   - OUTER: only a single-source or ungated outer reaches this path (the gated
//     multi-table outer ordinalized earlier); a buried-join / multi-source outer is
//     arity≠1 and declines LOUDLY (0AF00) regardless of the flag — so forcing
//     enclosure had no observable effect. Now inherits prevEnclosure.
//   - INNER: a correlated-scalar subquery is never merged into its parent, so it must
//     gate on its own arity, not the outer's enclosure. Now via translateSubqueryRef.
//
// This test pins BOTH sides: the outer residency (a derived-JOIN outer stays 0AF00,
// the reach gap that masks it), and the inner name-model constructs (JOIN, aggregate)
// answering CORRECT values with the inner rooted fresh — plus the StrictSingle
// cardinality guard, which must never silently pick one row.
func TestFDB_RFC173S4_4051CleanLift(t *testing.T) {
	if clusterFilePath == "" {
	...
	// (0A000, no SubqueryPlanner) — it never reaches the translator enclosure, so the
	// inner-EXISTS enclosed-gather reader is not reachable via this path. Pinned so a
	// future front-end that admits it forces a fresh certificate of the inner path.
	t.Run("inner_where_exists_front_end_declines", func(t *testing.T) {
		...
		if err == nil {
			t.Fatalf("inner WHERE-EXISTS scalar unexpectedly planned — re-certify the inner path (translateSubqueryRef enclosure)")
		}
