...
func (p *mmTxPair) exec(stmt string) {
	p.t.Helper()
	if _, err := p.itx.ExecContext(p.ctx, stmt); err != nil {
		p.t.Fatalf("indexed schema: exec %q: %v", stmt, err)
	}
	if _, err := p.ntx.ExecContext(p.ctx, stmt); err != nil {
		p.t.Fatalf("unindexed schema: exec %q: %v", stmt, err)
	}
}
...
func (p *mmTxPair) rows(tx *sql.Tx, q string) []string {
	...
	if err != nil {
		p.t.Fatalf("query %q: %v", q, err)
	}
	defer rows.Close()
	cols, err := rows.Columns()
	if err != nil {
		p.t.Fatalf("columns: %v", err)
	}
	var out []string
	for rows.Next() {
