...
// RFC-173 S4-rider pin — a computed UNALIASED projection in a JOIN-bodied
// recursive leg resolves through the ordinal read (its rendering carries a
// dot; the remap must never first-dot-split it into a garbage correlation).
func TestFDB_RFC173Rcte_ComputedLegProjection(t *testing.T) {
	t.Parallel()
