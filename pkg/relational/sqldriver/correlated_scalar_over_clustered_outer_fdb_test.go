...
// subquery in the projection over a MULTI-TABLE outer FROM. The outer
...
func TestFDB_CorrelatedScalarOverClusteredOuter(t *testing.T) {
	...
	// (d) SINGLE-SOURCE LEFT-join outer + NON-rightmost correlation: this
	// shape is GATED (ordinalizes). It used to decline because the
	// un-gated name model silently NULLed the first leg's alias; the gated
