...
// RFC-173 QP-REF-BIND item 2, commit 3 — the 2+1 flatten's ordinal seed.
// translateJoinWithExists consults the wedge gate and, for a gated INNER
// 2-leg flatten, seeds the baked ordinal RC (existential quantifiers
// contribute NO columns — Java's model) with the combined predicate list
// baked. Red-first observable: the §7 duplicate-name `SELECT *` fix arrives
// for the EXISTS-over-join class — the anchored flatten's name-keyed Datum
// conflates the duplicate ID/GID columns across legs (last-leg-wins), while
// the ordinal seed keeps each leg's value in its own slot (the NLJ birth +
// the commit-2 identity-FlatMap pass-through flow the positional row to the
// result set, which reads per-slot when aligned). The plain (no-EXISTS)
// cluster has behaved ordinally since W3b — pin C is the parity control.
//
// The pa/pb column-name collision (ID, GID on both) is load-bearing: it is
// exactly what the name map conflates and the positional row keeps distinct.
...
func TestFDB_RFC173Item2C3_FlattenOrdinalSeed(t *testing.T) {
	t.Parallel()
	...
	// (A) The §7 dup-name observable through the EXISTS flatten: four
	// columns [ID GID ID GID], each leg's ID distinct by position. The
	// name-model flatten conflates the dup names (last-leg-wins in the
	// Datum); the ordinal seed keeps both.
	...
	// EXISTS; the group-10 restriction spelled as a plain conjunct). Gated
	// ordinal since W3b: must pass before AND after the flatten seed.
