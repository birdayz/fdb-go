...
// Regression pin for the streaming-aggregate INPUT edge (group-by key /
// aggregate operand read). A plain single-table aggregate bakes each
// key/operand column to the inner frontier's output ORDINAL
// (NewFieldValueOfOrdinal), so the aggregate cursor reads the inner PositionalRow by
// Get(ordinal) — resolution is structural: the group key / operand resolves
// against the inner PositionalRow (by Get(ordinal), or by name-in-row for a
// layout-divergent covering row), never a name-keyed Datum map.
