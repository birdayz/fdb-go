...
// silent NULL. If the computed expression were applied above the
// inner and never landed in the inner row, the scalar reference
// would resolve to nothing (the query would return NULL — a silent wrong result
// a gate-decision-only unit test would never catch, since it only pins the gate
// decision, not end-to-end rows). The computation must materialize into the
// inner subquery's projected output (positional `_0`), so both name-based
// and ordinal resolution paths resolve it.
func TestFDB_ComputedCorrelatedScalarSubquery(t *testing.T) {
	t.Parallel()
