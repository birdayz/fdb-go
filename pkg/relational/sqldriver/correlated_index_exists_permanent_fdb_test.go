...
// The CORRELATED-INDEX EXISTS name-binding path is a PERMANENT, Java-correct
// architectural choice, not a migration remnant. A WHERE-EXISTS that
...
// into A's index. The ordinal (positional) resolution path is a Go-only
// optimization for the INDEPENDENT-legs materialized-NLJ case; it cannot
// express a positional correlation, and Java has no positional-correlation
// concept either — so this shape STAYS name-model, permanently. This pins
// that the name path keeps the SARG'd index plan (a cross-product would be an
// N³-vs-N regression on the shape where the index matters most) AND returns
// correct rows: NewAnchoredJoinRecord must never be deleted for this shape.
//
// This is a required permanence regression: it TRIPS if a future change
// re-ordinalizes the correlated-index existential and drops the SARG,
// regressing to a full cross-product join.
...
func TestFDB_CorrelatedIndexExistsStaysIndexed(t *testing.T) {
	t.Parallel()
	if clusterFilePath == "" {
	...
	if _, err := setup.ExecContext(ctx, "CREATE SCHEMA TEMPLATE corridxexists_tmpl"+
	...
	if _, err := setup.ExecContext(ctx, "CREATE SCHEMA "+dbPath+"/main WITH TEMPLATE corridxexists_tmpl"); err != nil {
	...
	if !strings.Contains(explain, "[=]") {
		t.Fatalf("correlated-index EXISTS lost its SARG'd index scan (a cross-product regression):\n%s", explain)
	}
