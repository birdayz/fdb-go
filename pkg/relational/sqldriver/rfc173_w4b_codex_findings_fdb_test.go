...
// TestFDB_RFC173W4b_FieldValuedComputedScalar pins the review finding P2-1: a
// PARENTHESIZED plain column in a correlated scalar projection walks to a bare
// FieldValue, and the shape-3 materialization must NOT re-key it to `_0` — the
// projection executor keys field-valued projections by the COLUMN NAME, so a
// seed expecting `_0` fails the leg adapter on a valid query.
func TestFDB_RFC173W4b_FieldValuedComputedScalar(t *testing.T) {
	t.Parallel()
...
// clustered-outer correlated scalar whose AGGREGATE OPERAND references an
// outer column. Aggregate operands evaluate against the aggregate's inner
// input row, which does not carry the level-2 positional outer binding — the
// pull-up must NOT bake aggregate-local outer refs. This shape has NEVER
// planned (0AF00 on master, verified in a master worktree); the un-guarded
// bake regressed the clean plan error into a loud baked-ref runtime error.
// The pin asserts master parity: the clean 0AF00, never the runtime error.
// (Making the shape actually WORK needs aggregate evaluation to receive the
// outer binding — out of W4b scope, tracked with the name-model residuals.)
func TestFDB_RFC173W4b_AggregateLocalOuterRef(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	dbPath := "/w4b_ag"
	setup := openTestDB(t, dbPath)
