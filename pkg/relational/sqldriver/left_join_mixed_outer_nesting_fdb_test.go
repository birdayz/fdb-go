...
// MIXED outer nesting runtime pins. BOTH nesting directions stay name-model:
// a single-source LEFT/RIGHT box that is a leg of an enclosing name-model
// INNER join (`d LEFT JOIN e ON … JOIN c ON …`) is kept name-model by the
// gate's enclosure guard (a positional box row under a name-merge parent
// reads the wrong source), and the reverse nesting (`a JOIN b ON … LEFT JOIN
// c ON …`) also stays name-model — the LEFT's preserved leg is itself a
// cluster. Both classes plan clean but had ZERO row-level pins before this
// file, and adding them immediately caught two real bugs: fabricated
// ALIAS.* keys on the null-padded box row (wrong-source rows) and a
// nondeterministic anchored re-enumeration planning panic. Row values,
// NULL-padding, and INNER drops are asserted for each nesting direction.
...
func TestFDB_MixedOuterJoinNesting(t *testing.T) {
	t.Parallel()
	...
	// preserved leg is a CLUSTER, so it fails the ordinal-safety gate and
	// stays name-model. Same rows, different plan class.
	wantMixed(t, "SELECT d.id, e.id, c.id FROM dept d"+
