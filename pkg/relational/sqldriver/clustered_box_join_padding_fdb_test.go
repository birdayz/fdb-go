...
// Row-level pins for mixed-nesting LEFT/FULL joins where one join leg is
// itself a multi-table cluster (a "box"): a clustered NULL-SUPPLYING leg's
// padded rows must read every buried column as NULL through the merged
// row (both nesting directions), and a WHERE-EXISTS over a clustered
// LEFT box must flow correctly through the existential machinery.
...
func TestFDB_ClusteredBoxJoinRows(t *testing.T) {
	t.Parallel()
	if clusterFilePath == "" {
	...
	if _, err := setup.ExecContext(ctx, "CREATE SCHEMA TEMPLATE clusterboxpad_tmpl"+
	...
	if _, err := setup.ExecContext(ctx, "CREATE SCHEMA "+dbPath+"/main WITH TEMPLATE clusterboxpad_tmpl"); err != nil {
	...
	// Clustered NULL-SUPPLYING leg — `pa LEFT JOIN (pb ⋈ pc)`.
	...
	// (the NULL padding spans the whole multi-column window).
	t.Run("clustered_null_supplying_leg", func(t *testing.T) {
		...
		if p := got[9]; p[0].Valid || p[1].Valid {
			t.Errorf("padded row = %v, want BOTH buried columns NULL (the NULL padding must span the whole window)", p)
		}
	})

	// The preserved-side twin — `(pa ⋈ pb) LEFT JOIN pc` with the ON
	// correlating the BURIED pa.
	t.Run("clustered_preserved_leg_buried_on", func(t *testing.T) {
		rows, err := db.QueryContext(ctx,
	...
	// FULL over a LEFT box (`(A LEFT B) FULL C`): the FULL join's
	// unmatched-C row pads the WHOLE nested LEFT-box side, never just
	// one of its columns.
	t.Run("full_over_left_box", func(t *testing.T) {
		rows, err := db.QueryContext(ctx,
	...
	// WHERE-EXISTS over the clustered LEFT box: existsOuterGatesFresh
	// must admit this shape, and the below-FirstOrDefault machinery must
	// serve the correlated read.
	t.Run("exists_over_clustered_box", func(t *testing.T) {
		rows, err := db.QueryContext(ctx,
