...
// TestFDB_ScalarSubqueryPerRowLimit pins a computed scalar subquery with a
// user LIMIT: the materializing Project (positional `_0`) wraps ABOVE the
// builder's Sort/Limit, so the translator's limit-peel does NOT see the
// Limit and it translates IN-POSITION inside the correlated inner. This pin
// proves the in-position translation still applies PER OUTER ROW — the
// hazard would be a LIMIT applied once globally instead (one outer row gets
// a value, every other gets NULL — silent wrong rows), which a
// discriminating two-outer-row probe catches.
