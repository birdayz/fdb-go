...
// White-box pins for RFC-173 B2 sub-slice A (the filtered box unnest).
// A BAKEABLE box-leg conjunct rides the gathered ordinal path and translates;
// an UNBAKEABLE one (here: a reference the buried window cannot resolve)
// declines LOUDLY (the name-model lowering the decline used to fall to was
// DELETED, RFC-173 S4 item B) — the discriminating control proving the verdict
// routes, not just passes. Rows/plan correctness is pinned e2e by
...
func TestRFC173B2_FilteredBoxUnnestCensus(t *testing.T) {
	...
	// the shape is untranslatable and LOUD (the name-model lowering the decline
	// used to fall to was DELETED, RFC-173 S4 item B). Proves the Bakeable path
	// above is the verdict ROUTING, not a vacuously-permissive gate.
