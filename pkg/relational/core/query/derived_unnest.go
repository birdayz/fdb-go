...
// the outer alias — structurally closes the hazard of misidentifying a
// same-named table elsewhere as the owner. A POSITIVE WHITELIST: only a bare
...
// derived-over-derived) declines loudly — honest "unsupported" over
// silent-wrong. Flowing a real ArrayType through the projection result
// (columnCascadesType) would retire this descriptor-recovery mechanism and
// the computed-output declines along with it.
...
// its CTE/derived owner's body. Returns the element type and the
...
func (t *cascadesTranslator) classifyDerivedUnnestArray(outerLeft logical.LogicalOperator, u *logical.LogicalUnnest) (elementType values.Type, outputName string, disp derivedUnnestDisposition) {
	...
	// The body's source must resolve to a SINGLE unambiguous BASE scan —
	// never a nested CTE/derived (which would re-open the wrong-same-named-
	// table hazard one level down) and never one of several scans
	// (ambiguous). singleBaseScan declines a body containing any
	// join/CTE/aggregate/union.
...
// or more than one scan — the ambiguity/nesting the whitelist forbids. A
// single scan whose table is itself a CTE is caught by the caller's
// outerSourceIsCTE check.
