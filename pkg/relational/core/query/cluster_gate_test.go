...
// Pins for the translation-time cluster-arity scoping gate: the decision of
// whether a join's leg gets an ORDINAL (positional) result value or the
// name-model anchored one. Two layers:
//
//   - clusterArity walk pins: the post-flattening arity computation per
//     logical shape, including the flattening-evasion shape and the poison
//     classes;
...
// TestExistsInOnGate pins the 2-way EXISTS-in-ON gate narrowing BOTH
// directions: a bare 2-way non-enclosed INNER EXISTS-in-ON GATES ordinal
// (the 2-leg fold is index-neutral for the existential shape — EXPLAIN-
// verified), while an ENCLOSED or N-WAY EXISTS-in-ON stays POISON (the
// existential would ride into the ≥3-quantifier partition machinery or hit
// the buried-inner-box index-matching wall).
func TestExistsInOnGate(t *testing.T) {
	t.Parallel()
...
func TestClusterArity_Shapes(t *testing.T) {
	t.Parallel()
	tr := newGateTranslator(t)
	...
	cases := []struct {
		...
		// LEFT OUTER: the dissolved select merges as preserved + the
		// null-on-empty quantifier — arity = preserved + 1. FULL OUTER stays
		// the genuinely opaque box (never rewritten/merged).
		...
		}), 1}, // the existential rides the merge; no ForEach added
		...
		}), 1}, // uncorrelated scalar = root-context binding, transparent
	}
...
// TestClusterArity_FlatteningEvasion pins the flattening-evasion shape: `FROM
// (a JOIN b) t1, (c JOIN d) t2` is 2-way at translation but 4-way
// post-flattening (SelectMergeRule merges the derived bodies up). The walk
// must see THROUGH the cteScope-registered bodies.
func TestClusterArity_FlatteningEvasion(t *testing.T) {
	t.Parallel()
	tr := newGateTranslator(t)
	...
	// The shape GATES: arity >= 2 is the whole rule now. A 2-way seed
	// mis-typed by post-translation flattening is legal composition today —
	// the derived bodies seed their own ordinal RCs and SelectMergeRule
	// composes them via translateValueCorrelations + the fuse arm.
	d := tr.ordinalWedgeGateDecide(evasion)
	if !d.Gated || d.Arity != 4 {
		t.Fatalf("flattening shape must gate at arity 4: %+v", d)
	}
...
// TestWedgeGate_Translation drives gate decisions through REAL translation,
// pinning the enclosure classification the arity walk cannot see from a join
// looking upward:
//   - a maximal 2-way inner join gates ordinal;
//   - the SAME 2-way join nested under a 3-way inner cluster does NOT (it
//     must remain name-model);
//   - a 2-way inner join under an OUTER-join leg roots a fresh cluster and
//     gates; the outer box itself gates too (a binary opaque box);
//   - a 2-way join inside an aggregate (derived GROUP BY) roots a fresh
//     cluster and gates;
//   - a maximal 2-way join under a WHERE-EXISTS filter GATES (the flatten
//     gate arm — the 2+1 select seeds the baked ordinal RC), and a 2-way
//     join INSIDE the EXISTS subquery roots a fresh cluster and gates too.
func TestWedgeGate_Translation(t *testing.T) {
	t.Parallel()
	...
	t.Run("three_way_gathers_flat", func(t *testing.T) {
		t.Parallel()
		// The 3-way root GATES (arity 3) and translates FLAT — the nested
		// inner join is GATHERED into the root's select, so it never seeds at
		// all (no recorded decision IS the invariant: a gathered join has no
		// translateJoin of its own).
		nested := inner(scan("Order", "o"), scan("Customer", "c"))
	...
	t.Run("left_outer_gated_both_legs_fresh", func(t *testing.T) {
		t.Parallel()
		// (a) the LEFT box GATES at the root even with a clustered preserved
		// leg; (b) legs of a GATED parent translate FRESH, so the 2-way inner
		// in the PRESERVED leg gates independently as its own root; (c) the
		...
		if d, ok := tr.wedgeGate[root]; !ok || !d.Gated {
			t.Fatalf("LEFT-outer box: %+v (ok=%v), want recorded and GATED", d, ok)
		}
		if d, ok := tr.wedgeGate[preserved]; !ok || !d.Gated {
			t.Fatalf("2-way in the PRESERVED leg: %+v (ok=%v), want GATED (fresh leg of a gated parent)", d, ok)
		}
	...
	t.Run("full_outer_box_gates_scan_legs_only", func(t *testing.T) {
		...
		// rewritten, never merged — it gates.
	...
	t.Run("full_box_over_gated_join_leg_gates", func(t *testing.T) {
		t.Parallel()
		// `(a JOIN b) FULL JOIN c` -- the FULL box's join leg GATES ITSELF, so
		// the leg is eligible (its output is a typed flat ordinal concat;
		// multi-accessor FieldPaths resolve upper references through it) and
		// the FULL box gates too. Both decisions recorded.
		nested := inner(scan("Customer", "c"), scan("TypedRecord", "t"))
		root := logical.NewJoin(scan("Order", "o"), nested, logical.JoinFull, "")
		tr := newGateTranslator(t)
		tr.translateRef(root)
		if d, ok := tr.wedgeGate[root]; !ok || !d.Gated {
			t.Fatalf("FULL box with a GATED join leg: %+v (ok=%v), want gated", d, ok)
		}
	...
	t.Run("enclosed_outer_box_stays_name_model", func(t *testing.T) {
		...
		// enclosure guard from the start; the outer arms shipped without it.
		// A LEFT/RIGHT box leg is ordinal-ELIGIBLE, so a plain
		// inner parent GATES and the box translates FRESH — the enclosure
		// guard survives VERBATIM but needs a GENUINELY
	...
	t.Run("joined_preserved_gates", func(t *testing.T) {
		t.Parallel()
		// The joined-preserved family — `a JOIN b LEFT JOIN c` — GATES at the
		// box root. The flattened preserved cluster's buried sources are
		// nameable per-leg (binding-keyed windows + positional binders), so
		// the box takes the ordinal seed and its inner(a,b) preserved leg
		// translates as a gated cluster leg.
		ab := inner(scan("Order", "o"), scan("Customer", "c"))
		root := logical.NewJoin(ab, scan("TypedRecord", "t"), logical.JoinLeft, "")
		tr := newGateTranslator(t)
		tr.translateRef(root)
		if d, ok := tr.wedgeGate[root]; !ok || !d.Gated {
			t.Fatalf("joined-preserved LEFT box: %+v (ok=%v), want recorded and GATED", d, ok)
		}
	})

	t.Run("mixed_nesting_leg_ineligible", func(t *testing.T) {
		t.Parallel()
		// `(A JOIN B JOIN C) FULL JOIN D` -- the 3-way inner leg GATES ITSELF
		// (arity 3 >= 2), so it is eligible and the FULL box gates over it;
		// the leg type is the 3-leg flat concat.
		...
		tr.translateRef(root)
		if d, ok := tr.wedgeGate[root]; !ok || !d.Gated {
			t.Fatalf("FULL box over a GATED 3-way leg: %+v (ok=%v), want gated", d, ok)
		}

		// The DERIVED-TABLE-wrapped join leg: the derived body join gates
		// itself (LogicalCTE transparency reports ELIGIBLE for a gating body,
		// but still guards NAME-MODEL bodies).
		...
		if d, ok := tr3.wedgeGate[root3]; !ok || !d.Gated {
			t.Fatalf("FULL box over a DERIVED-wrapped GATING join leg: %+v (ok=%v), want gated", d, ok)
		}
	})
	...
	t.Run("exists_outer_leg_enclosed_subquery_fresh", func(t *testing.T) {
		...
		// The flatten CONSULTS the gate: a maximal 2-way INNER join under
		// WHERE-EXISTS GATES — the flat 2+1 select seeds the baked ordinal RC
		// (flatten_gate_test.go's pins assert the seed itself; this pin holds
		// the recorded decision).
		if d, ok := tr.wedgeGate[outerJoin]; !ok || !d.Gated || d.Arity != 2 {
			t.Fatalf("flattened 2-way join under EXISTS must gate at arity 2: %+v (ok=%v)", d, ok)
		}
	})
	...
	t.Run("having_exists_untranslatable", func(t *testing.T) {
		t.Parallel()
		// HAVING-EXISTS over a 2-way join input:
...
// TestWalkArmParity closes a dimensional gap between two walks that ask the
// SAME classification two different questions: ordinalEligible and
// clusterArity. The LogicalCTE arm once existed in one but not the other —
// each walk had been verified against the rule it shadows, never against its
...
//     while clusterArity still counts inner joins (arity 2) for the SEED
//     walk;
...
func TestWalkArmParity(t *testing.T) {
	...
	cases := []struct {
		...
		{"cte_scoped_scan_join_body", scan("bodyjoin", "b"), true, 2}, // body join gates; composition is legal
		{"filter_plain", logical.NewFilter(scan("Order", "o"), "x > 1"), true, 1},
		{"filter_exists", &logical.LogicalFilter{Input: scan("Order", "o"), ExistsSubqueries: []logical.ExistsSubquery{{Alias: values.NamedCorrelationIdentifier("e")}}}, true, 1}, // an EXISTS rider stays transparent
		{"project_plain", logical.NewProject(scan("Order", "o"), []string{"order_id"}, []string{""}), true, 1},
		{"project_scalar_subq", &logical.LogicalProject{Input: scan("Order", "o"), Projections: []string{"order_id"}, ScalarSubqueries: []logical.ScalarSubquery{{Alias: values.NamedCorrelationIdentifier("s")}}}, true, 1}, // an uncorrelated-scalar rider stays transparent
		{"inner_join", inner(scan("Order", "o"), scan("Customer", "c")), true, 2},                                                                                                                                            // eligible iff the leg itself gates
		{"left_join", logical.NewJoin(scan("Order", "o"), scan("Customer", "c"), logical.JoinLeft, ""), true, 2},                                                                                                             // eligible as a leg; arity preserved+1
...
// TestWhereMergeBakesLegRelative pins the WHERE-merge bake site
...
// TestBoxLegBakeResolvesLeafLocal pins the bake window for a BOX leg: in
// `(o FULL JOIN c) JOIN t ON c.price = t.id` the FULL box enters the binary
// seed as ONE leg NAMED after its rightmost leaf ("c", sourceAlias) and TYPED
// as the {o,c} concat. The bare reference `c.price` addresses THAT LEAF's
// column — but Order also has a PRICE column earlier in the concat, so a
// whole-concat FieldIndex first-match would silently bake ORDER's price
// (wrong column, right range: undetectable at runtime). The bake must
// resolve within the rightmost leaf's own window and offset into the concat.
...
// TestGatedJoinLegTypes_BuriedConsistency pins the two predicate-bake
// legTypes maps in lockstep: the WHERE-pred map (gatedJoinLegTypes) must
// register the SAME buried bake windows for a clustered box leg as the
// seed's own map (ordinalJoinSeedFields via addBuriedBakeWindows). A WHERE
// cross-leg conjunct naming a buried source (`WHERE a.x = c.y` over
// `(A⋈B) LEFT C`) bakes through this map; without the buried entries
// predicateLegAliases counts it single-leg and the reference stays a
// grandchild-correlated lazy read.
func TestGatedJoinLegTypes_BuriedConsistency(t *testing.T) {
	t.Parallel()
	...
	s, ok := legTypes["S"]
	if !ok {
		t.Fatalf("gatedJoinLegTypes lacks the BURIED leg S: %v — the WHERE-pred bake map diverged from the seed's", keysOf(legTypes))
	}
