...
// The ordinal join SEED. When the cluster-arity gate admits a join into the
// wedge (wedgeGateDecision.Gated), translateJoin builds its result value as
// the ORDINAL concatenation of the legs (Java's
...
func (t *cascadesTranslator) ordinalLegType(op logical.LogicalOperator) *values.RecordType {
	...
	// A CLUSTERED box leg's concat records its buried-leg boundaries so the
...
func (t *cascadesTranslator) legScanTableName(op logical.LogicalOperator) string {
	for {
		...
		// had (a divergence), not restore one.
		default:
			return ""
		}
	}
...
// still reads the box's row by its dotted keys — the same Datum also carries
// the positional fields this function derives.
func (t *cascadesTranslator) ordinalLegColumns(op logical.LogicalOperator) []values.Field {
	...
	case *logical.LogicalJoin:
		if un, isUnnest := o.Right.(*logical.LogicalUnnest); isUnnest {
			...
			// `x.sub AS y`). The gate (ordinalWedgeGateDecide) POISONS every
			// unnest-right join, so the
		...
		// under a FULL boundary) contributes the flat ordinal concatenation
		// of ITS legs, in rv (FROM) order — the box's own output type;
		// duplicates survive (positional access). A NAME-MODEL join leg
		// reaching here is still a gate mis-scope: loud, never a silently
		// mis-typed leg.
		...
		if !d.Gated {
			panic("ordinal seed: a NAME-MODEL join leg reached the ordinal seed (cluster-arity gate mis-scope, planner bug)")
		}
...
// the seed types that leg's QOV RECORD-LEVEL nullable — Java's
// `pullUpResultColumnsWithNullability(true)` re-pulls through
// `QOV(alias, type.withNullability(true))`, and OuterJoinExpression VERIFIES
// nullability on the QOV's result type, never per column.
type clusterLeg struct {
	op    logical.LogicalOperator
	alias string
	// binding is the leg's BINDING correlation name (sourceBinding): equal
	// to alias for every non-duplicate leg; the parser-minted `Q$DUPN` for a
	// later duplicate-alias leg. Seed quantifiers, bake maps and windows key
	// on THIS; alias stays display. While the gate's dup-poison arm stands,
	// binding == alias on every leg that reaches a seed.
	binding       string
	nullSupplying bool
}
...
// exact re-derivation hazard a carried-not-rederived field is meant to avoid).
...
// binding — its rightmost leaf's binding + "$BOX". The bare leaf name cannot
// serve both levels: at the box's OWN select it names the leaf quantifier
// (typed by the leaf), while at the parent it would name the box (typed by
// the whole concat), and one plan carries both — the widen invariant's
// divergent-baked-types panic (aggregates over the mixed class). Fold-stable
// and unique (leaf bindings are dup-minted unique) — the Q$DUP discipline:
// internal plumbing users can never reference; display surfaces keep
...
// seeded on their own. Derived-table/filter boundaries stay LEGS and
...
type bakeLegType struct {
	...
	// has no quantifier of its own — the box's single quantifier is named by
	// its RIGHTMOST leaf (the sourceBinding convention), so every other
	// buried leaf bakes onto that correlation at its own offset within the
	// box concat. Empty = the referenced alias itself (every top-level leg
...
// gated inner cluster with nested direct joins (Java's aligned translation
// shape): one ForEach quantifier per gathered leg in FROM order, the flat
// N-leg ordinal seed RC, and the union of every nested join's ON predicates
// with cross-leg conjuncts baked. Legs translate ENCLOSED (their own nested
// content — derived bodies etc. — is inside this cluster). extraPreds
// carries WHERE conjuncts a caller folds INTO the cluster (the existential
// wrap passes the filter's non-EXISTS conjuncts so a comma-join's
...
func (t *cascadesTranslator) translateGatheredInnerCluster(j *logical.LogicalJoin, legs []clusterLeg, extraPreds []predicates.QueryPredicate) expressions.RelationalExpression {
	// Legs of a GATED parent translate FRESH: a leg's own inner joins gate
	// independently and SelectMergeRule composes the leg's ordinal RV into
	// this parent via translateValueCorrelations + the fuse arm — Java's
	// model. (Enclosure poisoning survives only for the few name-model
	// parents still standing: the residual unnest compositions and ungated
	// correlated-scalar outers — recursive-CTE REFERENCE joins gate ordinal,
	// and gated existential selects take the ordinal rebase.)
...
// legTypes map WITHOUT constructing/asserting the RC — the gathered-unnest
...
// correlation (boxCorr — the rightmost-leaf binding the box's QOV is named
// by)}. Nested gated boxes recurse with the
...
func bakeGatedJoinPredicatesChecked(preds []predicates.QueryPredicate, legTypes map[string]bakeLegType) ([]predicates.QueryPredicate, bool) {
	...
	bake := func(v values.Value) values.Value {
		...
		// exactly, and a case-mismatched ON conjunct would silently classify
		// as deeply-correlated instead (unreachable via production SQL,
		// which uppercases upstream; pinned white-box).
		bakeCorr := key
		if legType.bakeCorr != "" {
			// A BURIED leg: the reference binds the box's quantifier —
			// named by its rightmost leaf — not the buried alias, which has
