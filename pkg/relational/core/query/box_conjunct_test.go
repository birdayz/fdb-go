...
// White-box pins for the filtered box unnest: a box-leg WHERE conjunct over a
// gathered ordinal join. A BAKEABLE conjunct (every leg reference resolves in
// its buried window) rides the gathered ordinal path and translates; an
// UNBAKEABLE one (here: a reference the buried window cannot resolve)
// declines LOUDLY — there is no name-model fallback left to lower it to, so
// the decline must surface as a translate error rather than a silent 0-row
// plan. Rows/plan correctness is pinned end-to-end by the sqldriver filtered
// box unnest FDB test.
...
func TestFilteredBoxUnnestCensus(t *testing.T) {
	translate := func(t *testing.T, f *logical.LogicalFilter) (*cascadesTranslator, bool) {
		t.Helper()
	...
	// admits, the merge bakes over the recorded legTypes → the shape translates
	// (the ordinal gather is the sole lowering since the producer deletion).
	t.Run("bakeable_conjunct_ordinalizes", func(t *testing.T) {
		if _, ok := translate(t, b2FilteredBoxShape("ID")); !ok {
	...
	// the shape is untranslatable and LOUD (there is no name-model fallback
	// left to lower the decline to). Proves the Bakeable path above is the
	// verdict ROUTING, not a vacuously-permissive gate.
	t.Run("unresolvable_conjunct_declines_loud", func(t *testing.T) {
		tr, ok := translate(t, b2FilteredBoxShape("NO_SUCH_COL"))
		if ok {
			t.Fatalf("unbakeable filtered box unnest translated — the Unbakeable verdict must decline (correct-or-loud)")
		}
		if tr.translateErr == nil {
			t.Fatalf("unbakeable decline must be LOUD (a translate error), got nil")
		}
	})
	...
	t.Run("classifier_unbakeable_arms", func(t *testing.T) {
		...
		// SCALAR SUBQUERY operand is BAKEABLE: a ScalarSubqueryValue is a LEAF
		// (Children()==nil) — the bake closure leaves it untouched while the
		// sibling leg ref (T4.ID) ordinalizes; the subquery's result is bound by
		// the statement's scalar-subquery pre-eval pass (it was registered in
		// t.scalarSubqueries at predicate translation). So the conjunct bakes
		// and the gather owns the shape. Row correctness for this shape is
	...
	t.Run("classifier_shape_gates", func(t *testing.T) {
		...
		t.Run("nested_box_leg_classifies_bakeable", func(t *testing.T) {
			...
			if _, ok := translate(t, &logical.LogicalFilter{Input: j, Predicate: pred}); !ok {
	...
	// the sibling FDB scenario for a clustered box has path-independent rows,
	// so THIS census arm is the routing discriminator for the clustered
	// dimension.
...
// TestBakeGatedChecked_DriftDetection pins the DRIFT signal that backs the
...
// ref is left lazy (never baked), so it is NOT drift even when its field is
// absent.
func TestBakeGatedChecked_DriftDetection(t *testing.T) {
	t.Parallel()
