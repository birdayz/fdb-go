...
// Shape 1 — the CLUSTERED-OUTER correlated-scalar ordinal seed.
//
...
// classifier and keeps the pre-existing fallback behavior; the guarantee is scoped
// to provable refs.) The fresh-unique outer
// correlation id (the unique-correlation-id principle) structurally rules out the
...
// Limit.LimitValue; Join.OnPredicate (both children recursed — shape 2
...
func (t *cascadesTranslator) translateClusteredOuterScalar(p *logical.LogicalProject, csq logical.CorrelatedScalarSubquery) (expressions.RelationalExpression, bool) {
	...
	// positional and dotted rows. Residual until the enclosing parent
	// ordinalizes.
	if t.inInnerCluster {
		return nil, false
	}
