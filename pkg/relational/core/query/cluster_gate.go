...
// The translation-time cluster-arity scoping gate.
//
// The ordinal wedge covers exactly the joins that are NOT consumed as a
...
// rule is held by the contract pins (ClusterArity_Shapes / WalkArmParity) and
// the seed-side loud asserts — a decline here is forbidden (it would change
// plan shapes).
//
// The gate's per-seed decisions drive the ordinal seed: Gated joins get the
// baked ofOrdinalNumber result value + cross-leg predicate baking. The wedge
// covers N-way inner clusters (arity >= 2) and single-source LEFT/RIGHT boxes,
// and routes EXISTS-over-gated-joins through the ordinal existential rebase. A
// join over a RECURSIVE-CTE REFERENCE (a cteExprScope temp-table scan) gates
// too — the reference is an ordinal-eligible arity-1 leaf typed by
// cteColumnsScope; only the recursive DEFINITION node in leg position stays
// poison, and that shape is production-unreachable (derived-table WITH is
// 42F01). The name model survives only for the PINNED residuals
// (joined-preserved LEFT clusters, unnest declines, ungated correlated-scalar
// outers, dup-alias clusters).
...
// wedgeGateDecision records the ordinal-wedge gate outcome for one
// translateJoin seed, for the ordinal seed to consume and for tests to pin.
...
// in the ordinal wedge, and records the decision. Must be called
...
// existsOuterGatesFresh is the ENCLOSURE LIFT
// predicate: it reports whether a WHERE-EXISTS filter's OUTER input would gate
// ordinal AS A FRESH CLUSTER (the decision routes
...
// LEFT/RIGHT box stayed name-model and its ordinal existential rebase
...
// OUTER C`) is ADMITTED: the seed concats the clustered leg's
...
func (t *cascadesTranslator) ordinalWedgeGateDecide(j *logical.LogicalJoin) wedgeGateDecision {
	...
	if !t.ordinalEligible(j.Left) || !t.ordinalEligible(j.Right) {
		...
		// — an ordinal seed over it would type the leg wrongly. A
		// regression here (`(A JOIN B JOIN C) LEFT JOIN D` — the box gated
		// while its left leg stayed name-model) fires ordinalLegColumns'
		// mis-scope panic exactly as designed.
		return wedgeGateDecision{Arity: arityPoison, Reason: "a leg contains a name-model join"}
	}
	if j.Kind != logical.JoinInner && t.inInnerCluster {
		...
		// The enclosed inner-cluster case above ordinalizes-or-loud once its own
		// flat N-leg seed composes it; the OUTER-box case here still needs the
		// FULL/nested-box residual callers to ordinalize or go loud case-by-case
		// (booked). Keeping this decline holds those shapes
		// on the correct name-model rows until then, rather than stranding them.
		return wedgeGateDecision{Arity: arityPoison, Reason: "outer box enclosed in a name-model parent"}
	}
	...
	if j.Kind != logical.JoinInner {
		...
		// the box root (single-source legs, later widened to CLUSTERED legs):
		...
		// A flattened preserved/null-supplying cluster's buried
		// sources are nameable per-leg via binding-keyed
		...
		return wedgeGateDecision{Gated: true, Arity: 2, Reason: "binary LEFT/RIGHT-outer box (ordinal seed at translation; clustered legs supported)"}
	}
	if t.inInnerCluster {
		...
		// name-model until the ENCLOSED inner-cluster-under-unnest residual
		// ordinalizes. Collapsing this
	...
	a := t.clusterArity(j)
	if a >= 2 {
		// Every maximal inner-join cluster seeds the flat N-leg ordinal RC
		// (Java flattens inner joins at translation, QueryVisitor.java:429-434;
		// the partition rule's positional merge case re-collapses subsets
		// during exploration).
		return wedgeGateDecision{Gated: true, Arity: a, Reason: "maximal inner-join cluster (N-way flat seed)"}
	}
...
// output boundary must not be a NAME-MODEL join's merged row.
// Join-free shapes are eligible (scans, aggregates, unions, sorts — their
// outputs are single-namespace rows the leg adapter synthesizes by name). A
// GATED (ordinal) join leg is ALSO eligible: its output is a typed positional
// row that multi-accessor FieldPaths resolve through. A NAME-MODEL join leg
// (dissolved-LEFT clusters, unnest, mixed derived nesting) stays ineligible:
// its merged row cannot be ordinal-typed.
...
func (t *cascadesTranslator) ordinalEligible(op logical.LogicalOperator) bool {
	switch o := op.(type) {
	case *logical.LogicalJoin:
		// The probe runs Decide with enclosure
		// forced FALSE — a join leg roots a fresh cluster (it sits at an
		// outer-box or leg boundary) — and Decide is side-effect-free.
		//
		// A LEFT/RIGHT box is eligible AS A LEG exactly like any
		// gated join: clusterArity counts the box as preserved + 1 (the
		// rule's actual mergeability), so the parent's arity accounting and
	...
	case *logical.LogicalProject:
		if len(o.CorrelatedScalarSubqueries) > 0 {
			return false // per-row scalar — needs a clusterPullUp extension, booked
		}
		return t.ordinalEligible(o.Input)
	case *logical.LogicalScan:
	...
	case *logical.LogicalCTE:
		...
		// buried join at its leg boundary (a review catch: the walk
...
//
// The classification mirrors the rule's ACTUAL mergeability, not a simpler
// shorthand (two deliberate simplifications below, both failing toward the
// name model when in doubt):
//
...
//   - filter/project WITH rider subqueries: TRANSPARENT
...
//     ordinal seed threads; an uncorrelated scalar is a
//     pre-evaluated root-context binding (shape-agnostic).
//     Neither adds a ForEach quantifier. A CORRELATED projection scalar
//     still POISONS: per-outer-row evaluation needs a dedicated pull-up
//     rework (booked);
...
func (t *cascadesTranslator) clusterArity(op logical.LogicalOperator) int {
	...
	case *logical.LogicalJoin:
		...
		if o.Kind != logical.JoinInner {
			// LEFT/RIGHT OUTER: RewriteOuterJoinRule
			...
			// propagates from the preserved side.
			preserved := o.Left
	...
	case *logical.LogicalFilter:
		...
		// TODO: two PRE-EXISTING reach limits are newly VISIBLE on
		// gated paths (loud 0AF00 on master too): a
	...
	case *logical.LogicalProject:
		if len(o.CorrelatedScalarSubqueries) > 0 {
			...
			// seed cannot express (needs a clusterPullUp extension, booked) —
			// still poison.
			return arityPoison
		}
