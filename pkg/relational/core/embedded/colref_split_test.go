...
func TestParseColRef_SplitsAtDepthZero(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// inputs, and every one is a test row — three appear in this file
		// (`Q'.Z'`, this row, and `X.Y || '.'`) and the fourth, `R'.Z'`, only
		// in the yamsql corpus scenario pinning the SQL-visible side. "Appear
		// in", not "are exclusive to": `Q'.Z'` is in both. "All four are rows in
		// this file" was the first phrasing; the count held up and the locative
		// did not. Zero production decisions change, from the instrumented run
		// rather than from reading the algorithm.
		{
