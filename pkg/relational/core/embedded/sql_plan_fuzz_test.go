...
func FuzzSQLPlan(f *testing.F) {
	...
	seeds := []string{
		...
		"SELECT o.id, (SELECT d.amount FROM (WITH c AS (SELECT id, amount FROM orders) SELECT id, amount FROM c) AS d WHERE d.id = o.id) FROM orders o, orders d",
		"SELECT o.id, EXISTS (SELECT 1 FROM (SELECT * FROM orders WHERE id = 1) AS d WHERE d.id = o.id) FROM orders o, orders d",
		"SELECT o.id + other.id, EXISTS (SELECT 1 FROM (SELECT id FROM orders WHERE id = 1) AS d WHERE d.id = o.id) FROM orders o, orders other",
		"",
		"SELECT",
		"SELECT FROM",
