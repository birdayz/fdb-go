...
func TestLateralCollectionBinding(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"exists_inner", `SELECT ID FROM U WHERE EXISTS (SELECT 1 FROM T1, T1.ARR1 x WHERE x = U.V)`, "T1", []int{1}},
		{"exists_fast_inner", `SELECT ID FROM U WHERE EXISTS (SELECT U.ID FROM T1, T1.ARR1 x)`, "T1", []int{1}},
