...
// for the duplicate-FROM-alias binding-id mint (assignFromLegBindingIDs, the
// single mint authority). Design-ruling properties (RFC-173 item 1): DETERMINISTIC per query
// (position-keyed, never an atomic counter — two parses of the same SQL yield
// identical ids), first-occurrence-keeps-alias (zero change for non-duplicate
// queries), fold-stable `Q$DUPN` form, and structural carry onto the logical
// legs (LogicalScan/LogicalCTE.Binding) — carried, never re-derived.
...
func TestRFC173Item1_MintDuplicateFromAliases(t *testing.T) {
	t.Parallel()
...
// TestRFC173Item1_BindingCarriedOntoLogicalLegs pins the structural carry: the
// logical builder copies the minted id onto the join leg's LogicalScan /
// LogicalCTE — consumers read the carried field, never re-derive from the
// alias (the design ruling's carried-not-rederived condition).
func TestRFC173Item1_BindingCarriedOntoLogicalLegs(t *testing.T) {
	t.Parallel()
