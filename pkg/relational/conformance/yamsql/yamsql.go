...
type Test struct {
	// ID optionally identifies a semantic coverage cell within a scenario.
	...
	Rowcount *int64 `yaml:"rowcount,omitempty"`
	...
	// Args are typed database/sql arguments, passed to the driver unchanged.
	Args []Scalar `yaml:"args,omitempty"`
	// ExactRows distinguishes an absent assertion from an explicitly empty set.
	// It compares driver carriers and IEEE bits instead of numeric coercion.
	ExactRows *[][]Scalar `yaml:"exact_rows,omitempty"`
	// ColumnTypes independently asserts database/sql SQL type names.
	ColumnTypes []string `yaml:"column_types,omitempty"`
	...
	// whose Code matches. Result and plan assertions are incompatible with it.
	...
	// ErrorMessage additionally asserts the exact api.Error message.
	...
	PlanContains string `yaml:"plan_contains,omitempty"`
	...
	PlanNotContains string `yaml:"plan_not_contains,omitempty"`
	...
	AnsiGap []string `yaml:"ansi_gap,omitempty"`
}
...
func (s *Scenario) validate() error {
	...
	for i := range s.Tests {
		...
		switch {
		case t.Query != "" && t.Exec != "":
			return fmt.Errorf("tests[%d]: query and exec are mutually exclusive", i)
		case t.Query == "" && t.Exec == "":
			return fmt.Errorf("tests[%d]: one of query/exec is required", i)
		case t.Exec != "":
			if IsQuery(t.Exec) {
				return fmt.Errorf("tests[%d]: exec is for non-query statements; use query: for %q", i, t.Exec)
			}
			if t.Rows != nil {
				return fmt.Errorf("tests[%d]: exec cannot expect rows", i)
			}
			t.Query, t.Exec = t.Exec, ""
		}
		if t.Rowcount != nil && IsQuery(t.Query) {
			return fmt.Errorf("tests[%d]: rowcount is only valid on non-query statements", i)
		}
		if t.Rowcount != nil && t.EffectiveErrorCode() != "" {
			return fmt.Errorf("tests[%d]: rowcount and error assertions are mutually exclusive", i)
		}
		...
		if (t.Columns != nil && len(t.Columns) == 0) || (t.ColumnTypes != nil && len(t.ColumnTypes) == 0) {
			return fmt.Errorf("tests[%d]: column assertions must be non-empty", i)
		}
		if t.Rows != nil && t.ExactRows != nil {
			return fmt.Errorf("tests[%d]: rows and exact_rows are mutually exclusive", i)
		}
		if t.ErrorMessage != "" && t.EffectiveErrorCode() == "" {
			return fmt.Errorf("tests[%d]: error_message requires an error code", i)
		}
		if t.ErrorCode != "" && t.Error != "" {
			return fmt.Errorf("tests[%d]: error and error_code are mutually exclusive", i)
		}
		if _, err := decodeScalars(t.Args); err != nil {
			return fmt.Errorf("tests[%d]: args: %w", i, err)
		}
		if t.ExactRows != nil {
			for j, row := range *t.ExactRows {
		...
		if t.EffectiveErrorCode() != "" && (t.Rows != nil || t.ExactRows != nil || t.Columns != nil || t.ColumnTypes != nil || t.PlanContains != "" || t.PlanNotContains != "" || t.Unordered) {
			return fmt.Errorf("tests[%d]: result and plan assertions are incompatible with errors", i)
		}
		if !IsQuery(t.Query) {
			if t.PlanContains != "" || t.PlanNotContains != "" {
				return fmt.Errorf("tests[%d]: plan assertions are only valid on queries", i)
			}
			if t.ExactRows != nil || t.Columns != nil || t.ColumnTypes != nil || t.Rows != nil {
				return fmt.Errorf("tests[%d]: result assertions are only valid on queries", i)
			}
			if t.Unordered {
				return fmt.Errorf("tests[%d]: unordered is only valid on queries", i)
			}
		}
	}
	return nil
