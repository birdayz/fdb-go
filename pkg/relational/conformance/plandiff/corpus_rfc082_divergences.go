...
//
// 4.12.11.0 bump (RFC-135): six divergences were LIFTED because Java 4.12 fixed
// the behaviour the annotation pinned, so each now runs as plain cross-engine
// equivalence (verified by the conformance run). Two were map entries removed
// here — `left_outer_join_basic` (4.12 added LEFT OUTER JOIN) and
// `where_case_returns_bool_probe` (4.12 accepts a CASE-bool WHERE predicate).
// The other four were inline `DivergenceJavaWrongRowsGoCorrect` annotations in
// corpus.go that 4.12 corrected: `pk_literal_eq_in_join` and
// `three_way_join_shared_driver` (4.11 dropped ANDed join predicates) and
...
var rfc082Divergences = map[string]Divergence{
	...
	"error_delete_nonexistent_table":    {Direction: DivergenceBothErrorMessagesDrift, Reason: "RFC-082: both engines reject; cosmetic message wording differs (RFC-159: Go now reports a clean 42F01 Unknown table at build time, matching INSERT/SELECT, instead of the old generic 0AF00 'DML Cascades translation failed')", GoErrorContains: "Unknown table"},
	...
	"error_insert_arity_too_few":        {Direction: DivergenceBothErrorMessagesDrift, Reason: "RFC-082: both engines reject; Go ports Java's parseRecordFieldsUnderReorderings wording (ExpressionVisitor.java:1064) so only context wrapping can differ", GoErrorContains: "Value of column \"V\" is not provided"},
	"error_insert_arity_too_many":       {Direction: DivergenceBothErrorMessagesDrift, Reason: "RFC-082: both engines reject; Go ports Java's parseRecordFieldsUnderReorderings wording (ExpressionVisitor.java:1055) so only context wrapping can differ", GoErrorContains: "Too many parameters"},
	// error_not_null_violation: both engines now reject at CREATE with
	// Java's exact "NOT NULL is only allowed for ARRAY column type" (the
	...
	// string_concat_via_plus removed: the Java ADD string family landed
	// (`+` with a string operand concatenates in both engines), so the
	// entry runs equivalence like any other corpus shape.
	...
	"sum_over_string_rejected":         {Direction: DivergenceBothErrorMessagesDrift, Reason: "RFC-082: both engines reject SUM over a non-numeric operand at plan time; Go's numeric-operand gate now emits Java's verbatim SemanticException message", GoErrorContains: "unable to encapsulate aggregate operation due to type mismatch"},
	...
	"update_undefined_column_rejected": {Direction: DivergenceBothErrorMessagesDrift, Reason: "RFC-082: both engines reject; cosmetic message wording differs (Go now reports a clean 42703 at build time, matching INSERT/SELECT, instead of the old leaky executor error)", GoErrorContains: "column \"NO_SUCH_COL\" not found in table"},
