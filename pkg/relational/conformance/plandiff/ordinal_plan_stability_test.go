...
// change. ProjectionLabelIntegrationReference records both live engines' plans
// and pins these queries' rows and SQL labels: Go renders a logical tree here,
// Java a physical pipeline, so their plan text is not an equality oracle.
...
// qualified-alias pins (`P.NAME AS P.NAME`). Explicit bare output aliases now
// accompany unique columns as well; operators, join order and column ordinals
// are unchanged. SQL labels remain independent of duplicate physical keys.
var ordinalSeedPlanGolden = map[string]string{
	"ordinal_3way_chain": `Project(A.ID AS ID, C.NAME AS NAME)
  Filter((A.B_ID#1 = B.ID#0 AND B.C_ID#1 = C.ID#0))
    InnerJoin
      InnerJoin
        ...
        Scan(B)
      Scan(C)`,
	"ordinal_3way_star": `Project(H.ID AS ID, X.V AS V, Y.W AS W)
  Filter((H.ID#0 = X.HID#1 AND H.ID#0 = Y.HID#1))
    InnerJoin
      InnerJoin
        ...
        Scan(X)
      Scan(Y)`,
	"ordinal_self_3way": `Project(G.ID AS ID, P.NAME AS P.NAME, GP.NAME AS GP.NAME)
...
//
// The CROSS-ENGINE half is ProjectionLabelIntegrationReference: the same three
// queries execute against both engines with independently authored rows and
// labels, and both plans are retained. This test separately pins Go's exact
// logical rendering and determinism, not cross-engine plan-text equality.
		...
		t.Run(q.Name, func(t *testing.T) {
			...
			if first != want {
				t.Errorf("%s: ordinal-seed plan SHAPE changed:\n--- want (golden) ---\n%s\n--- got ---\n%s\n"+
					"If intentional, regenerate the golden AND verify the shared contract in ProjectionLabelIntegrationReference.",
					q.Name, want, first)
			}
		})
	}
