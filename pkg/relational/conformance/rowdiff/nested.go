...
type StructCol struct {
	// Name is the physical column, e.g. "N". Dotted column names are
	// "<Name>.<leaf>".
	Name string
	// TypeName is the struct type declared by CREATE TYPE AS STRUCT.
	TypeName string
	// Fields are the leaves in DECLARATION order, which is also the order a
	// positional struct literal in an INSERT must supply them. Each field's
	// Name is the LEAF name ("A"), not the dotted path.
	Fields []ColumnDef
}

// leafOf returns the struct leaf name of a dotted column, and whether the
// column is nested at all.
func leafOf(col string) (string, bool) {
	root, leaf, found := strings.Cut(col, ".")
	if !found {
		return "", false
	}
	_ = root
...
func (t TableDef) structDDL() string {
	...
	fmt.Fprintf(&b, "CREATE TYPE AS STRUCT %s (", strings.ToLower(t.Struct.TypeName))
...
func (t TableDef) structLiteral(r Row) string {
	...
	for i, f := range t.Struct.Fields {
		...
		b.WriteString(renderLiteral(r[t.Struct.Name+"."+f.Name]))
	}
	b.WriteString(")")
	return b.String()
}
...
//
// The struct is intentionally FLAT-LEAVED (depth 1). A struct inside a struct
// is generated by nothing here because the engine resolves exactly two dotted
// segments today: `walkColumnRef` refuses a 3-segment FullId, which is the
// known RFC-204 §4.4 representation gap pinned by
// TestFDB_NestedPathDepthGate. Generating three-segment paths would produce a
// batch in which every single candidate errors, which is not coverage — the
// gap is measured by that pin and by TestFDB_NestedProjectionAxes, and this
// generator grows the deeper leaves the day it closes.
...
func (c *Case) nestedProjections() [][]string {
	...
	// A nested leaf beside the FLAT column its last segment collides with.
	collision := []string{"ID", "A", "N.A"}
	...
	return [][]string{twoLeaves, collision, wide, reversed}
}
