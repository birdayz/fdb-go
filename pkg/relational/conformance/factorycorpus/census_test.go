...
func TestRatchetDetectsEveryShrinkDirection(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"blessing downgraded", func(c *factorycorpus.Census) { c.ByBlessing["cross-engine"] = 0; c.ByBlessing["metamorphic"] = 10 }},
	} {
		tc := tc
...
// TestStratifiedSampleCoversEveryDimension pins what the per-PR tier's sample
// is for. A uniform sample of a corpus dominated by one query family tests
// that family n times and everything else zero times — the volumetric fallacy
// the whole design is arranged against.
func TestStratifiedSampleCoversEveryDimension(t *testing.T) {
	t.Parallel()
	files, err := factorycorpus.LoadDir(factorycorpus.TestdataDir)
	if err != nil {
		t.Fatalf("LoadDir: %v", err)
	...
	all := map[string]bool{}
	for _, f := range files {
		all[f.Header.FeatureVector] = true
	}
	...
	for _, f := range sample {
		covered[f.Header.FeatureVector] = true
	}
	want := len(all)
	if n < want {
		want = n
	...
	if len(covered) < want {
		t.Errorf("sample of %d covers %d of %d feature vectors; a stratified sample must reach every dimension "+
			"before it repeats one", n, len(covered), len(all))
	}

	// Stability: the per-PR gate must run the SAME tests every time, or a real
	// regression becomes an intermittent.
	again := factorycorpus.StratifiedSample(files, n)
	for i := range sample {
