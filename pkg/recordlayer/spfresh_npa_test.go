...
var _ = Describe("SPFresh NPA reassignment", func() {
	...
	It("moves a boundary vector to a nearer split child (§6 step 3)", func() {
		...
		Expect(spfreshNPARun(ctx, sharedDB, storage, config, "npa-test", bFine)).To(Succeed())
		...
		Expect(spfreshNPARun(ctx, sharedDB, storage, config, "npa-test", bFine)).To(Succeed(), "re-run on a cleared task is a no-op")
	...
	It("a zombie NPA task (parent HDR gone) is deleted as a no-op", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(spfreshNPARun(ctx, sharedDB, storage, config, "npa-test", 12345)).To(Succeed())
		_, err = sharedDB.Run(ctx, func(rtx *FDBRecordContext) (any, error) {
