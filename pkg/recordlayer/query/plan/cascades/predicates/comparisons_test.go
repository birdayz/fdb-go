...
func TestComparison_Eval_BoolEquality(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := mustEval(Comparison{Type: tc.op, Operand: values.LiteralValue(tc.r)}, tc.l)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparison_Eval_NumericPromotion(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := mustEval(Comparison{Type: tc.op, Operand: values.LiteralValue(tc.rhs)}, tc.left)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparison_Eval_In(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := mustEval(Comparison{Type: ComparisonIn, Operand: values.LiteralValue(tc.list)}, tc.left)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparison_Eval_In_OnlyNullList(t *testing.T) {
	t.Parallel()
	got := mustEval(Comparison{Type: ComparisonIn, Operand: values.LiteralValue([]any{nil, nil})}, int64(5))
	if got != TriUnknown {
...
func TestComparison_Eval_In_NonSliceRHS(t *testing.T) {
	t.Parallel()
	got := mustEval(Comparison{Type: ComparisonIn, Operand: values.LiteralValue(int64(5))}, int64(5))
	if got != TriUnknown {
...
func TestComparison_Eval_In_CrossTypeLHS(t *testing.T) {
	...
	mustEval(Comparison{Type: ComparisonIn, Operand: values.LiteralValue([]any{int64(1), int64(2), int64(3)})}, "hello")
}
...
func TestComparison_Eval_Like(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := mustEval(Comparison{Type: ComparisonLike, Operand: values.LiteralValue(tc.pattern)}, tc.s)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparison_Eval_Like_TypeMismatch(t *testing.T) {
	t.Parallel()
	if got := mustEval((Comparison{Type: ComparisonLike, Operand: values.LiteralValue("abc")}), int64(5)); got != TriUnknown {
		t.Fatalf("got %v", got)
	}
	if got := mustEval((Comparison{Type: ComparisonLike, Operand: values.LiteralValue(int64(5))}), "abc"); got != TriUnknown {
		t.Fatalf("got %v", got)
	}
}
...
func TestComparison_Eval_StartsWith(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := mustEval(Comparison{Type: ComparisonStartsWith, Operand: values.LiteralValue(tc.rhs)}, tc.left)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparison_Eval_IsNullIsNotNull(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := mustEval(Comparison{Type: tc.op}, tc.left)
			if got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
...
func TestComparisonPredicate_IsNull_NonConstantLHS(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := mustEval(isNull, tc.row); got != tc.wantNull {
				t.Errorf("IS NULL: got %v, want %v", got, tc.wantNull)
			}
			if got := mustEval(isNotNull, tc.row); got != tc.wantNotNull {
...
func TestComparison_Eval_Strings(t *testing.T) {
	...
	if got := mustEval(c, "a"); got != TriTrue {
		t.Fatalf("a < b: got %v", got)
	}
	if got := mustEval(c, "c"); got != TriFalse {
...
func TestComparisonPredicate_EndToEnd(t *testing.T) {
	...
	if got := mustEval(pred, row); got != TriTrue {
		t.Fatalf("age=21 >= 18: got %v", got)
	}
	row["age"] = int64(15)
	if got := mustEval(pred, row); got != TriFalse {
		t.Fatalf("age=15 >= 18: got %v", got)
	}
	row["age"] = nil
	if got := mustEval(pred, row); got != TriUnknown {
...
func TestComparisonPredicate_NilOperand(t *testing.T) {
	...
	if got := mustEval(pred, nil); got != TriUnknown {
...
func TestComparisonPredicate_ComposesWithKleeneConnectives(t *testing.T) {
	...
	if got := mustEval(tree, row); got != TriTrue {
		t.Fatalf("AND: got %v", got)
	}
	row["rank"] = int64(7)
	if got := mustEval(tree, row); got != TriFalse {
...
func TestComparisonPredicate_ArithmeticOperand(t *testing.T) {
	...
	if got := mustEval(pred, map[string]any{"a": int64(5), "b": int64(7)}); got != TriTrue {
		t.Fatalf("5+7=12 > 10: got %v", got)
	}
	if got := mustEval(pred, map[string]any{"a": int64(3), "b": int64(4)}); got != TriFalse {
		t.Fatalf("3+4=7 > 10: got %v", got)
	}
	// NULL propagation: a=NULL -> a+b=NULL -> UNKNOWN.
	if got := mustEval(pred, map[string]any{"a": nil, "b": int64(1)}); got != TriUnknown {
...
func TestComparisonPredicate_NonConstantRHS(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := mustEval(pred, tc.row); got != tc.want {
				t.Fatalf("got %v, want %v", got, tc.want)
			}
		})
	}
...
func TestComparisonPredicate_IsDistinctFrom_NonConstantRHS(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := mustEval(dist, tc.row); got != tc.wantDist {
				t.Errorf("IS DISTINCT FROM: got %v, want %v", got, tc.wantDist)
			}
			if got := mustEval(notDist, tc.row); got != tc.wantNotDist {
...
func TestComparisonPredicate_NonConstantRHS_Arithmetic(t *testing.T) {
	...
	if got := mustEval(pred, map[string]any{"a": int64(5), "b": int64(4)}); got != TriTrue {
		t.Fatalf("5 = 4+1: got %v", got)
	}
	if got := mustEval(pred, map[string]any{"a": int64(5), "b": int64(5)}); got != TriFalse {
...
func TestComparison_Eval_LikeWithEscape(t *testing.T) {
	...
	if got := mustEval(c, "a%b"); got != TriTrue {
		t.Errorf("a%%b: got %v, want TRUE", got)
	}
	if got := mustEval(c, "axb"); got != TriFalse {
