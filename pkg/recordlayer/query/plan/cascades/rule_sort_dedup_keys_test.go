...
func TestSortDedupKeysRule_RemovesDuplicateKey(t *testing.T) {
	...
	a, b := sortRewriteField(q, 0), sortRewriteField(q, 1)
	keys := []expressions.SortKey{
		{Value: a, Reverse: false},
		{Value: b, Reverse: false},
		{Value: a, Reverse: false}, // dup of [0]
	}
	src := sortRewriteSort(keys, q)
	ref := expressions.InitialOf(src)
	yielded := fireSortRewriteRule(t, NewSortDedupKeysRule(), ref)
	if len(yielded) != 1 {
	...
	// Order preserved: a, b.
	if fv, ok := values.AsFieldValue(got[0].Value); !ok || fv.DisplayName() != "a" {
		t.Errorf("got[0] = %v, want FieldValue(a)", got[0].Value)
	}
	if fv, ok := values.AsFieldValue(got[1].Value); !ok || fv.DisplayName() != "b" {
...
func TestSortDedupKeysRule_AllUnique_NoFire(t *testing.T) {
	...
	q := expressions.ForEachQuantifier(expressions.InitialOf(scan))
	keys := []expressions.SortKey{
		{Value: sortRewriteField(q, 0)},
		{Value: sortRewriteField(q, 1)},
	}
	src := sortRewriteSort(keys, q)
	ref := expressions.InitialOf(src)
	yielded := fireSortRewriteRule(t, NewSortDedupKeysRule(), ref)
...
func TestSortDedupKeysRule_CooperatesWithConstantKeysElim(t *testing.T) {
	...
	progress, converged := exploreSortRewriting(NewPlanner(rules, nil), ref)
	if !converged {
...
func TestSortDedupKeysRule_FixpointTerminates(t *testing.T) {
	...
	src := sortRewriteSort(keys, q)
	ref := expressions.InitialOf(src)
	progress, converged := exploreSortRewriting(NewPlanner([]ExpressionRule{NewSortDedupKeysRule()}, nil), ref)
	if !converged {
