...
func TestLogicalSort_Unsorted(t *testing.T) {
	...
	s := mustExpression(UnsortedLogicalSortExpression(q))
	if !s.IsUnsorted() {
	...
	qov, ok := values.AsQuantifiedObjectValue(s.GetResultValue())
	if !ok || qov.Correlation() != q.GetAlias() {
...
func TestLogicalSort_Construction(t *testing.T) {
	...
	s := mustExpression(NewLogicalSortExpression(keys, q))
	if s.IsUnsorted() {
...
func TestLogicalSort_DefensiveCopy(t *testing.T) {
	...
	s := mustExpression(NewLogicalSortExpression(src, q))
	src[0].Reverse = true
	if s.GetSortKeys()[0].Reverse {
...
func TestLogicalSort_EqualsWithoutChildren(t *testing.T) {
	...
	s1 := mustExpression(NewLogicalSortExpression(keys1, q))
	s1Twin := mustExpression(NewLogicalSortExpression([]SortKey{{Value: values.NewBooleanValue(true), Reverse: false}}, q))
	s2 := mustExpression(NewLogicalSortExpression(keys2, q))
	s3 := mustExpression(NewLogicalSortExpression(keys3, q))
