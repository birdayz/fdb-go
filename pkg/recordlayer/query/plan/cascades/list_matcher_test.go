...
func TestListMatcher_PairsByPosition(t *testing.T) {
	...
	cv0 := &ConstantValue{Value: int64(1), Typ: TypeInt}
	...
	b := got[0]
	if Get[*ConstantValue](b, d0) != cv0 {
		t.Fatal("d0 didn't bind cv0")
	}
	if Get[*FieldValue](b, d1) != fv {
		t.Fatal("d1 didn't bind fv")
	}
	if Get[*ConstantValue](b, d2) != cv2 {
...
func TestListMatcher_LengthMismatch_ReturnsEmpty(t *testing.T) {
	...
	short := []any{&ConstantValue{Value: int64(1), Typ: TypeInt}}
...
func TestListMatcher_NonSliceInput_ReturnsEmpty(t *testing.T) {
	...
	if got := m.BindMatches(NewBindings(), &ConstantValue{Value: int64(1), Typ: TypeInt}); got != nil {
...
func TestListMatcher_ThreadsOuterBindings(t *testing.T) {
	...
	m := NewListMatcher(NewAnyValue())
	in := []any{&ConstantValue{Value: int64(1), Typ: TypeInt}}
