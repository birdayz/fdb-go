...
func mustRFC153VerifierConstruct[T any](value T, err error) T {
	if err != nil {
		panic("construct RFC-153 verifier fixture: " + err.Error())
	}
	return value
}

func rfc153VerifierRowType() *values.RecordType {
	return values.NewRecordType("RFC153VerifierRow", false, []values.Field{{
		Name:      "id",
...
func rfc153BuriedFieldOfA() values.FieldValue {
	...
	root := mustRFC153VerifierConstruct(values.NewQuantifiedObjectValue(aliasA, rfc153VerifierRowType()))
	field := mustRFC153VerifierConstruct(values.ResolveFieldOrdinals(root, []int{0}))
	result, ok := values.AsFieldValue(field)
	if !ok {
		panic("construct RFC-153 verifier fixture: resolved id is not a field value")
	}
	return result
}
...
func TestRFC153_Verifier_RecognizedCleanInner_NoDecline(t *testing.T) {
	...
	bareScan := mustRFC153VerifierConstruct(plans.NewRecordQueryScanPlan(
		[]string{"C"}, rfc153VerifierRowType(), false))
	...
	litProbe := mustRFC153VerifierConstruct(plans.NewRecordQueryIndexPlan(
		"c_idx", []*predicates.ComparisonRange{litRange},
		[]string{"C"}, rfc153VerifierRowType(), false))
...
func TestRFC153_Verifier_RecognizedSargWithBuriedRef_Declines(t *testing.T) {
	...
	buriedProbe := mustRFC153VerifierConstruct(plans.NewRecordQueryIndexPlan(
		"c_idx", []*predicates.ComparisonRange{buriedRange},
		[]string{"C"}, rfc153VerifierRowType(), false))
...
// TestRFC153_Verifier_UnrecognizedNode_FailsClosed: THE bug this verifier
// exists to catch. A node type the rebaser's walker does NOT rewrite (here a nested
...
func TestRFC153_Verifier_UnrecognizedNode_FailsClosed(t *testing.T) {
	t.Parallel()
	resVal := mustRFC153VerifierConstruct(values.NewQuantifiedObjectValue(
		values.UniqueCorrelationIdentifier(), rfc153VerifierRowType()))
	cleanScan := mustRFC153VerifierConstruct(plans.NewRecordQueryScanPlan(
		[]string{"C"}, rfc153VerifierRowType(), false))
	...
	nljWithBuried := mustRFC153VerifierConstruct(plans.NewRecordQueryNestedLoopJoinPlan(
	...
	cleanNLJ := mustRFC153VerifierConstruct(plans.NewRecordQueryNestedLoopJoinPlan(
	...
	wrapped := mustRFC153VerifierConstruct(plans.NewRecordQueryDefaultOnEmptyPlan(
		nljWithBuried, values.NewNullValue(rfc153VerifierRowType())))
...
func TestRFC153_Verifier_PassThroughsAreTransparent(t *testing.T) {
	...
	cleanScan := mustRFC153VerifierConstruct(plans.NewRecordQueryScanPlan(
		[]string{"C"}, rfc153VerifierRowType(), false))
	doe := mustRFC153VerifierConstruct(plans.NewRecordQueryDefaultOnEmptyPlan(
		cleanScan, values.NewNullValue(rfc153VerifierRowType())))
	tf := mustRFC153VerifierConstruct(plans.NewRecordQueryTypeFilterPlan([]string{"C"}, doe))
