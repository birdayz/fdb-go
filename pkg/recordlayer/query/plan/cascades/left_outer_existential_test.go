...
func leftOuterExistentialField(
	t testing.TB,
	alias values.CorrelationIdentifier,
	typ values.Type,
	ordinal int,
	...
	t.Helper()
	root := leftOuterExistentialQOV(t, alias, typ)
	var field values.Value
	var err error
	if frontierPinned {
		field, err = values.ResolveOrdinalSeedField(root, ordinal)
	} else {
...
func TestRebaseOuterLegValueOrdinal_LegLocalFrontierPinned_UsesBakedOrdinal(t *testing.T) {
	...
	mergedQOV := leftOuterExistentialQOV(
		t, values.NamedCorrelationIdentifier("M"), mergedType)
	...
	ref := leftOuterExistentialField(
		t, values.NamedCorrelationIdentifier("L"), dupLeg, 1, true)
	...
	if !isFV {
		t.Fatalf("rebase produced %T, want exact FieldValue", out)
	}
	if fv.Path() == nil || fv.Path().Len() != 1 {
		t.Fatalf("rebased ref is not a single-accessor path: %+v", fv.Path())
	}
	...
	if !isQOV || childQOV.Correlation() != mergedQOV.Correlation() {
		t.Fatalf("rebased ref child = %v, want merged QOV %v", fv.ChildValue(), mergedQOV.Correlation())
	}
}
...
func TestRebaseOuterLegValueOrdinal_LegLocalOrdinalPastWindow_Declines(t *testing.T) {
	...
	wideFields := make([]values.Field, 6)
	for i := range wideFields {
		wideFields[i] = values.Field{
	...
	wideFields[5].Name = "Q"
	wideLeg := values.NewRecordType("WideLeg", false, wideFields)
	ref := leftOuterExistentialField(
		t, values.NamedCorrelationIdentifier("L"), wideLeg, 5, true)
...
func TestRebaseOuterLegValueOrdinal_MergedRefPassesThrough(t *testing.T) {
	...
	baked := leftOuterExistentialField(
		t, mergedQOV.Correlation(), mergedType, 12, true)
...
func TestRebaseOuterLegValueOrdinal_DriftBetweenWindowsAndMergedLegs_Declines(t *testing.T) {
	...
	ref := leftOuterExistentialField(t, driftCorr, legType, 0, false)
	expectedOuterRoots := map[values.CorrelationIdentifier]struct{}{driftCorr: {}}

	if _, ok := rebaseOuterLegValueOrdinal(ref, windows, mergedQOV, expectedOuterRoots); ok {
	...
	innerType := values.NewRecordType("Inner", false, []values.Field{
		{Name: "IK", FieldType: values.NullableInt, Ordinal: 0},
	})
	innerRef := leftOuterExistentialField(t, innerCorr, innerType, 0, false)
	out, ok := rebaseOuterLegValueOrdinal(innerRef, windows, mergedQOV, expectedOuterRoots)
	if !ok {
	...
	forgedMergedQOV := leftOuterExistentialQOV(
		t, values.NamedCorrelationIdentifier("M"), forgedType)
	forgedRefType := values.NewRecordType("Forged", false, []values.Field{
		{Name: "FK", FieldType: values.NullableInt, Ordinal: 0},
	})
	forgedRef := leftOuterExistentialField(
		t, values.NamedCorrelationIdentifier("Q$11"), forgedRefType, 0, false)
	if _, ok := rebaseOuterLegValueOrdinal(forgedRef, windows, forgedMergedQOV, expectedOuterRoots); !ok {
