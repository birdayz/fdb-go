...
func TestTranslationMap_MergeCaseMiniature(t *testing.T) {
	...
	// A LAZY enclosing reference does NOT fuse — fuse requires both the
	// enclosing node and its replacement to be baked. The child is swapped
	// in place and the node stays lazy over the baked replacement, evaluated
	// as a chain.
