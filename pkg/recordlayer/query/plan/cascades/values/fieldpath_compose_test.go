...
// Pins for the multi-accessor FieldPath substrate (Java FieldValue.FieldPath,
// FieldValue.java:373) and the baked-gated compose rule
// (ComposeFieldValueOverFieldValueRule.java:57-69). Multi-accessor paths are
// constructible only through the compose rule, which is gated to fully-baked
// chains — a lazy chain (today's canonical nested access) never fuses, so
// these tests exercise the fuse machinery directly against the shapes the
// compose rule builds.
...
// TestFieldPath_WithSuffix_ImmutableFuse pins Java FieldPath.withSuffix
...
// TestFieldPath_ComposeGate pins the compose rule's gating: it fuses
...
// would rewrite memo identity and Explain renderings everywhere). Mixed
// chains (lazy over baked, baked over lazy) stay unfused too.
func TestFieldPath_ComposeGate(t *testing.T) {
	t.Parallel()
	qov := s3NestedQOV(t)
...
// TestFusedPath_Evaluate pins the fused node's runtime semantics
...
func TestFusedPath_Evaluate(t *testing.T) {
	...
	// proto.Message or a positional row. A pinned fused path over a map
	// nested value is LOUD, never a silent name read.
	row := &fakeOrdinalRow{
	...
	if _, err := fused.Evaluate(row); !errors.As(err, &mapORE) {
		t.Fatalf("pinned fused descent into a name-keyed map = %v, want loud *OrdinalResolutionError (no map name descent for a fused step)", err)
	}
	// A LAZY chain cannot compute this value: the inner step (a lazy
	// QOV-child "NESTED") carries no plan-time ordinal and there is no
	// runtime name->ordinal resolution, so it fails LOUD
	...
	if _, err := lazyChain.Evaluate(&fakeOrdinalRow{names: []string{"NESTED", "W"}, slots: []any{nestedOrd, int64(9)}}); !errors.As(err, &lazyORE) {
		t.Fatalf("lazy chain eval = %v, want loud *OrdinalResolutionError (an unbaked step does not resolve by name)", err)
	}
	...
	// contract (*BakedNameContextError) or the unbound-context tail
...
// TestFusedPath_CorrelatedContexts is a trap pin on the ordinal substrate:
// evaluateCorrelated is the fused node's PRIMARY eval path (the compose
// rule's only constructible output is fused-over-QOV), and it must descend
// from the ROOT step's ORDINAL — reading the display name (the last step,
// "Y") at the top level would return the planted trap value. The node is
// UNPINNED so the guard does not mask a misread. The correlation binds to
// the authoritative ordinal row (the sole runtime row); both the bare
...
// dot-joined, '#'-escaped — rendering keeps names; identity does not).
...
// TestAssertOrdinalJoinSeed_RejectsFusedPath pins the ordinal-join seed
// contract under the path substrate: the seed is SINGLE-accessor by
// construction — a fused path in a seed RC means the compose gate failed,
// and AssertOrdinalJoinSeed panics rather than letting a mis-shaped seed
// reach the executor.
...
// TestJoinSeed_DupBareNameMemoIdentity pins the memo-identity requirement for
// join ordinalization: a positional join seed carrying DUPLICATE bare names
// (two legs each with a column "X") keeps them as DISTINCT memo members iff
// the references are BAKED. writeSemanticHash forks FieldValue identity on
// Resolved (semantic_hash.go:120-139): baked → ordinal PATH, lazy → NAME
// bucket. So join ordinalization is memo-safe as long as it emits BAKED dup
// refs (exactly as the positional wrap does); a LAZY name-only ref is the
// ONLY way to conflate two logically-different columns into one memo member
// → wrong plans. This pins that mechanism as a regression.
func TestJoinSeed_DupBareNameMemoIdentity(t *testing.T) {
	t.Parallel()
	...
	// address the columns POSITIONALLY. That is precisely why join ordinalization binds
	// by baked ordinal, not bare name. The two legs' X columns sit at qualified slots
	// A_X (0) and B_X (1).
	...
	if SemanticHashCode(bx0) == SemanticHashCode(bx1) {
		t.Fatal("baked dup-bare-name refs at different ordinals must be DISTINCT memo members")
	}
