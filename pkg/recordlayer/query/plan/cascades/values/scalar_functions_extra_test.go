...
func TestEvalScalarFunction_FloorCeilRound(t *testing.T) {
	t.Parallel()
	if got := evalSF("FLOOR", []any{float64(2.7)}); got != int64(2) {
		t.Fatalf("FLOOR(2.7): got %v", got)
	}
	if got := evalSF("CEIL", []any{float64(2.1)}); got != int64(3) {
		t.Fatalf("CEIL(2.1): got %v", got)
	}
	if got := evalSF("CEILING", []any{float64(-1.2)}); got != int64(-1) {
		t.Fatalf("CEILING(-1.2): got %v", got)
	}
	if got := evalSF("ROUND", []any{float64(2.5)}); got != int64(3) {
		t.Fatalf("ROUND(2.5): got %v", got)
	}
	if got := evalSF("ROUND", []any{float64(2.49)}); got != int64(2) {
		t.Fatalf("ROUND(2.49): got %v", got)
	}
	// ROUND(x, decimals) — float result when fractional part remains.
	got := evalSF("ROUND", []any{float64(3.14159), int64(2)})
	...
	if got := evalSF("FLOOR", []any{int64(7)}); got != int64(7) {
...
func TestEvalScalarFunction_SqrtPower(t *testing.T) {
	t.Parallel()
	if got := evalSF("SQRT", []any{float64(16)}); got != float64(4) {
		t.Fatalf("SQRT(16): got %v", got)
	}
	if got := evalSF("SQRT", []any{int64(9)}); got != float64(3) {
	...
	if got := evalSF("POWER", []any{int64(2), int64(3)}); got != int64(8) {
		t.Fatalf("POWER(2,3): got %v", got)
	}
	// POW alias.
	if got := evalSF("POW", []any{int64(2), int64(10)}); got != int64(1024) {
		t.Fatalf("POW(2,10): got %v", got)
	}
	// POWER with float result.
	if got := evalSF("POWER", []any{float64(2), float64(0.5)}); got != math.Sqrt2 {
		t.Fatalf("POWER(2,0.5): got %v", got)
	}
	// Domain error → decline.
	if got := evalSF("POWER", []any{int64(0), int64(-1)}); got != nil {
...
func TestEvalScalarFunction_CoalesceNullif(t *testing.T) {
	t.Parallel()
	if got := evalSF("COALESCE", []any{nil, nil, "third", "ignored"}); got != "third" {
		t.Fatalf("COALESCE: got %v", got)
	}
	if got := evalSF("COALESCE", []any{nil, nil}); got != nil {
		t.Fatalf("COALESCE all-null: got %v", got)
	}
	if got := evalSF("COALESCE", []any{int64(5)}); got != int64(5) {
		t.Fatalf("COALESCE single: got %v", got)
	}
	// NULLIF: equal → NULL; unequal → first arg.
	if got := evalSF("NULLIF", []any{int64(5), int64(5)}); got != nil {
		t.Fatalf("NULLIF(5,5): got %v, want nil", got)
	}
	if got := evalSF("NULLIF", []any{int64(5), int64(6)}); got != int64(5) {
		t.Fatalf("NULLIF(5,6): got %v", got)
	}
	// Cross-numeric promotion mirrors embedded.
	if got := evalSF("NULLIF", []any{int64(5), float64(5)}); got != nil {
		t.Fatalf("NULLIF(5,5.0): got %v, want nil", got)
	}
	// Incomparable types → not equal.
	if got := evalSF("NULLIF", []any{"x", int64(5)}); got != "x" {
		t.Fatalf(`NULLIF("x",5): got %v`, got)
	}
	// Null first arg → NULL.
	if got := evalSF("NULLIF", []any{nil, int64(5)}); got != nil {
...
func TestEvalScalarFunction_TrimFamily(t *testing.T) {
	t.Parallel()
	if got := evalSF("TRIM", []any{"  hello  "}); got != "hello" {
		t.Fatalf("TRIM: got %v", got)
	}
	if got := evalSF("LTRIM", []any{"  hello  "}); got != "hello  " {
		t.Fatalf("LTRIM: got %v", got)
	}
	if got := evalSF("RTRIM", []any{"  hello  "}); got != "  hello" {
		t.Fatalf("RTRIM: got %v", got)
	}
	if got := evalSF("TRIM", []any{nil}); got != nil {
		t.Fatalf("TRIM(NULL): got %v, want nil", got)
	}
	if got := evalSF("TRIM", []any{int64(5)}); got != nil {
...
func TestEvalScalarFunction_Substring(t *testing.T) {
	t.Parallel()
	// 1-based position.
	if got := evalSF("SUBSTRING", []any{"hello", int64(2)}); got != "ello" {
		t.Fatalf("SUBSTRING(hello, 2): got %v", got)
	}
	if got := evalSF("SUBSTRING", []any{"hello", int64(2), int64(3)}); got != "ell" {
		t.Fatalf("SUBSTRING(hello, 2, 3): got %v", got)
	}
	// pos < 1 normalises to 1.
	if got := evalSF("SUBSTRING", []any{"hello", int64(-1)}); got != "hello" {
		t.Fatalf("SUBSTRING(hello, -1): got %v", got)
	}
	// pos past end → empty string.
	if got := evalSF("SUBSTRING", []any{"hi", int64(10)}); got != "" {
		t.Fatalf("SUBSTRING(hi, 10): got %v, want empty", got)
	}
	// Multibyte rune handling — `é` is one rune, two bytes.
	if got := evalSF("SUBSTRING", []any{"café", int64(4)}); got != "é" {
		t.Fatalf("SUBSTRING(café, 4): got %v, want é", got)
	}
	// SUBSTR alias.
	if got := evalSF("SUBSTR", []any{"world", int64(1), int64(3)}); got != "wor" {
...
func TestEvalScalarFunction_Replace(t *testing.T) {
	t.Parallel()
	if got := evalSF("REPLACE", []any{"hello world", "world", "Go"}); got != "hello Go" {
		t.Fatalf("REPLACE: got %v", got)
	}
	// Repeat replacement.
	if got := evalSF("REPLACE", []any{"aaa", "a", "bb"}); got != "bbbbbb" {
		t.Fatalf("REPLACE repeat: got %v", got)
	}
	// NULL `to` → empty (drop matches), mirrors embedded.
	if got := evalSF("REPLACE", []any{"abc", "b", nil}); got != "ac" {
		t.Fatalf("REPLACE NULL to: got %v", got)
	}
	// NULL string / NULL from → NULL.
	if got := evalSF("REPLACE", []any{nil, "x", "y"}); got != nil {
