...
// readable positionally; any name-based resolution collapses the two
...
//	    (no name fallback);
//	(3) over a non-ordinal context there is no name model to fall back to —
//	    the read is a LOUD *UnboundEvalContextError, and a flat name read
//	    over the ordinal row is likewise loud, never a silent NULL;
...
//	    columns.
func TestFieldValue_ResolvedOrdinal(t *testing.T) {
	t.Parallel()
	...
	// Control: a flat (name) read has NO plan-time ordinal, so
	...
	// (3) Non-ordinal context: a resolved-ordinal read has no name model to
	// fall back to. A nothing-matched eval over a non-ordinal context is a
	// loud *UnboundEvalContextError, never a silent NULL.
...
// TestFieldValue_ExplainOrdinalEscape pins the '#'-escape that keeps
// ExplainValue injective over (field text, ordinal). Plan identity is
// semantic, not rendering-based, but a quoted identifier may legally contain
// '#' (DOUBLE_QUOTE_ID accepts any non-quote character), so without the
// escape a plain name-read of a field literally named "X#0" would render
// identically to an ordinal read of X at slot 0 — debugging output
// collapsing two DIFFERENT reads. The same doubling keeps
// writeSemanticHash's FieldValue discriminator injective (a collision there
// is only a shared hash bucket — equality is structural and stays sound —
// but injective is strictly better). Raw field text has '#' DOUBLED; only an
// ordinal read ends in an unpaired '#'+digits.
