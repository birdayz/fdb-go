...
// AssertOrdinalJoinSeed is the LOUD ordinal-join seed-shape validator: the
// translator calls it on every ordinal join RC it builds, where the pristine
// shape IS guaranteed by construction — every field a BAKED FieldValue over a
// leg QuantifiedObjectValue flowing a *RecordType, consecutive full-coverage
// leg runs with baked ordinals 0..width-1 ascending. Any violation panics: at
// the SEED a malformed ordinal RC is unconditionally a planner bug
// (strictness lives at seed-time, where legitimate result-value rewrites —
// wrapper merges, folded projections, partial coverage — cannot yet have
// happened; the executor's cursor-side ordinalJoinSpans probe DECLINES those
// shapes instead of panicking).
//
...
func AssertOrdinalJoinSeed(rc *RecordConstructorValue) {
	if rc == nil || len(rc.Fields) == 0 {
		panic("ordinal join seed malformed: empty RC")
	}
	type run struct {
	...
	if len(runs) < 2 {
		panic(fmt.Sprintf("ordinal join seed malformed: %d leg runs — a join seed concatenates at least two legs", len(runs)))
	}
	for _, r := range runs {
		if r.width != len(r.legType.Fields) {
			panic(fmt.Sprintf("ordinal join seed malformed: leg %s run covers %d columns but its leg type has %d — the seed concatenates FULL legs", r.alias, r.width, len(r.legType.Fields)))
		}
	}
}
