...
func TestChargeCoverage_AllBufferPaths(t *testing.T) {
	...
	// 2. boundedBuffer.Append directly. (DELETE echo is not separately
	...
	advanced(t, "nljCursor hash-index", func(t *testing.T) {
		...
		c := mustNLJCursor(t, outer, innerRows, plans.JoinInner, values.NamedCorrelationIdentifier("O"), values.NamedCorrelationIdentifier("I"), preds, nil, EmptyEvaluationContext(), st)
		if c.hashIndex == nil {
