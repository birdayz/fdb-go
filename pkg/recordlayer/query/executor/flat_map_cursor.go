...
func (c *flatMapCursor) OnNext(ctx context.Context) (recordlayer.RecordCursorResult[QueryResult], error) {
	...
	for {
		...
		// bindMergedOuterLegs used to run here: a Go-only widening that bound EVERY
		// leg of a merged outer row under its own correlation, so a leg-correlated
		// read could resolve by NAME. Java has no such namespace — its FlatMap binds
		// the chained outer→inner PAIR and nothing else
		// (RecordQueryFlatMapPlan.java:135-140) — because where its planner collapses
		// sources into one quantifier it re-anchors every sibling reference by
		// ORDINAL and the alias ceases to exist (PartitionSelectRule.java:296-303).
		//
		// The producer of those merged multi-leg rows was the NLJ rule's
		// three-quantifier arm, retired by RFC-235. Measured before removal over the
		// whole real-FDB corpus: the binder bound 15,032 windows across 155 distinct
		// shapes and was READ ZERO TIMES — down from 3 reads across 303 shapes while
		// the arm existed. It ran once per OUTER ROW, so this was work on the
		// row-rate path with no consumer.
