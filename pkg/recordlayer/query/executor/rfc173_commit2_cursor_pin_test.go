...
// RFC-173 S4 commit 2 (C) — the CURSOR-side heterogeneity pin. The three
// white-box gate pins in the cascades package are all PLANNER-side: they prove
// the step-1 seed is built and gated. They do NOT reach
// the cursor's legWindowRowContext branch (computeResultLegs) — the actual
// heterogeneity absorption, the code path that reverted twice. And a functional
// FDB test cannot pin it: the step-1 merged row carries a coexistence Datum with
// QUALIFIED "A.V" keys (datumFromSpans), so a revert of the cursor branch to the
// name-model Datum fallback still resolves the fold's dotted reads correctly and
// the matrix stays green — the R1 blind spot, one layer down on the cursor
// dimension. This pin makes the positional path LOAD-BEARING: the outer row's
// Datum is EMPTY, so the fold's dotted AND QOV leg refs can only resolve through
// the positional row's leg windows. Correct output proves the branch fired; a
// revert to the Datum fallback yields NULLs and fails.
...
func TestRFC173Commit2_CursorResolvesFoldPositionally(t *testing.T) {
	t.Parallel()
