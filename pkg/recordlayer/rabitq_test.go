			...
			for _, nb := range exBits {
				t.Run("", func(t *testing.T) {
					...
					if data[0] != vectorTypeRABITQ {
						t.Fatalf("expected type ordinal %d, got %d", vectorTypeRABITQ, data[0])
					}
		...
		for _, dim := range dimensions {
			for _, nb := range exBits {
				t.Run("", func(t *testing.T) {
					...
					q := NewRaBitQuantizer(VectorMetricCosine, nb)
					encoded := q.Encode(v)

					est := NewRaBitEstimator(VectorMetricCosine, nb)
		...
		for _, dim := range dimensions {
			for _, nb := range exBits {
				t.Run("", func(t *testing.T) {
					...
					q := NewRaBitQuantizer(VectorMetricInnerProduct, nb)
					encoded := q.Encode(v)

					est := NewRaBitEstimator(VectorMetricInnerProduct, nb)
					result := est.EstimateDistance(v, encoded)
	...
	for _, seed := range seeds {
		for _, dim := range dims {
			t.Run("", func(t *testing.T) {
				...
				q := NewRaBitQuantizer(VectorMetricEuclidean, 7)
				encoded := q.Encode(v)
...
func TestAllSameValueVector(t *testing.T) {
	...
	q := NewRaBitQuantizer(VectorMetricEuclidean, 5)
	encoded := q.Encode(v)
...
func TestEstimatorCosineZeroQuery(t *testing.T) {
	...
	q := NewRaBitQuantizer(VectorMetricCosine, 4)
	encoded := q.Encode(v)

	est := NewRaBitEstimator(VectorMetricCosine, 4)
...
func TestQuantizerPanicsOnInvalidExBits(t *testing.T) {
	...
	for _, nb := range []int{0, -1, 9, 100} {
		func() {
			...
			NewRaBitQuantizer(VectorMetricEuclidean, nb)
		}()
	}
}
...
func TestEstimatedDistanceSpecialValues(t *testing.T) {
	...
	quantizer := NewRaBitQuantizer(VectorMetricEuclidean, 7)
	encoded := quantizer.Encode(v)

	estimator := NewRaBitEstimator(VectorMetricEuclidean, 7)
	result := estimator.EstimateDistance(q, encoded)
