...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/relational/core/embedded/logical_predicate.go:6376":          {1, "contract: aggregate group-key output name, same contract family"},
	"pkg/relational/core/query/cascades_translator.go:3763":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	"pkg/relational/core/query/cascades_translator.go:993": {1, "dotted: groupByOutputBaker asks whether a reference is qualified by comparing its name to stripColumnQualifier of itself, then re-looks-up the leaf; same shape as cascades_generator.go:4355"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4355": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
