...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/recordlayer/query/plan/cascades/values/values.go:948": {1, "boundary: the same nested-record descent as :930, one attempt further down. protoFieldByName now also tries the ESCAPED spelling of the accessor name (protoname.ToProtoBufCompliantName), because a descriptor emitted from a SQL identifier stores the escaped form and no case folding maps `a$b` onto `a__1b` -- without it a mangled field name resolved to nothing and read back as a silent NULL. It is the SAME debt as :930 and retires with it: the fix is to resolve the nested path to field NUMBERS once at the boundary, at which point neither spelling attempt exists"},
	"pkg/recordlayer/query/plan/cascades/values/values.go:930": {1, "boundary: descendResolvedPath's nested-record step resolves a ResolvedAccessor's per-step name against a PROTO DESCRIPTOR (protoFieldByName, called with acc.Field at :884) -- three spellings tried, then a case-insensitive scan. Newly visible: the name crossed a plain string parameter. This is the documented survivor of the accessor name (values.go's own contract: the name lives on ONLY for nested descent into a proto.Message or nested record map). The hedge this entry used to carry -- 'it may well be correct', 'the fix IF THERE IS ONE' -- is REFUTED by reading the reference: Java descends this exact step by ORDINAL. FieldValue.eval calls MessageHelpers.getFieldValueForFieldOrdinals (FieldValue.java:169), which indexes getFields().get(ordinal) and THROWS out of range (MessageHelpers.java:170-175) rather than missing quietly; the name is consumed once at construction, via recordType.getFieldNameToOrdinalMap() with RECORD_DOES_NOT_CONTAIN_FIELD when absent (FieldValue.java:284-290). The name-taking overload exists in MessageHelpers but FieldValue.eval is not a caller, so the fix is not conditional and is exactly the one already named: resolve the nested path to field NUMBERS once at the boundary. Also wider than one step -- protoFieldByName has three callers (values.go:884, :1102, :1106). Recorded rather than allowlisted, and now for a stronger reason than before: the name is NOT the identity here even at the descriptor layer, so an allowlist entry would assert something the reference contradicts. What still gates the conversion is the nested-descent audit -- ResolvedAccessor already carries an Ordinal, but whether it equals the proto descriptor's DECLARATION INDEX at every producer is unproven, and guessing it is a silent wrong-column read"},
	...
	"pkg/recordlayer/query/plan/cascades/values/values.go:1816": {1, "contract: explainValueOrdinals returns the rendered DISPLAY text of a value, and for a FieldValue that text is its leaf name — the rendering ProjectionColumnName (:1492) and every output-naming authority above it delegate to. Same producer family; surfaced once ReplaceAll joined nameLaunderers, since the '#'-doubling escape is what carried the name past the walk"},
	"pkg/recordlayer/query/plan/cascades/values/values.go:1803": {1, "contract: same renderer, the un-suffixed (ColumnNameValue) arm — which DROPS the '#ordinal' discriminator, so two baked reads of duplicate-named slots render identically. That is the collision this bucket is about, in the authority itself"},
	...
	"pkg/relational/core/query/cascades_translator.go:4829":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:5284": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
	...
	"pkg/relational/core/embedded/cascades_generator.go:4206": {1, "dotted: MINT. Composes 'CORR.FIELD' as the lookup key into the null-supplying-window metadata map, so a QOV-addressed reference and a flat one reach the same nullability answer. The correlation is RIGHT THERE in the expression being joined — the key could be the identity pair rather than its rendering, which makes this the cheapest of the five to convert and the one whose conversion proves nothing about the others"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4315": {1, "translator: parsed column ref matched against declared inner columns"},
