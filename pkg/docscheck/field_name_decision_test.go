...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/relational/core/query/cascades_translator.go:4733":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	// qualification probe.
	"pkg/recordlayer/query/plan/cascades/left_outer_existential.go:127":           {1, "dotted: leg-relative vs qualified ref probed via '.' in the name"},
	...
	"pkg/relational/core/query/cascades_translator.go:5760":         {1, "translator: QUALIFIER segment of a parsed identifier, single-ForEach flat baker -- guarded by `Child != nil || Resolved != nil → bail` at 5755, so the only value reaching the slice is a lazy carrier minted from parsed text, and a match emits NewFieldValueWithResolvedOrdinalInDomain (born-baked). The LEAF segment of the same identifier is the entry at 5766; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5766":         {1, "translator: single-ForEach flat baker scans the layout's declared column list for the parsed leaf and emits NewFieldValueWithResolvedOrdinal; same resolve-then-bake shape as 5928, reached through a local leaf"},
	"pkg/relational/core/query/cascades_translator.go:5808":         {1, "translator: QUALIFIER segment of a parsed identifier, multi-ForEach leg baker (bakeDottedRefsToLegQOV) -- same `Child != nil || Resolved != nil → bail` guard at 5801, so it sees only lazy carriers from parsed text, and the leg it selects bakes through legBake into NewCorrelatedFieldValueWithResolvedOrdinalInDomain. Its leaf segment is invisible to this gate -- see the plain-string-parameter blind spot in the header above, which is why only the qualifier half was ever recorded; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5938":         {1, "translator: QUALIFIER segment of a parsed identifier, bakeFlatRefsAgainstColumns leg-window arm -- same bail guard at 5922, born-baked on match via NewFieldValueWithResolvedOrdinalInDomain. The LEAF segment of the same identifier is the entry at 5946; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5946":         {1, "translator: multi-leg baker, column membership within the matched leg window"},
	...
	"pkg/relational/core/query/cascades_translator.go:3847":         {1, "translator: element slot lookup during translation (laundered map key)"},
	"pkg/relational/core/query/cascades_translator.go:5040":         {1, "translator: pullUpSortKeyValue resolves a bare ORDER BY key against the FOLDED projection's output fields -- guarded by `Child == nil && Resolved == nil` at 5038, so the key side is a lazy carrier from parsed text, and a match emits NewFieldValueWithResolvedOrdinal. Gated on cascades_translator.go:4723, NOT on values.go:1510: this site's `fields` are named from p.Projections/p.Aliases (parser text) or a positional `_i`, and the ONLY .Field-derived names in them are the hidden sort columns collectExtraSortColumns appends, each named by sortKeyFieldRef's strings.ToUpper(fv.Field) at 4733 -- a contract-bucket entry. Converting ProjectionColumnName leaves this one red"},
	"pkg/relational/core/query/cascades_translator.go:5928":         {1, "translator: column list membership during resolution"},
