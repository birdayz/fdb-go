...
var fieldIndexBlindSpotDebt = map[string]string{
	...
	"pkg/recordlayer/query/plan/cascades/leg_local_bake_census.go:367": "name-keyed: DIAGNOSTICS ONLY — classifies a census witness, never reaches a plan. The identically-shaped call that DID reach a plan (the leg-local bake) was deleted; this one is retained because the two residues it separates have different fixes. Retires with the census.",
}
