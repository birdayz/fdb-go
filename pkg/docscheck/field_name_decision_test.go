...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/relational/core/embedded/cascades_generator.go:5146": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
	...
	"pkg/relational/core/embedded/cascades_generator.go:4068": {1, "dotted: MINT. Composes 'CORR.FIELD' as the lookup key into the null-supplying-window metadata map, so a QOV-addressed reference and a flat one reach the same nullability answer. The correlation is RIGHT THERE in the expression being joined — the key could be the identity pair rather than its rendering, which makes this the cheapest of the five to convert and the one whose conversion proves nothing about the others"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4177": {1, "translator: parsed column ref matched against declared inner columns"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4191": {1, "translator: same inner-column lookup as the sibling entry above, leg-qualified arm -- the map key is a CONCATENATION, which is why that sibling was recorded and this one was not"},
	"pkg/relational/core/embedded/cascades_generator.go:4197": {1, "translator: inner-column lookup by parsed name (laundered map key)"},
