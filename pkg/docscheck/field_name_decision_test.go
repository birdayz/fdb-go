...
var fieldIndexBlindSpotDebt = map[string]string{
	...
	"pkg/recordlayer/query/plan/cascades/leg_local_bake_census.go:653": "name-keyed: DIAGNOSTICS ONLY — classifies a census witness, never reaches a plan. The identically-shaped call that DID reach a plan (the leg-local bake) was deleted; this one is retained because the two residues it separates have different fixes. Retires with the census.",
...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/recordlayer/query/plan/cascades/values/values.go:1827": {1, "contract: explainValueOrdinals returns the rendered DISPLAY text of a value, and for a FieldValue that text is its leaf name — the rendering ProjectionColumnName (:1492) and every output-naming authority above it delegate to. Same producer family; surfaced once ReplaceAll joined nameLaunderers, since the '#'-doubling escape is what carried the name past the walk"},
	"pkg/recordlayer/query/plan/cascades/values/values.go:1829": {1, "contract: same renderer, the un-suffixed (ColumnNameValue) arm — which DROPS the '#ordinal' discriminator, so two baked reads of duplicate-named slots render identically. That is the collision this bucket is about, in the authority itself"},
	...
	"pkg/relational/core/query/cascades_translator.go:4866":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:5292": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
	...
	"pkg/recordlayer/query/plan/cascades/rule_implement_nested_loop_join.go:2943": {1, "dotted: declines re-qualifying an already-dotted ref; Child is a live QOV, so this is the qualified-name channel, not the legacy flat shape"},
