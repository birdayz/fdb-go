...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/recordlayer/query/plan/cascades/rule_implement_nested_loop_join.go:2338": {1, "dotted: declines re-qualifying an already-dotted ref; Child is a live QOV, so this is the qualified-name channel, not the legacy flat shape"},
	...
	"pkg/relational/core/query/ordinal_seed.go:764":                               {1, "dotted: leg-ref detection via '.' probe on the merged-QOV leg.col channel"},
	...
	"pkg/relational/core/query/cascades_translator.go:5744":         {1, "translator: QUALIFIER segment of a parsed identifier, single-ForEach flat baker -- guarded by `Child != nil || Resolved != nil → bail` at 5739, so the only value reaching the slice is a lazy carrier minted from parsed text, and a match emits NewFieldValueWithResolvedOrdinalInDomain (born-baked). The LEAF segment of the same identifier is the entry at 5750; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5750":         {1, "translator: single-ForEach flat baker scans the layout's declared column list for the parsed leaf and emits NewFieldValueWithResolvedOrdinal; same resolve-then-bake shape as 5908, reached through a local leaf"},
	"pkg/relational/core/query/cascades_translator.go:5792":         {1, "translator: QUALIFIER segment of a parsed identifier, multi-ForEach leg baker (bakeDottedRefsToLegQOV) -- same `Child != nil || Resolved != nil → bail` guard at 5785, so it sees only lazy carriers from parsed text, and the leg it selects bakes through legBake into NewCorrelatedFieldValueWithResolvedOrdinalInDomain. Its leaf segment is invisible to this gate -- see the plain-string-parameter blind spot in the header above, which is why only the qualifier half was ever recorded; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5918":         {1, "translator: QUALIFIER segment of a parsed identifier, bakeFlatRefsAgainstColumns leg-window arm -- same bail guard at 5902, born-baked on match via NewFieldValueWithResolvedOrdinalInDomain. The LEAF segment of the same identifier is the entry at 5926; retired upstream by CQ-52"},
	"pkg/relational/core/query/cascades_translator.go:5926":         {1, "translator: multi-leg baker, column membership within the matched leg window"},
	...
	"pkg/relational/core/query/cascades_translator.go:5908":         {1, "translator: column list membership during resolution"},
