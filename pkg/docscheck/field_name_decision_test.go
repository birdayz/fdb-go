...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/relational/core/query/cascades_translator.go:4948":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:5300": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
	...
	"pkg/relational/core/embedded/cascades_generator.go:4214": {1, "dotted: MINT. Composes 'CORR.FIELD' as the lookup key into the null-supplying-window metadata map, so a QOV-addressed reference and a flat one reach the same nullability answer. The correlation is RIGHT THERE in the expression being joined — the key could be the identity pair rather than its rendering, which makes this the cheapest of the five to convert and the one whose conversion proves nothing about the others"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4323": {1, "translator: parsed column ref matched against declared inner columns"},
	...
	"pkg/relational/core/query/cascades_translator.go:5973":   {1, "translator: LEAF segment of a parsed identifier escaping as a bare string, leg baker's segmentsOf fallback -- reached only when the carrier arrived with NO parse-tree segments (ref.Present false), so it is the un-migrated producers' path; guarded by `Child != nil || Resolved != nil → bail` at the caller, and every match downstream emits a born-baked ordinal. Retires when the remaining LogicalProject producers carry ProjectionRefs"},
	"pkg/relational/core/query/cascades_translator.go:5977":   {1, "translator: QUALIFIER segment of the same fallback slice as 5742 -- one site now serves BOTH the single-ForEach and multi-ForEach arms, which previously sliced separately at :5769 and :5839. The leg-window comparisons it feeds are invisible to this gate (plain-string parameters into legWindowSlot / legBake)"},
	"pkg/relational/core/query/cascades_translator.go:6267":   {1, "translator: bakeSegmentedColumnRef's exact first-match of the SEGMENTED carrier's rendered name against the output column list -- the name is still what selects the slot, so it is still debt, but it is no longer the name that decides QUALIFICATION: that comes from the parser's segment count. Same resolve-then-bake shape as 6130, on the converted path"},
	"pkg/relational/core/embedded/cascades_generator.go:4337": {1, "translator: same inner-column lookup as the sibling entry above, leg-qualified arm -- the map key is a CONCATENATION, which is why that sibling was recorded and this one was not"},
	"pkg/relational/core/embedded/cascades_generator.go:4343": {1, "translator: inner-column lookup by parsed name (laundered map key)"},
