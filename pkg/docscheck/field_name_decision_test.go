...
var knownFieldDecisionDebt = map[string]fieldDebt{
	...
	"pkg/relational/core/embedded/logical_predicate.go:6195":          {1, "contract: aggregate group-key output name, same contract family"},
	"pkg/relational/core/query/cascades_translator.go:4781":           {1, "contract: sort-key hidden-field naming (RFC-141), same output-naming contract family"},
	...
	"pkg/relational/core/query/cascades_translator.go:1008": {1, "dotted: groupByOutputBaker asks whether a reference is qualified by comparing its name to stripColumnQualifier of itself, then re-looks-up the leaf; same shape as cascades_generator.go:4348"},
	...
	"pkg/relational/core/embedded/cascades_generator.go:4348": {1, "dotted: asks whether a group-key name is QUALIFIED by comparing it to its own bare form, then labels the output column from the answer; the flat representation is the debt, not the comparison"},
	//
	...
	"pkg/relational/core/embedded/cascades_generator.go:3379": {1, "translator: parsed column ref matched against declared inner columns"},
