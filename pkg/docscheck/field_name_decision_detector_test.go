...
func TestFieldDecisionsInsideClosuresAreReported(t *testing.T) {
	...
	// RFC-232 and the full scalar-query conversion retired the closure-local
	// compatibility readers. One is the measured production population; the
	// synthetic closure fixtures above pin traversal independently, so a future
	// population decrease remains distinguishable from the walker skipping
	// FuncLits.
	const closureSiteFloor = 1
