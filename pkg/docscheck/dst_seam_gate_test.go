...
var seamAllowlist = map[string]string{
	...
	"pkg/recordlayer/store_state_cache.go:getCachedSubspaceKeys: time.Now": "idle stamp bounding the derived-subspace-keys cache; the cached value is a pure function of the subspace, so eviction only costs a recompute and no byte depends on the clock",
	...
	"pkg/recordlayer/query/plan/cascades/values/values.go:StatementNow: time.Now":  "nil-Clock arm of RowEvalContext.StatementNow; every SQL-seeded evaluation carries the Execute-time statement instant (paginatingRows.statementTime → EvaluationContext → frontier wrap / ordinalJoinBuild.Clock), so only clockless non-SQL callers reach this fallback",
	"pkg/recordlayer/query/executor/evaluation_context.go:StatementNow: time.Now":  "unstamped-context arm of EvaluationContext.StatementNow; the SQL layer stamps the Execute-time statement instant (WithStatementTime from paginatingRows.statementTime, all pages), so only clockless non-SQL callers reach this fallback",
