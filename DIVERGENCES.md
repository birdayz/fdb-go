...
A read with no producer fires the alarm; a read once the bake is back is CQ-53 phase 2 working and must not. The one exclusion is a REGISTRATION, not a list: `TestFDB_MergedLegBinding_ReaderShapeIsRedundant` registers the one (alias, merged-row layout) it measured, only on its passing path, so a divergence between the two routes fails it, empties the registry, and turns those same reads into a red gate in the same run — the license cannot outlive its proof. A whole-suite floor of ≥1 read guards the other end: a dead read counter would otherwise report "0 READ" beside a full bind tally, which is exactly the shape of the claim this entry quotes.
...
- *shadowing* is never observed either: the corpus's only reads are of windows that displaced NOTHING (`6 multi-leg, 6 of those UNSHADOWED` in the census line), so no lookup ever chooses between a leg and an enclosing binding.
