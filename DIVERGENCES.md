...
| **`WithPrimaryKeyIntersector` is forward-PK-only and discards `requestedOrderings`** (vs Java's rich common-ordering gate) | Every intersection leg shares the directional comparison ordering consumed by the sorted merge | safe optimization miss today; partial reverse widening would become wrong rows | **SAFE PARTIAL** — RFC-181's explicit PK-monotonicity gate rejects range-first, vector, reverse, and otherwise incompatible legs before plan construction; unbakeable keys also decline at plan time. RFC-190.5a replaces the magic 3-way loop with bounded generic `ChooseK` enumeration without widening that contract. **TRACKED (190.5b):** atomically port rich common-order derivation, PK compatibility, redundancy proof, directional comparison parts, and reverse through plan/executor/ordering/rewrites; do not admit DESC legs piecemeal. |
