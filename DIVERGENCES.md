...
It was framed as blocked on CQ-63, on the grounds that a leg's quantifier states no row type and so there is no domain to state the ordinal in. **That gate is now closed:** `underivableLegs` in the leg-local bakeability census is **0 of 936 leg derivations** over the real-FDB corpus (it read 82 of 846 when this entry was written). Every leg states its row. What remains is not a missing type — it is that Go's two-level NLJ→FlatMap lowering reaches this cursor **without a stated merged layout** on the EXISTS-over-join and RFC-153 buried-leg paths, so there is no merged ordinal to re-anchor TO. `MergedReAnchor` in the same census measures that directly: **0 of 162** firings take the ordinal re-anchor; all 162 take the leg-alias pass-through.
