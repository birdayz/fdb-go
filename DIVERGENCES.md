...
- **102 firings — OPEN, and the LARGER block. It is a SHAPE residue, not a typing one, and that correction is measured.** A FlatMap leg flowing a bare `QuantifiedObjectValue` states no row layout, so there is nothing to position. **All 174 of this entry's reads belong to this block.**
  ...
  - **The population is 100% TYPED.** The outcome census's witness now spells the flowed type, and every declined leg carries a real `RecordType`: arity 2 ×30, arity 3 ×66, arity 1 ×4, plus arity 4 ×2 on a `RecordQueryNestedLoopJoinPlan` leg. There is nothing to type. The witness previously printed a boolean derived from `Typ != nil`, which no constructible QOV can make false, so the population read as typed for the wrong reason and would have read as *converted* on the day a typing sweep started.
  ...
  - **The producer is measured, not inferred.** All four non-test `RecordQueryFlatMapPlan` constructions are instrumented and each declined leg is attributed by its result value's own identity: every FlatMap-legged decline originates at `buildCorrelatedFlatMapPlan`, which emits **zero** untyped QOVs (25,644 constructions: 476 typed QOV, 6,732 positional-merge RC, 18,436 other). The structural hypothesis that the residue does *not* come from the minting site is confirmed.

  **A real and separate Java divergence survives, on a different population.** Three sites do hand FlatMap an untyped QOV — `implementExistentialSelect` 1,685, `yieldExistsFlatMap` 269, `implementJoinWithExistential` 249 — and Java cannot express any of them: `QuantifiedObjectValue.of` has no untyped overload (`QuantifiedObjectValue.java:187`) and `Quantifier.getFlowedObjectType` is a `Verify.verify` plus `requireNonNull` (`Quantifier.java:801-810`). None of those values reaches the decline classifier, so typing them advances THIS entry by zero reads. It is floored in the producer census so it stays counted while it stands.
