...
- **94 firings — OPEN, and the LARGER block.** A FlatMap leg flowing a bare `QuantifiedObjectValue` states no row layout, so there is nothing to position. RFC-200 fenced this on evidence (its §Residues): the untyped constructor at the obvious site is one of four producers and structurally not the main one, the type is unavailable precisely where it is needed, and typing it propagates into `GetFlowedObjectType`'s member agreement — where `DisagreeingLegs` and `UnderivableLegs` are asserted as hard zeros — and into the `correlatedStep1` wall. **All 174 of this entry's reads belong to this block.**
...
**This entry therefore retires with the 94, not before.** The 60 were a necessary structural step — the authority can now express a nested row at all, and the executor twin agrees with it bit-for-bit — but on the read axis this divergence is exactly where it was.

A further blocker was found while probing the 60 end-to-end and is pinned in `pkg/relational/sqldriver/nested_merge_leg_wrong_rows_fdb_test.go`: the projected-EXISTS fold over a THREE-source FROM — the shape that reaches the nested window — does not execute, failing LOUDLY with `multi-leg row cannot serve a source-relative ordinal … no frontier row resolved`. Measured with the nested acceptance disabled as well, so it is pre-existing and not an RFC-200 regression; it is loud rather than silent, so no wrong row reaches a user. It is what makes RFC-200's gate (a) unmeasurable today.
...
- presence: `TestFDB_MergedLegBinding_ReaderShapeIsRedundant` runs that exact query down both routes, once with the window serving the read and once with `EvaluationContext.WithMergedLegReadBypass` declining it, and asserts the rows agree;
