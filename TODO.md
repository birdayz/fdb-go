...
- [ ] **CRITICAL** — Pure Go FDB client wire protocol crashes FDB server (SIGSEGV). Root cause: our FlatBuffers serializer (`writer_direct.go` + codegen in `main.cpp`) produces a different byte layout than C++ `ObjectWriter` (`flat_buffers.h`). CGo stacktester passes same workloads. See `pkg/fdbgo/client/CRASH_BUG.md` for full analysis. Ground truth test vectors from C++ ObjectWriter in `pkg/fdbgo/wire/types/testdata.json`. Branch: `binding-tester-conformance`.
  ...
  **Fixed (codegen):**
  - [x] Bug 1: Nested struct serialization order reversed vs C++. Fixed in `main.cpp` `emitMeasureEndOff`/`emitWriteDirect`/`emitMarshalFDB`.
  - [x] Bug 2: `Optional<T>` fields completely skipped in marshal path. Fixed: presence tag + `WriteBytesOOL` for value.

  **Remaining (CRITICAL — blocks all pure Go client use):**
  - [ ] Bug 3: Empty `dynamic_size` fields not serialized. C++ `PrecomputeSize::visitDynamicSize` allocates 4 bytes for empty vectors (sentinel). Our `MeasureBytesOOL(endOff, nil)` returns `endOff` unchanged. Also need C++'s `emptyVector` re-use optimization. Fix in `writer_direct.go`.
  - [ ] Bug 4: Generated nil-guards skip fields C++ always serializes. `if m.Field != nil` suppresses entire field including vtable offset. C++ `serializer(ar, ...)` visits every field. Fix in codegen `main.cpp` — remove nil-guards on OOL writes, always emit reloff.
  - [ ] Bug 5: `CommitTransactionRequest` Go output LARGER than C++ (+40/+88 bytes). `Vector<struct>` blob sizing (`blobSize`/`writeBlob`) differs from C++. Needs step-by-step comparison.
  - [ ] Bug 6: VTable packing 2-byte padding difference. `VTableSet.pack()` produces 50 bytes, C++ produces 52. Comes from `save_with_vtables` layout (padding between footer and vtable region).
  - [ ] Mechanical port: replace `MeasureObject`/`WriteObject`/`PatchRelOff` with `PrecomputeSize`/`WriteToBuffer`/`SaveWithVTables` (ported in `serializer.go`, referencing `flat_buffers.h` by line). Update codegen to emit calls to new primitives. Validate all 10 ground truth test vectors byte-identical.
