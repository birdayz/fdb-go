...
- [ ] **InJoin plan selection** — InJoinRule fires correctly (produces InJoin(IndexScan) plans). Root cause is a **reference-structure issue**: InComparisonToExplodeRule creates a SelectExpression; during exploration, a restructuring rule duplicates it into a separate Memo Reference with a different explode alias. Data access generation produces the correlated IndexScan for the FIRST reference but NOT the second. The projection's child quantifier follows the second reference → extraction never sees the IndexScan variant. Fix: ensure partial-match propagation for inner filter references across all Memo References during data access generation. PreserveOrdering fallback added to InJoinRule so IndexScan partitions are included when they exist.
