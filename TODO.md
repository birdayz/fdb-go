...
- [~] **B0 — Phase 4.0 Type hierarchy**: seed landed nightshift-50, follow-ons landed swingshift-52. Now in `cascades/values/type.go` (1264 LOC):
  ...
  - Structured-type recursion in MaximumType: ARRAY element-by-element ✅, RECORD field-by-field ✅ (with Java's exact name-resolution rules) — both landed swingshift-52. ENUM/RELATION recursion still pending.
  - **Remaining**: type inference on `Value.Type()` so it returns rich Type instead of the legacy ValueType enum (largest remaining piece — every Value impl needs RichType()); plan-serialisation hooks (proto encoding for plan-cache); ENUM/RELATION recursion in MaximumType (small). When `cascades/values/type.go` exceeds ~1500 LOC (currently 1311), split into `cascades/typing/` per RFC-025. Sized 1–2 shifts to "complete enough to retire ValueType".
