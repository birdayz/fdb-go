...
- [ ] **Qualified GROUP BY on derived table** (discovered nightshift-42): `SELECT col1 FROM (SELECT col1 FROM t1) AS x GROUP BY x.col1` produces 2 groups (correct count) but the SELECT list emits NULL for each — projection resolution misses the column because the grouped-row map keys don't include the unqualified `col1`. Java's groupby-tests.yamsql line 117 passes this. Unqualified GROUP BY on the same shape works fine. Not related to the nightshift-42 alias-rewrite; the projection path itself needs to reconcile qualified group key names with the group-row map keys.
