...
- [ ] **Recursive CTEs** — `WITH RECURSIVE t(n) AS (...)`. Grammar accepts the keyword; our CTE evaluator errors 42F01 after swingshift-41 unwrapped the CTE error path (was 22023 via generic wrap). **Java HAS this**: `RecursiveUnionCursor` + `RecordQueryRecursiveLevelUnionPlan` in `fdb-record-layer-core`. Significant: needs iterative execution loop with seed + recursive branches, fixpoint detection. Worth doing.
...
- [ ] **CI cache invalidation on PR runs** (surfaced swingshift-41). Master push CI runs "Build, lint & test" in ~7s (fully cached). PR runs on the SAME final content take ~2m because 5 FDB-container tests (conformance, client, fdb, recordlayer, chaos) re-execute. Manual runs as the `runner` user on the Hetzner machine hit cache perfectly (~3s). So the miss is PR-specific — likely the synthetic merge commit that actions/checkout lays down has a file stat (mtime/ownership/mode) that Bazel treats as distinct from master's, invalidating test actions even when content is identical. Try: (a) re-run same PR twice with no changes — second run should ALSO miss if this theory is right, (b) diff `stat` output for `pkg/testcontainers/foundationdb/` between a PR checkout and master checkout, (c) add `--explain=/tmp/e.log --verbose_explanations` to CI's `bazelisk test` and grep for the reason, (d) consider a remote `disk_cache` to decouple from runner filesystem state. **NOTE**: swingshift-41 accidentally ran `bazelisk clean --expunge_async` on the runner while investigating — cache is cold. Next CI invocation will do a full rebuild (~15-20 min) to rewarm; that's a one-time cost.
