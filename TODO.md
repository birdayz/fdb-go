...
- [ ] **HIGH** — `cursor_combinators.go:578-586`: `FlatMapPipelinedCursor` continuation loses outer position when stopping mid-inner on the **first** outer value. Root cause: `priorOuterCont` is set from `c.outerCont` at line 492 (`c.priorOuterCont = c.outerCont`), but `c.outerCont` is nil before the first outer advance. So when `ToBytes()` enters the "inner NOT exhausted" branch (line 578), `w.priorOuterCont` is nil → `fm.OuterContinuation` stays nil → on resume `outerFactory(nil)` restarts outer from beginning → first outer value's inner results duplicated. Fix: initialize `priorOuterCont` to `&StartContinuation{}` (non-nil zero position) so the first outer value's continuation round-trips correctly. Reproducer: FlatMapPipelined with outer=[A,B,C], inner per outer returns 3 items, row limit=2 stops mid-inner on A, resume produces A's remaining items again. Not currently triggered by production code (combinator available but unused). Found 2026-03-26 via 10-agent audit.
