...
- [ ] **HIGH** — Binding tester: certain seeds (e.g. seed 6, `--num-ops 100 --api-version 730`) crash the FDB 7.3.75 server with SIGSEGV. 100% reproducible. FDB trace shows `FlowTransport.actor.cpp:749 "write buffer is full"` immediately before crash at `0x234cc19`. Our wire marshaling round-trips correctly (marshal→unmarshal produces identical data), but we are almost certainly sending something malformed that FDB doesn't validate before crashing. Needs packet capture + comparison against C client to find the bad message. Non-crashing seeds pass with 0 errors / 0 incorrect results.
