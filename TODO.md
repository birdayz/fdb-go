...
- [ ] **DML does not execute through Cascades (parallel pipeline).** `ExecContext` routes INSERT/UPDATE/DELETE through the naive `execStatement` path (`planOne` execMode branch → `planDDL` → `execStatement`), NOT the Cascades `planDML` funnel. `planDML` is reachable only via `QueryContext`, where its update plan is then rejected (`connection.go:359`, "only SHOW and SELECT"), so the Cascades DML plan is built and thrown away. This is exactly the "naive fallback alongside Cascades" CLAUDE.md forbids ("Java has one query path"). Java plans DML through Cascades like everything else. **Priority: architecture cleanup (Phase tier, not P0/P1/P2)** — DML is correct today via the naive path, so this is tech debt, not a deploy blocker. Surfaced by RFC-034 (P2.2): `planDML`'s metrics hook fires but logs a throwaway plan until DML execution is unified onto Cascades. Also blocks the Cascades INSERT path's "DML plan extraction failed" gap from mattering. See `pkg/relational/core/embedded/plan_logging_fdb_test.go` reachability note.
