...
- [x] **#84** **CRITICAL: Unified plan pipeline — eliminate naive generator.** Done: SELECT and DML (INSERT/UPDATE/DELETE) all route through Cascades. Naive generator retained only for DDL/SHOW/INFORMATION_SCHEMA (procedural, no optimization needed).
  ...
  **HN launch blockers (in priority order):**
  ...
  - [ ] **#98** Yamsql conformance: **81/98 pass (82.6%)**, up from 77/97 (79.4%). nightshift-80 fixed: aggregate alias threading, CTE column aliases, EXISTS column leak, recursive CTE stack overflow, derived table+join dropping, post-agg projection alias. Remaining 17 failures: derived table GROUP BY scope (~7), recursive CTE traversal order + error codes (~11), SELECT * expansion (~3), union star column mapping (~4), scalar subquery patterns (~4), datetime edge cases (~4), correlated subquery + ambiguous column (~6). (~1-2 shifts for most; derived table scope needs architectural work)
