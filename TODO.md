...
- [ ] **Bounded aggregate index scan (optimization).** Java's `AISCAN [EQUALS col1]` pushes WHERE predicates on group keys into the aggregate index scan range. `PushFilterThroughGroupByRule` already pushes predicates into GroupBy's inner, BUT: AggregateDataAccessRule fires on the ORIGINAL GroupBy in its Reference (inner=Scan) before seeing the pushed version (inner=Filter(pred,Scan)). The pushed GroupBy ends up in the Filter's Reference as an alternative member — AggregateDataAccessRule doesn't fire on it because it matches on the GroupBy Reference, not the Filter Reference. Fix: either (a) make AggregateDataAccessRule also check `ref.Members()` for GroupBy expressions that have a Filter inner, or (b) restructure so the pushed GroupBy goes into the same Reference as the original GroupBy, or (c) run AggregateDataAccessRule in a second pass after pushdown converges.
