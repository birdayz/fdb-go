...
- [x] **Performance testing under real workloads** — Go vs Java benchmark comparison (8 operations, 100 iterations + warmup). Go wins 5/8: LoadRecord 0.60x, ScanRecords 0.85x, StoreOpen 0.85x, SaveRecordWithIndex 0.93x, SaveBatch 1.02x. Java wins ScanIndex (2.5x, JIT) and DeleteRecord (1.19x). Four optimizations applied: parallel FDB reads in Open, skip proto.Clone/metadataVersion for uncached, fastUnpack in cursors, pool commit slices.
...
- [x] **Differential serialization fuzzer** — DONE. `cmd/fdb-diff-oracle/`: C++ oracle (ObjectWriter) + Go fuzz test. 18 message types, 17 oracle-compared + 1 Go-only (ReplyPromise). 600M+ executions at 10min/target. Branch: `diff-serialization-fuzzer`, PR #7.
  ...
  **Remaining issues found by fuzzer (not yet fixed):**
  ...
  - [x] **Codegen: stop emitting dead DirectWriter methods**: Verified — these methods no longer exist in current generated code.
