...
- [ ] **🚩 Aggregate result-type derivation diverges from Java (AVG→DOUBLE; SUM accumulator→int64). [RFC-gated, Graefe]** Surfaced by the A3 cross-engine `insert_select` scenario. Java types `AVG(BIGINT)→DOUBLE` and `SUM(BIGINT)→BIGINT`, then enforces assignability at PLAN time: `INSERT INTO dst(v BIGINT) SELECT AVG(v) FROM src` is **rejected** with `SemanticException` ("A value cannot be assigned to a variable … cannot be promoted …", SQLSTATE **22000**), while `INSERT … SELECT SUM(v)` is **accepted**. Go's `AggregateValue.Type()` (`pkg/recordlayer/query/plan/cascades/values/values.go`) derives SUM/AVG/MIN/MAX from the **operand**, so `AVG(BIGINT)` is mistyped `BIGINT` (never DOUBLE) and the plan-time assignability check that rejects in Java never fires; meanwhile the executor's aggregate accumulator yields `float64` for *both* SUM and AVG regardless of operand kind. Net: Go does not reject `AVG → BIGINT` at plan time (Java does) and only "works" for `SUM → BIGINT` via a coerce-on-write band-aid. **Correct fix (do NOT paper over with a downstream coerce in `goToProtoValue`/`ConvertToProtoValue` — principle 10):** (1) `AVG(x)→DOUBLE` always; (2) SUM accumulator returns int64 for integer operands so `SUM(BIGINT)→BIGINT` flows without a float round-trip; then the existing plan-time assignability check rejects `DOUBLE→BIGINT` exactly as Java does, emergently. This is a Cascades type-derivation change (needs an RFC + **Graefe ACK** on RFC and impl) with ripple into aggregate-index matching and AVG=SUM/COUNT decomposition — verify `avgScenario`, aggregate-index EXPLAIN pins, and the 1M stress before/after. Until done, A3 `insert_select` covers only the BIGINT→BIGINT INSERT…SELECT shapes both engines agree on; the aggregate-into-BIGINT shapes stay Go-only (insert_select yamsql corpus). The pre-existing `proto_value.go` whole-valued-float→int64 coercion on the INSERT…VALUES path is part of the same divergence and should be removed by this fix.
