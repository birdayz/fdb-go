...
- [~] **A1 — Phase B SqlSteps**: foundation, runSql + runWithSetup, harness, 31-entry corpus, **and Go-side runner against the embedded engine all landed swingshift-52** (PR #118). Real cross-engine VALUE assertions for 30/31 entries; column metadata gaps documented in CLAUDE.md but tolerated by the test harness via a separate `_ColumnGaps` audit. Detail:
  ...
  - **Go-engine conformance gaps surfaced by the harness** (see CLAUDE.md "Go embedded SQL engine — known divergences from fdb-relational"): identifier-case preservation, anonymous-projection naming, qualified-column retention, DOUBLE/empty-result type inference, UUID column-type DDL. Track these as separate Go-engine fixes; each one closes a `_ColumnGaps` log entry. Once all gaps close, fold `_ColumnGaps` into `_Values`.
...
- [ ] **DDL `UUID` column type** — Go embedded engine rejects `CREATE TABLE T (key UUID, ...)` with `42F59: unsupported column type: UUID`. Java supports UUID end-to-end (DDL, INSERT, projection — values come through as `java.util.UUID`). Surfaced swingshift-52 by the plandiff `uuid_round_trip` corpus entry. Wire UUID into the DDL column-type vocabulary in `pkg/relational/core/embedded/ddl.go` and through the cascades type system (singletons already exist in `cascades/values/type.go`). Sized 1 shift.
- [ ] **Result-set metadata: identifier-case + projection naming + qualifier stripping** — Go embedded engine returns column names as-typed by the user (`id`, `t.id`, `count(*)`, `id+10`); fdb-relational uppercases unquoted identifiers, strips qualifiers, and emits synthetic `_N` names for anonymous projections. Surfaced swingshift-52 by the plandiff `_ColumnGaps` audit (30/31 entries diverge). Pin three behaviours in projection naming: (a) uppercase unquoted identifiers in column metadata, (b) strip table qualifier in projected columns, (c) synthesize `_0`, `_1`, ... for non-trivial expression projections. Documented in CLAUDE.md "Go embedded SQL engine — known divergences from fdb-relational". When all three land, fold the `_ColumnGaps` audit into `_Values`. Sized 1 shift.
- [ ] **Embedded driver: `ColumnTypeDatabaseTypeName`** — implement on the `database/sql/driver.Rows` shim so callers (and the plandiff Go runner) get JDBC-style type names directly instead of inferring from runtime values. Today the runner has to coerce all numerics to `float64` to compare against Java's JSON-decoded numbers, then report `BIGINT` for everything numeric — DOUBLE columns can't be distinguished. Empty result sets have no type at all (`id|`). Implementing the driver method fixes both. Sized 1 shift.
