...
- [ ] **DML does not execute through Cascades (parallel pipeline).** → promoted to **🚩 P0.4** (top priority, do next) under Production readiness. `ExecContext` runs DML via the naive `execStatement` path, not the Cascades `planDML` funnel — the forbidden parallel pipeline. See P0.4 for the full writeup and fix plan.
...
- [ ] **🚩 P0.4 DML must execute through Cascades — DO THIS NEXT (top priority).** `ExecContext` routes INSERT/UPDATE/DELETE through the naive `execStatement` path (`planOne` execMode branch → `planDDL` → `execStatement`), bypassing the Cascades `planDML` funnel. `planDML` is reachable only via `QueryContext`, where its update plan is rejected (`connection.go:359`) and thrown away. **This is the forbidden "parallel pipeline" — two DML code paths that drift, double maintenance, and hide Cascades DML gaps (e.g. the "DML plan extraction failed" INSERT gap) behind the naive fallback.** CLAUDE.md: "Java has one query path (Cascades). Go has one query path (Cascades). Don't maintain a naive fallback alongside Cascades." **Fix:** delete the naive DML execution path; route INSERT/UPDATE/DELETE through `planDML` → executor so DML shares SELECT's single Cascades path, matching Java's `PlanGenerator.getPlan`. Make `QueryContext`/`ExecContext` execute the Cascades DML plan (`isUpdate` plans return row counts via the executor, not a rejection). Fix the Cascades INSERT extraction gap as part of this. Surfaced by RFC-034 (P2.2) — `planDML`'s metrics hook currently logs a throwaway plan. Promoted from architecture-cleanup tier to P0 by maintainer: a parallel pipeline is a correctness/divergence hazard, not mere debt.
