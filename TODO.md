...
- [~] **A3 — SQL semantic equivalence on yamsql corpus**: feed the 1587 yamsql statements through both engines via A1; require identical result sets for read queries. **Progress (dayshift-55):** genuine cross-engine harness wired in `conformance/yamsql_cross_engine_conformance_test.go`. Each test runs through BOTH the Java fdb-relational executor and the Go embedded engine (via plandiff's `NewJavaRunnerHTTP` + `NewGoSQLSetupRunner`) and asserts (a) Java vs scenario expected, (b) Go vs scenario expected, (c) Java rows == Go rows directly. 16 scenarios shipped (where_literal_on_left, arithmetic, cast, composite_pk, bytes, between, boolean, like, case_when, aggregate_empty_table, bitwise, avg, derived_table, coalesce_nullif, bare_col_with_agg, aggregate_nulls) — 118 active tests pass strictly cross-engine + 19 error_code/DML tests skipped. Surfaced and documented 9 new fdb-relational + Go-engine gotchas in CLAUDE.md (untyped NULL in arithmetic, FROM-less SELECT, parenthesized boolean predicate, CTE+ORDER BY, SQL-spec HAVING+empty-WHERE divergence, NULLIF unsupported, bare-bool-column-in-projection Go-stricter-than-Java asymmetry, plus the originals from earlier scenarios). Scenario expansion is mechanical follow-on; the harness (per-scenario Describe loop, multiset comparison for unordered tests, loose-numeric per-cell equality, three-way Java+Go+expected assertion) carries.
