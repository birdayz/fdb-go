...
- [x] **WHERE/HAVING on GROUP BY key with aggregate index.** Fixed: ImplementIndexScanRule now skips AggregateIndexMatchCandidate — prevents double-aggregation (IndexScan + StreamingAgg over pre-computed values). Query falls back to full scan + filter + streaming aggregation, which is correct. Future optimization: bounded-scan AggregateIndex plan (Java's `AISCAN [EQUALS col1]`).
