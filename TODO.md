...
- [x] **B5 Batch A — 5 of 6 rules**. New `pkg/recordlayer/query/plan/plans/` subpackage with `RecordQueryPlan` interface + 6 physical plans (Scan / Filter / Sort / Distinct / TypeFilter / Union). Rules: PrimaryScanRule, ImplementFilterRule, ImplementSortRule, ImplementDistinctRule, ImplementTypeFilterRule. All 5 have full 5-wrapper inner-recognition (asymmetry in initial port caught by reviewer + fixed). Bridges via `physicalScan/Filter/Sort/Distinct/TypeFilterWrapper` — RelationalExpression adapters with WithChildren + CostHinter. `BatchAExpressionRules()` composer. Remaining 1 (covering-index + index-equality / range rules) gated on MatchCandidate / IndexAccessHint infrastructure.
- [x] **4.0 Value hierarchy — 16 ports** (totals: ~28 of Java's 78 now). InOpValue (SQL IN, byte-slice-safe equality), EmptyValue, OfTypeValue (**Java-eval conformant** — NULL → IsNullable + STRICT primitive TypeCode match per OfTypeValueTest; non-primitive promotion + DynamicMessage gated), LikeOperatorValue (**routes through canonical values.LikeMatch shared with predicates.likeMatch — same fuzz-tested, Java-conformant matcher**), VersionValue, RecordTypeValue, IndexedValue, EvaluatesToValue (Java-eval conformant), DerivedValue, ExistsValue, ConstantObjectValue (with ConstantDeref interface), SubscriptValue (1-based per SQL spec, OOB → NULL), ArrayDistinctValue, CardinalityValue, FirstOrDefaultValue, ObjectValue.
...
- [x] **End-to-end integration tests through plangen.Convert + Planner.Plan + extraction** (~6 tests covering Filter/Sort/Distinct/Scan shapes, stats-driven extraction flipping, full SQL-shape Distinct(Sort(Filter(Scan))) at 4-deep).
