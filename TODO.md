...
- [ ] **ORDER BY on UNION with different-named sides.** dayshift-37: `SELECT id, v FROM a UNION SELECT id, w FROM b ORDER BY id, v` errors with 22023 because the parser plumbs ORDER BY through to each side's SELECT — `v` doesn't exist in b so the right side rejects. SQL standard says ORDER BY on a UNION refers to the result set (left side's column names). Fix: apply ORDER BY *after* `execUnion` combines the rows, using the left side's column names. Pinned as a known-broken case in `union_columns.yaml`.
- [ ] **Aggregate over arithmetic expression.** dayshift-37: `SUM(qty * price)` errors with 42703 because `extractAggFunc` resolves the inner expression via `selectExprToColumnName` which only accepts bare column references. Java and Postgres both accept `SUM(expr)`/`AVG(expr)`/`MIN(expr)`/`MAX(expr)`. Fix: let `extractAggFunc` capture the raw expression context and the evaluator evaluate it per-row before aggregating. Pinned as known-broken in `aggregate_expr.yaml`.
