...
- [~] **`SELECT tbl.*` qualified-star.** Partial fix nightshift-36: single-source FROM now accepts `SELECT tbl.*` as a SELECT * equivalent (correct when FROM has one source). Multi-source FROM (JOIN / comma-join) does NOT restrict to the aliased source's columns — it still projects everything, so `SELECT a.*, b.label FROM a, b` would return all of b's columns under `a.*` rather than just a's columns. Needs per-source column-list plumbing through the projector. Pinned for the single-source case by `boolean.yaml` (which now uses `SELECT lb.*`).
