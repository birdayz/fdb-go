...
- [~] **A3 — SQL semantic equivalence on yamsql corpus**: feed the 1587 yamsql statements through both engines via A1; require identical result sets for read queries. Today only parsing is verified. **Progress (dayshift-55):** harness wired in `conformance/yamsql_cross_engine_conformance_test.go` driving inlined scenarios through plandiff's `runWithSetup`. 7 scenarios shipped (where_literal_on_left, arithmetic, cast, composite_pk, bytes, between, boolean) — 67 active tests pass strictly + 14 error_code/DML tests skipped. Each scenario hand-adapted: drops NOT NULL on PK columns, uppercases BYTES literals (X'...' / B64'...'), drops untyped-NULL operands and constructs Java's planner rejects (per 12 fdb-relational gotchas in CLAUDE.md). Scenario expansion is mechanical follow-on; the harness (per-scenario Describe loop, multiset comparison for unordered tests, loose-numeric per-cell equality) carries.
