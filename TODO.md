...
- [~] **A1 — Phase B SqlSteps**: foundation, runSql + runWithSetup, harness, 31-entry corpus, **and Go-side runner against the embedded engine all landed swingshift-52** (PR #118). Real cross-engine VALUE assertions for 30/31 entries; column metadata gaps documented in CLAUDE.md but tolerated by the test harness via a separate `_ColumnGaps` audit. Detail:
  ...
  - **Go-engine conformance gaps surfaced AND closed by the harness in the same shift** (swingshift-52): identifier-case (`id` → `ID`), qualifier stripping (`t.id` → `ID`), anonymous-projection naming (`COUNT(*)` → `_0`), JDBC type-name plumbing (BIGINT/STRING/DOUBLE/etc.), empty-result type inference. The harness is now strict end-to-end (column metadata + row values), 30/31 entries pass byte-equivalence with Java; the one outlier is a UUID feature gap (DDL parser accepts UUID, proto-builder doesn't yet emit tuple_fields.UUID).
