...
- [~] **B0 — Phase 4.0 Type hierarchy**: seed landed nightshift-50, follow-ons landed swingshift-52. Now in `cascades/values/type.go` (1264 LOC):
  ...
  - **Remaining**: type inference on `Value.Type()` so it returns rich Type instead of the legacy ValueType enum (largest remaining piece — every Value impl needs RichType()); plan-serialisation hooks (proto encoding for plan-cache); structured-type recursion in MaximumType (RECORD field-by-field, ARRAY element-by-element). When `cascades/values/type.go` exceeds ~1500 LOC, split into `cascades/typing/` per RFC-025. Sized 2 shifts to "complete enough to retire ValueType".
