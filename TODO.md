...
- [~] **A1 — Phase B SqlSteps**: extend `conformance/sql_plan_steps.java` with a `runSql(clusterFile, schemaTemplate, sql)` step that returns the result rows as JSON (mirrors the existing `planSql` shape). Wire `pkg/relational/conformance/plandiff` to invoke it. Maven blocker is GONE (4.11.1.0 landed swingshift-50). **Foundation landed swingshift-52** (PR #118): Java `runSql` step + Go `Runner`/`RunResult`/`RowSet`/`Column` types + `javaRunner` HTTP plumbing + 6 unit tests + 3 end-to-end conformance tests. Bonus fix: long-standing schema-URL bug — `setSchema()` on JDBC wrapper doesn't propagate to `EmbeddedRelationalConnection`; must use `?schema=NAME` URL query string. DDL also requires `?schema=CATALOG` on `/__SYS`. Both `planSql` and `runSql` now actually exercise their full code path against real Java; previously every conformance test silently fell into a typed-error early-return. **Remaining**: Go-side runner (gated on Track C2 QueryExecutor), result-set-diff harness analogous to plan-tree diff, corpus comparison.
