...
- [ ] **`WITH RECURSIVE name AS (non-self-referencing-body)` Go-permissive divergence from Java** — Java rejects with "condition is not met!" because it requires an actual UNION ALL self-reference for RECURSIVE (CLAUDE.md gotcha). SQL spec + Postgres permit the non-self-referencing body (RECURSIVE is a scope enabler, not a requirement). Go matches SQL spec; Java is the strict-RECURSIVE outlier. Probed nightshift-61 — Java rejection confirmed. Aligning Go to also reject would invalidate yamsql `recursive_cte` scenarios using the non-self-referencing form. Defer to a dedicated cleanup shift.
...
- [ ] **Integer overflow Java-throws / Go-wraps silent drift** — Java raises `ArithmeticException: long overflow` on arithmetic overflowing int64; Go's embedded engine wraps two's-complement silently (CLAUDE.md gotcha). `SELECT v + 1 FROM t WHERE v = MAX_INT64` returns wildly different results across engines. Aligning Go requires swapping every plain `+ - *` for overflow-checked variants (or `math/bits` helpers) in `pkg/relational/core/functions/arith.go`'s `ApplyMathOp`, AND deciding the error class to raise (likely `ErrCodeNumericValueOutOfRange` 22003, the SQL standard for arithmetic overflow). Surfaced nightshift-61 by an integer_overflow probe scenario; scenario was dropped, gotcha documented. Defer to a dedicated cleanup shift.
