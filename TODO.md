...
- [ ] **INFORMATION_SCHEMA query gaps surfaced by nightshift-36.** Two issues: (a) parser rejects `FROM INFORMATION_SCHEMA.TABLES` with "syntax error near TABLES" — `TABLES` is a reserved keyword so it can't be used as a table name without quoting. (b) `SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'EMP'` returns 0 rows when the table is named `EMP` (uppercase). The system-tables handler may be comparing against an internal representation. Fix: either teach the parser to treat INFORMATION_SCHEMA.* identifiers as unreserved in that position, or require users to quote `"TABLES"`. No scenario file yet — try again after fix.
...
- [ ] **RIGHT OUTER JOIN: unqualified column from unmatched right row.** nightshift-36 harness-surfaced: when a right-side row has no matching left row, the combined-map merge assigns `b.id` to the unqualified `'id'` key, so `SELECT a.id FROM a RIGHT OUTER JOIN b ON a.k = b.k` returns b.id instead of NULL for unmatched right rows. The qualified lookup (`a.id`) resolves through the unqualified path because the a-side map is empty. Fix: when materialising the NULL-left row, explicitly set every `a.*` key to NULL rather than inheriting from b. `outer_join.yaml` avoids the case pending fix.
