...
- [~] **A2 — Catalog wire format Go↔Java round-trip**: PARTIAL nightshift-53. The FDBMetaDataStore wire-format **functional** round-trip is DONE: cross-language metadata save/load that PROVES the loaded metadata is functionally usable to drive a record store. Eleven new specs in `metadata_store_conformance_test.go` under "Cross-language functional round-trip (A2)":
  ...
  **dayshift-54 progress on SchemaTemplateCatalog** — peeled the onion two layers:

  - **Layer 1 (FIXED, dayshift-54)**: keyspace prefix matches across engines. Go's `catalog.OpenRecordLayerStoreCatalog()` and Java's `RecordLayerStoreCatalog` both target the SAME FDB subspace `(NULL, NULL, int64(0))` (3-byte prefix `0x000014`). Confirmed via direct subspace byte inspection. The Go sqldriver's three-string subspace is a separate concern (sqldriver writes elsewhere; the catalog package itself is Java-compat).
  - **Layer 2 (FIXED, dayshift-54)**: catalog metadata-version divergence. Java's `RecordMetadataSerializer.visit(SchemaTemplate)` calls `setVersion(schemaTemplate.getVersion())` where `CATALOG_TEMPLATE_VERSION = 1`, then 3 addIndex calls bump to 4. Go's `BuildCatalogMetaData()` was starting at 0 and ending at 3 → `StaleMetaDataVersionError{Local:3, Stored:4}` blocked any cross-engine catalog read. Fix: explicit `SetVersion(1)` in `BuildCatalogMetaData()` before the addIndex calls.
  - **Layer 3 (PINNED, NOT YET FIXED)**: SchemaTemplate proto-deserialize divergence. After Go can open the catalog, `LoadSchemaTemplate` reads the TEMPLATE record bytes successfully, but `RecordMetaDataFromProto` fails to rebuild the FileDescriptor with `proto: message field "RecordTypeUnion.T_0" cannot resolve type: "RecordTypeUnion.T": descriptor not found: RecordTypeUnion.T`. Java emits a self-referential `FileDescriptorProto` where the union message has nested-type references that Go's proto rebuild path doesn't resolve. Pinned via `conformance/schema_template_catalog_conformance_test.go` — the test asserts the SPECIFIC error substring so any Go-side deserializer change surfaces as test failure (caller updates the assertion when Layer 3 is fixed).

  **Remaining (next shifts)**: fix Layer 3 — comparing Java's FileDescriptor emission path against Go's parse path. Substantial follow-on work, likely 1-2 shifts. After Layer 3 lands, the negative pin becomes a positive (template loads + columns match) — cross-language SchemaTemplateCatalog round-trip closes A2. MIN_EVER_LONG / MAX_EVER_TUPLE / MIN_EVER_TUPLE index types follow the same atomic-mutation pattern as COUNT/SUM/MAX_EVER — adding cross-language tests is mechanical follow-on with the harness pattern documented inline at the head of the "Cross-language functional round-trip (A2)" Describe block.
