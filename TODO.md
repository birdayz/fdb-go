...
- [ ] **Cursor combinator bugs (12 documented issues)** — `EndContinuation` is overloaded: used for both "iteration truly done" AND "no continuation available for this value." Causes data loss in `ConcatCursors`/`FlatMapPipelined` (values silently dropped on pagination), infinite loop in `AutoContinuingCursor` (nil continuation → restart from beginning), resource leaks in `LimitRowsCursor(cursor, 0)`, silent truncation in `Seq()`. **Java solution:** `RecordCursorResult.withNextValue()` _throws_ if continuation `isEnd()` — you literally cannot construct a value result with an end continuation. Java also has `RecordCursorStartContinuation` (`isEnd()=false`, `toBytes()=null`) distinct from `EndContinuation`. Strict invariants enforced at construction: value→non-end continuation, SOURCE_EXHAUSTED↔end continuation. **Fix:** Add `StartContinuation`, add validation in result constructors matching Java's invariants.
- [ ] **Observability gaps** — `StoreTimer` exists but isn't wired into production paths (Save/Load/Delete/Scan). **Java approach:** 260+ optional timer events on `FDBRecordContext` (nullable, zero overhead when disabled). No built-in Prometheus/otel — apps subclass `StoreTimer` (e.g. `MetricRegistryStoreTimer` wraps Dropwizard Metrics). Structured logging via `KeyValueLogMessage` + SLF4J with 100+ standard `LogMessageKeys`. **Fix:** Wire existing `StoreTimer` into production paths, keep optional/nil-safe. Don't bake in monitoring system — match Java's library-agnostic design.
- [ ] **OnlineIndexer adaptive throttling** — Missing `shouldLessenWork()` and `recordsPerSecond` rate limiter. **Java approach:** No circuit breaker either — backpressure is error-reactive. `IndexingThrottle.Booker` reduces per-transaction record limit on specific FDB errors (1501 timeout, 1007 too_old, 1020 conflict): consecutive failures → 90%→80%→50%→10% reduction, 8+ consecutive → give up. `recordsPerSecond` (default 10,000) adds inter-transaction delays. Default 100 max attempts (we use 10). **Fix:** Implement `shouldLessenWork()` with adaptive limit reduction, add `recordsPerSecond` config. Skip circuit breaker (not in Java's design).

### HIGH

- [ ] **Index maintainer code duplication** — 5 atomic mutation maintainers (COUNT, SUM, MIN_EVER, MAX_EVER, COUNT_NOT_NULL) repeat ~1000+ lines of nearly identical evaluation→common-entry-removal→mutation logic. **Java solution:** Single `AtomicMutationIndexMaintainer` class (242 lines) handles ALL 11 atomic types via Strategy pattern. `AtomicMutation` interface (10 methods: `getMutationType()`, `getMutationParam()`, `isIdempotent()`, etc.) with `AtomicMutation.Standard` enum (11 variants). One factory, one maintainer, ~779 total lines for 11 types. **Fix:** Create single `atomicMutationIndexMaintainer` with `AtomicMutation` interface. Each index type becomes a struct implementing that interface. One maintainer, one factory dispatch, zero duplication.
- [ ] **Metadata builder API footguns** — `GetRecordType("Order").SetPrimaryKey(...)` nil-derefs if type doesn't exist; no validation until `Build()`; `GetTypedRecordStore[T](store, "Order")` type name string redundant with generic param. **Java approach:** `getRecordType("nonexistent")` throws `MetaDataException` (never null, never auto-creates). Semantic validation (field existence, type compatibility, index structure) is late — at `build()` via `MetaDataValidator` + `KeyExpression.validate(descriptor)`. Early checks only for duplicates/nulls. Typed store uses generics + proto serializers + field descriptors — no string param. **Fix:** (a) `GetRecordType()` should return `(*RecordType, error)` not nil-deref, (b) add `KeyExpression.Validate(descriptor)` called from `Build()`, (c) try to derive type name from `T`'s proto descriptor to eliminate string param.
