...
FoundationDB Record Layer â Go Port. Java version: **4.12.11.0**. FDB wire protocol: **7.3.77**.
...
# â ALL WORK FROZEN â sole priority is RFC-173 (ordinal/group column-resolution migration)

**Owner directive (2026-07-01): pause ALL other project work until RFC-173 lands.** Do NOT pick up
any item below, any handover follow-up, or any new hunt â RFC-173 (`rfcs/173-ordinal-column-resolution.md`)
...
merge independently; atomic Slice 3 as its own PR), RFC-ack â per-slice re-ack. Foundational,
**~25â30 shifts**. See RFC-173 Â§4 for the slice order and Â§5 for the (execution-pin, not dark-diff)
validation gate.

### RFC-173 progress (slice tracker)

- [x] **RFC** â `rfcs/173-ordinal-column-resolution.md`, all-four-acked, merged (#422).
- [x] **P1 â ordinal `FieldPath` substrate** (dark): `FieldValue.resolveOrdinal` + `RecordType.FieldIndex`
  (list-position = Java ordinal) + `NewRecordType` normalises `Fields[i].Ordinal == i`. All-four-acked,
  merged (#423, `a20794e9b`).
- [~] **P2 â positional/typed runtime row** (in gauntlet, PR #427). Typed positional row emitted
  ...
  **Deferred to Slice 1** (before the ordinal path goes live): (i) the dual-emission per-row cost
  benchmark (RFC Â§4 P2 hard part); (ii) [@claude] a dedicated **e2e shadow test for the projection
  producer** (`executeProjection`), analogous to `TestBuildCoveringRow_ShadowAndCollision_RFC173P2`
  â projection's `slots[i]=val` has no index arithmetic so risk is low, but pin it before Slice 1
  ...
  are near-duplicate scans â dedup (`LookupField` â `FieldIndex` + index) when a slice touches both.
- [x] **P3 â alias-bijection interning â FOLDED INTO SLICE 3** (gauntlet call, PR #429 NOT merged:
  Graefe + Torvalds + codex all ACK-with-fold; @claude n/a). The dark-shadow spike proved the
  mechanism (tier-3 predicate minus the `aliasAware` gate â `would=true` == the flip's extra dedup)
  and quantified it (â259 extra dedups / 1500 planned corpus exprs â an **approximate, Insert-only
  under-count**, not a pinned number). But the observer is a nil-in-prod hook + an unasserted `t.Logf`
  â transitional scaffolding deleted at the flip, so it lands **with its Slice 3 consumer**, not
  ...
  member-count-delta (the pin the spike omitted); (iii) certify no CTE-rename NULL-read via Â§5
  execution pins + RFC-077 task-count baseline (safety is flip-live-gated, un-shadowable). Full
  analysis banked in RFC Â§4 P3.
  - [x] (ii) **shadow-delta pin DONE** (`TestRFC173S3_AliasAwareInterningShadowDelta`, branch
    ...
    â measured member-count delta (via `SetDisableAliasAwareInterning`), NOT the naive equality â
    cascade makes delta > shadow (3-chain 4â20, 4-chain load-bearing: converges on / blows budget off).
  - [ ] Follow-up (non-blocking, Torvalds ACK w/ next-slice defer): `disableAliasAwareInterning` is a
    ...
    Insert â deferred because `Reference.Insert` has no Memo handle. Do when a slice gives Insert Memo
    context (the alias-aware tier itself survives S4, so this is not auto-retired).
  - [ ] Follow-up (W4b review residuals, master-parity â broken on master too, pinned as clean
    errors): (a) aggregate-local outer refs in a correlated scalar (`SUM(o.amount + e.ref)` over a
    clustered outer) never planned â the true enablement is an EXECUTION-CONTEXT property (the
    ...
    stay on the pre-W4b path even when gated (@claude scope note â intended W4b narrowing); (c) the
    UPDATE-transform row-context site (`executor.go` ~:2754) still gates the binder on the old
    params/scalar-subqueries-only condition â the same class round 5 fixed for projections
    (`hasBindingContext`); PROBED: unreachable through SQL today â a subquery in UPDATE ... SET never
    reached the binder at all (the builder wrote the LITERAL SUBQUERY TEXT into the row â silent data
    ...
    from the walked QOV; the ambiguous-column probe returned the WRONG LEG before the fix â pinned);
    OUTER-scope parenthesized columns (scope discrimination via the binder-exact innerSourceAliases;
    materialized, keyed by the executor's shared naming contract â a wrong-scope read before the fix);
    the unnest AT-alias scope corner (the collector mirrors unnestSourceCorrelation: AS-else-AT-only);
    the name-model projection binder gap (round 5 â outer refs over Datum rows silently NULLed).
- [x] **Slice 1 non-join ordinal â MERGED (#437, `12516e33f`), all four gates ACKed at the merge
  HEAD** (Graefe impl+delta ACK Â· Torvalds fix-firstâACK incl. MUTATION-TESTING the authority proof Â·
  codex clean (P2 fixed; delta finding = documented Goâ¥1.22 loopvar false positive) Â· @claude "nothing
  blocks merge"). Ordinal resolution authoritative on the non-join frontier; buried-reference
  reverse-map retired (4 RFC-082 divergences lifted, real-Java-validated); Â§5 dual-window differential
  standing (1617 entries, first catch fixed: recursive-CTE computed-column silent-wrong, known-red
  lock shrank); dual-emission benchmark satisfied (+71% window, `positionalTypeCache` â Slice 4 ends
  net-faster); 1M stress FASTER than pre-merge master (scan_all_wide 4.52s vs 5.57s â the cache repays
  the window). Full execution log: RFC Â§4 Slice 1.
  ...
  - [ ] Graefe recorded obligations live in RFC Â§4: Slice-4 kill list (sort-comparator fallback dies
    loud Â· dualwindow retires with the name map Â· `legPhysicalOutputNames` must not outlive the window Â·
    bound `positionalTypeCache` dynamicpb leak) + Slices 2â3 oracle-gate rule (new birth sites extend
    `DisablePositionalEmission`).
- [x] **Slice 2 2-way wedge â DONE, MERGED as PR #447 (squash 7f7100199, 2026-07-02).** All four
  gates at the final HEAD: Graefe â (design + impl), Torvalds â, codex â (clean P1/P2/P3),
  @claude â ("ð¢ Clean"). Master moved twice mid-gauntlet (PR #446 recursive-CTE alias frontier +
  PR #450 RFC-176 P1); both merged in with the suite green. #446 had independently invented a
  SECOND baked-ordinal mechanism (`ResolvedOrdinal`/`HasResolvedOrdinal`, childless, quiet) â
  unified onto `ResolvedAccessor` in 51e3327ca with a `FrontierPinned` contract bit (Graefe
  pre-code ruling: bit on the accessor, NOT child-presence â passthrough copies strip Child;
  ...
  - [x] **ENTRY GATE: the name-burial inventory â SATISFIED** (`rfcs/173-name-burial-inventory.md`,
    ...
    sites enumerated); `qualifyTypeFallback` exists (executor.go:2140 â the "not found" was a
    directory-scope artifact).
  - [x] **W1 baked-ordinal FieldValue substrate (dark)** â `Resolved *ResolvedAccessor`,
    `NewFieldValueOfOrdinal`, (name, ordinal) identity for baked / bakedâ lazy, marker through every
    ...
    compose/push-down + lazy dup-name DECLINES, Â§5 dup-name identity pin. Graefe ACK Ã2, Torvalds
    NAKâACK Ã3 (e74c4d192, ec739f83f, fd9d83636, 6168a56e9, 089979ea8).
  - [x] **W2 cluster-arity scoping gate + drift asserts (dark)** â `rfc173_cluster_gate.go`
    ...
    pinned redâgreen), flattening-evasion + enclosure-matrix + HAVING-EXISTS pins. Two Graefe-confirmed
    errata vs the contract shorthand (subquery-carrying filter/project = poison; outer boxes gate
    unconditionally). Graefe ACK, Torvalds NAKâACK (53ba2a9ac, 089979ea8).
  - [ ] **BOOKED (Graefe, RFC-173 S4 :3234 clean-lift condition 3): retire the mutable `inInnerCluster`
    ...
    CALLER-CONTEXTUAL â "is this filter a merge leg or a fresh cluster root?" is caller state, NOT
    ...
    NOT a producer-retirement blocker â the `:3234` lift landed without it. Do AFTER the remaining
    enclosure setters are retired (it touches all of them).
  - [x] **Slice 2b: filtered-chained ordinalizes â DONE, FULLY 4-GATED on 60cbb0c08 (Graefe design+impl ACK, @claude ACK, Torvalds ACK, codex clean).** A chained lateral
    unnest under an ancestor WHERE (`FROM t, t.SARR AS x, x.SUB AS y WHERE <pred>`) now ORDINALIZES
    instead of declining to the name-model residual (buildUnnestResultValue â NewAnchoredJoinRecord).
    The coarse `chainedUnnestUnderFilter` "any filter suppresses" decline is RETIRED (field + set/restore
    + gate all deleted). Predicate placement is per-conjunct via the â-outerLegs pushable-to-scan rebase
    gate (`rebaseChainedOuterLegPredicate`/`chainedPredScanPushable`): outer-col-only (correlated-to â
    {t}) â SARG on Scan(t); anything referencing an in-chain correlation (x/y) â keep the rebase at the
    inner Explode. Axis-audited every filter shape (eq/AND/IN-list/BETWEEN/arithmetic/IS-NULL/NOT/
    multi-conjunct/straddling â all row-verified correct; end-to-end plan asserts pin the SARG/inner-filter
    ...
    first-link row â @claude's NAK caught it; NARROW `chainedUnderOrFilter` bit declines OR filters to
    name-model, correct rows); (ii) a scalar subquery in the filter â LOUD `0A000` (typed, gated
    `len(f.ScalarSubqueries) > 0 && filterInputHasChainedUnnest(f.Input)` â the detector WALKS the join
  ...
  - [x] **Slice 2c: 2-chain OR ordinalizes via the POSITIONAL bake â DONE (Graefe design-ACK'd).** A
    ...
    POSITIONALLY â `rebaseUnnestOuterLegPredicateOrdinal(p, ordType, ordType, â¦)` with
    ...
    the outer QOV is the 5-field ordinalLegType), NOT an `OrdinalSeedLegWindows` disagreement â the
    ...
    on `isChainedUnnest`, so it fired on the name-model FALLBACK too â a mixed-inner-ref clause over a
    3+-link chain (or a buried 2-chain) baked `ofOrdinal(QOV)` against the name-keyed row â ordinal -1
    ...
    and stays name-model â its mixed-inner-ref STRAND lives in a DIFFERENT layer (pushBuriedUnnestPredicateDown
    ...
    OR-with-scalar-subquery â 0A000, B1 (1641) green.
  - [x] **Slice 2d (deeper-nesting): LINEAR 3+-link chains ORDINALIZE (filtered + unfiltered) â DONE after a
    ...
    name-model), its "SARG â ordinal" cert discriminator was FALSE (model-independent â the certs passed
    verbatim on the parent, caught by a neutered-gate control + a reviewer's independent parent-worktree
    control), and the unscoped gate admitted a FORK chain (`t.arr X, X.substruct Y, X.sub W` â owner two links
    ...
    `chainedBaseOrdinalizes`, which walks `j.Left` NOT firstBase â firstBase alone would miss a fork one level
    below the top); forks decline to name-model. (2) The box-leg-conjunct arm SCOPED via
    `unnestExistsSeedSafe(left, spineBase)` â `flag && !spineBase && multiAlias`; only the chained gate passes
    ...
    off `translateChainedUnnestJoin`: linear 2/3/4-link Ã filtered/unfiltered â ordinal; top/mid-spine fork,
    box-base Â±filter â name-model; twin-fork + 1-seg-mid-spine â loud upstream). ONE-TIME feature-off control:
    gate neutered â all six linear pins FAIL (discriminating by construction). Fork rows-certs e2e + honest
    ...
    AT) â PRE-EXISTING semantic-resolver, reproduces name-model, upstream of translation.
    SECOND corrective round (two more gate kills on 59eb67aaa): a P1 silent-wrong â `clusterArity(FULL
    OUTER)==1` let a FULL-box-BOTTOMED spine pass spineBase=true, suppressing the box-leg-conjunct arm for
    genuine box legs (chained link ordinal OVER the first link's name-model seed â wrong A-side values);
    fixed by the walk returning (admitted, pureSpine) with pureSpine = `len(outerBoundAliases(bottom))==1`
    (admission unchanged â FULL-box bottoms still ordinalize unfiltered, pre-slice parity), pinned white-box
    + e2e (`fullbox_bottom_boxleg_filter`, the exact repro). And a false "pinned below" coverage claim in the
    white-box file (the walk's Segments<2 check had NO walk-test case â a reviewer control neutering it
    stayed green); the 1-segment walk case added. Cert harvest: depth-3 shadow-slot straddle `T4.SUB=Z`
    (positive slot-3 pin) Â± OR, AT-first/AT-mid ordinal rows.
  ...
  - [x] **FORK SLICE: fork spines ORDINALIZE via owner-slot rooting (Graefe design-ACK'd fork-first).**
    ...
    "resolves to exactly ONE deeper link" â forks admitted, table-owned/orphan/dup-alias declined
    defensively, dup 42712-loud upstream + pinned); `chainedOwnerElementSlot` roots the collection at the
    OWNER's element slot (`len(ordinalLegColumns(owner.join.Left))` â the layout law, AT-invariant, pinned
    per combination incl. the AT-only-upstream case). Purely translation-side. Controls at introduction:
    full feature-off â all four fork seed-form pins FAIL; MIS-ROOT control (tip slot) â the colliding-schema
    ...
    {box-base, FULL-box-filtered, enclosed, defensive !ok} â NEXT: the box-substrate slice (box-base +
    ...
    all correct; independent teeth-check), Torvalds comments-NAKâACK (seven chainedBaseOrdinalizes ghosts
    rewritten; controls F/F2/G all reproduced under his hand), codex clean Ã2. Probe extras booked for a
  ...
  - [ ] **BOX-SUBSTRATE slice (in de-risk; Graefe design consult round 3 in flight).** Commit 1 landed
    ...
    CORRECTION owed: commit 1's message attributes the spike's silent-wrong to "empty rt.Legs â flat
    fallback" â REFUTED by a layout probe (windows already propagate through chained merged types incl.
    box bottoms: [{A 0 3},{B 3 3},{X 6 1},{Y 7 1}]; a prototyped bottom-only propagation BROKE the 2c
    certs â per-link windows are load-bearing for runtime qualified-name resolution â reverted
    uncommitted). TRUE mechanism (trace-confirmed): the box-leg conjunct takes rebaseChainedOuterLegPredicate's
    LAZY fork ({B} â outerLegs "scan-pushable"), leaving a FOREIGN-correlation name read at the merged
    select; the runtime bare-name fallback over an ordinal row first-matches across legs â A's slot 0.
    The pureBottom lazy-fork gate is the correct semantic fix but NOT sufficient: with it, the bake is
    altitude+slot-correct yet evaluates NULL â the chained-over-box FlatMap's runtime binding does not
    flip positional in sync with the seed (the seed/birth mismatch class at the CHAINED FlatMap birth).
    ROUND-3/4 RESOLUTION (executor-birth mapping + gather:93 spike): gather:93 is NOT vestigial â its
    ...
    and adaptLegPositional's synthesis from a merged multi-namespace box Datum is unfaithful â silent
    NULL/empty). Even the full three-piece lift (gather:93 + arm + WHERE-merge bake) fails: TWO more
    uncoupled sites gate the simple case â legIsOrdinalSafe categorically rejects non-INNER legs
    (rule_implement_nested_loop_join.go:1106) and the FULL NLJ is built with sel.GetResultValue()
    VERBATIM (:166). LANDED (Outcome A): the coherence guard on the chained gate â
    `!pureSpine && !boxOuterBirthsPositional(bottom) â decline` â AMENDED per two independent review
    falsifications: NOT a demonstrated-bug fix (pre-guard rows were CORRECT on every probed observable â
    ...
    FULLY 4-GATED (b8441025a + 90e9fbe82 + amendment 2095b0ce8): Graefe ACKâre-confirm (with ownership;
    NEW STANDING RULE: latent-bug-on-HEAD claims require a rows-probe+trace on the PRE-change tree),
    Torvalds NAKâACK (theory #4 falsified by his instrumented probes; zero refuted-claim residue; the six
    tripwires "the real prize"), @claude ACKÃ2 (22-probe differential byte-identical; harvest faithful),
    codex cleanÃ3. Arc: FOUR falsified theories, THREE revertible spikes, ZERO unsound code shipped.
    BOOKED (Outcome B, the real box-substrate ordinalization â sequenced WITH the circular arms, LAST):
    the 5-site checklist â (1) chained gate â birth coupling (landed as A), (2) the enclosure declines
    ...
    consumer row-loss â both surfaced only under the lift, both stay declined today.
  - [ ] **BOOKED (Slice 2d discovery â PRE-EXISTING semantic-resolver gap, orthogonal): a WHERE ref to a
    DEEP chained alias â 42703 "column does not exist".** Boundary: a 3-link AS-alias (`Z` in `â¦Y.DEEP AS Z
    WHERE Zâ¦`) resolves; a 4+-link AS-alias (`v` in the 4-link chain), a 3-link AT-alias (`o`), and a
    ...
    SemanticAnalyzer scope-depth fix, separate slice â filtered 3-link (the expressible filtered depth) already
    ordinalizes.
  - [ ] **BOOKED (RFC-173 S4 Slice 2b discovery â PRE-EXISTING, ORTHOGONAL, engine-wide silent-wrong):
    scalar subquery in a WHERE comparison returns `[]`.** `SELECT id FROM t WHERE id = (SELECT MAX(id) FROM
    t2)` returns SILENT `[]` for a PLAIN table (verified MAX/MIN/`>` all `[]`; want the matching rows) â so
    the "scalar subquery in a filter" gap is NOT chained-specific; it's a general scalar-subquery-in-WHERE
    hole (projection-position scalar subqueries DO work â see the W4b entries). The chained case additionally
    ...
    shape LOUDLY (0A000) to prevent that regression from silent-`[]` â crash. Proper fix: implement
    scalar-subquery-in-WHERE comparison (a general feature; makes the plain case answer rows AND unblocks
    the chained 0A000 â rows). Java answers both â reach gap, must eventually close; the 0A000 sentinel
    flips green when it lands. Sibling upstream gap: IN-subquery / EXISTS in a filter over a chained unnest
    fail 0AF00 UPSTREAM of the chained dispatch (Java answers) â orthogonal reach gaps, book if the endgame
  ...
  - [ ] **BOOKED SLICE (Graefe, RFC-173 S4 Slice 2b follow-up â orthogonal resolver gap): scalar sibling
    of an unnested array element does not resolve (42703).** `SELECT X.K FROM T4, T4.SARR AS X` where the
    SARR element has a scalar sibling `K` â `42703 column "X.K" does not exist`, IN THE BASE CASE (no
    ...
    Java resolves it (standard lateral ref) â real Go divergence, must eventually close (likely a small
    resolver fix once scoped). Discovered during the Slice-2b de-risk (the would-be x-column conjunct
    `WHERE X.K = v` is unreachable via this gap; the â-outerLegs gate needs no special handling for it â
    an x-ref flows through the keep-rebase branch once the resolver closes).
  - [ ] **BOOKED (EXISTS-composition commit-1 follow-ups, RFC-173 S4 collision mint â see the RFC entry):**
    (a) **mint-per-leg for MULTI-SOURCE colliding EXISTS inners** â `EXISTS (SELECT 1 FROM OT OI, ST WHERE
    ...
    (b) **buildCorrelatedScalar sibling mint** â the correlated-scalar fallback shares buildCorrelatedExists'
    walk/qualify structure and therefore the same capture class; needs its own three-way-discriminating probe
    first, then the same mint. (c) **Java's array-constructor literal** (`INSERT â¦ VALUES (1, 100, [10, 200])`,
    RelationalParser `arrayConstructor`) â Go 0A000 "unsupported expression atom ArrayConstructorExpressionAtom"
    â reach gap found by the live-Java probe; Java populates array columns via SQL, Go cannot. (d) **Case-2
    nested-EXISTS hoist dangles a middle-ref** â `EXISTS(SELECT 1 FROM M WHERE EXISTS(SELECT 1 FROM N WHERE
    N.b = M.a))` (middle has ONLY the nested EXISTS) hoists the inner plan and drops M entirely, leaving
    `M.a` unbound â pre-existing, unchanged by the mint (minted it resolves loud-deterministic instead of
    accidentally binding a same-named outer); needs its own probe + decline-or-fix. (e) **outer-CTE-leg scope
    registration** â buildOuterScopeSources.addSrc silently drops a resolve-failed leg, so an OUTER CTE leg
    ...
    correlated pair OT.k>X AND MA.c<X → {10,20} on the probe fixture) → shared-surface parity gap,
    needs its own slice (likely the exists-filter path only carries one ExistsSubqueries consumer per
    unnest select).
  - [x] **W3 the coupled 2-way flip â DONE, ALL ACKs.** W3a-1/W3a-2 (36297a253, fd07e2f49,
    ...
    **W3b-2 pins LANDED** (a3d323808 + fc653b4b0 fail-closed ON-drop fix â a PRE-EXISTING
    ...
    rfc173_slice2_gate_pins_test.go goes RED the instant the planner learns the shape â when it
    ...
    blocked on green â pre-commit runs the suite). Fallout fixed so far (each = a gate/executor
    correction, all reviewed-in-principle): (1) LEFT OUTER = POISON â RewriteOuterJoinRule
    ...
    shape name-model e2e â, pin RewriteOuterJoinRule declines FULL â³TODO, dup-alias poison noted as
    S3/S4 unique-quantifier-alias item â³RFC); FULL OUTER stays gated; preserved leg ENCLOSED /
    null-supplying leg fresh. (2) JOIN legs categorically ineligible (nested bare concat erases
    buried aliases â S3 FieldPath territory). (3) Dup-leg-alias poison (FROM p JOIN p). (4)
    ...
    (select_star_cross_join_all_cols + cte variant) â Â§5 dup-name SELECT * over cross join reads
    ...
    "NAME" â executeProjection projNames (values.ProjectionColumnNameâfv.Field) collide in the
    ...
    slot columnIndex-1 â the Â§7 dup-name fix arriving via the positional row; verify projection
    ...
    before/after).
  - [x] **W4 â DEFERRED TO S3 (Graefe ruling, f034707eb RFC amendment).** The correlated-scalar
    2-leg seed is a pre-rewrite LEFT OUTER select (the ephemeral object the W3b premise
    correction covers) and the unnest port needs S3 FieldPaths â both moved into S3's scope with
    rationale; S3 honestly resized 4â5 shifts; the FINALIZED S2 wedge (pure inner/cross 2-way over
  ...
  - [x] **W5** gauntlet â PR #447 MERGED (squash 7f7100199). Full four-gate trail at every HEAD
    (461c38074 branch content; 51e3327ca unification; final merges mechanically verified â
    ...
    the contract delta (Graefe condition 4: reviewers review against the amended Â§4, not the
    stale text): premise correction, LEFT-outer poison, join-legs ineligible, W4 deferral,
    finalized wedge scope.
- [x] **Slice 3 â DONE, MERGED as PR #464 (rebase-merge `38454886a`, 2026-07-03).** Graefe's design
  gauntlet REFRAMED it from an "atomic deletion" to a **dispatch-authority flip + certification**
  slice: the premise "widen gate â `parentIsMerge` never true â trio dead" is FALSE (anchored seeds â
  scalar-subquery, multi-source unnest, non-wedge â stay constructible), so the trio + `AnchoredJoin`
  flag + seed constructors + `InternsAliasAware` anchored arm ALL move to Slice 4; W5 stays a separate
  post-green PR. This slice is strictly ADDITIVE instrumentation + six Q1âQ6 gate pins (dispatch
  ...
  HEAD: Graefe â (design + impl + delta), Torvalds â (+ delta; nits folded), codex â (caught + fixed
  a real P3 â Absorb dropped the loser's alias-aware dedup counter on `Memo.merge`, pinned by
  `TestReference_AbsorbFoldsAliasAwareDedups`), @claude â ("the fourth ACK"). CI green.
- **Slice 4 is a GATED DEMOLITION, not a free delete** (6-agent producer-retirement map + Graefe
  boundary DESIGN-ACK, RFC Â§"Slice 4 â retire AnchoredJoin: boundary design"). After S3 the
  `AnchoredJoin` flag axis has ZERO individually-dead symbols (the flag is a field on the SHARED RC
  type) â undeletable while ANY of FOUR live seed producers survives. **Canonical sequence (each a full
  four-gate slice):**
  - [x] **W4b â correlated-scalar ordinalization â MERGED (PR #465), all four gates ACKed at the
    merge HEAD** (Graefe design+impl+5 delta ACKs Â· Torvalds 2 NAKs fixed â "merge it" Â·
    codex 4 P2-class findings fixed â clean Â· @claude 2 passes + coverage-gap catch â gate closed).
    ...
    text-corruption writes in UPDATE...SET (subquery/EXISTS/IN-subquery RHS â pre-existing on master,
  ...
  - [x] **W5 â multi-source lateral UNNEST** â MERGED (PR #466, master 1050831ff). Five commits
    ...
    PRE-EXISTING buried-unnest spanning-WHERE 0-row class); the Â§5 oracle rebuilt for gathered
    ...
    disjoint-schema FDB e2e (WSRC/WAUX â the only disjoint pair; every other corpus pair shares
    ...
    51377â42788, -17%, chain/dispatch pins unchanged; typed/untyped pins). Commit 3b LANDED: the
    ENCLOSED class (`FROM A, A.arr AS x, B`) â rotation to the root form (gatherLegsWithBuriedUnnest
    â Join(Join(plain legs, FROM order), Unnest), collected ONs on the rebuilt root where the
    element is in scope), the translateFilter enclosed merge arm (rewrite+bake, the root form's
    treatment), and the pushBuriedUnnestPredicateDown stand-down when the gather fires â FIXING a
    ...
    via the Â§7 positional-aligned read) + FROM-order preservation through the rotation (unnestPos
    threaded; element mid-list) + the MANDATORY Â§5 oracle rebind: the dual-mode FDB differential
    ran RED (gathered family NIL/0-row oracle-side; the PLAIN 3-way merge-sub-product class was
    silently broken the same way, corpus-blind) â recoverOracleDatumSpans (NLJ twin of the
    ...
    corpus caught a.k=b.kâk=k instantly). 7-query differential + 1621-entry corpus green.
    Commit 5 LANDED: the F4-rider dead-for-gated pins (gated=empty, emergent owner edge,
    residual dotted still classifies â the reachability proof deferring physical deletion).
    ...
    BARE duplicate column Names ([TAID K TBID K TCID K]) â the NLJ-planned cross form keeps
    ...
    collection-field name via descriptorForColumn, whose ambiguity rule is first-match â two
    ...
    - RETRACTED (probe-verified with discriminating data): the "KNOWN-BROKEN spanning residual" was
      a PHANTOM â the commit-1 seeds (WV {5,6} vs elements {7,8}) made `EL > WV` all-true, so the
    ...
    - Commit-2 investigation results (probe-verified end-to-end): the case-A dotted-resolution
      ...
      (`downstreamLegWindows` â `ordinalJoinSpansOf` with legRVs â `spanAwareRow.GetByName`); the
      R18 divergence = `resolveSpanLeaf` stops at merge quantifiers for SINGLE-accessor paths, so
      the mixed (no-AT) bare-QOV element yields partial leg coverage â windows decline â strict
      positional context â loud miss (the AS+AT full-baked ON-carrying gathered shape ALREADY WORKS
      end-to-end). Commit-2 core = the ~40-line span-derivation extension (synthesize the 1-field
      element leg for a single-accessor bare-non-record-QOV merge slot â the existing :145-155
      ...
      uppercases â normalize + pin); the NLJ birth's nil-legRVs Datum dimension.
    - Pre-existing (NOT W5, worktree-verified on master): a filtered cartesian with only per-leg
      ...
      CANNOT PLAN (0AF00) â a partition gap, booked so it is not mistaken for a W5 regression.
      (The no-WHERE dotted 3-way works.)
  - [x] **W4-left + EXISTS + recursive-CTE joins** â MERGED (PR #467, rebase; four-gate tally:
    ...
    EXISTS-over-join â the I2 latent RIGHT+EXISTS column-order fix, the OUTER-flatten
    ...
    rejection, Java 42702 byte-equal, F3 REVISED against live Java runs â 2 marked divergences
    + 1 parity entry). Commit 5 LANDED (exit gates: producer audit in the RFC â the INNER
    ...
    column derivation, all-priors tracking, 42F01 unmasked â live-Java-verified corpus
    entries); mixed outer-nesting soundness (the demanded runtime pins caught TWO pre-existing
    MASTER bugs â fabricated ALIAS.* keys from merged-row bare keys = wrong-source rows, and
    the nondeterministic anchored re-enumeration panic â both fixed; enclosure guard on the
    ...
    the dup-alias collision check (a."COUNT(*)" over duplicate legs read one side silently â
  ...
  - [ ] **S4 atomic demolition** (LAST â gated on QP-REF-BIND items 1+2+3, the riders, and the
    ...
    feat/rfc173-next. **The correlated-scalar prerequisite is DISCHARGED (W4b #465)** â JOIN-inner
    ...
    it; (2) direct ungated-shape probes â a correlated scalar over an UNNEST outer
    (`SELECT c.name,(SELECT COUNT(*) FROM o WHERE o.cid=c.id) FROM c, c.arr AS x`) does NOT reach
    :3860 (it declines at PLANNING, "could not plan query" 0AF00 â a pre-existing limitation, not
    ...
    anchored record" is WRONG â it declines elsewhere. **BUT S4 is NOT near-done: only the SCALAR
    producer (#1) is near-dead. The name model's CORE is heavily live** â `NewAnchoredJoinRecord`
    ...
    **B1 DISCHARGED (corpus-level ordinal-spike certificate GREEN):** the whole-gate
    ...
    :143/:157/:190, in `pkg/relational/conformance/ordinalspike`) preserves EVERY corpus row â
    **1641 entries, 0 carve-outs, 0 mismatches**. This PROVES the atomic cap (flag + producers +
    Â§5 oracle deletion) is SAFE TO FIRE: any shape the cap would change now surfaces as a spike
    ...
    branches + 4 executor consumers + `select.go:251` arm-1 + the Â§5 oracle (load-bearing until
    now) â ONE commit, which also LIFTS the W5 bare-twin duplicate-column decline (the
    ...
    harness with DATUM-KEY accounting â datumFromSpans was a fourth layout site that
    ...
    the coexistence Datum outright â decide at demolition time.
  - [x] **F2-LEFT DONE** (projected-EXISTS over LEFT JOIN, scan legs â Java-parity reach gap: Go
    ...
    (correlatedStep1 false on the undissolved LEFT â gatedSeedStep1 true) + a JoinLeftOuter NLJ that
    null-extends the null-supplying leg **positionally** â NO name-model `:698` producer (verified: the
    scan-leg query never hits buildJoinResultValue). Buried box `(a JOIN b) LEFT JOIN c` DECLINES
    cleanly (`isScanFamilyLeg` gate â Â§8 â 0AF00) rather than mint a producer; INNER keeps its
    buried-box `:698` (task-scope, no null-extension) â asymmetry deliberate. noop-(J) flipped
    declineârows; live-Java-parity corpus entry added â **dualwindow green BOTH windows, 0 new
    ...
    Go-beyond-Java reach) â so all tests use the VERIFIED ORDER-BY-free shape, sorting in Go. Four-gate pending.
...
- [ ] Slice 5 closure invariant Â· [ ] Slice 6 extensions + ANSI headroom.

## ð RESUME AFTER 173 â where we pick up (do not lose this)
...
1. **RFC-164 WS-2 â FINISH IT.** Correlation-completeness invariant becomes always-on **for free** via
   RFC-173 Slice 5 (closed-by-construction). Then re-evaluate the three **field-level** invariants
   (set-op comparison-key columns, `COVERING â referenced-fields`, result-type consistency) â RFC-164
...
2. **RFC-164 WS-3** â `RecordQueryPlanVisitor` port + `comparandReadsField` audit (single-source-of-truth).
   Mostly independent of 173.
3. **RFC-164 WS-4** â map-iteration lint + **Phase-1b InJoin inner-tie determinism**; the latter is
...
4. **RFC-164 WS-1** â generative Go-vs-Java row differential. Heavy-infra (live Java + FDB), multi-week.
   Untouched by 173.
5. **3 open Cascades hunt bugs** (from `hunt/cascades-bug-hunt`) â own focused Graefe cycle each.
6. **INFRA â Nightly Fuzz** (see item directly below): dispatch-timing bug **AND two red nightly runs
   (06-29, 06-30)**. â ï¸ The red runs are a live "red-CI-is-red" signal that arguably should NOT wait for
   173 (a red fuzz could be a real bug, possibly in code 173 touches) â flag for the owner whether to
...
cost-monotonicity/IN-determinism landed (#411â#419); WS-2 correlation-completeness + field-level,
WS-3 visitor, WS-4 map-lint/tie, WS-1 all remain â the list above.

---

# NEXT

> ## [ ] INFRA â scheduled nightlies dispatch HOURS late ("nightly" fuzz runs at noon)
...
> usage** â a documented GitHub behavior; the off-minute cron (`:17`, not `:00`) mitigates but does not
...
> the delay â it just renames the nominal time. Real options (pick later): (a) accept GitHub best-effort
> scheduling for nightlies (they're regression nets, exact time doesn't matter); (b) reduce the backlog â
> a second runner box, or a larger box to lift `max-runners` to 2 (RFC-155 Â§3 locked it at 1 for
> dependency isolation â revisit); (c) dispatch nightlies via an external scheduler (`workflow_dispatch`
> from a reliable cron outside GitHub) so they fire on time; (d) a dedicated nightly runner so scheduled
> jobs don't contend with per-PR CI. Also: the last two Nightly Fuzz runs FAILED (06-29, 06-30) â a
> separate real signal to investigate (a fuzz target has been red for two nights; no-unrelated-flakes rule).

> ## [ ] INFRA â stress-1M thresholds violated on MASTER (baseline rot; INVESTIGATE)
> Discovered by RFC-176 P2's stress gate (PR #453): on an idle box, **current master violates the
> "Stress test 1M baseline" Threshold column** â in_list 14.97ms (<10ms), needle_pk 5.4ms (<5ms),
> needle_filter 6.4ms (<5ms), pk lookups 5.0â7.2ms (<5ms) â vs May-baseline values of 10/2.0/2.4/1.5-1.7ms.
...
> â box `workstation`, Linux 7.0.10-arch1-1, idle, back-to-back master/branch runs; three-way table
> in PR #453 (comment "Quiet-box stress-1M: branch AND master control").
> **Decision path (in order):** (1) re-qualify the environment against the May baseline run â
> machine, Docker version, FDB image, kernel; if any changed, re-measure and UPDATE the baseline
> table + thresholds; (2) if the environment is unchanged, bisect MayâHEAD on the violated rows
...
> ## RFC-176 P2 registrations (review of PR #453) â two follow-up cycles
>
...
>   winner â PR #453 is the existence proof: RFC-176 P2's alias-invariant plan hashes flipped the
>   vector fold/decline winner and a pinned sentinel went red (fixed there by making the tie a COST
>   decision â vector scan CPU â but the tie-break CLASS remains for every other equal-cost pair).
...
>   cycle (Graefe).
> - **[ ] Predicate/selector semantic hash â retire rendering-keyed `HashCodeWithoutChildren`, the
>   RFC-176 Â§1 defect class one type-family over.** Five sites fold non-Value renderings into plan
...
>   `planSelector.Equals`). equalâ¹same-hash holds only while every rendering stays exactly injective
>   over the structural discriminators â the standing whack-a-mole RFC-176 killed for Values. Wanted:
...
> ## Cascades bug hunt (branch `hunt/cascades-bug-hunt`) â 9 confirmed bugs
>
> Multi-agent + differential hunt across the Cascades engine. 6 FIXED in this PR (redâgreen tests,
> full `sqldriver` suite green); 3 OPEN below (riskier â own focused Graefe cycle each).
>
> **FIXED (this PR):**
> - **[x] AGG-RESIDUAL (critical, wrong results).** `AggregateDataAccessRule` dropped any input filter
>   predicate that wasn't a grouping-key equality (non-group column, or inequality) â aggregate read over
>   ALL rows. `SELECT g,SUM(v) FROM ga WHERE f=1 GROUP BY g` returned the unfiltered sum. Fix: decline the
>   aggregate-index match when a residual isn't a grouping-key equality (Java compensation = impossible) â
...
>   `isLeafReplaceable`, so `â¦ WHERE c IN (â¦) LIMIT k` over a Projection kept the eager nil-inner snapshot â
>   `Limit(Project(Fetch(<nil>)))` â 0 rows. Fix: always relink (the fetch wrapper's RFC-070 fix, applied to
...
>   with zero columns â col read as NULL â COUNT=0. Fix: gate on a true COUNT(*) (mirror executor `isCountStar`).
...
>   (`RecordQueryUnionPlan`) as distinct â `SELECT DISTINCT` over UNION ALL elided the dedup. Fix: report it
>   non-distinct. `plan_properties.go`. Pin: `TestBugHunt_DistinctOverUnionAllKeepsDedup`.
> - **[x] CAST-ROUND (low, Java parity).** `CAST(double AS INT/BIGINT)` used pre-Java-7 `floor(x+0.5)` â
...
>
> **OPEN (riskier â separate Graefe cycles; repros in PR):**
> - **[x] COST-SELECTIVITY â FIXED (PR pending).** The equality bound was costed at `FilterSelectivity=0.5`
>   > `RangeSelectivity=0.33` â inverted, so a broad range index was costed cheaper than a selective equality
...
>   (`physical_wrapper.go` Ã2, `planning_cost_model.go` scanLikeCost). `WHERE a=5 AND b>10` now drives off
...
> - **[x] NULLS-ORDER â FIXED (PR pending).** Restored the NULLS axis to `RequestedSortOrder`
...
> - **[x] VECTOR-PARTITION-INTERSECTION-K>1 (HIGH, wrong rows â was ACTIVE on master, pre-existing) â FIXED.**
>   Pin: `TestFDB_VectorSearch_MultiPartition_InequalityResidualK2` (`WHERE zone='z1' AND region>'r1' â¦ <=2` now
>   returns `{21,22}`, was `{22}`). The fix is TWO pieces (piece 3 collapsed into piece 2 â the realization path
>   already existed): (1) exclude ALL `VectorIndexScanMatchCandidate` from the pk-intersection candidate set â one
>   condition in `pushDataAccessTasks` (planner.go ~628), the single home for the rule â since a vector scan is
...
>   consolidated into this upstream candidate-level check â Torvalds/Graefe nit, provably equivalent);
>   (2) `compensationSafeForYield` `residualIsPartitionContiguous` exception â a residual over the partition columns
...
>   scan, only against index-only predicates) then realizes `Filter(region>'r1') â VectorScan(self-limiting rank<=2)`.
>   The DistanceRank stays in the scan binding (never leaks to the residual â rule_match_intermediate.go:470-483 keeps
...
>   HISTORICAL (kept for context â root cause). Found by Graefe during RFC-167 Phase 4 review (PR #411). For a
...
>   emits `Intersection(VectorTopK, PrimaryRange)` keyed on the full pk â and it is the ONLY physical plan (the safe
...
>   no re-sort â for K>1 rows per partition whose distance-order â  doc_id-order, the merge DROPS rows. Worked example:
...
>   (selects whole partitions, never within-partition rows) â yields the correct `Filter`. PIN with a K>1 partition-
>   residual FDB rows test FIRST. **CONFIRMED empirically** (probe, dropped during cleanup): `WHERE zone='z1' AND
>   region>'r1' â¦ <=2` plans to `Intersection(VectorIndexScan(rank<=2), Scan(DOCS,[=,<>]))` and returns `{22}` â
>   DROPS id 21 (correct is `{21,22}`; in r2 the nearest vector is id 22 (dist 1.20) but id 21 (dist 1.41) sorts
>   first by pk â the pk-merge advances past 21). **FIX ATTEMPTED (reverted) â it is THREE pieces, not two:**
>   (1) the pk-order gate in `WithPrimaryKeyIntersector` (per-leg `computeWrapperRichOrdering(leg).Satisfies(pkReq)`)
>   correctly drops the invalid vector intersection â VERIFIED; (2) the `compensationSafeForYield` partition-residual
>   exception (with a CONTIGUITY check: the residual columns must be exactly the partition columns immediately after
>   the bound equality prefix `len(prefixComparisons)` â else a leading partition col like `zone` is left unbound,
>   which must stay unplannable per `TrailingEqualityResidual`) â VERIFIED via the plan field
>   `RecordQueryVectorIndexPlan.partitionColumns` set in `ToScanPlan` from `columnNames[:partitionCount]`. BUT pieces
>   1+2 alone make BOTH the K=1 and K>1 inequality queries UNPLANNABLE ("index-only predicate â¦ cannot be a residual
>   filter") â because **(3) the standalone `Filter(region>r1, VectorTopK)` plan is never GENERATED**: a partitioned
...
>   intersection â there is no realization path building a `Filter` over a self-limiting vector scan (the compensation
>   is blocked/non-realizable, so only the intersection ever carries the residual). **The real fix (Graefe-corrected â
>   the earlier `Limit(k)âFilterâordered-scan` framing was WRONG):** do NOT switch to ordered-stream + a global
>   `Limit(k)` â a global Limit over a fanout stream returns the k nearest rows across ALL surviving partitions and
...
>   HNSW search) and **realize a partition-contiguous `Filter` directly above it**: `Filter(region>r1) â
>   VectorScan(self-limiting per-partition top-k, fanout prefix=[z1,*])`. The self-limiting fanout returns top-k for
>   every region in z1 in region order; the partition-column Filter drops whole regions â¤ r1; survivors are exactly
...
>   So Piece 3 is NOT "extend ordered-stream to partitioned" â it is "realize a partition-contiguous Filter over the
>   self-limiting per-partition scan" (Piece 2 already certifies its safety). THEN gate the intersection (pieces 1+2).
>   The K>1 pin must assert the winning plan is `Filter(...) â VectorIndexScan(... rank<=k ...)` (self-limiting), NOT
>   an ordered scan. RFC-046/156 vector-planning change â needs the K>1 FDB rows pin + the existing vector suite green + 1M stress.
> - **[~] PLAN-NONDETERMINISM (medium, flaky plans / cache churn) â RFC-167; Phase 0 + 1a done, rest designed.**
...
>   Intersection) are a separate landing needing the full ordering machinery + 1M stress (see RFC-167 Â§4 IMPLEMENTATION
...
>   (per-candidate-type participation) â the RFC-167 Phase 1b landing, needing the full ordering machinery + 1M stress.
...
>   tie-break; an orthogonal pk-intersector ordering bug â `intersector_primary_key.go` dropping requestedOrderings
>   â is a plausible wrong-rows risk that Phase 4 fixes). **Phase 0 (this change):** fixed the two map-iteration
...
>   are costed without their eventual inner â rank artificially cheap; and the extraction relink
>   `findPhysicalPlan` (physical_wrapper.go) resolves a shell's inner to the FIRST physical member of the child
>   reference by member-iteration order â on a cost tie the relinked index varies. Naive fixes tried and reverted:
>   excluding all shell types from `OptimizeGroupTask` selection deterministically picks the cost-cheapest REAL
>   plan, but that's an Intersection (e.g. `idx_a â© idx_b`) â a plan-shape regression vs the single-index plan
...
>   stress (it changes index selection broadly). Rows are always correct â medium. `FuzzPlanner_Determinism`
...
> false) so fan-out value-index access never emits the per-leg PK Distinct Java applies â but the embedded
...
>
> ## RFC-164 â Port-fidelity drift: make this bug CLASS un-shippable (tracked workstream)
>
> Post-mortem of the hunt: every bug was a spot where Go reimplemented/simplified Java instead of porting 1:1,
> dropping an invariant â and CI stayed green because the test gap is *dimensional* (each feature tested alone,
...
> reviewed). **Execution order = ships Ã leverage** (cheap always-on class-killers first, then the heavy net);
> every found bug gets a committed minimized seed. Owner: query-engine cycle per WS.
> - **[ ] WS-2 â Structural plan invariants** (highest ROI; always-on, Go-only CI). Each must run clean across
>   the WHOLE existing corpus with ZERO runtime skip-lists (exemptions only as compile-time optional slots).
>   - [x] no `<nil>` child in the FINAL extracted plan â LANDED (`ValidatePlanInvariants`, plan-tree walk via
...
>   - [ ] `WithChildren(GetQuantifiers())` round-trip identity â most direct relink-class catch.
>   - [ ] correlation/quantifier-binding completeness (no dangling correlation).
>   - [ ] set-op comparison-key correctness; result-type/schema consistency; COVERINGâreferenced-fields (â COUNT-COL).
>   - NOT "DistinctRecordsâhas-dedup-node" (unsound â unique/PK/agg/streaming-agg/intersection are distinct w/o a node) â runtime no-dup or WS-3.
> - **[ ] WS-3 â Single source of truth.** [ ] shared `isCountStar` + guard==consumer audit (cheap, first);
...
> - **[ ] WS-1 â Generative Go-vs-Java row-level differential** (highest coverage; heavy-infra, nightly lane).
...
>   budget; [ ] mutation proof. Honest effort ~1-2wk (or narrow first cut 2-3d) â NOT 1 day. Torvalds + /code-review.
> - **[ ] WS-4 â Property/metamorphic tests for Go-only paths.** [ ] cost monotonicity invariant (equality â¤ range);
...
> - **[ ] WS-5 â Audit & enumerate Go-only divergences in `DIVERGENCES.md`** ("what Java invariant does this drop?").
>   Means "known reservoirs documented", not "all found". Graefe.
>
> Open hunt bugs: **NULLS-ORDER** is wrong-rows â fix DIRECTLY (extend `RequestedSortOrder` NULLS axis + thread
> through `Ordering.Satisfies`), pinned by WS-2/WS-1 â not "through" the audit. COST-SELECTIVITY + NONDETERMINISM
> ride WS-4 (pin-then-fix). Closing them WITH the nets that would have caught them is the test the workstream works.

> **[ ] BUG (query-engine, wrong results, Graefe-gated) â local residual silently DROPPED on GROUP-BY over a
> correlated join.** `SELECT o.id, COUNT(*) FROM o, t WHERE t.fk = o.id AND t.k = 5 GROUP BY o.id` plans
> IDENTICALLY with and without `AND t.k = 5` â `StreamingAgg(keys=[O.ID], InMemorySort(â¦, FlatMap(outer=Scan(T),
> inner=PredicatesFilter(Scan(O),[t.fk=o.id]))))`. The local `t.k=5` residual on the driving leg vanishes â
> COUNT over ALL matching `t`, not the `t.k=5` subset â **over-counts**. PRE-EXISTING (verified byte-identical on
> base 33b7307ce + HEAD â NOT the muzzle/Piece-1; found by Graefe's adversarial battery). Non-aggregate is fine
> (`FROM o,t WHERE t.fk=o.id AND t.k=5 AND â¦` correctly applies the residual on `Scan(T)`); the aggregate/GROUP-BY
> orientation drops it. Repro is the redâgreen sentinel. Suspected root: the residual-placement/orientation logic
> the GROUP-BY-over-FlatMap path shares with Phase-2b's `RewriteOuterJoinRule` work â confirm before assuming;
...
compatibility; everything here is polish/parity/infra â **none gates adoption**. Priority order below;
...
1. **[x] C3 (conformance) â Ride their test designs: port FDB's adversarial workloads. COMPLETE.** Cycle /
   ...
   **Increment 1 DONE:** Cycle workload â pure-client serializability oracle (RFC-119, PR #308).
   ...
   **Increment 3 DONE:** Cycle under `process_behind (1037)` + `wrong_shard (1001)` faults (RFC-122,
   PR #320) â 1037 the fixed/QueueModel read row, 1001 its own relocate+invalidate ring-survival
   assertion (flake-free: budget exhaustion â retryable 1007).
   **Increment 4 DONE:** Cycle under a dropped commit reply â `commit_unknown_result (1021)` (RFC-123,
   PR #321) â the faithful commit-path fault (1021 is client-minted from an ambiguous RPC, so a dropped
   reply, not a synthetic error; `not_committed` deliberately NOT injected â unfaithful on an applied
   ...
   **Increment 5 DONE:** AtomicOps workload (RFC-124, PR #322) â atomic-op + unique-per-attempt
   ...
   confirmed the same atomic op double-applies under 1021 (faithful â no idempotency IDs), which is why
   the fresh-per-attempt logKey is load-bearing. Serializability gap is already covered by Cycle.
   **Increment 6 DONE:** ConflictRange workload (RFC-125, PR #323) â a two-directional read-conflict-range
   oracle on key-selector getRange, driven through the real `fdb` facade. A concurrent writer (tr2) commits
   between a pinned reader's (tr3) read version and its commit; the oracle is `resultChanged â¹ foundConflict`
   ...
   **Increment 7 (FINAL) DONE:** FuzzApiCorrectness (RFC-126, PR #324). The RFC pivoted under review:
   ...
   fixed points + differentials), so the `ExceptionContract` was used as an *audit checklist* â which
   surfaced real, libfdb_c-confirmed wire-contract divergences where Go silently accepted input
   libfdb_c/Java reject: `getRange` row `limit < -1` â `range_limits_invalid (2012)`;
   AddRead/WriteConflictRange + getRangeSplitPoints endpoint `> maxKey` â `key_outside_legal_range
   (2004)` (with the read/write `maxReadKey`-vs-`maxWriteKey` asymmetry); and the metric-op early-return
   precedence (inverted 2005 â cancelled 1025 â poison 2000 â timed_out 1031 â maxKey 2004). Each
   revert-proven + pinned by redâgreen differentials / deterministic unit tests. Also fixed a
   pre-existing flake in the RFC-121 conflict differentials (conservative-resolver false-positive 1020,
   proven via libfdb_c hitting it too â retry). FDB-C-dev + Torvalds + /code-review + codex + @claude +
   CI all green.
   **C3 COMPLETE:** Cycle (+ read/commit faults), AtomicOps, ConflictRange, FuzzApiCorrectness
   (error-contract axis), Serializability (via Cycle) all covered. Detail: "Native fdbgo client" â C3.
2. **[ ] RFC-056 continuation item 3 â ongoing `/hunt-divergences`.** Standing differential-axis hunt
   vs libfdb_c (atomic-op edges across `Atomic.h`, error-code/option semantics, key/tuple/versionstamp
   encoding). RFC-059â067 closed. Detail: conformance section, "Fresh differential axes".
   **Atomic-op axis hunted (2026-06-25): one concrete divergence found â RFC-149 â DELIVERED (PR #358).**
   The MinâMinV2 / AndâAndV2 op-code upgrade lived only in the `fdb` facade, so `client.Transaction.Atomic`
   (and the `cmd/fdb-stacktester` binding tester) shipped legacy `Min(13)`/`And(6)` where libfdb_c ships
   `MinV2(18)`/`AndV2(19)` â diverged on absent-key fold. Fixed: the upgrade now lives in `client.Atomic`
   (the `RYW::atomicOp` analog), 1:1 with `ReadYourWrites.actor.cpp:2243-2248`, gated `apiVersionAtLeast(510)`
   with the API version threaded into `client.database` (mandatory-set â `api_version_unset` 2200). Pinned by
   a cgo in-txn-RYW + committed redâgreen differential + the 509/510 boundary. FDB-C-dev + Torvalds + codex +
   ...
   **Full-surface hunt (2026-06-30, PR #403, branch `hunt/fdbgo-client-bughunt`):** two multi-agent
   discovery sweeps over 22 axes â adversarial verify (3 refuted) â **11 fixed redâgreen** (getKey
   ...
   StreamingModeExact-no-limitâ2210). **Open (in the PR's severity table + `shifts/2026-06-30-fdbgo-client-
   bughunt.md`):** 3 architectural â **RFC-165** (watch-at-committed-version), **RFC-166** (reset() must
   clear non-persistent options â also closes snapshot-ryw-survives-reset), **RFC-167** (getKey isBackward
   shard-location, needs multi-SS/SimTransport) â all Draft, need FDB-C-dev ACK; plus bounded LOWs (atomic
...
3. **[ ] C2-followup â confirm RFC-057's lazy iterator closed the go-vs-cgo 1007-rate** near the 5s
...
4. **[ ] Query-engine "one query path" unification.** Route `buildSelectShell`/SimpleTable builder +
   INSERTâ¦SELECT through `visitSelectGroupBy`, delete the legacy builder (CLAUDE.md "no parallel
   pipelines" endgame). Graefe-gated. Detail: "vs Java" follow-ups (RFC-079b + RFC-084) + Â§7.6 history.
5. **[~] 7.7 â RFC-148 split into Phase 1 (RFC-148) + Phase 2 (RFC-150), both Graefe direction+text ACK'd.**
   ...
   - **[x] Phase 1 follow-up â index-only `ImplementFilterRule` gate (NAMED, Graefe condition) â DONE (RFC-151).**
     Root cause was NOT unconsumed match-level residual (the match already binds `DistanceRank` via
     `flattenConjuncts`) â it was a SCHEDULING coupling: `pushDataAccessTasks` ran inline before the matching
     ...
     is **RETAINED as the catch-all backstop** â Graefe + Torvalds both reproduced a JOIN leak (the distance is a
     `Select` predicate â physical residual via `ImplementSimpleSelectRule`/NLJ, which the `ImplementFilterRule`
     ...
     **Sentinels (green):** `TestVectorPlan_QualifyPlansToVectorScan` (plans) + `MetricMismatch` (single-table clean
     error) + `MetricMismatchInJoinDoesNotLeak` (join clean error â the regression pin) +
     `TestFDB_VectorSearch_MultiPartition_TrailingEqualityResidual` (unplannable via the kept inner-scan guard).
   - **[ ] Follow-up â gate the remaining physical-filter builders to fully retire the net.** Gate
     ...
     residual â only then is `validateNoIndexOnlyResidual` genuinely dead and removable (Graefe's design-#10 path).
   - **[~] Phase 2b (RFC-150) â split into Piece 1 (DONE) + Piece 2 (in progress).**
     - **[x] Piece 1 â B1 task-graph invariant + retire the `!refIsJoinLeg` muzzle (PR #363, 4 gates green).**
       Ported Java's structural property: `OptimizeInputs` is scheduled only for PHYSICAL/plan members
       (CascadesPlanner.java:524; both construction sites â ExploreGroup :744-748 + executeRuleCall :1064-1070),
       ...
       is intentionally NOT gated â load-bearing (gating it breaks `TestFDB_ArrayUnnestOrdinality`), and a
     ...
     - **[~] Piece 2 â retire `tryFlatMapPlan` (PATH A). Step 1-3 DONE (commit b8b3b6ad7); deletion blocked on
       INNER-multiway PATH-B coverage.**
       - **[x] RewriteOuterJoinRule + DefaultOnEmpty null-extension (the LEFT-OUTER enabler â the one shape PATH B
         ...
         when an ON-pred references the preserved leg (uncorrelated LEFT â ON FALSE/NULL â stays on the
       ...
       - **[ ] Make PATH B cover INNER join legs (multiway chain + PK probe), THEN delete `tryFlatMapPlan`.**
         Three layers root-caused over 3 DFS rounds (all validated fixes REVERTED â they only pay off together
         ...
         - **Layer 1 (multiway chain) â VALIDATED FIX.** `PartitionBinarySelectRule`'s idempotency guard
           (`rule_partition_binary_select.go:88-93`) blocks on *any* predicate-free 2-quant select in the group,
           so sibling bipartitions of an N-way join never partition â no chained index-probe. Narrow it to the
           SAME quantifier-alias-set as `sel` (Java has no such guard; relies on memo interning). Verified: 3-/4-way
           chain to byte-identical index-probe FlatMaps. Bumps `ChainInterningBaseline` (3-way 9095â11122, 4-way
           31210â46483, < 100k).
         - **Layer 2a (PK probe never generated) â VALIDATED FIX.** `matchSingleSourceAgainstSelect`
           (`rule_match_intermediate.go:350+`) only tries the predicate LHS (`cp.Operand`) as the candidate column,
           so a join pred with the leg's key on the RHS (`O.CUSTOMER_ID = C.ID` â customers PK on RHS) never SARGs
           the leg's PK. Fix: add `ComparisonType.Commute()` (=â=, <â>, <=â>=) + a `bindOrientedComparison` that
         ...
         - **Layer 2b SOLVED + DESIGN-ACK'd (Graefe) â SARG the correlation as a sargable BOUND, not a residual.**
           ...
           bound-vs-residual line: `PredicateWithValueAndRanges.java:423-432` (`containsKey(alias) â
           noCompensationNeeded`). The 0-row guard is UNCHANGED â it still rejects the genuine residual-correlation
           ...
           **D.2** (physical scan/index wrappers must surface ScanComparisons correlations â Go returns empty,
         ...
         - **Layer 2c (THE ACTUAL GATE â a COST-MODEL change, distinct RFC, Graefe-gated, PR-#201 perf surface).**
           ...
           driver = perf regression). **D.1 cannot land standalone** â it makes multiway WORSE without the cost-model
           fix. Fix: make join-order costing prefer driving from the smallest/most-selective table â run
           ...
           larger table). HIGH blast radius (every join plan) â its own RFC + Graefe ACK + full plandiff/row-count/
           1M-stress. Also: the JoinSelPred FILTERED leg (`o.id<10` sibling) doesn't reach
           `matchSingleSourceAgainstSelect` cleanly â a separate match-firing fix.
         Sequence to finish: cost-model RFC (Layer 2c) â re-apply Gap#1 + D.1 + D.2 â filtered-leg match-firing â
         ...
         Detail: RFC-150 Â§3/Â§4.
   - **[ ] PROCESS HAZARD (found this shift) â the codex-review CLI can leave the repo on a detached HEAD,
     orphaning the branch tip.** Commit a567acb68 (a Torvalds F1 fix) was silently dropped this way â its content
...
7. **[~] Java target bump to 4.12.11.0 (from the 4.11 series; RFC-135).** Mechanical bump landed (pins + proto
   ...
   38, `PExistsPredicate`â`PExistentialValuePredicate`, added `PExistsValue.value` +
   `PRecordQueryExplodePlan.with_ordinality` â all `gen/`-only on the Go side, schema pinned by
   ...
   - **[x] R1 â DONE (RFC-136, merged in PR #336 `2095a4a7b`).** metadata-evolution field renames
     (`allow{Field,DeprecatedFieldRenames,Undeprecating}` + `RenameFieldsVisitor`) vs Java
     `MetaDataEvolutionValidator`. Landed in the same change as the RFC-135 4.12 upgrade â
     `rename_fields_visitor.go` + all three flags + the `validateField`/`comparePrimaryKeys`/index rewrite.
     RFC-136 was just never flipped from Draft (now corrected). **Small residual follow-up â DONE (RFC-157).**
     The per-node-type shapes + error paths the follow-up named were already ported (stale TODO); RFC-157
     closed the only genuinely-missing axis: the `messageTypeForField` re-derivation at depth â¥ 2 + the
   ...
   - **[x] R2 â DONE** â indexer 4.12 changes. **(a) DONE (RFC-137):** erase-indexing-metadata-after-readable â
     ...
     `SetEnforcedPostTransactionDelay(ms)` â fixed per-transaction delay replacing records-per-second
     when >0 (Java `setEnforcedPostTransactionDelay` #4229). **(c) DONE (RFC-139):** typed-record build-range
     preset (#4244) â `computeRecordsRange` (over the indexed types; null if any lacks a record-type PK
     ...
     `+0xff` end â fixed via `unpackRangeEndBoundary`/raw-boundary mark; codex P2 caught non-integer
     record-type keys â preset now gives up for them); added `RecordType.PrimaryKeyHasRecordTypePrefix()` +
     ...
     NAME for string/bytes explicit `SetRecordTypeKey` â a wire divergence from Java (which encodes every
     key type); the R2c preset already guards against it but the encoding itself should be fixed.
     **N/A:** `SlidingWindowIndexMaintainer` (+163, #4233-adjacent) â pure metrics
     instrumentation for an HNSW window-decorator index type Go does not have; index-scrub rangeSet fix
     (#4226) â Go has no scrubber. Gate: Torvalds + codex + @claude.
   - **[x] R3 â DONE (RFC-140)** â parser grammar: `(AT atAlias=uid)?` on `atomTableItem` (#4112) +
     ...
     it â codex caught 3 silent-drop holes (column collision, DDL, DDL-JOIN). Graefe + Torvalds + codex ACK.
   - **[x] R4** â EXISTS in the projection list (`PExistsValue.value`), RFC-141. Phase 1 (ExistsValueâValueWithChild + ExistentialValuePredicate, WHERE-EXISTS) + Phase 2 (FirstOrDefault re-architecture + projected `SELECT EXISTS(...)` + structural reject-the-rest backstop). Graefe + Torvalds + codex (14 rounds) ACK; full `just test` green; pushed (PR #336).
     **Phase 1 DONE:** representation collapse to Java 4.12's single mechanism â `ExistsValue` is now an
     ...
     NOT-EXISTS suite green after the swap, 10Ã deterministic. codex caught 3 eval/detection subtleties
     (QOV outer-row fallback, non-NOT_NULL misclassification, typed-nil binding). Graefe+Torvalds+codex ACK.
     **Phase 2 DONE (single existential):** re-architected the existential join to Java's emergent shape â
     ...
     WHERE-EXISTS is a residual `QOV IS NOT NULL` (NOT-EXISTS: `IS NULL`) filter above the FOD â Java's
     ...
     (`projected_exists_fdb_test.go`); WHERE-EXISTS + NOT-EXISTS suite green + 10Ã deterministic;
     ...
     **Phase 2 DONE (ORDER BY / LIMIT + scalar subquery alongside the EXISTS):** the fold now sees THROUGH
     intervening `Sort`/`Limit` (`findExistsFilterUnderUnaryChain`) â the builder emits `Project(Sort(Filter))`,
     so the existential filter is not the project's direct input â folds the projection into the existential
     ...
     no-leak, scalar in both column positions) â each revert-proof (all-false / NULL without the fix). Full
     sqldriver + conformance green, EXISTS suite 10Ã deterministic. Graefe+Torvalds ACK.
     **Phase 2 FOLLOW-UP (computed ORDER BY over projected EXISTS â Graefe):** `sortSource.sortKeyName` only
     ...
     bare-column bug just fixed, zero wire impact, exotic shape â next item, not buried.
     **Phase 2 ROUND-3 (safety guard + 3 fold shapes â codex r3):** the fold pattern-matched plan shapes and
     SILENTLY fell through to a plan evaluating the projected ExistsValue ABOVE the FlatMap (dead binding â
     ...
     3 round-3 fixes: **(1)** projected EXISTS + JOIN in FROM no WHERE â `attachOrSynthesizeExistsFilter` now puts
     ...
     uses the rebased projection as the FlatMap result (leg refsâmerged-outer qualified keys, existential
     QOVâinner FOD) for a projected EXISTS over a join; **(2)** ORDER BY on the EXISTS alias â `pullUpSortKeyValue`
     ...
     `NOT (EXISTS(...))` â `existsAtomOf`/`existsAtomInExpressionAtom` unwrap the paren-wrap RecordConstructor to
     ...
     ...` green; EXISTS suite 10Ã deterministic. **Supported:** projected EXISTS/NOT-EXISTS (corr/non-corr/empty/
     ...
     **(1) projected EXISTS + CORRELATED scalar subquery** (`SELECT id, EXISTS(...), (SELECT v FROM t2 WHERE
     ...
     leaving the correlated `ScalarSubqueryValue` unbound â that column read NULL every row. The existential
     SelectExpression and the correlated-scalar LEFT-OUTER-join select are incompatible structures (composing
     them is the multi-quantifier boundary the port rejects), so this shape is now CLEANLY REJECTED â both at
     the logical guard (`findUnfoldableProjectedExists`: a projected-EXISTS `LogicalProject` carrying
     `CorrelatedScalarSubqueries` â `ErrCodeUnsupportedQuery`) and defense-in-depth in `translateProject`
     (`len(CorrelatedScalarSubqueries) > 0` before the fold â nil). UNCORRELATED scalar + projected EXISTS
     ...
     folded record carries the bare output column â key NULL every row â DESC silently fell to scan order.
     ...
     qualified `FieldValue` key onto the bare output column (only when a bare output field matches â a JOIN-FROM
     ...
     `pkg/relational/core/...` green; projected-EXISTS suite 10Ã deterministic. **Rejected (added R4):**
     projected EXISTS + correlated scalar subquery in the same SELECT.
     **Round-5 (two more codex-found silent-wrong regressions, fixed):** **(P1) `SELECT * â¦ WHERE EXISTS(â¦)`
     ...
     the PROJECTED-EXISTS RecordConstructor, then fell through to merging outer+inner columns â the driver
     ...
     The round-4 fix stripped `t2.id`âbare `ID` for non-selected qualified keys; for a JOIN source the FlatMap
     ...
     strip-to-bare ONLY for single-table sources; for a JOIN source keep the QUALIFIED key (`T2.SK`) â the
     ...
     Revert-proof pins: `projected_exists_round5_fdb_test.go` â P1 `SELECT *`/`SELECT * NOT EXISTS` column-metadata
     tests; the full ORDER BY matrix {single-table, 2-table INNER JOIN}Ã{selected, NOT selected}Ã{qualified,
     unqualified}ÃDESC/ASC with colliding `sk`/`id` columns whose inverse leg orderings make a wrong-leg or no-op
     ...
     **Round-6 (two more codex-found silent-wrong regressions, fixed via the consistency root-cause):** both bugs
     ...
     had, so the key resolved by NAME against the output record â reading field `ID` (= `col1 AS id`), the WRONG
     column. Fix: `pullUpSortKeyValue` now runs the output-field-value match (`pullUpToOutputField`, extracted as
     the shared helper) FIRST for EVERY key shape â the same keyâoutput-field correspondence the normal ORDER BY
     alias path uses â so an alias key pulls up to the output field it IS (`X`), not a same-named column; the
     name-based resolution is the fallback for keys appended via remainingOrderBy. **(P2b) column LABEL regression
     for qualified projections** (`SELECT t1.id, EXISTS(...) FROM t1 â¦`): `deriveColumnsFromFlatMap`'s folded
     ...
     unaliased â label = bare leaf). A projected EXISTS now produces IDENTICAL label/type/nullability to a
     non-EXISTS control query. Revert-proof pins: `projected_exists_round6_fdb_test.go` â P2a ORDER BY by
     ...
     Full sqldriver bazel suite + `pkg/recordlayer/query/...` + `pkg/relational/core/...` green; EXISTS suite 10Ã
     deterministic; all round-1..5 tests still green.
     **Round-7 (computed non-selected ORDER BY over projected EXISTS â codex):** `ORDER BY col1 + 1` where
     `col1 + 1` is NOT a SELECT output. `collectExtraSortColumns` can only append NAMED columns, so the sort
     re-applied above the folded FlatMap evaluated the expression against a record lacking its inputs â NULL
     every row â no-op sort (wrong order). Fix: `translateProjectOverExistsFilter` now BAILS the fold for any
     computed ORDER BY key absent from the projection (â Â§8 guard cleanly rejects with `ErrCodeUnsupportedQuery`)
     ...
     **Round-8 (two more codex-found metadata/alias divergences, fixed via the alias-provenance root-cause):**
     ...
     by the alias `ID` â a Scan read NULL; for an unaliased `t2.id` over a JOIN the NLJ-rebased composite value
     (`FieldValue{Field:ID, Child:QOV}`) skipped the `Child==nil` bare-compare so the qualified `f.Name` leaked as
     a fake alias â label `T2.ID` not bare `ID`. Root fix: `foldedColumnDef` derives the column metadata DIRECTLY
     from the `RecordConstructorField.Name` â the SAME name the fold set and that `RecordConstructorValue.Evaluate`
     keys the executed row by (`out[f.Name]`): datum `Name = f.Name` (cannot diverge from the record key â never
     ...
     it). Revert-proof pins: `projected_exists_round8_fdb_test.go` â P1 explicit-alias-over-JOIN + unaliased-qualified
     ...
     + `pkg/relational/core/...` green; EXISTS suite 10Ã deterministic; all round-1..7 tests still green.
     **Round-9 (a WHERE-EXISTS correctness REGRESSION + a metadata divergence â codex):** **(P1, SILENT-WRONG,
     ...
     correlation (the inner CLOBBERED the outer â the pass-through row was NULL: `converting NULL to int64`),
     ...
     references the rename can't reach, so those keep the source-alias (leg) routing â the alias-shadow clobber
     ...
     named by expression text.** `SELECT id + 1, EXISTS(...) AS e FROM t` â the fold named the folded computed
     ...
     subqueries â all returned NULL/wrong before the fix) and `exists_computed_column_fdb_test.go` (P2: column-name
     parity with a `SELECT id + 1` control read dynamically, + correct values). Full sqldriver bazel suite +
     `pkg/recordlayer/query/...` + `pkg/relational/core/...` green; EXISTS suite 10Ã deterministic; all
     ...
     **(P2a, silent-wrong) MULTI-TABLE EXISTS inner correlating to a NON-rightmost leg.**
     `EXISTS (SELECT 1 FROM t2, t3 WHERE t2.t1_id = t1.id)` â the existential inner is a multi-table (comma/JOIN)
     ...
     neither the inner-correlation test NOR "outer-only" correctly â it was evaluated with NO inner binding and
     ...
     correlation OTHER than the FlatMap's outer leg(s) â i.e. it touches the inner subquery (`predicateTouchesInner`,
     ...
     correlation, inner-only join predicates, explicit `JOINâ¦ON` inner, NOT-EXISTS, non-correlated, outer-predicate
     combos, projected, and JOIN-in-FROM variants â NO shape needs rejection; the merged inner row resolves leg
     ...
     **(P2b, silent-wrong ORDER) qualified ORDER BY key whose bare name collides with a SELECT alias.**
     `SELECT col1 AS id, EXISTS(...) FROM t1 ORDER BY t1.id` â the fold stripped `t1.id`âbare `ID`, which equals the
     ...
     now VALUE-based (`sortKeyInOutput` / `sortKeySourceValue` â an output field must genuinely PROJECT the source
     ...
     match (raw key first â SELECT-list aliases incl. the computed EXISTS boolean â then the source-column value).
     The bare-alias ORDER BY path (`upgradeSortKeyValues` sets `k.Value` to the projected value) is UNCHANGED.
     Revert-proof pins: `projected_exists_round10_fdb_test.go` â P2a {2-leg non-rightmost/rightmost, 3-leg,
     inner-join-pred, explicit JOINâ¦ON, NOT-EXISTS, outer-pred, projected, projected-JOIN-from, WHERE-JOIN-from}
     ...
     EXISTS suite 10Ã deterministic; all round-1..9 + WHERE/NOT-EXISTS tests still green.
     **ROUND-11 (codex; the round-10 routing fix REGRESSED a scalar-subquery shape â two silent-wrong bugs, both
     fixed at root, NO shape rejected; Â§8h):** **(P1, silent-wrong) route by the KNOWN inner-leg set, not "any
     non-outer".** Round-10's `predicateTouchesInner` routed a predicate BELOW the FirstOrDefault iff it
     referenced ANY correlation other than the outer leg(s) â an ABSENCE test. An UNCORRELATED SCALAR SUBQUERY in
     a predicate (`price > (SELECT MAX(x) FROM t2)`) has its OWN `ScalarSubqueryValue` alias â non-outer yet NOT
     an inner table leg (a pre-evaluated EXTERNAL binding) â so the absence test pushed the scalar predicate below
     the FOD; alongside an empty NOT-EXISTS the FOD yields NULL, its IS-NULL residual admits every outer row, and
     the below-FOD scalar comparison never ran â the scalar predicate was SILENTLY DROPPED (`price > MAX(x) AND
     ...
     inner's FROM-source-alias set (innerCorr âª all legs the subplan DECLARES â every `SelectExpression.GetSource
     Aliases()` + ForEach/Physical quantifier alias, never a value-tree binding), distinguishing multi-table (innerCorr
     IS a declared leg â return ALL legs, keeping round-10) from renamed single-table (innerCorr NOT declared â return
     {innerCorr} only, re-avoiding the round-9 alias-shadow leak by construction); `predicateReferencesInnerLeg`
     routes below the FOD iff the predicate's correlation set INTERSECTS that set â scalar-subquery / parameter /
     ...
     subquery.** `SELECT id, EXISTS(...) FROM t1 WHERE price > (SELECT MAX(x) FROM t2)` â the fold's early return in
     `translateProject` bypasses `translateFilter`, the only place `f.ScalarSubqueries` is registered for
     pre-evaluation, so the WHERE scalar stayed unbound (NULL) â `price > NULL` â 0 rows. Fix:
     ...
     silent-wrong. Revert-proof pins: `projected_exists_round11_fdb_test.go` â scalar+NOT-EXISTS (empty inner),
     scalar+EXISTS, scalar+multi-table-NOT-EXISTS, projected-EXISTS+WHERE-scalar, parameter-marker control, audit
     controls; dataset built so the scalar EXCLUDES a NOT-EXISTS-true row (id 0, price â¤ MAX) so a dropped scalar
     loudly INCLUDES it. Routing revert â scalar+NOT-EXISTS returns `[0 1 2 3 4]` (want `[2 4]`), scalar+EXISTS `[]`
     (want `[3]`); fold-collection revert â projected+WHERE-scalar `[]`. NLJ-rule change â **Graefe ACK needed**.
     Full sqldriver bazel suite + `pkg/recordlayer/query/...` + `pkg/relational/core/...` green; EXISTS suite 10Ã
     deterministic; all round-1..10 + WHERE/NOT-EXISTS tests still green.
     **Round-12 (the CONVERGENCE BACKSTOP â codex r12 found EXISTS in WRAPPED/NESTED positions silently wrong):**
     ...
     parse tree, never `GetText`) and REJECTED cleanly with `ErrCodeUnsupportedQuery` â never silently mis-evaluated.
     ...
     regular-predicate bucket where the empty FirstOrDefault's NULL default is never removed â every outer row
     silently passed. New `query.CheckBuriedExistentialPredicate` (post-translation, alongside
     `CheckProjectedExistsFolded`, run on BOTH the SELECT and DML planning paths â `DELETE/UPDATE â¦ WHERE NOT (NOT
     ...
     `ExistentialValuePredicate` at any depth (`predicates.ContainsExistentialPredicate`) â reject. **(P1b nested projected EXISTS):** `CASE WHEN EXISTS(...) THEN ... ELSE ... END`, `EXISTS(...) AND x`,
     `(EXISTS(...) OR x)`, `NOT (EXISTS(...) AND x)` took the predicate path â the ExistsValue evaluated ABOVE the
     FlatMap with the binding dead â constant false / NULL. New `expr.NestedExistsProjectionError` (raised in
     ...
     â a DISTINCT error from `UnsupportedExpressionShapeError` (which callers swallow to the silent-wrong text
     ...
     never validating the (all-ELSE, silent-wrong) rows â now pins the clean rejection. Revert-proof pins:
     `projected_exists_round12_fdb_test.go` â P1a {double-NOT, OR, buried-in-AND, NOT-of-AND} + P1b {CASE-WHEN, AND,
     ...
     positions, all where the EXISTS is not a top-level boolean term: (a) JOIN ON (`ON EXISTS(...)`) â ON resolver
     has no SubqueryPlanner, EXISTS dropped, every joined row passed; (b) ORDER BY key (`ORDER BY EXISTS`, `ORDER BY
     CASE WHEN EXISTS`) â sort resolver has no SubqueryPlanner, key kept raw text, never evaluated â wrong ordering;
     (c) WHERE EXISTS BURIED in a scalar (`WHERE CASE WHEN EXISTS THEN 1 ELSE 0 END = 1`, `WHERE (EXISTS) = true`) â
     lowered into a CASE/comparison operand, folded to constant false â every row dropped. (a)+(b) via
     ...
     directly-handled iff reached through ONLY boolean nodes AND/OR/NOT/paren; buried via any scalar node) â run on
     the SELECT WHERE (plan_visitor.go), the DML WHERE (`DELETE/UPDATE â¦ WHERE <buried EXISTS>`, at the DML dispatch),
     and across an `INSERT â¦ SELECT` subtree (`expr.AnyWhereExistsInScalarPosition`; the INSERT-SELECT body is rebuilt
     ...
     <EXISTS-alias>`, a top-level WHERE EXISTS/NOT-EXISTS/AND-conjunct/paren, and a direct DELETE/INSERTâ¦SELECT WHERE
     EXISTS are NOT rejected (preserved).
     Both backstops verified revert-proof (disabling them returns the
     silent-wrong rows). NLJ-rule reasoning change â **Graefe ACK** + **Torvalds ACK** (got both). Full sqldriver bazel suite +
     `pkg/recordlayer/query/...` + `pkg/relational/core/...` green; EXISTS suite 10Ã deterministic; all round-1..11
     + WHERE/NOT-EXISTS tests still green. **Final supported = exactly the directly-handled positions; cleanly
     rejected = ANY EXISTS elsewhere** â codex round-13 bar: NO silent-wrong EXISTS case.
     **Round-13 (convergence fix â boundary stop; the round-12 detectors must NOT descend into nested subqueries):**
     codex round-13 found the FINAL convergence issue â an OVER-rejection (not silent-wrong, so that surface stays
     closed). The round-12 structural detectors recursed into nested subqueries, so an EXISTS in a nested scalar / IN /
     derived-table subquery's OWN clause was mis-attributed to the outer expression and falsely rejected â
     ...
     with the standalone subquery, which rejects) â guard added there; and the WHERE-walk error handlers swallowed a
     nested `*api.Error` into text-fallback (generic "could not plan") â now propagated verbatim. Tests:
     ...
     sqldriver + yamsql + plandiff + query/core green; EXISTS suite 10Ã deterministic; round-1..12 still green.
     **Phase 2 FOLLOW-UP (computed ORDER BY over projected EXISTS â Graefe):** `sortSource.sortKeyName` only
     classifies a bare/qualified column reference; a *computed* ORDER BY expression (`ORDER BY a+b` where
     `a`,`b` aren't in the SELECT) is skipped rather than appended, so it silently mis-sorts. Java's
     `Expressions.difference` uses a semantic `canBeDerivedFrom` check that appends the non-derivable computed
     ...
     **Phase 2 FOLLOW-UP (multi-existential, separate larger extension):** >1 existential quantifier in one
     query â multiple projected EXISTS, and EXISTS in WHERE *and* SELECT together (Java exists-in-select.yamsql
     lines 85, 94) â needs nested FlatMaps with intermediate record-bundling (`RETURN (..., exists(q5._0),
     exists(q5._1))`) and projection-QOVâbundled-field rewriting. This was NEVER supported in Go â multiple
   ...
   - **[x] R5** â correlated array UNNEST in FROM (`FROM t, t.arr AS x`) + `AT ordinality`
     ...
     a correlated `FlatMap(outer, Explode(FieldValue{arr} over QOV(outer)), â¦, resultValue, false)` â
     ...
     resetting per outer row); a name-based ordinal `FieldValue` (`ofOrdinalNumber` analog) binds ASâ`_0`,
     ATâ`_1`. AT on a non-array source converges on `ErrCodeWrongObjectType` (42809). Works: base unnest,
     ...
     **Refactor follow-ups (acknowledged-not-blocking by Graefe/Torvalds in the R5 full-pass review):**
     - **Dedup the group-key-output-name helper.** The aggregate group-key output name is computed three
       times under three names â `aggKeyName` (executor), `aggregateGroupKeyOutputName` (embedded), and the
     ...
     - **Refactor the NLJ rule's `rebaseOuterLegRefsToMerged`** to call the translator's generic
       `mapPredicateValues` walk instead of carrying its own predicate-tree recursion â the same recursion
     ...
     - **Unify the two SELECT-build paths behind one driver.** There are two SELECT builders â `PlanVisitor`
       (top-level) and the catalog builder `buildLogicalPlanForSelectWithCTECatalog` (subquery/DML/derived) â
   ...
   - **R5 final codex pass â DEFERRED to Jun 25 (codex quota exhausted).** R5 went through 32 codex rounds
   ...
   - **[x] R6** â `CARDINALITY()` function + index support (RFC-143). Phase 1 (scalar function, `e3adb2a4a`) +
     Phase 2 (cardinality index â function-key bridge, evaluator, DDL, planner-matching + general IS-NULL
     ...
     - **[x] Phase 1 â the scalar function (no index).** `CARDINALITY(array) â nullable INT` wired SQLâ
       `CardinalityValue` via a BY-NAME built-in dispatch at the `walkFunctionCall` leaf
       (`expr.walkUserDefinedScalarFunction` â `walkCardinality`; CARDINALITY parses as a bare-ID
       `UserDefinedScalarFunctionCall`). Fixed the 3 divergences in the orphaned `CardinalityValue`:
       `Type()` â `NullableInt` (Java `primitiveType(INT)`, nullable â metadata INTEGER, was NotNullLong);
       array-type validation at the walk site (non-array arg â `CANNOT_CONVERT_TYPE`/22000, matching the
       ...
       `IsCascadesSafeScalarFunction`/`evalScalarFunction` lists â it's a dedicated Value). Resolved array
       ...
       EXPLAIN). **Â§3a note:** Go writes plain-repeated arrays (no nullable-array wrapper), so an
       empty/unset array is wire-indistinguishable from NULL â reads as NULL â `CARDINALITY([])` is NULL,
       not 0 (Java's wrapper distinguishes them). The function is faithful; the empty-vs-NULL distinction
       is the latent Â§3a nullable-array-wrapper-WRITE gap (below), out of scope for Phase 1.
     - **[x] Phase 2 â index support** (the 4.12.3 delta; Graefe-gated planner matching). A `CARDINALITY()`
       ...
       - **Step 4 â evaluator:** `CardinalityFunctionKeyExpression` (`key_expression_cardinality.go`) embeds
         ...
         Empty/unset Go array â NULL key (Â§3a-consistent with the scalar).
       - **Step 6a â KeyExpressionâValue bridge:** `ValueIndexScanMatchCandidate` carries a parallel
         ...
         from the index's `KeyExpression` (the recordlayerâcascades half of the bridge).
       - **Step 5 â index DDL:** `CREATE INDEX â¦ AS SELECT CARDINALITY(arr) â¦ ORDER BY` recognises the
         bare-ID CARDINALITY call by typed node and routes to `Builder.AddCardinalityIndex` â
         `CardinalityExpr(field(arr, Concatenate))` value index (`buildCardinalityIndex`).
       - **Step 6b â predicate matching:** `WHERE CARDINALITY = N` falls out of `valuesMatchColumn`; added a
         `*CardinalityValue` alias-invariant arm (mirrors the FieldValue / distance-row-number arms).
       - **Step 6c â ORDER-BY rework:** `rule_ordered_index_scan.go` now matches the sort key by Value-tree
         equality against the candidate's `ColumnValue` (the `sortKeyMatchesColumn` helper), not a
         `FieldValue`-name string â so `CardinalityValue` sort keys bind (incl. REVERSE). Plain-field
         ORDER BY unchanged.
       - **Value-index NULL ranges (Java-aligned, surfaced by the `IS [NOT] NULL` cases):** `IS NULL` is now
         ...
         matching â Java's `ScanComparisons.getComparisonType(IS_NULL)==EQUALITY` / `NOT_NULL==INEQUALITY`.
         ...
         divergence (no value index bound `IS [NOT] NULL` before) â `TestPlanHarness_IsNull` updated to the
       ...
       - Tests: `array_cardinality_index_fdb_test.go` (EXPLAIN-asserted: `= N`, `IS [NOT] NULL`, ORDER BY
         ...
         `cardinality_ddl_test.go` (DDL â `CardinalityFunctionKeyExpression` + catalog proto round-trip),
         `key_expression_cardinality_test.go` (evaluator fast paths + wrapper + wire round-trip). 10Ã
     ...
     - **[ ] Â§3a follow-up â nullable-array-wrapper WRITE.** Go's metadata builder emits a plain repeated
   ...
   - **[ ] R6 follow-up â BITAND/BITOR/BITXOR unreachable through the walker (pre-existing drift).** The 3
     ...
     `ScalarFunctionValue("BITAND"/...)` that is then rejected by `isCascadesSafeValue`/the catalog â they
   ...
   - **[x] R7** â LEFT/RIGHT OUTER JOIN reclassification + 4.12 null/boolean fixes (RFC-144). The parity
     sweep (53 ported `join-tests-outer.yamsql` cases) found + fixed **6 real outer-join divergences**
     (JOIN USING â cartesian; chained outer joins + INNER-then-LEFT dropped NULL-padding; derived-table-
     ...
     hygiene â no SQL producer) with BC1 faithful `rejectsNull` + BC2 `ImplementSimpleSelectRule` exact-
     ...
     - **[ ] R7 follow-up (Torvalds) â JOIN USING typed-path lowering.** `synthesizeUsingOnExpr`
       ...
       `resolveJoinUsingClause` â `resolveFunction("=")`). Replace the text-splice with typed Value/expr
       construction. Non-blocking; pre-existing style deviation.
     - **[ ] R7 follow-up (Torvalds) â USING `.asHidden()` + `SELECT * USING` test.** Go does not implement
       Java's right-column hiding for USING, so `SELECT * â¦ USING (id)` emits the join column twice
   ...
   - **[~] R8** â conformance rebaseline from a live 4.12.11.0 run. **Partial in the bump PR:** the 7
     RFC-082 annotations 4.12 lifted were reclassified to keep the conformance gate green (4 Java bug-fixes
     â plain equivalence; `left_outer_join_basic` + `where_case_returns_bool_probe` lifted â plain
     equivalence; `bare_bool_where_rejected` â JavaSucceedsGoRejects). **Remaining:** full corpus re-sweep,
...
> **CI: the single self-hosted box is intentional â NOT a tracked problem.** We work locally + sequentially;
> the slowness during the RFC-115â117 merge wave was a one-off (four PRs squeezed through one runner at once).
> Don't re-file a "second / ephemeral runner" or "CI reproducibility off the box" item. (The Â§7 CI-volume
> tofu/cloud-init is fail-safe dead-ish code â `prevent_destroy` protects the box and nothing auto-applies â
> harmless to leave; revisit only if the box actually starts failing on disk.)

> **C3 (RFC-056 lazy GetKey iterator) â DONE (RFC-057):** `rywSegCursor` replaced the materializing
> `buildSegmentsLocked` (55,437Ã faster at N=100k, behavior-identical). The residual go-vs-cgo 1007-rate near
> the 5 s MVCC edge is characterized (RFC-056 #235, TODO `C2-followup`) as accepted perf/timing, not a wire
> bug. Don't re-file.

- [x] **D1 â `SimTransport`** (frame-level fault injection) â DONE (RFC-118; FDB-C-dev + Torvalds +
...
- [x] **B2 â libfdb_c escape hatch** â DONE (RFC-109, PR #295). `BackendDatabase` interface
  (`pkg/fdbgo/fdb/backend.go`) + a CGo-backed impl over `cgofdb` (`pkg/fdbgo/libfdbc/backend.go`),
  selected at BUILD time via the `libfdbc` build tag (`pkg/internal/fdbclient`, netgo/netcgo idiom) â
...
> Shipped this session (stacked on `master`, merging bottom-up #303â#304â#305/#306):
...
## Client launch-readiness â prioritized stack (2026-06-13)
...
remaining launch-readiness work, ordered by priority â **Go-code correctness first, escape hatch
last** (it's a pre-launch safety net, not a blocker for adoption). Driven one at a time via the
`fdb-client-engineer` skill (RFC â FDB-C-dev + Torvalds + codex review â implement â review-clean),
each on its own stacked branch.

1. **[x] GRV cache parity â `USE_GRV_CACHE` opt-in (default off), client correctness.** DONE
   (RFC-104; also fixed the `updateMinAcceptable` MAXâMIN divergence = the filed "RFC-056 item 2").
   `M` Â·
   ...
   wrong answer: a Go txn served a cached version OLDER than a libfdb_c-committed seed â seed keys
...
2. **[x] Retry-predicate fidelity â `fdb.IsRetryable` vs `client.isRetryable`.** DONE (RFC-105):
   no bug â pinned each to its C++ analog + deleted the dead 4th predicate `wire.FDBError.Retryable`.
   `S` Â·
   fdb-client-review. The two predicates list different code sets. The fix is NOT naive unification:
   in C++ `fdb_error_predicate(RETRYABLE)` â  `Transaction::onError`'s set (1039 predicate-retryable
...
3. **[x] Resource limits / backpressure (multi-tenant launch safety).** DONE (RFC-106a) â clean
   tri-ACK (Graefe + Torvalds + codex), HEAD `a396227e`. `M` Â· query-engine-gated. Statement timeout
   (per-request ctx deadline â 54F01), scan-limit options wired to `ExecuteProperties` with Java
   ...
   value drain helper, and cursor iterator â none silently truncates. The per-query MEMORY byte budget
...
4. **[x] Make CI gates real.** DONE (RFC-107) â Torvalds ACK + codex clean, HEAD `b1779f49`. `M`.
   ...
   preflight on EVERY FDB-driving gate (else `FDB not available` skips â exit 0 â green with no
   ...
   skill â root cause: `codex exec` blocks on open stdin.) (TODO-production P1.6.)
5. **[~] CI reproducibility â off the single Hetzner box. UNTRACKED (owner decision, 2026-06-18).**
   The single self-hosted box is intentional: we work locally + sequentially; the RFC-115â117
   ...
   TODO-prod P1.8.)
6. **[x] libfdb_c escape hatch (Backend interface + CGo-backed impl) â DONE (RFC-109, PR #295).**
...
`buildCorrelatedExists` (pkg/relational/core/embedded/logical_predicate.go) is ~412 lines â mostly essential
complexity (INNER/LEFT/RIGHT/FULL Ã correlation Ã ordering Ã CTE Ã nested-subquery), not cruft (Torvalds
ACK'd it). The one clean extraction: pull the per-join ON classification loop body (~70 lines: node-vs-lift-vs-decline)
into `classifyJoinOn(...) (nodeOn, lifted, err)` â the densest, most independent, most nameable sub-decision;
would drop the function to ~340 and make the loop legible. Recommended, not a should-fix.

### [ ] query-engine follow-on (Torvalds ACK book): extract `buildExistentialFlatMapTail(...)` â the existential-wrap tail (belowFOD â FirstOrDefault(NULL) â optional IS[NOT]NULL residual â FlatMap) is now stamped out 4Ã (implementExistentialSelect ~:924, the 2-leg fold arm ~:2230, the N-way arm ~:2612, yieldExistsFlatMap ~:2836). A future residual-polarity/FOD-contract fix has FOUR landing sites (the EXISTS correlation-leak fix already had to be applied in >1). Extract the invariant tail once the N-way arm is battle-tested; the differing parts (rebasing, FlatMap outer, memo bookkeeping) stay at the call sites. Not a blocker â booked follow-up.

### [x] query-engine: a DERIVED-TABLE inner in a correlated EXISTS loses its body â wrong rows â FIXED 2026-07-08 (resolve-body path)
A correlated EXISTS whose inner FROM is a DERIVED TABLE (`(SELECT â¦) AS d`) entered via the `buildCorrelatedExists` fallback's no-WHERE/no-ON fast path (entered only because the ignored inner SELECT references an outer column) returned silent-wrong rows: the fast path rebuilt each derived source as `NewScan("d")` â a scan of the non-existent catalog table `d`, which the executor treats as EMPTY â EXISTS tested the wrong (empty) relation. Confirmed at BASELINE `a34e9e21d` and reproduced fresh (`[[1 false],[2 false]]` where correct is `[[1 true],[2 true]]`, d={1} non-empty) â PRE-EXISTING master bug, not introduced by the JOIN-ON work.

DEFECT CLASS (both source positions, found in review): the silent-wrong hit BOTH the PRIMARY source (`correlatedInnerPrimarySource`) AND a comma/JOIN **LEG** (`correlatedSubqueryJoinRight` â `NewScan(j.tableName)`). @claude's correctness gate caught the leg twin: `â¦ EXISTS (SELECT â¦ FROM ord a, (SELECT â¦) AS d) â¦` scanned leg `d` as EMPTY â `ord Ã â` = â â EXISTS wrong. FIX: a shared `buildDerivedInnerCarrier` helper builds the derived BODY via `buildLogicalPlanForQueryBodyWithCTECatalog` and wraps it in the `LogicalCTE(alias)` carrier (mirroring `buildOuterPlanOnDerived` and the CTE-aware resolution); BOTH the primary and every leg route through it, so the inner FROM carries the subplan. A body that can't be planned declines loudly. Its SQLSTATE is FAITHFUL to the inner query and POSITION-INDEPENDENT: an undefined column in the derived body surfaces 42703 in BOTH the projected (`SELECT â¦, EXISTS`) and the WHERE (`WHERE EXISTS`) position (codex P2) â `buildDerivedInnerCarrier` returns the wrapped `*api.Error` UNWRAPPED so `mapPredicateWalkError` doesn't rewrite the WHERE-position code to 0A000 (it matches `CorrelatedExistsError`â0A000 before the raw api.Error); a body that returns no plan with no structured cause â 0A000. The EXISTS WHERE/ON path and the correlated-SCALAR path (`buildCorrelatedScalar`) build their inner SCOPE first (`ResolveTable`), so a derived source there declines loud (0A000) before any operator build â correct-or-conservative, no wrong rows; pinned so they never regress to a silent bare scan. Regressions: derived_inner_correlated_exists_fdb_test.go (17 bipolar-discriminating EXISTS subtests incl. real/derived-primary Ã derived-LEG poles + the helper's own undefined-column loud-decline branch pinned across projected, WHERE-primary, and WHERE-leg positions) and scalar_derived_source_decline_fdb_test.go (scalar derived-primary + derived-leg 0A000 pins). NOTE for future red-first on this repo: in-place `git checkout` of a source file + re-run bazel test is UNRELIABLE (action cache serves the stale library) â EDIT the library content (forces a content-hash rebuild), use a FRESH test file, or `bazelisk clean` to verify red-first.

### [x] query-engine: CORRELATED EXISTS with an explicit `JOIN..ON` inner DROPS the inner ON â FIXED 2026-07-08 (codex-surfaced on the N-way review; root-caused + fixed in the front-end; 7 codex rounds folded: INNER-drop, OUTER-fold, mixed-orderings Ã2, nested-subquery, shadowing+SQLSTATE, CTE fast-path)
ROOT CAUSE (the filed framing was WRONG â it blamed the Cascades 2-leg fold arm; the real defect is UPSTREAM
in the SQLâlogical front-end): `buildCorrelatedExists` (pkg/relational/core/embedded/logical_predicate.go)
rebuilt the inner FROM's join tree with `NewJoinWithPredicate(op, right, kind, nil)` â the ON hardcoded to
nil â and its no-WHERE early return dropped it entirely. So `EXISTS(SELECT 1 FROM e JOIN f ON f.fid=e.fid
WHERE e.eid=p.id)` produced a bare cross-product â EXISTS silently true over an empty inner join. This is why
the three Cascades-layer guard attempts all mis-scoped â wrong layer; the ON was gone before Cascades ran.
...
an inner-inner ON below the FOD and lifts an ON-embedded correlation â same as a comma-join WHERE conjunct.
...
A correlated projected/WHERE EXISTS whose inner is an **explicit inner join** â `EXISTS (SELECT 1 FROM e
JOIN f ON f.fid=e.fid WHERE e.eid=p.id)` â returns EXISTS=true even when the inner join `e JOIN f` is
...
- **The N-way arm** (`implementNWayJoinWithExistential`, RFC :2908/:3033) is FIXED conservatively: it
  fail-closes on ANY non-scan existential inner (`existInnerIsScanSafe`) â declines the buggy explicit-join
...
- **The 2-leg fold arm** (`implementJoinWithExistential`) has the PRE-EXISTING silent-wrong and is NOT yet
  fixed â three guard attempts all mis-scoped: (a) `!existInnerIsScanSafe` alone over-declines the working
  ...
  `JOIN..ON` (ON on the join node) under correlation" vs "comma-join WHERE under correlation" â a distinction
  ...
  once fixed: 2-leg `EXISTS(... e JOIN f ON ...)` over an empty inner join â `[[10 false]]`.

`GROUP BY <qualified-col> + <expr>` returns NULL for the key on ANY multi-table query:
`SELECT WSRC.SID + 1, COUNT(*) FROM WSRC, WAUX GROUP BY WSRC.SID + 1` â `K=<nil>` ("unresolved
reference WSRC.SID + 1"). Reproduces with NO unnest/gather/shadow, and identically on the pre-RFC-173
name-read wrap. A BARE qualified group key (`GROUP BY WAUX.WV`) resolves fine â only the COMPUTED
...
rfcs/173-ordinal-column-resolution.md Â§NOT-OUR-BUG #2). Sibling of the derived-table+GROUP-BY 42703
gap (Â§NOT-OUR-BUG). Fix: trace the computed-group-key operand resolution over the qualified reference.
...
:669, â¦) BEFORE the underlying actor observes `resetPromise` â so a transaction that is both
poisoned (deferred 2000/2018) and Cancel()ed surfaces the deferred code from every op. Go's
uniform entry order is cancelled-first (checkCancelled â deferredErr â checkTimeout, in
...
and pinned. Resolution needs a differential probe (poison, Cancel, Get on both clients â mind
...
### [ ] fdbgo/client: GRV reply's ProxyTagThrottledDuration is discarded â GetTagThrottledDuration undercounts vs libfdb_c
...
the field's comment falsely claimed reply-side accumulation â comment fixed there; the
wiring (batcher â per-waiter accumulation, mind GRV batching fan-out semantics) needs its
own small FDB-C-dev cycle with a differential pin against libfdb_c.

### [ ] fdbgo/client: rywDisabled is a plain bool read lock-free on every read-path gate â same reset-boundary class as timeoutNs/deadlineNs
...
re-application â the exact class fixed for timeoutNs/deadlineNs (atomics) on the reset boundary.
...
### [x] planner: `LIMIT 0` returned ALL rows unless the inner was a bare table scan (Go-only LIMIT extension) â FIXED 2026-06-28
...
`NewFullUnorderedScanExpression(nil, UnknownType)`, believing nil record-types meant an empty source â but nil means
"scan ALL record types", i.e. a full table scan. The broken full-scan alternative won on cost over the correct
`Limit(0, â¦)` whenever the inner was more than a bare scan (the bare case kept `Limit(0, Scan)`). **Fix:** deleted
...
pre-existing `TestFDB_LimitZeroReturnsNothing` only covered the bare case â a dimensional gap.

### [~] executor/types: cross-type numeric SARG on an INDEXED column â PARTIALLY FIXED 2026-06-28 (int-const vs DOUBLE-col done; IN / float-const / col-col remain)

**FIXED (2026-06-28):** the common + severe direction â an INTEGER literal vs a DOUBLE indexed column, for both
comparison ops (`=,<>,<,<=,>,>=`, via `expr.ResolveComparison`â`widenIntConstAgainstDouble`) AND IN-lists (`d IN
(5,7)`, via `expr.ResolveIn`). The int constant(s) are widened to DOUBLE (`5`â`5.0`) when the other operand /
the LHS is a non-constant DOUBLE, so the SARG packs the right tuple type while the indexed column stays bare (index
still matched â verified with an EXPLAIN IndexScan assertion). Regression: `crosstype_const_sarg_fdb_test.go`. Full
53-target suite green (no plan-shape/result regression); Graefe + Torvalds ACK. **STILL BROKEN (deferred â need the
broader MaximumType+PromoteValue design that SUBSUMES this special case, not a parallel branch):**
- DOUBLE/FLOAT literal vs INT/LONG column (the narrowing direction): `n_bigint = 5.0` / `n > 6.0` â `[]`. Needs
  per-operator floatâint exactness (floor/ceil + integral check), so it was NOT folded into the safe intâdouble fix.
- col-vs-col cross-type join: `a.xbig(BIGINT) = bd.ydbl(DOUBLE)` (both non-constant) â still empty.
- FLOAT (not DOUBLE) columns â only DOUBLE handled. **SEVERE + now pinned
  (indexed_float_sarg_probe_test.go, 2026-06-28):** an INDEXED FLOAT(32-bit) column
  returns ZERO rows for EVERY equality/range comparison â even `f = 1.5` where 1.5 is
  ...
  index with a mismatched FDB tuple type code â matches nothing. Non-indexed FLOAT and
  indexed DOUBLE are both correct. Note `promoteConstant`
  (value_constant_object.go:150) has no `float64âTypeCodeFloat` case. The fix is a
  cross-WIDTH SARG decision (compare in float32-space, or widen the float32 scan +
  residual-filter in double-space) â part of the MaximumType/PromoteValue design
  below, Graefe-gated.
...
which equals the stored 5.0). Same for a cross-type index-probe join `a.xbig(BIGINT) = bd.ydbl(DOUBLE)` â empty
...
The RESIDUAL (non-index) path is CORRECT â `xbig = 5.0` matches via `cmpAny`'s runtime numeric coercion â and an
...
no-op passthrough â an incomplete port; Java's PromoteValue coerces) PLUS the data-access matcher handling a
`Promote(col)`-wrapped operand. That touches EVERY comparison's resolution + core value-eval semantics + the
matching/SARG infra (Graefe-gated, high blast radius) â not a safe unattended change. Repro shape lives in
`cross_type_join_probe_test.go` (the BIGINT=DOUBLE case is noted, not asserted, pending the fix). intâbigint joins
work (identical tuple encoding); the gap is specifically int/bigint â double/float (and presumably â string).

**EXPERIMENT FINDING (2026-06-28, saves the next implementer a dead end):** I tried the "obvious" Java-aligned
approach â make `PromoteValue.Evaluate` coerce (via `promoteConstant`) and wrap the narrower int operand in
...
regressed to `[]` â i.e. the data-access matcher does NOT route the comparand through `PromoteValue.Evaluate` when
...
because the matcher packs `ConstantValue.Evaluate()` directly â a Promote wrapper is transparent-to-the-matcher and
gets unwrapped/ignored. **The real fix must coerce at the matcher / SARG-range-build level** (where the comparand
is turned into a tuple element â e.g. thread the index column's key type into `scanComparisonsToTupleRange` and
...
"index key column types" accessor â `executeIndexScan` has `idx` whose `RootExpression` (a KeyExpression with
`ColumnSize()`) lists the indexed columns, but per-position TYPES must be derived by mapping each key field to its
record-type field type (handle nested / grouping key expressions). intâdouble coercion is exact (the common +
col-col + severe-inequality direction); floatâint (narrowing) needs per-operator floor/ceil + an integral check.

**SEVERITY UPDATE (broader + worse than first thought):** the gap is not limited to equality missing rows. With a
DOUBLE indexed column `d â {5.0,7.0,10.0}` and INT literal comparands:
- `d = 5` â `[]` (misses; should be `{5.0}`) â equality, as documented.
- `d IN (5,7)` â `[]` (misses; should be `{5.0,7.0}`) â IN-list has the same bug.
- `d > 6` â `{5.0,7.0,10.0}` â returns 5.0 which is NOT > 6 (**WRONG ROWS**, not just missing).
- `d < 8` â `[]` â returns nothing though 5.0,7.0 ARE < 8 (**WRONG ROWS**).
- `d BETWEEN 5 AND 8` â `{5.0,7.0}` (CORRECT â inconsistent with `>`/`<`; likely a residual re-check on the
...
ints, so an int-bound range over a double index degenerates to all-or-nothing. This RAISES priority â `WHERE
...
fix). Graefe should pick. Either way the int/float-exactness rules (floatâint inequality bound: floor vs ceil per
operator) must be handled.

**Scope note (good news):** the INSERT/UPDATE *store* side is CORRECT and wire-safe â an int literal written to a
...
and narrowing doubleâBIGINT is conformantly rejected (22000, no doubleâlong promote). So the bug is confined to the
COMPARISON/SARG comparand promotion, not record storage â the wire format of stored records is fine.

**Implementer caveat (found while scoping):** a naive "promote both operands to MaximumType" will REGRESS plan
shapes. INTâLONG (and any tuple-encoding-compatible pair) must NOT be promoted â wrapping the indexed column in a
...
operand that is (or could be) the indexed column â wrap only the narrower NON-indexed comparand, leaving the wider
(indexed) operand bare so its index is still matched. Also confirm whether FLOATâDOUBLE tuple encodings differ
(if so they need the same treatment). This is why it needs a Graefe DESIGN review, not just an ACK.

### [~] query-engine: scalar-subquery cardinality (21000) NOT enforced for CORRELATED subqueries â Go-extension inconsistency (Graefe design, found 2026-06-28)

**Projection non-aggregate case DONE (option (a) â extend 21000 to the correlated path).** With no user
...
raises 21000 on a second â a non-pushable barrier that runs fresh per outer row under the driving FlatMap (so
at-most-one is per outer row). A user-written LIMIT keeps `StrictSingle=false` and truncates (deliberate
intent). Pinned by `scalar_subq_correlated_card_test.go` (multi-rowâ21000, singleâvalue, emptyâNULL,
user-LIMITâtop-1) and the flipped `scalar_subquery_correlation_probe_test.go`.

**REMAINING (kept open â `StrictSingle` is projection + non-aggregate only, by design of the focused PR):**
- **Aggregate correlated scalar WITH GROUP BY, >1 group** (`(SELECT status FROM o WHERE o.cid=c.id GROUP BY
  ...
  silently truncates. (Bare aggregates â no GROUP key â are always single-row, so no gap there.)
- **WHERE-clause correlated scalar** (`WHERE v = (SELECT â¦ WHERE correlated)`): a different lowering path;
...
- **Java enforces NO cardinality at all** â its `ErrorCode` enum (fdb-relational-api) has no 21000 /
...
- **Go added 21000 enforcement** (`executor/scalar_subquery.go`, SQL-standard, stricter than Java) â but ONLY on
  the NON-correlated path. `SELECT (SELECT salary FROM emp) FROM dept` â 21000. â
...
(SQL-standard, consistent â the RFC-077 join's inner needs an at-most-one-or-error operator, replacing the
...
`scalar_subquery_correlation_probe_test.go` (`corr_scalar_multi_row_currently_unenforced` â flip to expect 21000
if (a) is chosen). Not a safe unattended change (Cascades/RFC-077, high blast radius); needs the Graefe RFC.

### [ ] driver: NO read-your-writes inside an explicit transaction â SELECT auto-commits (divergence, found 2026-06-28)

Inside `BeginTx`, DML (INSERT/UPDATE/DELETE) joins the explicit FDB transaction (`runInTx` â `activeTx.rctx`) and
is atomic on Commit / undone on Rollback â correct. But **SELECT runs in a FRESH auto-commit transaction**
...
- **No read-your-writes:** a SELECT in the tx does NOT see the same tx's uncommitted DML (`UPDATE v=777` then
  `SELECT v` â 100; `INSERT id=2` then `SELECT WHERE id=2` â no rows).
...
executor opens its own record store; binding SELECT to the user write-tx would add read-conflict ranges â the same
...
serializable reads) â a Cascades/executor + driver-tx architecture change (Graefe). Until then it's a real
...
### [x] DDL error classification â duplicate-column + PK-over-unknown-column now clean 42-class errors (2026-06-28)
...
- duplicate column (`..., x BIGINT, x STRING, ...`) â clean **42701** (was `XX000: protodesc.NewFile: descriptor
  "T.X" already declared`).
- PRIMARY KEY over an undefined column (`... PRIMARY KEY (nope)`) â clean **42703** (was `XX000: build
...
Unquoted identifiers work correctly (case-insensitive, folded to upper case â `MyCol` resolves as
...
- But NO explicit reference resolves it: `SELECT keepcase` / `KEEPCASE` / `KeepCase` / `"KEEPCASE"` / `"KeepCase"`
  all â `42703 column does not exist`. The column is effectively write-and-`SELECT *`-only â unreferenceable by
...
resolution for quoted identifiers. Java has a consistent case-sensitivity model â `SemanticAnalyzer.normalizeString
(string, caseSensitive)` ("taken as-is if caseSensitive, upper-cased otherwise") with an `isCaseSensitive` flag set
per-identifier by quoting â so quoted identifiers round-trip (created and queried by the same quoted name). Fix =
...
**Empirical diagnosis (CORRECTS the earlier framing â verify the real symptom before fixing):**
- The `SELECT *` "shows KEEPCASE" observation is a RED HERRING â that is the JDBC result-set *display* name
...
- DDL does NOT fold the quoted case: `DOUBLE_QUOTE_ID` (RelationalLexer.g4:1332 = `'"' ~'"'+ '"'`) keeps the quotes,
  so `parseTableDefinition` â `StripIdentifierQuotes(Uid().GetText())` (ddl.go:677) yields `"KeepCase"`â`KeepCase`
  (preserved), and `builder.go:485` writes the proto field as `col.name` = `KeepCase`. So the on-disk proto field
  IS `KeepCase` (case preserved) â and that should be wire-faithful, but VERIFY what Java's descriptor names it.
- The real bug is in RESOLUTION: a `Fields().ByName(...)` lookup never matches the preserved-case field. Smoking gun
  ...
  column-ref path. Empirically `SELECT "KeepCase"`, `KeepCase`, `KEEPCASE`, `keepcase`, AND `"KEEPCASE"` ALL â
  42703 â i.e. no reference form (not even the exact stored/displayed name) resolves, which rules out a simple
  one-sided case fold and points at the scope/column-registration step, not the reference normalizer alone.
- Next-session fix: unify ALL column-resolution lookups on ONE Java-faithful `normalizeString` (quotedâas-is,
  unquotedâupper) AND ensure the record-typeâsemantic-scope column registration keys columns by that same form;
  then `SELECT "KeepCase"` resolves and `SELECT KeepCase` (unquotedâKEEPCASE) correctly does not. Add the e2e probe
  + verify the proto field name matches Java (wire) before landing.

### [x] dml: wire DML DRY RUN through to the dry-run store primitives (Java parity) â DONE (RFC-158)

`<DML> ... OPTIONS (DRY RUN)` now PREVIEWS the would-be-affected rows without committing, matching
Java (AstNormalizer.visitQueryOptions â Options.DRY_RUN â ExecuteProperties.setDryRun â the DML plans
branch to dryRunSave/DeleteRecordAsync). Replaces the former fail-closed reject (the data-loss
stopgap). Threading is STATEMENT-scoped (Torvalds NAK on the v1 connection-options design â that
would have gone sticky / never-fired, resurrecting the data-loss bug): `dmlHasDryRunOption` â
`cascadesPlan.dryRun` â `paginatingRows.dryRun` â `ExecuteProperties.DryRun`, where
executeInsert/Update/Delete branch onto `DryRunSaveRecord`/`DryRunDeleteRecord`. Existence checks
still fire (INSERT of an existing PK under DRY RUN â 23505, parity). EXPLAIN renders the plan (never
...
### [x] dml: DryRunSaveRecord secondary-UNIQUE / intra-statement-PK preview scope â RESOLVED (matches Java, NOT a bug)
...
=true)` EARLY-RETURNS at FDBRecordStore.java:578 â BEFORE `serializeAndSaveRecord` (staging) and
...
Java exactly. Adding secondary-index validation would make Go STRICTER than Java â a conformance
...
+ documented at `DryRunSaveRecord` (store_api.go). No action â do NOT "fix" it into a divergence.

### [ ] dml: DELETE/UPDATE ... RETURNING silently ignored â Java supports it (divergence, found 2026-06-28)

The shared grammar carries `(RETURNING selectElements)?` on `deleteStatement` and
`updateStatement`, and **Java supports it** â `QueryVisitor.visitDeleteStatement:848` /
...
NOT data loss (the DML is correct) â a Java-supported feature left unimplemented.
...
RETURNING is a 42601 â not in the INSERT grammar â so it's a separate, larger gap.
...
mutation operator's output â so Go needs a `Project`-over-DML the Cascades planner can plan
...
### [x] ddl: in-template index/column errors wrap to 42F59, burying the specific SQLSTATE â DONE (RFC-161)
...
0A000 unsupported INCLUDE, â¦) as its own SQLSTATE instead of masking it under the generic 42F59
(ErrCodeInvalidSchemaTemplate â the wrong code for a duplicate column). Confirmed real vs Java: its
...
### [ ] ddl: implement covering indexes â CREATE INDEX ... INCLUDE (cols) (Java parity; found 2026-06-28)

`CREATE INDEX ... ON t (a) INCLUDE (b)` is currently REJECTED (0A000 "INCLUDE clause
(covering index) is not yet supported", ddl.go parseIndexDefinition) â a fail-closed
stopgap for what was a SILENT divergence: Go dropped the INCLUDE clause and created a
PLAIN index, while Java (DdlVisitor.java:249 â addValueColumn) creates a COVERING
...
Go's record layer ALREADY supports covering indexes â KeyWithValueExpression
(index_maintainer.go:107/217/362, "Matches Java's KeyWithValueExpression path"). The
gap is only the SQLâmetadata DDL wiring: (1) Builder.AddIndex (core/metadata/builder.go)
...
### [x] metadata: UUID columns are indexable end-to-end â RFC-162 DONE (item 1021, PR #397)
...
(TIMESTAMP, DATE, FLOAT, INTEGER, BOOLEAN, BIGINT, DOUBLE, STRING, BYTES â pinned in
...
treats UUID as a first-class indexable PRIMITIVE â `DataType.Primitives.UUID` /
`Type.uuidType()` (SemanticAnalyzer.java:724, DataTypeUtils.java:152) â so a UUID
...
**DONE â RFC-162 (Graefe design + impl ACK'd; PR #397).** Sites 1-2 LANDED: the validator accepts the
tuple_fields.UUID message (`isTupleField`) and the maintainer writes the entry as a `tuple.UUID`
(`scalarToInterface`â`uuidMessageToTuple`), byte-identical to Java â pinned by
`recordlayer/uuid_key_encoding_test.go` (unit, wire format) + `indexable_types_probe` (CREATE INDEX +
INSERT succeed). REMAINING = the read side. Mapped to exact sites â it is ONE ATOMIC change (each piece alone regresses
something; UUID must flow as `tuple.UUID` end-to-end, string only at the driver boundary, per Graefe):
- **A â field read.** `query_result.go protoFieldToGo` (~:107) currently renders a UUID field via
  `uuidMessageToString`; change it to return the 16-byte `tuple.UUID` (reuse `uuidMessageToTuple`'s msbâlsb
  layout) so the FILTER path compares `tuple.UUID == tuple.UUID`.
- **B â comparand. THE CRUX (substantial; Graefe impl review).** Two parts, both needed:
  - **B1 (insertion):** wrap the comparand in `PromoteValue(literal, NotNullUuid)` where a UUID column
    meets a STRING literal. NOTE: Go currently inserts NO comparison promotes at all â numeric promotion
    ...
    So B1 is a NEW pattern in the predicate builder (the comparand evaluated as STRING-typed today â that
  ...
  - **B2 (eval arm):** `PromoteValue.Evaluate` (values.go:2380) is a NO-OP today (delegates to Child).
    Give it the single StringâUUID arm Graefe approved: when `Target` IsUuid and the child evaluates to a
    string, `uuid.Parse` â 16-byte `tuple.UUID`. With B1+B2 the index/PK probe, range/IN, and INL key all
    pack `tuple.UUID` from the value's type (no out-of-band masks).
- **C â materialization.** `cascades_generator.go paginatingRows` row build (~:1618) converts a
  `tuple.UUID` column value â canonical string at the driver boundary (the ONE place string appears),
  also covering the covering-index `tuple.UUID` from `IndexEntryObjectValue` â which stays a pure ordinal
  extractor (Graefe).
...
be queried by `WHERE v='â¦'`.

**ARCHITECTURE RESOLVED â Graefe DECISION: (b).** A UUID flows as a neutral `[16]byte` inside the
Cascades value layer (`values` stays wire-agnostic â NO `tuple` import there). The `tuple.UUID`
conversion lives ONLY at the wire boundaries that already import `tuple`: the scan-range packing
(`scanComparisonsToTupleRange`, executor) and the index-entry write (`scalarToInterface`, recordlayer â
...
- A: `protoFieldToGo` UUID field â `[16]byte` (parse from the msb/lsb message; reuse the msbâlsb layout).
- B2: `PromoteValue.Evaluate` (values.go:2380) Stringâ`[16]byte` via `uuid.Parse` (google/uuid is a
  NEUTRAL lib â allowed in values; only `fdbgo/fdb/tuple` is the banned wire dep).
- Executor scan-range packing: when the equality comparand evaluates to a `[16]byte`, append
  `tuple.UUID(that)` to the probe tuple (the [16]byteâtuple.UUID conversion at the wire boundary).
- C: `paginatingRows` materialization (cascades_generator.go ~:1618) `[16]byte` â canonical string.
- `cmpAny`: ensure `[16]byte == [16]byte` (filter path) â likely already byte-compares; verify.
Original design notes below.

**DESIGN READY â see RFC-162.** Prototyped end-to-end and PROVED the
approach (the index probe returns the right row), but it spans **6 sites across 3 packages** (incl. a
sensitive Cascades `values` file) and touches the **wire format** â so it needs a Graefe design review
first, not a rushed multi-site landing. WIRE VERIFIED: `tuple.UUID = msbâlsb big-endian` =
...
projection site makes `SELECT v WHERE v=â¦` / covering `SELECT v` silently wrong â worse than the
...
corpus has `FROM (SELECT â¦ ) AS t` entries). But an alias introduced in an INNER
derived table is not visible TWO levels up:
- works: `SELECT x FROM (SELECT a AS x FROM t) i` (1-level alias)
- works: `SELECT a FROM (SELECT a FROM (SELECT a FROM t) i) s` (2-level, NO alias â
  the real column name `a` propagates through any depth)
- FAILS: `SELECT x FROM (SELECT x FROM (SELECT a AS x FROM t) i) s` â `42703 column
  "X" does not exist`; likewise `â¦ (SELECT x AS y FROM (SELECT a AS x FROM t) i) â¦`.
Only an alias-introduced name is dropped at depth â¥2. Fail-CLOSED (clean 42703, not
wrong rows). Standard SQL allows it and Java supports derived tables, so this is most
likely a Go column-anchoring gap, not a shared limitation â confirm against Java.
...
inner-alias â 42703; flip when fixed). Needs query-engine review. WORKAROUND: the gap
is specific to INLINE derived tables â the structurally-identical CTE chain
...
### [x] dml: UPDATE/DELETE with a nonexistent WHERE-column or table give generic 0AF00 (vs SELECT/INSERT's cleaner 42703/42F01) â DONE (RFC-159)

Fixed: (1) `buildWherePredicateForTableE` classifies the WHERE walk error via `mapPredicateWalkError`
(bare `ColumnNotFoundError` â 42703), matching SELECT; (2) explicit target-table existence check
...
Sibling to the now-fixed "UPDATE SET undefined column â 42703" leak. Remaining DML
error-classification asymmetries (all have a SQLSTATE, so lower severity than the SET
leak which had none):
- `UPDATE t SET a=5 WHERE nope=1` (nonexistent WHERE column) â `0AF00: DML Cascades
  translation failed`, whereas `SELECT â¦ WHERE nope=1` â clean 42703 ("column
  NONEXISTENT does not exist", pinned as error_undefined_column_where).
- `UPDATE notable â¦` / `DELETE FROM notable â¦` (nonexistent table) â `0AF00: DML
  Cascades translation failed`, whereas `INSERT INTO notable â¦` â clean `42F01:
  Unknown table`.
The WHERE/table resolution failure in the DML builder
(buildLogicalPlanForUpdateWithCatalog â upgradeDMLWhereWithCatalog /
...
### [x] executor: UPDATE of a PRIMARY KEY column â XXXXX is JAVA-FAITHFUL, not a bug â RESOLVED (RFC-160)

**Misframed (like the secondary-UNIQUE dry-run item).** `UPDATE t SET id=99 WHERE id=1` retargets the
save to the new PK (no record) â existence check fails â XXXXX. **Java is IDENTICAL:**
...
DEFAULT `ErrorCode.UNKNOWN` (not in its RecordCoreException switch) â and `ErrorCode.UNKNOWN("XXXXX")`
...
own comment (~2461) assumes "an UPDATE does not change the PK" â an assumption the
...
(delete-old + insert-new), whichever matches Java â needs a Java-behavior check
...
### [ ] query-engine: GROUP BY ignores SELECT-list COLUMN ORDER â emits keys-then-aggregates (Go-extension bug, Graefe design, found 2026-06-28)

A standalone `SELECT <aggregate>, <key> â¦ GROUP BY <key>` returns its output columns
in the aggregate's native KEYS-FIRST order, NOT the SELECT-list order â both the
...
Data is correct; NAME-based access is a sound workaround (the nameâvalue map is
right). GROUP BY is a Go-only extension (Java's fdb-relational has no GROUP BY), so
this is an extension defect, not a Java divergence â but it still violates SQL
...
name-based workaround; flip when fixed). The bug is UNIFORM â a computed expression
OVER an aggregate placed before the key (`SELECT SUM(v)+1, a â¦ GROUP BY a` â cols
...
interleaving â the order is lost in `logical_builder`/`logical_predicate` before
...
`GroupByExpression(groupKeys, aggSpecs, â¦)` keys-first; `aggregateOutputColumns`
mirrors it.

Fix path (Graefe review required â cross-cutting): track the SELECT-list output
order in `LogicalAggregate` (e.g. an output-spec list or `[]int` permutation) and
build a reordering Project over the GroupBy in `translateAggregate` â the infra
already exists (`buildPostAggregateProjection` builds a SELECT-order Project, reused
today only for INSERTâ¦SELECTâ¦GROUP BY via `wrapBareAggregateInsertSource`).
CONFIRMED no data corruption: the bare INSERTâ¦SELECTâ¦GROUP BY path already runs the
SELECT through buildPostAggregateProjection and so honors SELECT-LIST order
correctly (`INSERT INTO dst SELECT SUM(v), a â¦ GROUP BY a` â g=SUM, total=a;
pinned in insert_select_groupby_probe_test.go), and an explicit target column list
is fail-closed (0AF00) â so the bug is confined to standalone-SELECT display
...
### [x] translation: subquery conjunct in a compound JOIN ON clause â CROSS PRODUCT (pre-existing) â FIXED (RFC-154, 2026-06-27)
...
SQLâlogical translation. `upgradeJoinOnPredicates` installs no SubqueryPlanner, so `WalkPredicate` declined the
subquery shape, a permissive `continue` dropped the WHOLE ON predicate, and the translator ignores `OnText` once
`OnPredicate==nil` â cross product (NLJ with zero preds â EXPLAIN confirmed). **Fixed (RFC-154 Phase 1):**
fail-CLOSED â `expr.ContainsSubqueryAtom` rejects IN-subquery / scalar-subquery in ON with `0AF00` (Go, like Java,
...
### [x] ARCHITECTURE â eliminate the legacy embedded SQL interpreter (a "No parallel pipelines" violation, surfaced 2026-06-23 during R8) â DONE (RFC-145)
...
compiler-as-oracle restored 3 functions the static audit wrongly classed island â genuinely shared with
...
connection state the trim left â `EmbeddedConnection.validQualifiers` + `outerScopes` are now
read-but-never-written (their writers were island-only); `validQualifiers` is read by the kept
`eval_map.go:57` qualifier check (always-nil â branch never fires) and `outerScopes` by `scope.go:85`.
Removing them touches the kept map-path eval logic (behavior-preserving since both are always nil for the
kept consumers â single-source system-table WHERE + constant INSERT-VALUES never set them). Small,
separate cleanup. (`cteData`/`ctes` was the third such orphan â removed in Phase 2.) **â RFC-147 â DONE.**
Deleted both fields + their resets, collapsed the always-nil branches in `eval_map.go`/`eval_proto.go`,
removed `scope.go` + `scope_test.go` (the only non-nil writer), and pinned the kept qualifiedâbare
fallback with `TestFDB_InfoSchema_SchemataWhere_QualifiedRef` (redâgreen: 42703 when disabled). Net
â111 LOC. Torvalds LGTM (collapse proven behavior-preserving; fixed an orphaned `resolveOuterColumn`
comment ref he caught).

Original writeup (kept for context):

`pkg/relational/core/embedded` still contains a complete hand-rolled SQL interpreter â `execSelect` â
`execSelectQuery` â `execSelectQueryFull` â `execSelectJoin` / `aggregateMapRows` / `cte_scan` /
`execUnion` (~3k+ lines across `select_query_full.go`, `join.go`, `aggregate.go`, `cte_scan.go`,
`union.go`, â¦) â that duplicates Cascades' WHERE/GROUP BY/HAVING/join/CTE/UNION/aggregate execution. This
...
`newCascadesGenerator(c).Plan` â `planSelectCascades` (`cascades_generator.go:206`). The interpreter is
reached only via two fallbacks in `planSelect`: (a) `referencesInformationSchema(q)` â `execSelect`
(`:172`) â INFORMATION_SCHEMA is itself a **Go-only extension Java rejects** (`DIVERGENCES.md`), so there
is no cross-engine reference for that path; and (b) `planSelectExplainOnly` â `execSelect` (`:216`) â
...
INFORMATION_SCHEMA gap to be fixed in Cascades. Big, separate effort â query-engine-gated (Graefe +
Torvalds). Do NOT "keep the two aggregate executors in sync" â that is the anti-pattern; remove one.

### [x] relational/planner: bare boolean column as a single-table top-level WHERE predicate (`WHERE flag`) â DONE (RFC-146)

**DONE (RFC-146):** `walk.go` now lifts a bare boolean value to the COMPARISON form `value = TRUE` â the
byte-identical `ComparisonPredicate` that `flag = TRUE` produces â so `WHERE flag` and `WHERE flag = TRUE`
unify (same plan, semantic hash, and **index match** â Graefe's v1 NAK caught that a bare `ValuePredicate`
would never use a boolean index). NULL â `ConstantPredicate(TriUnknown)` (value-type detection, Java
`instanceof NullValue`); non-boolean â 42804 (clause-agnostic, shared WHERE/ON). The `isBareFieldPredicate`
...
flagâ[1], NOT flagâ[2], idâ42804), `TestWalkPredicate_BareNull`; corpus `bare_bool_where_rejected`âparity
`bare_bool_where`. Graefe ACK (RFC v2 + impl) + Torvalds. Original analysis below.

### (history) relational/planner: bare boolean column WHERE â surfaced by RFC-144 Â§3d, 2026-06-23

A bare boolean column as a single-table top-level WHERE predicate â `SELECT id FROM a WHERE flag` â fails with `0AF00: Cascades planner could not plan query`, even though: (a) the parser/resolver correctly lift it to `ValuePredicate(flag)` (`expr/walk.go` walkPredicatedExpression; `TestWalkPredicate_BareBooleanColumn` passes), (b) explicit comparisons work (`WHERE flag = TRUE`, `WHERE flag IS TRUE`), and (c) the SAME `ValuePredicate(flag)` shape plans fine inside a join ON clause (`SELECT a.id, b.name FROM a LEFT JOIN b ON a.flag` â pinned green in `TestFDB_OuterParity_BooleanOn`). Java 4.12 supports it: `Expression.Utils.toUnderlyingPredicate` (`fdb-relational-core/.../query/Expression.java:371-399`) lifts a bare boolean value to `ValuePredicate(value, EQUALS TRUE)` and rejects a non-boolean bare value with `DATATYPE_MISMATCH` (42804).

**Root cause corrected (RFC-146 research, 2026-06-25):** the gap is NOT the implement leg (the TODO's original hypothesis). `ImplementSimpleSelectRule` already builds a `RecordQueryPredicatesFilterPlan` from a top-level bare `ValuePredicate` â proven by deleting the guard. The actual bail-out is a conservative guard in the **translator**: `translateFilter` short-circuits to `nil` via `isBareFieldPredicate` (`cascades_translator.go:1687-1689` + helper `:2867`, added commit `85d0dd9f2`). Fix = mirror Java's single lift point: add the boolean type-assertion at `expr/walk.go:1334` (non-boolean â 42804, covers BOTH WHERE and ON), remove the guard, and propagate the type error hard (don't let `buildWherePredicateForTable` swallow it to `0AF00`). Do NOT just delete the guard â that makes `WHERE <non-boolean>` plan and silently return 0 rows instead of raising 42804. Graefe-gated. Pin: flip `TestFDB_OuterParity_BooleanWhere` + the `bare_bool_where_rejected` plandiff corpus entry. Detail: RFC-146.

### [~] fdbgo/client: GetAddressesForKey `ip:port` vs ip-only â MISFRAMED, NOT a real gap at API 730 (re-verified 2026-06-25)
...
`omit_unsplit_record_suffix` is set â across load, scan, `scanRecordKeys`, `recordExists`, save,
...
Java's transactional format upgrade (`maybeUpgradeFormatVersion` â `checkRebuild` /
...
versions and unsplit records â a data-correctness bug on the wire-compat hard line. Pinned by
...
### [x] fdbgo/client: Get/GetRange over-conflict vs libfdb_c â RFC-121 DONE (PR #319; conflict-range audit 2026-06-19)

Two confirmed serializability-outcome divergences (both SAFE over-conflicts â Go aborted where C/Java
...
`keyAfter(lastKey)` â ReadYourWrites.actor.cpp:271-274 / NativeAPI.actor.cpp:4576-4579). **D2:**
Get/GetRange added the read-conflict unconditionally, not skipping keys served by a local independent
write (RFC-058 had wired this RYW filter into GetKey only â ReadYourWrites.actor.cpp:328/342). Fix
...
batch â fixed so every batch is a serializable read adding its own clamped conflict (the C-client
per-batch model). Pinned by redâgreen differentials + `FuzzDifferential_ConflictOutcome` (63k+ execs)
...
### [ ] fdbgo/client: system-key DB-default applied to a tenant txn â tenant audit (2026-06-19); user-path FIXED
...
default) while `tenantId == NoTenantID`, and `SetTenantId` runs *after* â so a tenant txn created
...
returns void today, so it's a signature/ordering change â deferred. Rare (a DB-wide system-key
default + tenants is unusual). Also documented in-code: the D3 `stripTenantPrefix` clamp divergence
(unreachable â the commit proxy guarantees prefixed boundaries; comment at `locality.go`).

### [ ] fdbgo/client: special-key-space (`\xff\xff/...`) unimplemented â locality audit D1 (2026-06-19)
...
no-op â matches apiâ¥630 default, not a real divergence), D4 (`ParseClusterString` whitespace not
...
### [ ] fdbgo/client: watch-path divergences (D1/D2/D3/D5) â found by the quality-grind watch audit (2026-06-19); D4 fixed

The watch audit fixed **D4** (WatchPoll now retries the SS poll-signals â watch_cancelled/process_behind/
timed_out/future_version â instead of breaking the watch). Four remaining, ranked:

- **D1 [concrete, fixable] â no `too_many_watches` (1032) limit.** C++ `Transaction::watch`
  ...
  errors (`:5679`). Go has NO outstanding-watch counter â watches are unbounded; 1032 is never thrown;
  ...
  cancel) â the lifecycle is the tricky part. Test with a low limit via a `MAX_WATCHES` option.
- **D2 [architectural â RFC] â watch registered at READ version, not commit-gated.** C++ defers the
...
- **D3 [architectural â RFC] â no RYW pending-write watch semantics.** C++ `RYWImpl::watch`
  ...
  baseline (via `tx.ryw.get`) but has no watchMap/immediate-fire â the watch's baseline becomes the
  post-write value and it long-polls for the *next* change (wrong fire point).
- **D5 [small] â cancel returns `context.Canceled`, not `transaction_cancelled` (1025); failed commit
  doesn't cancel watches; stale comment.** `reset()âcancelWatches()` cancels the watch *context*, so
  ...
  `reset()âcancelWatches()` path â cleanup.

### [~] fdbgo/client: `makeSelfConflicting` (`\xFF/SC/<uuid>` synthetic conflict range at commit) â NON-tenant LANDED; tenant + idempotency-id add remain

**STATUS (landed):** The primary `makeSelfConflicting` port is DONE for **non-tenant** transactions
(`transaction.go` `maybeMakeSelfConflicting` â the `!intersects(write, read)` gate â `makeSelfConflictingLocked`,
...
(1:1 with `intersects`, `NativeAPI.actor.cpp:6211` â O(n log n), not the old O(wÂ·r) scan). Revert-proven by
`self_conflict_test.go` (wire-level: SC in both vectors for a non-tenant write-only commit; gated OFF for a
tenant commit; gated OFF when real ranges already intersect). **REMAINING:** (1) the **tenant** case â
...
feature â distinct, gate on `tr.idempotencyId`).
...
automatic-idempotency feature â distinct, gate on `tr.idempotencyId`.) Go has neither: a write-only
commit (read conflicts empty â no intersection) ships WITHOUT the synthetic range, and
`commitDummyTransaction`'s `intersectConflictRanges` (`commitpath.go:250-265`) falls back to
`writes[0].Begin` â a real user key â where C++'s dummy uses the synthetic key
(`NativeAPI.actor.cpp:6744-6750`).

**Two effects:** (a) Go's commit-request conflict-range vector diverges from libfdb_c for the same
write-only transaction (request-frame semantic difference â not persisted bytes, but affects the
...
traffic. PARTIALLY mitigated today: Go's `OnError(1021/1039)` copies writeConflictsâreadConflicts on
the RETRY (`transaction.go:1850`), so the retry is self-conflicting via a different mechanism â but
the original commit's wire shape and the dummy's key choice still diverge.

**Why a dedicated RFC, not a grind fix:** the commit_unknown_result â makeSelfConflicting â
...
differential-tested at the data plane (conflict ranges go to the resolver, not storage â a
...
### [ ] fdbgo/client (#28, HIGH): commit ships the UNFOLDED mutation log; libfdb_c commits the COALESCED RYW write map â Go throws 2101 (`transaction_too_large`) on a transaction C++/Java commit fine â needs its own `fdb-client-engineer` RFC (commit-path materialization; verified vs cgo)

**App-breaking behavioral divergence** (not a wire-bytes-of-stored-data issue â the final DB state is
...
**Root cause.** Go's `tx.Commit` marshals `tx.mutations` â the raw, unfolded append log (one entry per
...
- **SET/CLEAR** â last-writer-wins / clear-clears-the-stack (an absolute op resets the operation stack).
- **Same-type associative atomic op** (ADD, OR, AND, MIN, MAX, â¦) â `WriteMap::coalesceOver`
  ...
  atomic-op types â those `stack.push` instead.)
`writeRangeToNativeTransaction` then emits clears FIRST (`:2004-2018`), then per-key the (folded) operation
stack `op[i]` for `i in 0..op.size()` (`:2035-2065`) â so the shipped mutation vector is the coalesced one.

**Fix shape (RFC-grade, high regression risk â this is the most critical wire path).** Commit from the
coalesced write map for the RYW-enabled path (mirror `writeRangeToNativeTransaction`): iterate Go's RYW
write map (`ryw.go` â `rywEntry`/operation-stack, the WriteMap.cpp analog already used for READS), emit
...
impl. Do NOT rush at a session tail â 2/2 commit-path fixes this session needed rework.

### [ ] fdbgo/client: transaction-level options are PRESERVED across `onError` retry; C++ resets them to DB defaults â needs its own RFC (found by the quality-grind options audit, 2026-06-19)
...
â it copies the old options ONLY `if (!cx->apiVersionAtLeast(16))` (ancient APIs). So for every modern
app, a retry RESETS `priority`âDEFAULT, `causalReadRisky`â0 (grvFlags), `lockAware`â`cx->lockAware`,
tx-level `sizeLimit`âDB default, `tags`âempty, `snapshotRYWDisableCount`âDB default, then re-applies
...
priority/causalReadRisky/lockAware/readLockAware/sizeLimit/tags/snapshotRYWDisableCount â the comment
...
options) â a change to the hot retry path with per-option DB-default subtleties (lockAwareâcx default,
not false; causalReadRisky consistency), and the existing code deliberately chose the wrong behavior, so
it needs FDB-C-dev design review. Pin with a unit test (set a tx-level option â reset â assert reverted
to DB default; persistent options survive).

**Other options-audit findings (silent no-ops where C++ acts â `fdb/options.go`):** `REPORT_CONFLICTING_KEYS`
...
slot â tag throttling non-functional; also no `tag_too_long`/`too_many_tags` validation),
...
rather than no-op â but each is a small feature, scoped separately.

**GRV / read-version audit (same grind) â NO consistency divergence found** (version-vector is OFF by
...
- **Write-only commits omit `CAUSAL_READ_RISKY` on the commit-path GRV.** C++ `tryCommit` does
  `startTransaction(GetReadVersionRequest::FLAG_CAUSAL_READ_RISKY)` (`NativeAPI.actor.cpp:6578`) â a
  write-only/no-prior-read commit doesn't need full causal consistency for its `read_snapshot`. Go's
  commit path (`transaction.go:1507`) calls plain `ensureReadVersion` â `grvFlags()`, setting the flag
  only if the USER did. Effect: an extra TLog epoch-confirmation round-trip per write-only commit
  (latency/throughput, NOT consistency â the read_snapshot is equally valid). **Infra implication, why
  not a grind fix:** Go's `grvBatcherIndex` keys batchers only on the PRIORITY mask, NOT the risky flag
  (unlike C++'s `readVersionBatcher`, keyed by full flags) â so adding the flag would mix risky/non-risky
...
- `SetReadVersion` accepts `v<=0` / double-set silently where libfdb_c `setVersion` throws â
  `CATCH_AND_DIE` aborts the process (`NativeAPI.actor.cpp:5519`, `fdb_c.cpp:932`). Go's graceful
  defer-to-1007 is arguably BETTER (no panic in library code per CLAUDE.md) â leave as a documented,
...
- Dropped GRV-reply observability (no consistency impact): `proxyTagThrottledDuration` (the
  ...
  `\xff/metadataVersion` â correct, one extra round-trip), `midShardSize` (no clear-range cost estimator).

**Minor OnError/knob-audit findings (same grind, low priority â note, don't necessarily fix):**
hedge `secondDelay` uses a fixed `2.0Ãprimary-latency` where C++ uses a runtime-adaptive
`secondMultiplier (â¥1.0) Ã second-best latency + BASE_SECOND_REQUEST_TIME(0.5ms)`
...
full window); GRV `batchTime` floors at 100Âµs where C++ has no floor.

### [x] fdbgo/wire: register WatchValueRequest/Reply in the schema extractor (pre-existing gap, surfaced by RFC-115 Â§6) â DONE (branch `wire/watchvalue-extractor-registration`, stacked on #303)
...
types) DROPS them â a regen footgun. RFC-115 Â§6 restored them after its regen; the proper fix is
...
`Optional<Error>`, so re-emitting it picks up the Â§6 union fix too. Not caught by per-PR CI
(`just generate` â  `just generate-wire-types`). Verify the re-emitted bytes are wire-identical
...
`main.cpp` `extractType<>`); a regen now PRODUCES them. The regen surfaced â and this branch also fixes â **two
deeper extractor wire bugs** the registration depended on:
1. **`Optional<UID>` mis-emitted as `[]byte`.** `scalar_traits<UID>` (flow/IRandom.h) â UID is a fixed 16-byte
   ...
   `Optional<scalar>` codegen path (restricted to UID â the lone fixed-array struct-scalar). Fixed `DebugID` on
   ...
   (Correction to the note above: `WatchValueReply` has NO `Optional<Error>` â it's just `{version int64, cached bool}`.)
2. **`ReadOptions` field-name mis-registration â a live client bug.** The old `REGISTER_FIELD_NAMES(ReadOptions,
   ...
   (`readpath.go`) set the debugID field thinking it was lockAware â **lock-aware reads never actually requested
...
**Follow-up â DONE (RFC-117, commit `b5bdbc00`):** **`Optional<primitive-scalar>` codegen.**
...
only `ReadOptions.consistencyCheckStartVersion` `[]byte`â`int64`; un-skipped in `cmd/fdb-diff-oracle`
(`TestDiffReadOptions`, C++ byte-truth). The UID `[16]byte` array path is unchanged.

### [x] fdbgo/client: stamp the GRV/watch/locate requests with a trace SpanContext â DONE (RFC-116)

RFC-115 Â§4 stamped the per-op child SpanContext on reads + the tx span on commit, but the GRV,
watch, and getKeyServerLocations requests still carried a ZERO/raw SpanContext. **RFC-116** closes
all three, faithfully to the C++ (NOT the naive "thread a representative tx span" â that would put a
...
- **locate** stamps the `getKeyLocation` child (`:3017/3037`, derived once in `refresh`, reused
  across proxy retries â `basicLoadBalance` reuse).
...
### [x] fdbgo/client: read-path RPC reply timeout is retryable, not a terminal leak (C++ divergence) â FIXED (PR #288)
...
exhaustion surface a RETRYABLE `transaction_too_old` (1007) â matching libfdb_c's `loadBalance`,
...
### [x] TOP â SPFresh churn flake on MASTER: live record not findable after concurrent churn (094.3 race)

**ROOT-CAUSED + FIXED on the 094.4 branch (PR #283): the csplit pause-window orphan.**
The fingerprint (`membership=[393217] fine 393217@cell 2 state=0` â membership and
posting entry both present, centroid ACTIVE, search still misses) is the capped-read
truncation shape: the query path fetches postings with a 4ÃLmax+1 cap
...
(`spfreshCSplitPaused` skip in the insert probe) never got its split task re-filed â
...
`spfresh_cascade_test.go` ("csplit move re-files split tasksâ¦"). Verified: 45/45
focused runs green on the branch vs ~1-in-8 red on master. The churn test now also
asserts post-quiescence that every ACTIVE posting is within the 4ÃLmax envelope (the
...
### [x] CORRECTNESS FIXED â re-enumerated indexed multi-way joins (was: NULL / 0 rows)
...
`uppersDependingOnLowersAliases`, computed from `getCorrelationOrder()` â
...
unbound upper aliases â wrong rows. The physical FlatMap then merges via
`JoinMergeResultValue`, which cannot resolve columns nested under `_0` â NULL.
...
degenerate partitions" guard). The valid associativities â where the spanning
predicate stays at the join level â then win identically for every FROM-order.
...
executed the query) â now retrofitted with **row-correctness** assertions for
both FROM-orders, which is the load-bearing check.

**Remaining (cost-optimality, NOT correctness) â RFC-042.** Under the bigâsmall
FROM-order the re-enumerated `(t2ât3)` sub-product still prefers a cross-product
...
byte-identical FROM-order invariance for Nâ¥3 (the `TestFDB_MultiwayJoinOrder_Probe`
goal) depends on closing this cost gap + FROM-order-deterministic winner selection.
Likely levers: the index-probe cardinality cost (criterion #2 â make the FlatMap
...
- [ ] **Re-verify `joinOptimizationProbesScenario` (RFC-082 cross-engine exclusion) against RFC-042 (@claude flag).** The A3 builder is excluded from `crossEngineScenarios` with the note "Go's join enumeration is still non-deterministic on some arithmetic-predicate shapes â a 3-way / arithmetic-join can return a different ROW COUNT across runs." That row-count *nondeterminism* (a correctness flake) is NOT the item tracked above â line 11-40 is the now-FIXED FROM-order-dependent (but per-order deterministic) bug, and line 42 is cost-optimality (correct results, just slower). So either the exclusion note is stale (the row-count flake was the fixed PartitionSelectRule bug â the scenario may be re-enableable cross-engine now) or there is a genuinely-still-nondeterministic join-enum shape that needs its own root-cause. Verify with a focused multi-run of the probe shapes; if still nondeterministic, the Go-only yamsql coverage for `join_optimization_probes` is itself flaky (same code path) and must be pinned, not just excluded cross-engine. Out of scope for RFC-082 (conformance determinism); tracked here for the RFC-042 follow-up.

### vs Java (correctness/feature parity)

- [x] **Correlated filter without index.** Fixed in 56874f23 â ImplementFilterRule sets innerAlias on RecordQueryPredicatesFilterPlan. All correlated paths (scalar subquery, EXISTS, JOIN) work without indexes. 14+ integration tests verify.
- [x] **RIGHT/FULL OUTER JOIN.** Done in RFC-036. (The old "only LEFT OUTER" note was stale â RIGHT already worked via operand-swap normalization in `cascades_translator.go`, pinned by `TestFDB_RightJoin`.) FULL OUTER added as a Go-only query extension: Java's SQL layer has **no** outer joins at all (`visitOuterJoin` is a no-op, zero tests), so LEFT/RIGHT/FULL are all read-path-only extensions with **zero wire-format impact** â Java apps still read/write the same records. FULL OUTER is implemented exclusively by the materialized NLJ cursor (`streaming_cursors.go`): LEFT-OUTER outer loop + a `matchedInner` bitmap + a drain phase emitting unmatched inner rows NULL-padded on the left. Routed away from the correlated FlatMap path (cannot observe global inner-match state); FULL+EXISTS rejected with a clear error. 9 FDB integration tests (all four row classes, NULL-key 3VL, many-to-many, large-inner hash+drain, WHERE-above-join, determinism, RIGHT NULL-key regression). Graefe+Torvalds ACK.
...
- [ ] **No *general-purpose* window functions â and Java has none either.** Investigation (RFC-045): Java's relational layer has **no** general streaming window operator. The general `windowClause` is commented out in Java's grammar ("don't want to deal with them now"); `LAG`/`LEAD` are grammar tokens with **no** value class; `RankValue implements Value.IndexOnlyValue` (computable only from a rank/leaderboard index, never over a result set). The **only** working window function in Java is `ROW_NUMBER() OVER (... ORDER BY <distance>) <= K` via `QUALIFY`, used exclusively for **vector/HNSW K-NN search**. So "match Java's window functions" â¡ "finish the vector/HNSW relational parity" â tracked as **Phase 9** below. General windowing over plain tables would be a *Go-only extension Java lacks entirely* (allowed if wire-compat holds + deep tests), not parity â deferred, not in Phase 9.
- [x] **GROUP BY/HAVING in correlated scalar subqueries.** Done in RFC-047 â a Go-only read-side extension (Java rejects correlated scalar subqueries at the grammar level entirely; zero wire impact). The stale "PredicatePushDownRule AliasMap.Compose conflict" blocker no longer applies: GroupByExpression is already a push-down barrier (no case in `pushPredicateToExpression`) and the panicking `AliasMap.Compose` has no production callers. `buildCorrelatedScalar` now builds GROUP BY (+ HAVING) into the inner plan and caps with `LIMIT 1`; the scalar contract is FirstOrDefault (first group + LEFT-OUTER NULL-on-empty), NOT a runtime cardinality assertion (Graefe). Empty input â 0 groups â NULL falls out naturally (vs no-GROUP-BY COUNT â 0). Group keys + aggregate operands resolve via the semantic scope (`ResolveIdentifier`), scalar column named with the bare operand to avoid an embedded-`.` qualifier mis-parse. 42803 enforced via `validateGroupByProjection`; multi-column + EXISTS-in-HAVING + unresolvable-expr-arg/key rejected. 23 FDB integration probes (incl. EXPLAIN-pins-StreamingAgg, emptyâNULL contrast, expression group key, join+GROUP BY, determinism 10Ã).
  - [x] **Follow-up: `ORDER BY` over grouped output in a correlated scalar subquery.** Done in RFC-085 â a Go-only read-side extension. The interim rejection is gone; `ORDER BY` + `GROUP BY` now inserts a `LogicalSort` over the post-aggregate row (between the aggregate and the FirstOrDefault `LIMIT 1`) so the multi-group choice is deterministic. Sort keys resolve to the **exact** datum key the aggregate cursor emits (`groupedScalarSortKeys`, single-source: group keys â bare-upper, aggregates â the materialised alias) â translateSort/FieldValue do exact-case lookup, so a mismatched key would silently sort every row equal. ORDER BY a column that is neither grouped nor a *selected* aggregate is rejected loudly (no silent-nil sort). Wired in BOTH aggregate paths (hasRealAgg + group-key-only). **Sub-fix (same exact-case-datum-key bug class):** a qualified projection (`SELECT o.amount`) and a qualified ORDER BY key in the **non-aggregate** single-table path used to keep the `o.` qualifier and resolve to NULL / miss the sort â now stripped to the bare key (mirroring the join-vs-single-table convention at :910). Pinned by `ordered_grouped_scalar_subquery_fdb_test.go` (ASC/DESC group choice, determinism 10Ã, loud reject, qualified projection + qualified key) and `quality_probes_test.go` (order_by_with_group_by_deterministic, ASC+DESC SUM per group).
  - [x] **Follow-up (single-source): expression/constant-argument aggregate that meets a *differing* aggregate via HAVING in a correlated scalar subquery.** DONE â the addendum unified producer and consumer on **one** canonicaliser (`canonicalAggName`, called by both `buildCorrelatedScalar` and `rewriteAggregateValue`), so the two name schemes can no longer drift; the prior fail-safe rejection is gone for single-source. The last silent-wrong corner (nested-arithmetic args like `SUM((amount+10)*2)` returning NULL â dropped groups) was a *separate* root cause â an inverted `!isArith` guard in `translateAggregate` that preferred a lossy text reparse over the resolved operand â fixed in RFC-048 (4dc3276c): the resolved `AggregateOperands[i]` is now always the source of truth. Works now (single-source): `SELECT COUNT(1) â¦ HAVING COUNT(*)` both directions; `SELECT SUM(a*2) â¦ HAVING SUM(a*3)`; decimal-literal args (`SUM(a*1.5)`); nested-arith args (`SUM((a+10)*2)`). `COUNT(DISTINCT 1)` correctly still rejected (DISTINCT unsupported here). Pinned by `quality_probes_test.go` (count_constant_with_having_works, expression_aggregate_in_having_works, decimal_literal_aggregate_arg_in_having, nested_arithmetic_aggregate_arg_in_having). **Residual (join only):** over a JOIN an expression-argument aggregate in HAVING is still rejected (the operand binds to the wrong quantifier through the parser round-trip) â pinned by `join_expression_aggregate_in_having_rejected`.
- [x] **ð© IN over an indexed column drops the outer projection (wrong result schema).** Fixed in **RFC-070**. Root cause was two defects: (1) `MergeProjectionAndFetchRule`'s fallback dropped the projection when the fetch's child was an InJoin (not a coverable index scan), leaking a bare `InJoin` ([ID,A]) into the root projection group where it won on cost; (2) `physicalProjectionWrapper`/`physicalFetchFromPartialRecordWrapper` `WithChildren` didn't relink a compound-join inner during extraction (left `Project([id], InJoin(<nil>))` / `Fetch(<nil>)`), because of an `isLeafReplaceable` gate â same gate RFC-069 removed from the in-memory sort wrapper. Fix: fallback retains the projection; the two transparent caps relink unconditionally. `SELECT id FROM t WHERE a IN (1,7)` â `Project([ID], InJoin(IndexScan(IDX_A,[=])))`; `SELECT id+100 ...` (was 0 rows) â `{101,107}`. Pinned by `TestFDB_INProj_OuterProjectionOverInJoin` (indexed+unindexed, multi-column, expression-projection, 8Ã determinism). Graefe+Torvalds ACK.
  - [ ] **Follow-up (RFC-070): `pushValue`-into-covering-result-value modeling gap.** Java's `MergeProjectionAndFetchRule` yields a bare `fetchPlan.getChild()` because `RecordQueryFetchFromPartialRecordPlan.pushValue` rewrites the projected value into the covering plan's own result value. Go's `WithCovering` only sets a flag (the scan still flows the full partial record), so Go compensates with a thin outer `Project`. Pushing the value into the covering result value would let both rule branches collapse to a bare child yield, matching Java. Cosmetic/architectural â current behaviour is correct.
  - [ ] **Follow-up (RFC-070): other transparent unary wrappers over joins.** `Map`, `Distinct`, `Limit`, `TypeFilter`, `FirstOrDefault`, `DefaultOnEmpty` still gate `WithChildren` on `isLeafReplaceable` and could exhibit the same nil-inner-over-join bug if a rule ever builds them with a placeholder inner over a join. Not currently reachable via SQL (projections route through `LogicalProjectionExpression`, not `Map`); the **blanket** gate removal is unsafe â it regressed `TestFDB_AggregateIndexUsage` by dropping the eq-filter on aggregation/DML wrappers (which embed filter semantics in their own plan). Each wrapper needs individual analysis if/when reachable.
- [x] **DML does not execute through Cascades (parallel pipeline).** Fixed as **P0.4** â all DML now executes through Cascades (`planDML`); the naive `execStatement` DML path is deleted. See P0.4.
- [x] **ð© `INSERT â¦ SELECT â¦ GROUP BY` wrote the wrong columns (spurious 23505).** Fixed in **RFC-084**. A plain GROUP BY SELECT builds a bare `LogicalAggregate` with NO Project (standalone derives its schema from the physical plan), so as an insert source its datum was keyed by the aggregate's own canonical names (`G`, `SUM(V)`) â `buildInsertRecord` maps by TARGET field name, found none, left every field unset â each grouped row collapsed to the same all-default record â second group collided â spurious 23505. Java accepts this exact shape (`insert_select_java.yaml:60`). Fix: `wrapBareAggregateInsertSource` wraps the bare aggregate in the canonical post-aggregate Project (reusing `buildPostAggregateProjection` â visible-only via `ac.visible`, canonical-named to match the runtime datum key, in SELECT order), filling `ProjectedValues` with upper-canonical `FieldValue` refs; `alignInsertSelectColumns` then sets target aliases positionally. A sole `SELECT COUNT(*)` (tracked as `sq.countStar` with empty `aggCols`) is synthesised into the wrap so `INSERT INTO t SELECT COUNT(*) [GROUP BY g]` is aligned too. Pinned by `groupby_insert_select_fdb_test.go` (core/was-23505, multi-aggregate Java shape, COUNT(*) scalar+GROUP BY, lowercase arg, AS-aliases, reordered SELECT, ungrouped HAVING-over-non-visible `keys==0`, qualified-stays-loud, HAVING-strip-Project path, determinism 10Ã). Graefe + Torvalds ACK (RFC + impl).
  - [ ] **Follow-up (RFC-084): qualified aggregate operand on the insert-source path computes NULL.** `INSERT â¦ SELECT g, SUM(s.v) â¦ GROUP BY g` leaves the qualified aggregate's operand unresolved (`AggregateOperands=[nil]`) so it sums NULL; the wrap therefore SKIPS qualified-operand sources (a `.` in the canonical agg/group-key name) to avoid silently inserting NULL â they stay at the original loud 23505. Fix the operand resolution on this path (then drop the skip + flip `qualified_source_stays_loud` to assert correct rows).
  - [ ] **Follow-up (RFC-084 / RFC-079): unify INSERTâ¦SELECT onto `visitSelectGroupBy`.** The one-query-path end-state MOVES this coercion into the Insert expression and **deletes** `wrapBareAggregateInsertSource` (no third parallel coercion path) â per Graefe's condition. Tracked with the RFC-079 SimpleTable-builder unification.
- [x] **ð© Aggregate result-type derivation diverges from Java: `AVG(x)âDOUBLE`. DONE â RFC-083.** `AggregateValue.Type()` now types `AVG â NullableDouble` (function-determined, matching Java `AVG_*âDOUBLE`); SUM/MIN/MAX stay operand-derived, COUNTâLONG. The "ZERO new code / existing IsPromotable check" framing was **inaccurate** (no plan-time promotion check existed â `IsPromotable` had zero callers; the only enforcement was a runtime band-aid), so the fix is three coordinated parts: (A) the AVG `Type()` arm + collapse the duplicate AVGâDOUBLE SQL-name encodings onto it (`valueTypeName`/`aggregateResultType` route through `Type()`); (B) a **plan-time promotion guard** at the INSERTâ¦SELECT chokepoint (`checkInsertSelectPromotable`, the first production `IsPromotable` caller) keyed on aggregate **provenance** â `LogicalProject.AggregateSlots` (captured pre-rewrite via `containsAggregate`) for computed exprs like `AVG(v)+1`, and name-resolution against the producing `LogicalAggregate` for bare `AVG(v)` (whose projection slot carries a nil value) â so `AVGâBIGINT` is rejected 22000 **even over an empty source** (emergent from the lattice, not a materialized float); `rewriteAggregateValue` now preserves `Typ: av.Type()` (was discarding it as UnknownType); (C) converge the runtime converters â remove `ConvertToProtoValue`'s whole-floatâint64 coercion (VALUES doubleâBIGINT now rejects 22000), and give `goToProtoValue` the promotable INT/LONGâFLOAT/DOUBLE widenings + an **emergent 22000 fallthrough** (also fixes the adjacent `SUM(BIGINT)âDOUBLE` gap that used to error). Pinned: `values_test` AVG-type pins, flipped both `ConvertToProtoValue` whole-float unit tests, new `goToProtoValue` widening/reject tests, `avg_double_insert_fdb_test.go` (scalar/empty-source/`AVG+1`-empty/`âDOUBLE`/`SUMâBIGINT`/`SUMâDOUBLE`/plain-arith/VALUES double-reject/index-presence EXPLAIN). insert_select.yaml corpus corrected. Ripple guard holds (AVG never lowered to `Sum/Count` ArithmeticValue division; no aggregate index â streams). Full `just test` green. Graefe+Torvalds ACK'd RFC (v4) + impl.
  - [ ] **Follow-up (RFC-083): replace the guard + `AggregateSlots` marker with Java's `PromoteValue` projection nodes** â the single mechanism that both rejects-at-plan and widens-at-runtime, dissolving the dual lattice-encoding (guard + converters) and the load-bearing "aggregate-slot â guard" coupling (Graefe's end-state). Subsumes reliably typing `FieldValue`/`ArithmeticValue` projections, which then closes the **residual deferred cases**: bare-column `SELECT double_col â BIGINT` over an empty source, and `UPDATE â¦ SET int_col = <double-expr>` â both currently rely on the runtime converter (correct for non-empty rows, miss the 0-row case).
  - [ ] **Follow-up (RFC-083): bare GROUP BY-aggregate INSERTâ¦SELECT source.** `INSERT â¦ SELECT g, AVG(v) â¦ GROUP BY g` has a `LogicalAggregate` as the insert Source (no `LogicalProject`), so the guard can't read column order and defers it (runtime rejects the non-empty case). Also observed a possible PK-mapping/grouping anomaly on that execution path (a 23505 where the rows shouldn't collide) â investigate separately.
  - [ ] **Adjacent (separate index-type bug): `GetIndexTypeName` hardcodes `MIN_EVER_LONG`/`MAX_EVER_LONG`** â MIN/MAX over a non-long operand needs `MIN_EVER_TUPLE` (Java `permuted_min/max`).
- [x] **ð© TODO 7.6-union-remap â aggregate UNION branch with a mismatched output alias drops rows (pre-existing executor gap).** Fixed for STREAMING aggregates in **RFC-078**: (1) `executeUnorderedUnion` (executor_new_plans.go) now remaps later branches' columns to the first branch's names by position â it previously concatenated branch cursors with NO normalization at all (unlike the ordered `RecordQueryUnionPlan`/`executeUnionStreaming`); (2) `planColumnNamesWithMD` (executor.go) reports a `RecordQueryStreamingAggregationPlan`'s output names (group keys + alias-or-canonical) instead of descending through `GetInner()` to the input scan. `SELECT u.x FROM (SELECT COUNT(*) AS x FROM a UNION ALL SELECT COUNT(*) AS y FROM b) u` now returns both counts (was `[2, NULL]`). Pinned by `TestFDB_UnionAggregateColumnRemap`. Graefe + Torvalds ACK.
  - [x] **Follow-up (RFC-078) c â FIXED in RFC-080: re-enable the union-as-join-leg / derived-table aggregate case for UNGROUPED aggregates.** The gate's `LogicalAggregate` case is hit only by a *bare* aggregate branch (no Project). Graefe's review caught that a bare aggregate can be GROUPED (an unaliased, all-visible `SELECT g, COUNT(*) FROM t GROUP BY g` skips `buildSelectShell`'s stripping Project). Only the UNGROUPED sub-shape is safe to normalize: an ungrouped aggregate produces **no** aggregate-index candidate (`tryAggregateIndexCandidate` returns nil when `groupingCount == 0`, `cascades_generator.go`), so it always plans as StreamingAgg, which flows every aggregate under its alias (RFC-078). So `unionBranchNormalizable`'s `LogicalAggregate` arm relaxed from `false` to `len(Aggregates) >= 1 && len(GroupKeys) == 0`. `TestFDB_UnionJoinLeg` case (3) flipped clean-errorâcorrect-rows. Pinned by `TestFDB_UnionScalarAggregateAlias` (single + multi ungrouped unions read by name + no-AggregateIndex invariant), `TestFDB_UnionGroupedAggregateStillGated` (grouped union, which DOES plan as AggregateIndex, stays gated), `TestUnionBranchNormalizable_AggregateArity`. plandiff byte-identical. Graefe + Torvalds ACK.
    - [x] **Follow-up (a) â GROUPED bare aggregate union by name â FIXED in RFC-081.** A bare GROUPED aggregate union branch (`SELECT g, COUNT(*) FROM a GROUP BY g UNION ALL â¦` read by name) plans as `AggregateIndex` (single agg) or `MultiIntersection`/`StreamingAgg` (multi agg). The fix was *reporting*, not cursor changes: the AggregateIndex and MultiIntersection cursors already write rows keyed by their output names (group cols + canonical aggregate name; a bare aggregate is always unaliased, so no alias to carry). Added `RecordQueryAggregateIndexPlan.OutputColumnNames()` + `planColumnNamesWithMD` arms for AggregateIndex (group cols + `CanonicalAggColumnName`) and MultiIntersection (result-value field names, verbatim), then dropped the `len(GroupKeys) == 0` clause â gate is now `len(Aggregates) >= 1`. `TestFDB_UnionGroupedAggregate` (single + multi grouped union join legs, mismatched group-key names â correct rows; EXPLAIN-pins AggregateIndex), `TestPlanColumnNames_{AggregateIndexReportsOutputSchema,MultiIntersectionReportsResultValueNames}`, `TestAggregateIndexPlan_OutputColumnNames`, gate unit test groupedâtrue. plandiff byte-identical. Graefe + Torvalds ACK.
      - [ ] **Sub-follow-up (codex): DIVERGENT-NAMED aggregate union branches.** A bare aggregate whose output name differs between the logical leg schema (`aggregateOutputColumns`, raw text) and the physical row key (`aggResultName`/AggregateIndex canonical) NULLs when union-remapped by name. Divergent forms: qualified operand (`SUM(t.c)`â`SUM(C)`), constant (`COUNT(1)`/`COUNT(NULL)`â`COUNT(*)`), expression (`SUM(a*b)`), DISTINCT. RFC-081 GATES all of them **in the GROUPED case** via `aggregateNamesStableForUnion` (whitelist `COUNT(*)`/`FUNC(bare-col)`; clean error, `TestFDB_UnionQualifiedAggregateGated` + `TestFDB_UnionGroupedCountConstantGated`). UNGROUPED branches are left as RFC-080 (always StreamingAgg, not re-gated, to avoid regressing working ungrouped legs); any ungrouped divergent form (e.g. bare ungrouped `SUM(t.c)`/`COUNT(NULL)`) is a pre-existing RFC-080 latent NULL, fixed by the same naming-unification below. To OPEN them: unify aggregate output naming so the logical schema and the physical row key agree for every form (strip qualifier consistently + reconcile count-star normalization between StreamingAgg and AggregateIndex), then relax the whitelist. NOTE: a separate pre-existing bug â `SELECT u.*` star-expansion over an aggregate union join leg mis-derives the aggregate column name (NULL) even for ALIASED aggregates (Project-topped) â is orthogonal to the gate and also needs fixing. Trivial cleanup (@claude): `deriveColumnsFromAggregateIndex` (cascades_generator.go) builds the canonical `FUNC(col)`/`FUNC(*)` name inline (a third copy alongside `CanonicalAggColumnName` + the cursor) â for schema-metadata column-type derivation, not row-key naming, so it doesn't interact with the union remap, but it should call `aggIdx.CanonicalAggColumnName()` to complete the single-source consolidation.
  - [x] **(b) ordered-union projection-alias â FIXED in RFC-079.** A UNION branch projecting a post-aggregate EXPRESSION with an alias (`SELECT COUNT(*)+1 AS x FROM a UNION ALL SELECT COUNT(*)+1 AS y FROM b`, read by name) returned `[NULL,NULL]` â the legacy `buildSelectShell` builder (the UNION-branch path) built the post-agg projection with `nil` aliases, dropping the `AS x`. Fixed by extracting the projection-building loop into one shared `buildPostAggregateProjection` helper called by both `visitSelectGroupBy` (modern) and `buildSelectShell` (legacy) â one source of alias truth. Pinned by `TestFDB_UnionAggregateExprAlias` + `TestBuildLogicalPlan_PostAggExprAlias_CarriesAlias`. Modern path plandiff byte-identical. Graefe + Torvalds ACK.
  - [ ] **Follow-up (RFC-087, Graefe): reject aggregate-in-scalar-context at PLAN time.** `WHERE COUNT(*) > 0` reaches `AggregateValue.Evaluate` at row eval; RFC-087 made it a clean runtime `AggregateEvalError` â 42803 (was an uncaught goroutine crash on master â Graefe confirmed). Java rejects this at semantic-analysis / plan time ("unable to eval an aggregation function with eval()"). Detect an aggregate in a per-row scalar predicate (WHERE / JOIN-ON / projection-not-under-GROUP BY) during planning and reject there, matching Java exactly. Runtime 42803 is the safety net; plan-time is the parity fix.
  - [ ] **Follow-up (RFC-087, Graefe): thread `ComparisonKeyFunc` error channel.** The 5 executor merge/sort comparison-key sites (`intersectionCompKeyFunc`, `multiIntersectionCompKeyFunc`, `mergeSortCursor.isBetter`/`extractKey`, executor.go:1391) `panic(err)` on a stray key-eval error â pre-existing behaviour (no recover before/after RFC-087), and keys are pre-projected field refs so the typed-error family is unreachable today. To make it airtight, give `ComparisonKeyFunc` an `error` return and thread it (ripples into wire-adjacent `merge_cursor.go`). Low priority â not reachable from current SQL.
  - [ ] **Follow-up (RFC-088, Graefe condition): converge `validateGroupByProjection`'s existence check onto the semantic resolver.** Java does NO standalone existence check for GROUP BY keys â `SemanticAnalyzer.resolveIdentifier` over the full multi-source scope already guarantees existence, and `validateGroupByAggregates` enforces only the algebraic 42803 rule (key must be grouped-or-aggregated). Go currently runs a SECOND, hand-rolled existence oracle (`tableFields` = union of all source descriptor field names, bare-name match) that is deliberately qualifier-blind, so it would false-ACCEPT a wrong-qualifier key (`e.dname` where dname is on the joined dept) â SAFE today ONLY because the precise resolver runs first at every call site (top-level `resolveColumnName` ~L1002; correlated-scalar GROUP-BY-key resolution in `buildCorrelatedScalar`), an ordering invariant now pinned by a code comment at `validateGroupByProjection` and by `TestFDB_GroupByWrongQualifierRejected`. End-state: route existence through `resolver.ResolveIdentifier` and leave `validateGroupByProjection` enforcing only 42803, removing the duplicate oracle and the ordering dependency.
  - [ ] **Cleanup (RFC-079 follow-up b): unify the SimpleTable logical builder onto `visitSelectGroupBy`.** The "one query path" endgame (CLAUDE.md "no parallel pipelines"). `buildSelectShell`/`buildLogicalPlanForSelect` is a second SELECT builder reached by plain-table SELECTs, derived tables, AND UNION branches; it has repeatedly drifted from the modern `visitSelectGroupBy` (the RFC-079 alias bug was one such drift). Route ALL of its callers through `PlanVisitor.visitSelectGroupBy` and delete the legacy builder. Larger than a single-bug fix (multiple callers, full regression surface) â Graefe's condition: must unify the WHOLE SimpleTable builder, not graft a special case onto the union entry.

### Beyond Java (Go-only improvements)

- [x] **Full Graefe Memo with cross-group merging.** Done in RFC-037 â union-find group merging (the Cascades-paper "merge two groups discovered to be one", Â§2 + Â§3.5), a Go-only extension beyond Java (which, like the pre-RFC Go memo, only interns at insertion time). `Reference` gains a monotonic `id` + `forwardedTo` + path-compressed `Canonical()`; every state-bearing method resolves the receiver to canonical, so a merged-away (loser) Reference transparently forwards â no in-flight task, Quantifier, or binding is rewritten. `GetRangesOver()` resolves at the single chokepoint (444 sites). `Memo.Integrate` hooks the REWRITING yield path: when a yielded expression equals a member of a different group, the two merge (survivor = lower id, deterministic), folding members + exploration state, repointing the topology index, invalidating correlation caches up the DAG, and recursively re-merging parents (paper's bottom-up recursion). Scoped to REWRITING (PLANNING winners/partial-matches embed raw References â guarded by `mergeable`); ancestor/descendant (idempotence) merges skipped to avoid DAG cycles. Wire compat untouched (read-path-only sharing). Merge fires through the real planner (`TestMemoMerge_FiresThroughRealPlanner`); 9 merge unit tests + determinism 50Ã/10Ã; 46/46 targets green; stress-1M unchanged. Graefe+Torvalds ACK (NAK'd v1 on in-flight-task stranding + cache staleness + index repoint + upward re-merge â all fixed in v2). **Reach caveat (honest):** the merge is correct and fires, but its practical reach is narrow today â the memo's interning/equivalence is alias-sensitive, and rule-rewritten equivalents mint fresh quantifier aliases, so equivalent sub-expressions intern to *different* child References and rarely surface as merge candidates (measured: exactly one merge on a K-branch equivalent UNION regardless of K; ~2% planner-time delta; no execution-time effect â same plan). Broad merging (and any real speedup / multi-way-join-order benefit) is **gated on alias-namespace unification (item 7.1 below)**; this PR lands the correct merge *infrastructure*, not a present-day perf win. Remaining (Future Work): **alias-normalized equivalence (7.1) â the lever**; reduction-rule-triggered merges (Â§3.6); PLANNING-phase merging; cost-model exploitation of shared sub-products for full N-way join-order optimality.
  - **PR-A landed the lever (RFC-038 epic / RFC-039 + RFC-040).** The reach caveat is now closed: the memo's structural-equivalence compare sites use alias-aware `expressions.MemoEqual` (faithful port of Java `Reference.isMemoizedExpression`) on top of the RFC-040 foundation (alias-aware `EqualsWithoutChildren` + alias-invariant `HashCodeWithoutChildren`). Rule-rewritten equivalents that differ only in fresh quantifier aliases now intern/merge into the SAME Reference â proven by `memo_activation_test` (K=6 alias-variant filters â 1 shared Reference, was K distinct). Zero plan-shape regression (plandiff conformance green), 10/10 deterministic, stress-1M before/after within noise. Graefe+Torvalds ACK. Still ahead in the epic: **PR-C** join-order enumeration (associativity/commutativity, capped) and **PR-D** cost selection + the e2e "multi-way join ordering proven" test (N-table join, EXPLAIN-pinned optimal order â  FROM-order, shared sub-products merged).
  - **PR-C scope corrected (RFC-074).** PR-C was framed as "efficient â¥5-way enumeration via sub-product interning (collapse the dual merge values)." **Measurement falsified the premise:** collapsing `JoinMergeResultValue`/`JoinMergeAllValue` to one canonical type does NOT reduce `distinctRefs`/`tasksRun` (N=5 stays 127k tasks / 171 refs) â the duality is a ~2Ã constant, not the exponential. The exponential is that logically-equivalent join sub-products land in SEPARATE memo References (even identical scans fork Ã3) and never coalesce: `mergeable` (memo_merge.go:84) refuses once a group `HasWinnersOrMatches`, and `OptimizeGroup` interleaves `SetWinner` with `Integrate` yields, so a group holds a winner before its equivalent twin is born. RFC-074 now ships ONLY the **merge-value collapse** â a correct Go-only-divergence removal + prerequisite for single-type interning, **behavior-preserving (NOT a budget fix)**. Graefe re-ruled.
  - **PR-C2 â the actual â¥5-way budget fix (NEW, separate RFC).** Java does NOT solve the blowup by merging-under-winners (RFC-037 broad merge is a Go-only extension Java lacks); it **bounds the bipartition lattice at the source** via `shouldDeferCrossProducts` + `shouldJoinRightDeep` (Java `PartitionSelectRule.java:92,122`) and builds legs in a canonical interning-friendly form. Port/enable that pruning into `rule_partition_select.go` (the hooks exist â `ShouldJoinRightDeep`/`ShouldDeferCrossProducts` â verify defaults + why a pure connected chain isn't bounded). 1:1 Java-aligned. Do NOT decouple exploration from optimization (Java interleaves identically) and do NOT extend broad-merge-under-winners. Graefe-ruled.
- [x] **Correlated scalar subqueries.** Go-only extension â Java rejects at grammar level. Implemented via FlatMap with JoinTypeLeftOuter.

---

## Production readiness (Graefe review, 2026-05-28)

The Cascades architecture is solid â task stack, two-phase REWRITING+PLANNING, 16-criteria cost model, match-candidate infra all well-ported. The production risks are all at the **boundaries**: plannerâexecutor, executorâruntime, systemâoperator. Priority tiers below.

### P0 â fix before deploying anywhere (correctness/availability)

- [x] **ð© P0.4 DML executes through Cascades.** Fixed in RFC-035 â all DML (INSERT VALUES/SELECT, UPDATE, DELETE) routes through `planDML` â Cascades executor; `planOne` no longer branches on exec mode and the naive `execStatement` DML path (`execInsert`/`execUpdate`/`execDelete`/`execInsertSelect`, `pkPushdownCursor`) is deleted. INSERT VALUES reuses the Explode operator (RecordConstructorâArrayâExplodeâInsert) with plan-time arity/NOT-NULL/coercion; UPDATE SET RHS resolves to Values; DELETE/UPDATE WHERE gets EXISTS/scalar-subquery support via `upgradeDMLWhereWithCatalog`; INSERTâ¦SELECT maps projectionâtarget positionally and materializes (Halloween-safe). `IsUpdate()` derived from physical plan type; `RowsAffected` counted (Java's countUpdates); DML respects explicit transactions via `runInTx`. Fixed a latent non-correlated-EXISTS semi-join bug that also affected SELECT. QueryContext rejects update plans before executing (use Exec) â documented divergence in DIVERGENCES.md. Corner-case tests in `dml_cascades_fdb_test.go`. Graefe+Torvalds ACK (direction + implementation).
- [x] **P0.1 NLJ memory bomb.** Fixed in PR #203 â `CollectAllBounded` with configurable materialization limit (default 100K rows) on all 6 `CollectAll` sites. `MaterializationLimitExceededError` typed error. All cursor leaks on error path fixed. 11 regression tests. RFC-028.
- [x] **P0.2 Plan cache serves wrong plans.** Fixed in RFC-029 â cache keys on normalized SQL string directly (was uint64 FNV-64a hash with no text comparison on hit â collision = wrong plan). Scalar subquery staleness was a non-issue: `scalarSubqueryBinding` stores plans not results, re-evaluated per page fetch. `QueryHash` retained for tests only.
- [x] **P0.3 No context cancellation in executor.** Fixed in RFC-030 â `ctx.Err()` checks at the top of every cursor OnNext loop and drain function (44 sites across 19 files). `autoContinuingCursor` was the worst offender (created new FDB transactions on cancelled contexts). All cursor combinators, executor cursors, utility drains, DML drains, legacy query path drains, and iterator adapters now respect Go context cancellation. 24 unit tests verify prompt cancellation.

### P1 â fix before relying on the optimizer for real workloads (plan quality)

- [x] **P1.1 Wire statistics from FDB.** Fixed in RFC-031 â `fetchTableStatistics` was already wired (nightshift-100) but had two bugs: used read-write transactions for read-only stats (wasted commit), and fabricated equal-distribution counts for intermingled schemas. Fixed: `FDBDatabase.RunRead()` for no-commit snapshot reads, dropped intermingled fallback (returns nil â safe DefaultStatistics). E2E FDB integration test proves full pipeline: count maintenance â stats read â cost model â plan selection â execution.
- [x] **P1.2 Complete QOV-based FieldValue migration.** Fixed in RFC-032 â all 10 `stripAlias*` calls deleted (8 NLJ rule, 2 PushFilterBelowJoinRule). Predicates now retain `FieldValue(QOV(correlationId), "column")`; filters use `PredicatesFilterPlanWithAlias` so the executor binds each row under its correlation alias and resolves via `evaluateCorrelated` â zero string manipulation. `executePredicatesFilter` binds the inner alias whenever present (was gated on params only). Root cause exposed: `PartitionBinarySelectRule` (Java inner-join rule) fired on LEFT OUTER joins, pushing nullable-side predicates pre-join; guarded with `JoinInner`. `mergeRows` string qualification untouched (operates on executor row maps, not planner Values â separate concern). All 46 targets pass; determinism verified.

### P2 â fix before scaling operations

- [x] **P2.1 Plan cache LRU is O(n) per hit.** Fixed in RFC-033 â replaced the slice-based LRU order tracking (linear scan + slice splice in `promote()` on every hit, under the lock) with a `container/list` doubly-linked list + `map[string]*list.Element`. Promote-on-hit/update and eviction are now O(1), matching Java's Caffeine-backed cache. `RWMutex` downgraded to `Mutex` (the read path always mutated the list, so the read lock was a lie). `BenchmarkPlanCache_HitLargeCache` confirms position-independent O(1) hits at maxSize=1024; all existing LRU-semantics tests pass unchanged + new interleaved-eviction test.
- [x] **P2.3 Intersection cursors don't resume mid-stream (codebase-wide).** Fixed in **RFC-071**. `DecodeIntersectionContinuation` (exact inverse of `buildIntersectionContinuation`) splits the per-child `IntersectionContinuation` proto into START/MID/END resume states; `executeIntersection` and `executeMultiIntersection` create each child cursor accordingly (fresh / resume-from-bytes / `Empty`) via the shared `buildIntersectionChildCursors`, then use `IntersectionResume`/`IntersectionMultiResume`. `started` is now tracked **per child** in `mergeChildState` (matching Java's `KeyedMergeCursorState`, not derived from cursor-level state) and seeded from the decode, so a resumed mid-stream child can't be re-encoded as START. The loud guard is dropped. Also fixed a latent continuation-timing bug the paged test caught: both cursors captured the continuation *after* the post-match advance, losing every other match on resume (`[2,4,6]`â`[2,6]`) â now captured before. Pinned by white-box paged-resume tests (no dup/loss, asymmetric exhaustion, no-common, 3-child, both cursors) + decode round-trip/error/nil tests in `intersection_resume_test.go`. Graefe+Torvalds+@claude+codex ACK (v1 NAK'd â Graefe caught a limit-before-first-row child silently terminating the intersection + held-match loss on out-of-band stops, driving the full Java `MergeCursorState` consume-model port; @claude caught `intersectionMultiCursor` returning bare END on a limit instead of checkpointing; codex caught a decode child-count validation gap for 1/2-child tokens). Surfaced by @claude + codex on PR #249; landed as PR #252.
  - [ ] **Follow-up (RFC-071, Go-only optimization beyond Java): skip re-scanning discarded non-matching rows on intersection resume.** Because the cached per-child continuation sits at the last *consumed* (matched) position (faithful to Java `MergeCursorState`), an out-of-band stop resumes a child from there and re-scans the non-matching rows discarded since its last match (bounded by the inter-match gap; the whole prefix-to-first-match for a never-matched child). Correct (no dup/no loss) and Java-faithful, but for very sparse intersections under a tight per-page limit the re-scan is wasted work and â pathologically â could fail to make progress within one page. Tracking the position just *before* the currently-held candidate (so resume re-reads only it) would eliminate the re-scan; this diverges from Java's model, so it's a Go-only read-path optimization, not parity. Flagged by codex on PR #252.
- [x] **P2.2 Operational debuggability.** Fixed in RFC-034 â `PlanGenerationLogger` hook (nil = silent) emits one `PlanGenerationInfo` per Cascades planning call: SQL (truncated, rune-safe), plan hash (`plans.PlanHash`), plan explain, planning duration, cache event (hit/miss/skip/inconclusive), cache size, slow-query flag, error. Go analog of Java's `RelationalLoggingUtil` + `PlanGenerator` finally block; wired into `planSelectCascades` (real query) and `planDML` via a shared `planLogScope` with a named-return defer. EXPLAIN re-entry suppressed via `logMetrics bool`. No scalar "estimated cost" â the Cascades cost model is a comparator, not a number (matches Java; logs plan hash + explain instead). Threshold default sourced from the canonical `api.OptLogSlowQueryThresholdMicros` (single source of truth); `OptLogQuery` left intentionally unwired (no SLF4J level concept in Go â handler owns level + sampling), documented at `options.go:55`. Sampling is the handler's responsibility. 11 unit tests + 2 FDB integration tests (DML Skip event, SELECT miss-then-hit through the public driver). Graefe ACK, Torvalds ACK.

---

## Stress comparison â RFC-173 Slice 2 W3b live flip (2026-07-02, same machine)

Master (`/tmp/fdb-master` worktree) vs branch `feat/rfc173-slice2-wedge` @ 5ead4e149 â the ordinal
...
**2026-07-05 (RFC-173 item-1 commit 1 + fix round, PR #481 â dup-alias binding-id
...
4.03â4.35s vs 4.28â4.49s, sparse filter 3.58s vs 3.77s, needles 5.6/6.3ms vs
5.4/7.7ms, IN-list 14.6ms vs 14.9ms). All 23 subtests PASS both sides. No
regression.

**2026-07-05 (RFC-173 item-1 c2+c3 â the front-end flip + SELECT-* star layout):**
...
**2026-07-06 (RFC-173 item-3 c1+c2 â mixed-nesting LEFT widening: box roots + boxes as
legs): branch 154.35s total, all 23 subtests PASS â FASTER than the master baseline
...
**2026-07-06 (RFC-173 item-3 c4 â stranded-correlation keystone + review batch:
GetCorrelatedTo own-alias filter, SelectMerge surgical box-ref translation, twin
Legs alignment, ofOrdinal nullability flow): branch 160.51s total, PASS â faster
than the master baseline (161.68s), within noise of the c1+c2 row (154.35s). No
regression.

**2026-07-06 (RFC-173 unnest-residual c1 â box-leg owners gather, multi-segment
struct paths, SelectMerge/Explode arm, struct-column schema surface): branch
156.73s total, PASS â faster than the master baseline (161.68s); the NAK-round fix HEAD (proto-converter unification on the executor hot path) 160.53s, still clean. No regression.


**2026-07-06 (RFC-173 item-1 c4 â the review-round fixes: binding-keyed sort/group
keys, buried-EXISTS rebase, fold binding correlations):** branch 161.84s total, all
23 subtests PASS â equal to the master `8c179a025` baseline (161.68s) and faster
...
**2026-07-05 (RFC-173 item-2 commit 5b, PR #480 â cluster-gate rider transparency):**
baseline master `4f836f941` 156.14s total vs branch `bd802e83d` 156.32s (+0.1%, noise);
every subtest within measurement resolution (pk lookups 10â60ms both, index equality 20ms,
full scans 3.6â4.4s both sides). No regression.
...
### 8.1 Evaluate `pushDataAccessTasks` as CascadesRule â RESOLVED (keep procedural)

Graefe flagged this as procedural code that should be a rule. After investigation: **the procedural approach is architecturally correct.** `pushDataAccessTasks` operates on Reference-level PartialMatch state, not expression types â CascadesRules require expression-type pattern matching. Java uses explicit `TransformMatchPartition` task types for the same reason: this is task-level logic, not rule-level. Go's direct method call in `ExploreExprTask.Run()` is simpler and equivalent. No change needed.

### 8.2 Verify `promoteByDataAccessCost` heuristic absorbed â VERIFIED

`promoteByDataAccessCost` was deleted in eb94291a (dead code cleanup). Its heuristic (prefer lower-cardinality data access) IS absorbed into `PlanningCostModelLess` at `planning_cost_model.go:191â208` â Criterion #2: `maxDataAccessCardinality`, lower wins. This fires via `stampOrderingWinners(ref, costModel)` after every data access insertion. The cost model uses the same `findExpressionsByType` + `maxDataAccessCardinality` comparison. No heuristic was dropped.

### 8.3 Document `maxRoundsPerRef = 10` cap â DONE

Added comment at `unified_tasks.go:59` explaining: prevents divergence from rule cycles (AâBâA) that produce distinct-but-equivalent members. Java relies on memo dedup for fixpoint; Go's per-Reference dedup is weaker, so pathological rule interactions can produce new members indefinitely. 10 rounds >> typical 2â3 needed, safely under MaxTasks budget.

---

## Phase 7: Cascades alignment â close remaining Java divergences

### 7.1 Unify alias namespaces â DONE

Quantifier aliases now match table aliases at creation. Three band-aids removed: `rightAliasSet`, `planContainsJoin`, `collectPlanAliases` (â114 lines). Root-cause fix in `mergeRows`: bare inner keys overwrote qualified keys from nested joins (missing `!exists` guard). 46/46 tests, 15/15 determinism.

### 7.2 Port matching infrastructure for index intersections â DONE
...
### 7.3 Convert remaining predicateReferencesAlias sites â DONE
...
### 7.4 FlatMap wrapper correlation propagation â NOT NEEDED (Graefe confirmed)
...
4. NLJ rule: detect NullOnEmpty â wrap inner with FirstOrDefault + FlatMap

NLJ wrapper correlation propagation (walks predicates) is already correct and active.

### 7.5 + 7.6 (HOLISTIC â RFC-077): Source-anchored join result + structural interning

**Bundled per maintainer decision (2026-06-04):** 7.5 (structural interning key) and 7.6
(source-anchored field pull-up) are two facets of ONE change â retire the opaque, name-keyed
...
falls out for free** â the anchored RC is canonical (one type, alias-set-keyed), so it interns
...
(`values.go:2148`), so Go uses **name-based anchored resolution** â NOT Java's full ordinal-substrate
...
threading). 7.5 IMPLEMENTED â and the documented root-cause was CORRECTED by an implementation spike:
...
alias-sensitive sites are `Reference.Insert`/`InsertFinal`, which dedup alias-IDENTITY only â a
...
re-enumeration selects only â gating avoids over-deduping CTE column-rename selects, which silently
...
deterministic chain task-count gate (Â±2%, pinned 3-chain 8999 / 4-chain 30593; naive un-gated uniqueId
DOUBLES the 4-chain to 60044) + full suite green + 5Ã determinism. The opaque-type retirement
(JoinMergeAllValue/Seed/composeFieldOverJoinMerge) and anchored RC remain 7.6, deferred on column
threading (F3). See RFC-077 "Precise root-cause â CORRECTED".

**7.6 DONE (2026-06-05, RFC-077 v4):** column threading landed in the 7.6 core (#259); this follow-up
(a) anchors EVERY reachable join-leg shape â correlated scalar subqueries (incl. dotted scalarCol),
derived tables / aggregate subqueries / CTE references as join legs, recursive-CTE legs (outer +
recursive-branch self-reference), Sort/Distinct/Union/Aggregate legs â and (b) DELETES the opaque
...
- [x] **7.5 + 7.6 (RFC-077) â DONE.** 7.5 merged (#258), 7.6 core merged (#259), 7.6 retirement
  (anchor-all + delete opaque types) on `feat/7.6b-retire-opaque-merge`.

### 7.7 Retire `ImplementIndexScanRule` â unify on the data-access/`Compensation` path (RFC-045 follow-up)
...
- [x] **DONE (RFC-076 v5, 2026-06-05).** `ImplementIndexScanRule` + both registrations + its 3 test
  ...
  costing â 3a constraint-pass activation + stub-chain costing â deletion + **data-access compensation
  ...
  / index-only / vector-or-aggregate-inner / join-leg shapes â see `isSimpleResidualCompensation` +
  `refHasCorrelatedMatch`). `validateNoIndexOnlyResidual` KEPT (still load-bearing). Full suite green,
  plandiff byte-identical, determinism 5Ã. The data-access/`Compensation` path is now the sole scan
...
- [ ] **Follow-up (Graefe v5 ACK condition): replace the `isSimpleResidualCompensation` allowlist with
  ...
  `FinalYields.yieldUnknownExpression` â a non-`RecordQueryPlan` lands in the *exploratory* set and is
  ...
  allowlist arm (falls through to the dead-final-member path â silent no-plan). The honest fix is a Go
  `yieldUnknown`/exploratory-insert that re-optimizes all compensations and shrinks the allowlist to
  nothing â BLOCKED on Go's compensation re-optimization correctly handling IN-explode / correlated /
...
winner). Java has ONE path (`AbstractDataAccessRule` â `toEquivalentPlan`) and enforces "index-only
...
the rule â removing its member collapses the filter Reference and breaks the data-access intersection
...
so there is **no live bug** â but the layering is a smell whose root is the duplicated paths. Root fix
...
once, as in Java. See DIVERGENCES.md "ImplementIndexScanRule is a Go-only second index-scan path".
  - **RFC-076 v3 ACK'd (Graefe + Torvalds), committed `75bf8d17`. v2's leaf-matching diagnosis was
    ...
    match infra fires correctly (leaf scanâscan `EqualsWithoutChildren=true`; `matchSingleSourceAgainstSelect`
    binds the predicate to the candidate Placeholder; `pushDataAccessTasks` fires) â the gap is that
    every seed-match path builds its MatchInfo with `maxMatchMap=nil`, so `PartialMatch.PullUp`
    (`partial_match.go:117`) returns nil â `CompensateCompleteMatch` â `ImpossibleCompensation` â
    `DataAccessForMatchPartition` skips â ZERO scans. `ImplementIndexScanRule` is the SOLE producer.
    `ComputeMaxMatchMap` (`max_match_map.go:167`) exists but is never called by the seeds.
  - **WIP STASHED (`git stash list` â top of stack on this branch).** Implemented the data-access
    ...
    predicates as filters via `OfPredicateCompensation` â Java produces the match even when fully
    residual), an IN-sargable guard (an IN comparison is NOT a contiguous range â leave it to the
  ...
  - **REMAINING (multi-shift, per-feature vs Java â bigger than v2 stated):** broad `just test`
    ...
    `UnplannableIndexOnlyResidualError` (4 `TestVectorPlan_*`); (4) **DELETE over-deletes** â
    `TestFDB_DeleteOldAndLowValue` panic (correctness); (5) sort-elim ordering parts now computed but
    the satisfactionâordered-scanâ`RemoveSort` chain is incomplete (4 `TestSortElim_*`); (6) covering
  ...
  - **RFC-076 v4 (2026-06-04): step 1 DONE (5 correctness fixes, Graefe+Torvalds ACK), full retirement
    ...
    activate the dormant ordering-constraint pass (`constraintOnly` never set true â `PushRequestedOrderingThrough*`
    inert); (3b) template-aware costing (a nil-inner `Fetch` shell hides its inner from the cost model
    â join-order flip on `TestFDB_JoinSelPred_Repro`). See RFC-076 "v4 amendment" for the sequenced plan
    ...
    leaf/intermediate match rules, so the agg-skip type-switch â currently duplicated 4Ã (`planner.go:465`
    data-access boundary [new], `rule_implement_index_scan.go` [dies with the rule], `rule_streaming_agg_from_index.go`,
    `rule_aggregate_data_access.go`) â collapses to one. Torvalds flagged the boundary guard as a
    defensible transition shim, NOT the permanent design; the don't-seed fix is the root cause.

### 7.6 â MERGED into 7.5+7.6 (RFC-077)

7.6 (source-anchored field pull-up / retire `composeFieldOverJoinMerge`) is no longer a separate
item: it is the same change as 7.5 (anchored RC retires the opaque merge â structural interning
...
`vectorIndexDefinition` (`CREATE VECTOR INDEX â¦ USING HNSW â¦ PARTITION BY â¦ OPTIONS(â¦)`),
`qualifyClause`, `overClause`, `windowSpec`, `nonAggregateWindowedFunction(ROW_NUMBER â¦)`.

**The gap = the relational front-end + Cascades wiring** (the "just not relational bits"):

**Status: DONE (RFC-045, Graefe+Torvalds ACK).** 9.1â9.4 all landed, tested, green. The full
SQL vector K-NN read path works end-to-end: a partitioned HNSW index +
`SELECT â¦ WHERE <partition> QUALIFY ROW_NUMBER() OVER (PARTITION BY â¦ ORDER BY
...
partitioned) â fails to plan rather than returning wrong results; revisit if needed.

- [x] **9.1 DDL: `CREATE VECTOR INDEX â¦ USING HNSW â¦ PARTITION BY â¦ OPTIONS(â¦)`** â metadata vector
...
- [x] **9.2 Query front-end: `QUALIFY ROW_NUMBER() OVER (PARTITION BY â¦ ORDER BY <distance>(vec, q)) <= K`.** Done â walk.go builds DistanceValue + RowNumberValue; predicates.TransformRowNumberDistanceRankMaybe ports transformComparisonMaybe; QUALIFY lowers to a DistanceRank ComparisonPredicate.
  No `qualifyClause` handling and no window-functionâValue visitor exist (`grep QualifyClause` â 0 hits;
...
- [x] **9.3 Cascades wiring + vector physical plan.** Done â (9.3a) tryVectorIndexCandidate enumerates the candidate + ExpandVectorIndex builds the distance placeholder + valuesMatchColumn matches it; (9.3b) ToScanPlan splits partition prefix from the DistanceRank binding; (9.3c) RecordQueryVectorIndexPlan + executeVectorIndexScan dispatch BY_DISTANCE; physicalVectorIndexScanWrapper + the index-only compensatability guard (valueContainsUncompensatable via values.IsIndexOnly on the match path + the residual-skip loop in ImplementIndexScanRule) make the vector scan the sole physical winner â the DistanceRank predicate, being index-only, is never lowered to a residual filter, exactly as Java's match-then-implement does. Three pieces (Torvalds catch â not a single
  ...
  distance-rank comparison (which rides as an *equality-shaped* range, Ã  la Java
...
### RFC-010 audit findings (the original 15 â correctness fires)
...
- [x] **#2** `ErrWrongShardServer` 1062 â 1001 + anti-self-confirming fault test (Phase 0)
- [x] **#3** pipelined `Get` shares full classifyâinvalidateâretry; 1006 surfaced correctly (Phase 0)
- [x] **#4** tenant commit builder uses a scratch `[]MutationRef` â no in-place mutation of `tx.mutations`, no double-prefix on rebuild (build-twice regression; Torvalds + FDB-C ACK)
- [x] **#5** hedge loser/timeout/cancel QueueModel deltas released (Phase 0)
- [x] **#6 â HIGH.** Conn shutdown â fixed in RFC-050. One `failConnection(err)` path (`sync.Once`: cancel ctx + close socket + `failAllPending`) is the single teardown, used by `Close`, `connectionMonitor` death, and `readLoop` read errors. **(1)** `SendFrame`/`Flush` now wait on `errCh` **or** `ctx.Done()` (and deliberately don't pool `errCh` on the `ctx.Done()` path â audit #13 stale-value hazard), so a sender whose frame is still queued when `writeLoop` exits no longer hangs forever. **(2)** `connectionMonitor` death now calls `failConnection` â adding the missing `conn.Close()` that unblocks `readLoop`'s blocking `Read` (the old bare `cancel()` leaked the fd + goroutine until the 10 s TCP keepalive). Single-delivery to a pending reply still comes from the pending-map + `pendingMu` + delete-as-you-go; `closeOnce` only guarantees the meaningful error wins. SimTransport scope: built the in-process `net.Pipe` fake-server test harness #6 needs (handshake + stall / go-silent / abrupt-close modes) and made the monitor cadence injectable (unexported `withMonitorCadence` on an unexported `dialWith`; public signatures unchanged); the full seeded multi-mode SimTransport is deferred to C4 (YAGNI). 6 deterministic in-process `-race` tests (the two core ones verified failing on the pre-fix code: stranded-sender hang + monitor-no-socket-close leak). FDB-C + Torvalds ACK.
- [x] **#7 â MEDIUM.** Honor the "methods safe for concurrent use" contract â fixed in RFC-049. Writers already appended under `conflictMu`; the unprotected readers/clears now do too: `Commit` validation + read-only check snapshot `mutations`/`len(writeConflicts)` under the lock and **thread that validated snapshot into the marshal** (so a `Set` racing `Commit` can't ship an *unvalidated* mutation to the proxy â FDB-C catch); `buildCommitTransactionRequest`/`commitDummyTransaction` snapshot the conflict headers under the lock (append-only + `conflictBuf`-only-grows â snapshot-and-release is race-free for them); `GetApproximateSize` iterates **under** the lock (not a released snapshot â it can race `Commit`'s in-contract auto-reset, which `[:0]`-reuses the backing arrays); `mutations[:0]` clears moved inside `conflictMu` in reset/postCommitReset; `addWriteConflict*` moved the `nextWriteNoConflict`/`writeConflictsDisabled` gate inside the lock (the one-shot flag is read+cleared on the `Set` path â two concurrent `Set`s raced). `Set`/`Clear`/`ClearRange`/`Atomic` now publish the mutation + its write-conflict range **atomically** under one `conflictMu` acquisition (codex catch â the old two-lock split let a `Commit` snapshot ship a mutation *without* its conflict range â a missed conflict; this also subsumes the `nextWriteNoConflict` fix and drops `Set` from two locks to one). Contract doc narrowed: option setters (`SetXxx`) + `Reset` are configure-before-use, not concurrent-safe (matches `fdb_transaction_set_option`); RYW lost-update stays documented-not-safe. 6 deterministic concurrency tests (verified failing on the pre-fix code) + tenant no-alias sentinel + validated-snapshot pin + Set-atomicity invariant. FDB-C + Torvalds + codex review.
- [x] **#8** `ReadErrorOr` parses the union tag (not field count); error code uint16 (Phase 0)
- [x] **#9** rename `isSystemKey` â `isSpecialKey` (tests `\xff\xff` special-key space; behavior unchanged)
- [x] **#10** decoupled `ACCESS_SYSTEM_KEYS` from `LOCK_AWARE` in `fdb/options.go` (C sets them
  independently â confirmed NativeAPI 7159 / RYW 2557 / TenantManagement). Facade no longer
  ...
  relying on the implicit coupling must set `SetLockAware` explicitly (as a Java/CGo app must) â only
...
- [x] **#11 â MEDIUM.** TLS wired end-to-end â fixed in RFC-051. `ParseClusterString` parses the `:tls` coordinator suffix (faithful to C++ `NetworkAddress::parse`: strip `(fromHostname)` then `:tls` when len>4; uniform-cluster, mixed rejected) â `ClusterFile.UseTLS`; `database` carries `tlsConfig *tls.Config` and `getOrDialConn` dials TLS; `resolveTLSConfig` loads `FDB_TLS_{CERTIFICATE,KEY,CA}_FILE` (â `/etc/foundationdb/{cert,key}.pem` default) into a standard config, C++-precedence-faithful. **Go-idiomatic user-facing API (bradfitz review):** `transport.Dial(ctx, addr, *tls.Config, dialFn)` â the non-nil config is the *only* "use TLS" signal (nil = plaintext), so the silent-downgrade footgun is gone by construction (the `useTLS bool` + `DialWith`/`DialWithTLS` overloads + bespoke `transport.TLSConfig` are deleted); `fdb.OpenDatabase(clusterFile, WithTLSConfig(*tls.Config), WithDialFunc(...))` functional options, precedence explicit > `FDB_TLS_*` env > default; `upgradeTLS` clones + fills `ServerName`/`MinVersion` only if unset. 6 deterministic tests incl. a real in-process mutual-TLS handshake (FDB ConnectPacket inside the tunnel) + wrong-CA/missing-client-cert rejects. FDB-C + Torvalds + bradfitz ACK. Follow-ups: per-address TLS flag (dual-listen), `FDB_TLS_VERIFY_PEERS` rule DSL, `FDB_TLS_PASSWORD`/encrypted keys, FDB-TLS testcontainer e2e.
- [x] **#13 â LOW (concurrency-sensitive).** Fixed in **RFC-072**. The reply channel is now returned to the pool exactly on the no-send-can-race paths: `Release()` pools it on the success path (caller received, no `Cancel`); `Cancel()` pools iff it won the `delete` race and nils `h.ch` so `Release` never double-pools; `SendAndWait` pools on success and via `cancelPending` (delete + pool-iff-won) on timeout, leaving the rare race-loser to GC (it may hold a stale buffered value). The false "readLoop returns it after dispatch" comment is corrected â readLoop only delivers. Pinned by `reply_pool_test.go` (won/lost-race + success + no-double-pool, `-race`-clean) via a `putReplyChannel` seam (deterministic, not `sync.Pool`-reuse-dependent). Full multi-goroutine timeout-vs-delivery race coverage awaits `SimTransport` (C4). FDB-C + Torvalds ACK.
- [x] **#14 â LOW.** Monitor ping on a saturated `writeCh` â fixed in RFC-052. The send was already non-blocking (`select â¦ default`), but the drop path returned a **closed** `done`, which the monitor read as `case <-replyCh:` "PING reply arrived â connection alive" â so a *stuck* connection (writeLoop blocked on an undrained socket â `writeCh` saturates) falsely passed as alive and never reached the `bytesReceived` liveness check (the one state where the monitor must act). Fix: the drop path returns **nil** (never selected in the monitor's `select`) so it falls through to the timer â `bytesReceived` kill â faithful to C++ `connectionMonitor`, whose liveness verdict is solely bytes-received (the ping-reply arm only restarts the cycle; C++ `Peer::send` is an unbounded buffer with no "couldn't send" path). Pinned by `TestSendPingWithReply_DropsToNilOnFullWriteCh` (verified failing on the pre-fix closed-`done`); the sent-path kill stays covered by `TestConn_MonitorDeathClosesSocket`. FDB-C + Torvalds ACK.
- [x] **#15** range-iterator next-begin via `keyAfter` helper that copies (no alias/scribble of `lastKey`); spare-capacity unit pin
- **#12 â FALSE POSITIVE.** Locality never panics (invariant guarantees non-empty); add a defensive guard at most.
...
mutated. (Determinism for our OWN retry/LB/wire-error paths â `PendingGet.Resolve`'s
flush/transport/timeout arms, the codex 1006 drop-between-dial-and-send race, transparent
wrong-shard retry â comes from a seeded in-process `SimTransport` fake server behind
`transport.DialFunc`, extending the existing `wrongShardConn`; tracked as a separate Phase-1 item.)

- [x] **C1. Ride their oracle â FDB `ConsistencyCheck` after Go-client writes.** DONE
  (`pkg/fdbgo/conformance/consistencycheck_test.go`). `RunCluster(3, double, ssd)` â
  pure-Go client writes 1000 keys â wait replication-healthy â run FDB's one-shot
  `fdbserver -r consistencycheck` role â parse its JSON trace and assert it completed
  (`ConsistencyCheck_FinishedCheck`), examined data, and emitted **no** Severity-40
  inconsistency/`TestFailure` event. **Double redundancy is required** â under single
  ...
  baselines (one per shard) â i.e. some single shard was read on â¥2 replicas, which a bare
  "â¥2 reads total" count can't prove (N single-replica shards defeat it). `FirstValidServer`/
...
- [x] **C2. Ride their client â differential vs the official C binding (`libfdb_c`).** Landed in
  **RFC-053 (PR #231)**. Differential harness in `pkg/fdbgo/bench` (reuses the dual-client fixture):
  L2 write battery (byte-identical persisted state â Set shapes incl. exactly-VALUE_SIZE_LIMIT, every
  atomic on a missing key pinning the MinâMinV2/AndâAndV2 upgrade, SetVersionstampedValue offset,
  ...
  (reverting MinâMinV2 fails it byte-exactly). **Surfaced & fixed FOUR real client divergences**, each
  ...
  **Follow-up RFC-054: `FuzzDifferential`** â random op sequences through both clients,
  ...
  **Follow-up RFC-055: RYW-read differential (Get/GetRange)** â found+fixed a getRange
  ...
  DATA plane, never the wire.** Request frames are legitimately NOT byte-identical â reply-promise
  ...
  range chunk boundaries all vary per client. So:
    - **Writes â byte-exact on PERSISTED bytes.** Write the same logical mutation via each client,
      ...
      + magic `6773487359078157740`. This is the cross-client compatibility hard line â where
    ...
    - **Reads â semantic, control-plane excluded.** Same key/range + a pinned read version â
    ...
    - **Continuations â mutually resumable** (a Go-produced continuation resumes correctly when fed
...
- [ ] **C2-followup. RYW key-selector + read-version correctness audit (RFC-056).** Remaining
  ...
  behavioral divergence** â both clients correctly return 1007 once a read version genuinely ages
  ...
  version via the canonical `gofdb.IsRetryable` predicate â `differential_getkey_ryw_test.go`).
  REMAINING: profile go-getKey 1007-rate vs cgo to confirm item-1 closes it. See rfcs/055.
  - [x] **(1) `Transaction.GetKey` ignores pending writes** â FIXED (RFC-056): faithful port of
    ...
    loop), wired into `Transaction.GetKey` (+ the baseâresolved RANGE read-conflict, fixing the
    ...
    `{orEqual, offset>1}` â not used. Pinned by `ryw_getkey_test.go` + the
    ...
    â it eagerly folds resolved atomics into plain entries and moves a matched CompareAndClear
    into the cleared list; faithfully closing either needs a write-map that retains op-type, like
    C++'s):
    (a) **phantom offset slot** â a PENDING atomic that resolves to no value (CompareAndClear, or
    ...
    writes until then.
    (b) **conflict-range filtering** â C++ `updateConflictMap` SUBTRACTS independent-write/cleared
    segments from the getKey read-conflict (no DB read there). Go keeps the FULL baseâresolved
    range: it OVER-conflicts on those segments (extra retries, always SAFE) rather than risk a
    missed conflict on a folded dependent atomic (an UNSAFE under-conflict â a naive
  ...
  - [x] **RYW applyAtomic on present-empty values** â FIXED: the chain conflated `nil` (absent)
    with present-empty, so a V2 op after `Xor(k,"")` took the absentâoperand path (`Min(k,"0")`
    â 0x30 vs libfdb_c 0x00). The get/merge chains now keep present-empty non-nil (nil reserved
  ...
  - (3) **versionstamped-pending read = unreadable.** A SetVersionstampedKey/Value pending on a
    key reads as ABSENT in Go pre-commit (Getânil, GetRangeâomit); C++ marks it `is_unreadable`
    and THROWS `accessed_unreadable`. Go has no unreadable state â approximated as absent,
    ...
    needs an explicit unreadable concept â part of the RFC-056 audit.

- [ ] **RFC-056 continuation â ordered, ONE AT A TIME (do 1, then 2, then 3).** After the merged
  ...
  1. **[x] DONE (RFC-057).** Lazy `rywSegCursor` replaced the materializing
     `buildSegmentsLocked`: getKey cost is now FLAT in cache size â **57 ms / 39 MB â
     1 Âµs / 816 B at N=100k (55,437Ã)**, measured before/after (Torvalds's "no benchmark =
     ...
     **Lazy/windowed segment iterator for getKey-RYW.** `buildSegmentsLocked`
     ...
     [allKeysBegin, maxKey) â O(writes + cacheKeys) per resolution attempt â whereas libfdb_c's
     ...
     ALSO shrinks **item (2)** below: less work per getKey under heavy parallel-container load â
     less likely to drift past the 5s MVCC window mid-loop â fewer transient
  ...
  2. **[x] DONE (RFC-058).** Op-type-preserving write-map closed BOTH sub-edges. Added `absent`
     ...
     at the landing â not "counted and landed on." Modeled as `segPhantom` (count-in-walk +
     ...
     (`doMax(_,"")`ânil misread as absent by a later CompareAndClear). (b) Ported `updateConflictMap`
     (ReadYourWrites.actor.cpp:335) as `conflictRangesLocked` â the getKey read-conflict now
     ...
     (a) **phantom-slot offset counting** â a PENDING atomic that resolves to no value
     ...
     (b) **exact `updateConflictMap` conflict filtering** â getKey's read-conflict should SUBTRACT
         independent-write + cleared segments (no DB read there); Go currently keeps the
         conservative FULL baseâresolved range (safe over-conflict). With op-type preserved, the
         subtraction is safe (a naive `!hasAtomics` filter was UNSAFE â it dropped the conflict
         for a Get-folded dependent atomic; codex caught it on #235 â reverted). Port
  ...
  3. **Fresh differential axes (`/hunt-divergences`).** Probe axes still uncompared vs libfdb_c:
     ...
     - [x] **[RFC-059 â MERGED #238] RYW-disable-after-op poison.** Differential characterization
       ...
       GetEstimatedRangeSizeBytes, GetRangeSplitPoints, Commit) returns 2000 â the whole txn is
     ...
     - [x] **[RFC-060 â MERGED #239] tuple-codec byte-identity differential.** The tuple/key encoding is the wire
     ...
     - [x] **[RFC-061 â MERGED #240] SNAPSHOT_RYW_ENABLE/DISABLE counter.** Found via the
       ...
       boolean with `SetSnapshotRywEnable()` a no-op â so `disableâenable` left snapshot reads
       ...
       redâgreen + a counter-vs-boolean discriminator + negative-count axis + RYW-disable
       dominance).
     - [x] **[RFC-062 â MERGED #241] atomic-fold width/edge differential.** Atomic fold semantics
       ...
       client-side fold (doAdd/doMin/â¦) runs ONLY on in-txn reads â a commit-then-read-back test
       ...
       shared a key â missing-key fold saw the other's committed value).
     - [x] **[RFC-063 â MERGED #242] versionstamp-mutation differential.** SetVersionstampedKey/Value
       ...
       reject, negative, past-body, too-small, empty â 2000 go==cgo), multi-op. Mask offset is
       ...
       - [x] **Follow-up (tenant +8 offset) â DONE + found a BIGGER bug.** Built the tenant
         ...
         Pinned by `TestDifferential_TenantCrossClientCRUD` (goâcgo create/open/write/read/list) +
         `tenant_crud_internal_test.go` (FDB-spec vectors, round-trip, legacy decode, errors).
     - [x] **[RFC-064 â MERGED #243] explicit conflict-range API differential.** AddReadConflictRange/
       Key + AddWriteConflictRange/Key feed the resolver (isolation) but had no differential coverage
       (RFC-058 covered only getKey-DERIVED conflict ranges). Empirically NO divergence â edges
       (invertedâ2005, emptyâaccept, oversizedâaccept) match go==cgo (the C++ NativeAPI source has no
       release inverted-check, but the C binding cgo uses returns 2005 â the differential is the spec,
       ...
       pinning (both A+B SetReadVersion(vSetup), transientâretry, fresh prefix/attempt, bounded) â
       flake-free (5 runs). Teeth: empty key-conflict range â key_exact_r5 diverges. Oversized
       committed-truncation is unobservable (keys > maxKeySize are unwritable).
     - [x] **[RFC-065] getKey boundary resolution â REAL BUG FIXED.** The existing
       ...
       readThroughEnd after the walk for offset>1. Fix: direction-aware off-end branch â forward
     ...
     - [x] **[RFC-067 â MERGED #247] error-CODE differential â TRANSACTION_SIZE_LIMIT + 4 linked fixes.**
       ...
       the Go client did NOT enforce `TRANSACTION_SIZE_LIMIT` by default â it committed >10 MB txns
       ...
       indexer lessen-work codes (Torvalds â wrong numbers, missing 2101, made latent-live by the
       limit; now matches Java `IndexingThrottle.lessenWorkCodes` 1:1); (3) commit-validation ORDER
       (codex â read-only fast path + per-mutation-before-size; then the full eager-vs-deferred model:
       ...
       contract (codex+FDB-C+++Torvalds â a blanket `continue` silently committed every illegal mvk
       ...
       buffer (codex â a Set racing Commit could fail a small commit for an unshipped mutation; pinned
...
- [x] **GRV `locked` enforcement â DONE (RFC-096, FDB-C++ + Torvalds ACK on RFC; found by the
  ...
  `:7409` precedes `:7425`), and â because Go's GRV cache is ALWAYS-ON unlike C++'s opt-in
  USE_GRV_CACHE (divergence filed below) â `locked` rides the cache (`grvCache.lastLocked`,
  ...
  READ_LOCK_AWARE ok, unlock+poll recovery) â revert-proven red without the check â plus the
  production-parser `locked` assert in the `GetReadVersionReply_locked` reply vector.

- [x] **GRV cache is ALWAYS-ON in Go; opt-in (USE_GRV_CACHE) in C++ â DONE (RFC-104).** Closed:
  ...
  (`grv.go:293`) â matching C++ `NativeAPI.actor.cpp:7504-7517` (gate `:7505`, default false
  `:6148`). The opted-in cached path fail-opens on `locked` exactly as C++ does (`:7514-7516`), so
  RFC-096's `lastLocked` ride-along â which existed ONLY to compensate for the previous always-on
  cache â was removed (`grv.go:38-45`). The RFC-098 wrong-answer (a default Go txn serving a
...
- [ ] **C3. Ride their test designs â port FDB workloads as scenario + invariant specs.** FDB's
  `fdbserver/workloads/*.actor.cpp` (Cycle, AtomicOps, ConflictRange, Serializability,
  FuzzApiCorrectness, â¦) are unrunnable for us (Sim2-only), but each scenario + invariant is
  language-agnostic. Port the adversarial designs â e.g. Cycle: maintain a ring of pointer K/Vs,
  hammer it concurrently (+faults), verify the ring stays unbroken â to drive our client against
...
- [x] **C4. Deferred Phase-0 test gaps â DONE (RFC-118 SimTransport).** All four closed with
  revert-proven regressions (`client/simtransport_test.go`, migrated `client/fault_test.go`):
    - **Inline `LoadBalancedReply.error` on `parseGetKeyReply` / `parseGetKeyValuesReply` / `parseGetValueReply`** â
      ...
      real FDB delivers a read wrong-shard. (RFC-115 Â§6 had already fixed the `Optional<Error>`
      marshal â the "generated writer mis-marshals" caveat above was stale.)
    - **`PendingGet.Resolve` flush-error arm** â a `Close()`d real conn â `Flush()` returns
      `errConnClosed` deterministically (`TestPipelinedGet_Resolve_FlushErrorRetries`).
    - **Range wrong-shard mid-scan (`more=true`), fwd+rev** â `flipMoreReply` forces a continuation,
      1001 injected on the continuation frame; asserts no dup/drop (`TestSimRangeWrongShardMidScan`).
    - **`future_version` (1009) / `process_behind` (1037) â QueueModel backoff** â inline 1009/1037
...
- [ ] **Parallelize the whole `//conformance` suite via stdlib `t.Parallel` (drop Ginkgo). [LOW PRIO â RFC-082 follow-up]**

  **Goal.** Cut the GoâJava conformance suite wall time (~122s today) by running *every* cross-engine
  check concurrently, uniformly â no bespoke fan-out. Today only the two SQL loops are parallel
  ...
  **once, directly** (serial invocation). So the only available parallelism is **in-process**.
  Ginkgo cannot parallelize in-process â its only parallel mode is the `ginkgo --procs=N` CLI, which
  spawns N worker *processes* (each would spin its own FDB container â the 290-failure resource
  ...
  `t.Parallel()`") â it's the lone serial holdout.
  ...
  **Measured profile (121.6s wall, 112s in specs; `ginkgo-report.json` from a `--nocache_test_results`
  ...
  36s â **already** 8-Java-server parallel; `yamsql A3` (859 specs) 20s â **already** 8-server
  parallel; **~40 FDB conformance families â 56s â SERIAL, on the single global Java server.**

  **The load-bearing finding â the ceiling is JVM count, not Go concurrency.** The suite is
  Java-JVM-throughput-bound and JVM count is **memory-capped on CI** (16 JVMs is exactly what caused
  the earlier conformance CI timeout; 8 is the safe ceiling). The SQL work already runs 8-way â that
  ...
  window â **~122s â ~70-75s (~1.7x) @ 8 JVMs**. Beating ~70s needs **more JVMs** (memory), not more
  ...
  **Approach (incremental, safe).** stdlib `Test*` funcs coexist with Ginkgo's `TestConformance` in
  ...
  batch together) â so migrate **family-by-family** with a green + spec/assertion-**count-parity**
  gate after each (silent coverage drops are the exact CLAUDE.md failure mode). Steps: (1) move
  container + Go DB + a pool of N Java servers into `TestMain` (all servers spawned before any test â
  preserves the "no JVM spawn during a query" GRV-lag discipline); Gomega assertions stay verbatim via
  `g := NewWithT(t)`; `BeforeEach` â a setup helper; nested `Describe` â flat test names / `t.Run`
  subtests. (2) Convert each FDB family (already UUID-tenant-isolated â inherently parallel-safe).
  (3) Convert A3 + SeedRunCorpus to `t.Run(..., t.Parallel())` subtests and **delete** the hand-rolled
  worker pools + `precomputed` map + `results[]` â this is the "stop special-casing A3" cleanup.
  (4) `-test.parallel=N` via the `go_test` `args`. Keep the FDB-1020 conflict-retry (shared catalog).
  Benchmark stays gated (`CONFORMANCE_RUN_BENCHMARK`). Query-engine-adjacent â needs Graefe +
  Torvalds + @claude + codex.

  **Cheaper alternative (no rewrite, ~zero risk, ~1.3x):** just raise the existing SQL pool 8â12
  (`CONFORMANCE_A3_POOL_SIZE` / `CONFORMANCE_SEED_PARALLELISM`) if the CI runner's memory allows â
...
## Exploration: a second, FDB-native vector index (Go-only â NOT Java parity)

- [x] **Explore an FDB-native ANN index for a high-latency networked KV store â REALIZED by SPFresh (RFC-094).**
  *Status: the headline question ("build an FDB-native ANN index for this substrate, and which?") is
  answered â **SPFresh**, the top candidate below, is built, shipped, and SQL-exposed; the authoritative
  ...
  needed-by SPFresh â future ideas on file, not open SPFresh work.* This is a deliberate Go-only extension, NOT a Java-parity item â
  ...
  read/write it, so this index is **Go-built and Go-read only** â it forfeits cross-engine sharing for
  ...
  **Motivation.** The existing HNSW index is now **100% Java-faithful** (the Go-only cross-transaction
  `sharedNodeCache` was removed for compliance â see `hnsw.go`). Being faithful, it inherits Java's
  latency profile on FDB: classic HNSW assumes O(1) RAM and does 50â200 *sequential, data-dependent*
  pointer-chasing reads per op; on FDB every hop is a ~0.3â0.5 ms round-trip, so search/build are
  ...
  cannot. The honest fix is not more caching bolted onto HNSW â it's an index whose *algorithm* fits a
  networked KV store.

  **Candidates (ranked by fit / payoff):**
  - **SPFresh** â *in-place incremental update for disk-based ANN* (LIRE/centroid-partitioned posting
    ...
    local to a partition â far less cross-writer contention than HNSW's shared adjacency mutation.
  - **DiskANN / Vamana** â single flat graph, higher degree + long-range edges â a search touches
  ...
  - **SPANN** â cluster + posting-list; turns the random-access graph walk into a few large
    `GetRange` reads (one round-trip for many keys â exactly what FDB is good at). Recall/locality
    tradeoff vs a navigable graph.
  - **Batched beam search** â *not a new index*: keep HNSW but expand the whole `ef` frontier in one
    batched multi-get per round instead of node-at-a-time, collapsing N sequential hops into log-depth
    batched rounds. **Wire-neutral** (no format change) â the cheapest real query-latency win and a
    good first step regardless of which index above we pick. Could even land on the existing HNSW.
  - **FDB-native build primitive â atomic-append neighbor lists.** If adding an edge is an FDB atomic
    mutation (no read-modify-write), writers don't register a read-conflict range on the neighbor â
    no 1020 storm â concurrent multi-writer build becomes correct *and* fast without the single-writer
    lock. Applicable to HNSW or a new index.

  **Outcome:** SPFresh was chosen, prototyped, and shipped (RFC-094) â that step is **done**. The one
  genuinely-still-open, wire-neutral idea from the candidates above is **batched beam search** on the
  existing HNSW (collapse N sequential hops into batched rounds â the cheapest query-latency win, no
...
- [x] **fdbgo/wire: `TestPrecomputeSize_GetReadVersionRequest` never runs in CI and fails when run.**
  â DONE (RFC-095, wire ground-truth net repair). The hand test was stale (it omitted the 8-byte
  fake-root object C++ `save_helper` allocates) â deleted; the production serializer is pinned
  ...
  (`flat_buffers.h:964` unconditional write) â Go commit-request bytes diverged from libfdb_c;
  (2) `parseSplitRangeReply` decoded ZERO split points from every real reply (splitPoints is a
  FlatBuffers offset-vector, not an inline blob) â production `GetRangeSplitPoints` never worked,
  ...
  `NativeAPI.actor.cpp:6726`; latent â proxy only sends that shape under report_conflicting_keys).
  (`parseWaitMetricsReply`'s envelope-`UnmarshalFDB` was originally claimed as a 4th bug; Torvalds'
  mutation probe disproved it â correct by layout, ErrorOr's value offset coincides with FakeRoot's
...
## SPFresh â tracked in RFC-094 (status)

All SPFresh tracking â current state, shipped work, open items, frozen
performance, and measured-negative levers â is consolidated in the authoritative
...
- **Tier 1:** SPFresh has no chaos/model-based fault coverage â the whole
...
## RFC-165 â ANSI SQL Core conformance backlog (read-side reach beyond the Java port)

Generated scorecard: `SQL_ANSI_CONFORMANCE.md` (Ledger A) tracks SQL:2023 Core (176 features per
PostgreSQL 18). `Go?` is derived from `# ansi:` corpus tags â never hand-typed. The **shared-gap**
...
- [ ] **E091-07 `COUNT(DISTINCT)` / DISTINCT quantifier** â rejected 0A000 in both engines. The
      single highest-value Core gap. (Pinned today as `# ansi-gap: E091-07` on `count_distinct.yaml`.)
- [ ] **E071-01 / E071-03 `UNION DISTINCT` / `EXCEPT DISTINCT`** â only `UNION ALL` works (E071-02).
      No Cascades dedup rule. (`# ansi-gap: E071-01` on `union.yaml`.)
- [ ] **E061-11 subqueries in `IN` predicate** â rejected 0AF00 in both. (`# ansi-gap: E061-11` on `subquery_in.yaml`.)
- [ ] **E021-04/05/06/08/09/11 string functions** (`CHARACTER_LENGTH`, `OCTET_LENGTH`, `SUBSTRING`,
      `UPPER`/`LOWER`, `TRIM`, `POSITION`) + **E021-07** string concatenation â no function-catalog
      entry in either engine (42883). A whole Core subfeature family.
- [ ] **E011-03 `DECIMAL`/`NUMERIC`** â no exact-decimal type; BIGINT division truncates.
- [ ] **E141-04/06/07 `FOREIGN KEY` / `CHECK` / column defaults** â only NOT NULL/UNIQUE/PK today.
...
is tagged â never by hand-editing the doc.
...
- [ ] **Verify the `Java?` roster facts against the live 4.12.11.0 server.** The `Java?` column in
      ...
      SQL_CONFORMANCE.md), structurally contained (it can't inflate the Go headline â see RFC-165 Â§4.6)
...
"the 7.1 charter" â TODO 7.1 [alias-namespace unification] is DONE; this is the SUCCESSOR item):
1. **Per-reference ambiguity + fresh-id gating for duplicate FROM aliases** â Java's exact
   ...
   **DESIGN SUBSTRATE BANKED (2026-07-05, RFC Â§ "QP-REF-BIND item 1 â design substrate"):
   ...
   Mechanism M1âM6: scope accepts duplicates + per-attribute qualified resolution; F3-ruled
   ...
   constraint â front-end acceptance and back-end binding never live separately
   (mis-pushdown wrong-rows hazard).
   **ITEM-1 COMPLETE (2026-07-05, PR #481): c1 (dark mint + binding-keyed seed, 34872539b +
   fix round) + c2+c3 (the lift â per-attribute resolution + FROM-walk 42702 retirement +
   ...
   RFC Â§ "QP-REF-BIND item 1 â c2+c3 record". codex + @claude remain the PR-side gauntlet.**
   **c4 â the review round (2026-07-05): the PR gauntlet caught two REAL post-"COMPLETE"
   ...
   group keys) â sort+group keys now route through ResolveQualifiedProjection, group-key
   datum keyed by bare Field. P2: correlated EXISTS over an un-collapsed cross join â bisect
   pinned the buried-reference class as an ITEM-2 regression (worked pre-item-2) â
   duplicate-preserving outer scopes + gated flatten binding correlations +
   rebasePlanOuterRefsOrdinal (buried refs in the existential subplan â merged positional
   row, expression-gated, fail-closed verified). P3: the projected-EXISTS fold served NULL
   for a later dup leg's columns â buildExistentialJoinSelect + classifySortSource now speak
   ...
   record in RFC Â§ "QP-REF-BIND item 1 â c4 record". Follow-on booked below: aggregate
   ...
   loud-decline guard, RFC Â§ c5 record) and c5b (P4e/P4f pins) â four-gate ACK on a789b66a9
   (architecture + code-quality + both PR-side reviewers), CI 6/6, stress at master parity
   (161.84s vs 161.68s). Item 3 UNBLOCKED.**
2. **Existential-flatten ordinalization** â translateJoinWithExists keeps the ANCHORED seed
   ...
   **DESIGN SUBSTRATE BANKED (RFC Â§ "QP-REF-BIND item 2 â design substrate"): E2-validated
   executor binder; the W4-left rebase machinery is currently DEAD on live SQL (enclosure
   forced under EXISTS â the slice's commit-4 lift). DESIGN RULING: ACK with amendments
   (RFC Â§ "design ruling + commit-1 record"). COMMIT 1 MERGED (PR #469, master 33291617d,
   four-gate ACK â Graefe, Torvalds after two converted NAKs, codex P1+P2 fixed-and-pinned,
   @claude Ã4): the no-op existential residual (EXISTS == NOT EXISTS rows on
   LEFT+EXISTS) root-caused across four layers and fixed with the Java-shaped correlated
   step-1 (buildCorrelatedFlatMapPlan; the audited decline-only fix was insufficient â
   REWRITING promotion drops the unmerged member; full record in the RFC) + the 1+1 path's
   buried-leg rebase; unmasked matrix AâH pinned. DISCOVERED + FIXED on the branch:
   ...
   (matrix class K â exit gate = the item-2 positional binders, commits 2â4);
   CTE-shadowed derived alias + EXISTS (buildDerivedTableSource resolves derived bodies
   against the catalog only â teach it CTE bodies); fetch-shell walk-terminators under
   planResultValue (same binder exit gate). The alias-unchecked frontier fallback
   follow-up (values.go evaluateCorrelated) dies with commits 2â4.**
   **CHARTER EXTENDED (post-W4-left sequencing ruling): absorbs the under-existential unnest
   class (`FROM t, t.arr AS v WHERE EXISTS(â¦)` â the W5 F4 rider's booking gap, closed here)
   and the EXISTS-rider clusterArity poison (a cluster whose leg filter/project carries
   exists subqueries) â same root cause, one slice, one review. The SCALAR-rider poison
   absorbs CONDITIONALLY: same binders â same slice; W4b-seed rework needed â immediate
   follow-on. Each absorbed class gets its own gate-reason string + dualwindow pins.**
   **ITEM-2 CHARTER COMPLETE (2026-07-05): commits 1â4 (PRs #469/#471/#472/#475) + 5a (#476,
   the structural exit gate) + the B/C/D/E wrong-rows batch (#478) + 5c (#479, class-K â
...
3. **Mixed-nesting LEFT widening** â the joined-preserved class (clustered legs under a
   ...
   **IN FLIGHT (PR #483, feat/rfc173-item3): design ruling banked (three gate-arm commits,
   amendments AâJ; the zero-callers claim STRICKEN â deletion rides S4; the FOURTH site
   ...
   FULL-over-LEFT pin; pins re-cut per H). c3 in flight: LEFT-box-dup flip PINNED (P5 â
   the item-1 c5 loud class narrows as designed), records, exit gates.**

**Sequencing (Graefe ruling, banked in the RFC):** (riders â¥ item 2 â¥ item 1) â item 3 â
unnest-residual slice â S4. The riders are standalone and start immediately:
- [x] **Rider: bound `positionalTypeCache`** â MERGED (PR #468): wipe-at-cap 4096 with a
      miss-path mutex (exact bound, 8-worker -race churn pin; the lock-free variant
      transiently overshot â review finding).
- [x] **Rider: recursive-CTE leg remap hardening** â MERGED (PR #468): the read arm now
      ...
      renderings, float literals, dotted quoted aliases â all red-first pinned + live-Java
...
- [ ] **Unnest-residual completion slice** (books A3's W5 fail-open declines: box-leg
      ...
      rides until S4 â folded into the atomic commit per the circularity ruling, with the
      differential covering it name-model-side until then).
      **Progress (on `feat/rfc173-item3`, pending atomic-slice merge + gate re-ACK):**
      c1 (classes 1+2 â box-leg owners + multi-segment struct paths) DONE, both in-session
      gates ACK + codex re-confirm tracked for quota. c2 (class 3 â CTE/derived owners via
      body-projectionâdescriptor, positive whitelist, P2a-closed) DONE, both in-session
      gates ACK. c3 (class 4 â chained unnests `t.arr AS x, x.sub AS y`) DONE: nested
...
- [ ] **Rider: the minted-binding loud-decline class flips to rows** (item-1 c5 â
      the review-round guard): the declared-loud shapes over duplicate FROM aliases,
      each pinned in rfc173_item1_keybinding_exists_fdb_test.go (P4aâP4f) with the
      never-wrong-rows drain assert. Exit gates per shape: (a) leg-independent EXISTS
      over a minted-binding gated flatten (P4e) â flips when the executor's
      ...
      (existential-alias collision P4a, arity â  2 P4b, enclosure) â flip per-path as
      each learns the ordinal seed (item 3 / the N-way flatten); (c) correlated SCALAR
      subqueries over a dup outer (P4c/P4d) â flips when the scalar lowering speaks
      ...
      (P4f) â a dup-alias branch's per-attribute reference stays display-keyed and
      ...
      stays a LOUD ordinal decline (the c4 buried-reference rebase is arity-2 â
      ...
      per-attribute (`q AS a, u AS a, a.arr AS e` â loud 42703 naming the wrong
      source) â classify vs live Java when the unnest-residual slice lands.
- [x] **Rider: aggregate output METADATA drift vs Java** (item-1 c4 probe finding â
      ...
      Name stays the datum-lookup key the aggregate cursor writes â three-mirror
      ...
      descriptive `COUNT(*)` against Java's anonymous `_N` when the type matches â a
