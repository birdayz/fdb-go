...
- [ ] **Explicit `NULLS FIRST`/`NULLS LAST`** — default ordering now matches Java (swingshift-35: ASC=NULLS FIRST, DESC=NULLS LAST via `compareValues` NULL-direction fix). Grammar does parse `NULLS FIRST`/`NULLS LAST` clauses (ANTLR `orderClause`) but Go's `extractOrderBy` doesn't carry the flag through — users cannot override the default. Java does. Wire it into `orderByCol` struct.
...
- [ ] **`SELECT tbl.*` qualified-star unsupported.** Grammar accepts it but evaluation errors with `0A000: unsupported SELECT element type *antlrgen.SelectQualifierStarElementContext`. Common idiom in the Java yamsql corpus — blocks porting scenarios that use it. Likely a 20-line fix in `buildSelectExpressions` to treat the qualified star as "all columns from aliased table".
- [ ] **Parenthesised boolean expressions in WHERE** parse as `RecordConstructorExpressionAtomContext`. `WHERE NOT (x = NULL)` fails, workaround is to rely on precedence (`WHERE NOT x = NULL`). Grammar quirk from swingshift-35, needs parser or parse-tree normalisation step.
- [ ] **`AVG()` return type.** Java's `AggregateValue` returns `DOUBLE` for `AVG(bigint_col)`. Go may return `int64`; not yet verified by the harness. Add an `avg_return_type.yaml` scenario that pins the expected type, then audit `aggregateMapRows` / the proto aggregate path.
- [ ] **LEFT / RIGHT OUTER JOIN support.** Grammar accepts the keywords; execution path unclear. Conformance scenarios that rely on outer joins can't be ported from yamsql until this is wired up. Survey `execJoin*` / planner JOIN mode dispatch.
- [ ] **Arithmetic overflow SQLSTATE.** Currently uses `22023 INVALID_PARAMETER` (closest existing code). Java raises `ArithmeticException` which most SQL engines map to `22003 NUMERIC_VALUE_OUT_OF_RANGE`. Need to (a) verify what code Java's `RelationalException` translator uses for `ArithmeticException`, (b) add `ErrCodeNumericValueOutOfRange = "22003"` to `api/errcode.go` and switch the overflow sites to it, (c) update `overflow.yaml` to match.
- [ ] **INSERT with duplicate PRIMARY KEY silently REPLACES.** `INSERT INTO t VALUES (pk_that_exists, ...)` currently succeeds and overwrites the existing row, because SaveRecord semantics at the record-layer pass through. Java's relational layer raises SQLSTATE 23505 (unique_constraint_violation) per the SQL standard. The UNIQUE secondary-index path (CREATE UNIQUE INDEX) does raise 23505 correctly — only the PK path is silent. Fix: check for existing record at the target PK before save on INSERT (not INSERT OR REPLACE / MERGE), raise 23505 if present. Surfaced by the nightshift-36 harness (`unique_violation.yaml` currently avoids the case).
- [x] **Integer overflow silent wrap (`+` / `-` / `*`).** Java uses `Math.addExact / subtractExact / multiplyExact` which throw `ArithmeticException`; our `applyMathOp` integer fast-path used Go's native operators which silently wrap on overflow (e.g. `MAX_INT + 1 → MIN_INT`). ✅ nightshift-36: added `addInt64Checked`/`subInt64Checked`/`mulInt64Checked` helpers with Hacker's-Delight overflow tests (sign-bit XOR for +/-, divide-back for *). MinInt64/-1 for `/` also caught (abs value doesn't fit). SQLSTATE 22023 (INVALID_PARAMETER) — closest existing code to Java ArithmeticException. Pinned by `overflow.yaml` in the conformance harness.
...
- [~] **Java↔Go SQL conformance harness.** `conformance/sql/` directory with `.sql` + `.json` expected-output files; drive both Go (`sql.Open("fdbsql", ...)`) and Java (`EmbeddedRelationalConnection` via the Bazel-built conformance server) against the same inputs; diff result sets. Seed with the existing yamsql corpus (1587 statements already parse — just execute them and diff). Also run write-in-Go / read-in-Java round-trips (and reverse) to catch wire-format drift — would have caught the catalog subspace bug above. Opt-in target (`just conformance-sql`), gated behind `@manual` to stay out of default `bazelisk test //...`. **nightshift-36 status**: Go-side harness landed in `pkg/relational/conformance/yamsql/`. 12+ seed scenarios pinning swingshift-35 correctness fixes (three-valued NULL, aggregate NULLs, ORDER BY NULLs, mixed-type equality, CAST, GROUP BY NULL, IN (list with NULL), NOT NULL violation, integer division, DISTINCT aggregates, BETWEEN Kleene, GREATEST/LEAST NULL). Scenarios are authored in YAML with explicit rows (not yamsql's ambiguous `{1, true}` flow-map). Java side (write-in-Go / read-in-Java round-trip) not yet — needs fdb-relational maven deps wired into the Bazel build.
