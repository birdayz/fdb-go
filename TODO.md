...
- [ ] **UPDATE on PK column — silent re-shuffle in Go vs Java rejection.** Surfaced dayshift-62 by round-8 probe. `UPDATE T SET id = 2` on a row with id=1: Java errors `record does not exist` (fdb-relational's UPDATE path is in-place; modifying the PK column triggers an inconsistent-state lookup that surfaces RecordDoesNotExistException). Go silently re-shuffles the row to id=2. Java's reject is more conservative — silent PK re-assignment can collide with other rows or disrupt index entries unpredictably. Fix: detect when an UPDATE SET clause touches a PK column (in `update_delete.go`) and reject with the verbatim Java message, matching SemanticAnalyzer rejection. Sized 1 small shift; ~3 yamsql tests likely affected.
