...
- [~] **A3 — SQL semantic equivalence on yamsql corpus**: feed the 1587 yamsql statements through both engines via A1; require identical result sets for read queries. **Progress (dayshift-55 + swingshift-56):** genuine cross-engine harness wired in `conformance/yamsql_cross_engine_conformance_test.go`. Each test runs through BOTH the Java fdb-relational executor and the Go embedded engine (via plandiff's `NewJavaRunnerHTTP` + `NewGoSQLSetupRunner`) and asserts (a) Java vs scenario expected, (b) Go vs scenario expected, (c) Java rows == Go rows directly. 43 scenarios shipped: 16 from dayshift-55 + 27 added swingshift-56 (full list in `shifts/2026-04-27-swingshift-56.md`). 372 active cross-engine specs pass strictly. **Plus a real fdbsql-driver leak fix surfaced and fixed swingshift-56**: process-global cache for FDB databases by cluster_file path — without it, plandiff's per-corpus-entry sql.Open accumulated 340 leaked FDB connections per run, causing i/o timeouts at corpus entry ~170 under parallel `just test //...` load. Pre-fix suite took ~12 min; post-fix ~45 s. 13 new fdb-relational + Go-engine gotchas added to CLAUDE.md this shift. Scenario expansion is mechanical follow-on; the harness (per-scenario Describe loop, multiset comparison for unordered tests, loose-numeric per-cell equality, three-way Java+Go+expected assertion) carries.
...
- [~] **B3 — Phase 4.3 Memo & references**: `Reference` (= Cascades "group"), implicit DAG via `Reference` pointers, `PlanContext`, `CascadesRuleCall`. Gates B4, B5, the partial-match matchers in B2. **Seeds shipped dayshift-58:** Reference (single-member equivalence class with EqualsWithoutChildren-based dedup; in expressions/), PlanContext + PlannerConfiguration + MatchCandidate + EmptyPlanContext (cascades root), ExpressionRuleCall (parallel to Predicate/Value RuleCall — Yield inserts into a Reference). **Remaining**: full Memo machinery (multi-member equivalence, partial-match propagation, cost-driven extraction), planner phases / yieldExploratoryExpression / yieldFinal distinctions. Sized 2–3 shifts remaining (down from 3–4 with the seeds in place).
...
- [~] **C1 — `PlanGenerator`**: `LogicalOperator → RelationalExpression` adapter. Bridges today's text-based logical builder to the new RelationalExpression hierarchy. **dayshift-58 ships 8 operator types**: `pkg/relational/core/query/plangen/Convert(op)` covers LogicalScan / LogicalFilter (with QueryPredicate) / LogicalUnion (incl. UNION DISTINCT wrapper) / LogicalDelete / LogicalInsert (Source non-nil) / LogicalProject (bare-column projections only) / LogicalSort (bare-column keys only) / LogicalUpdate (bare-column SET RHS only). `FuzzConvert` pins the no-panic invariant (88M execs/60s clean) over random trees of bounded depth. **Remaining**: text→Value parsing for expression projections / sort keys / SET RHSes / Insert VALUES (gated on cascades catalog-aware walker), LogicalLimit needs a RelationalExpression equivalent, LogicalAggregate needs GroupByExpression port, LogicalJoin needs SelectExpression-with-multi-Quantifiers + predicate placement, LogicalValues / CTE / DDL have no equivalent. Sized 1 shift remaining.
...
- [~] **4.5 — Rules**
  ...
  - [~] RelationalExpression-shaped logical-rewrite rule set — **dayshift-58 shipped 15 rules** (8 initial + 7 follow-ons in the same shift): `FilterMergeRule` (collapse nested filters), `FilterDropTruePredicatesRule` (drop TRUE conjuncts), `DistinctMergeRule` (collapse nested DISTINCT), `DistinctOverSortElimRule` (Distinct(Sort(X)) → Distinct(X) — sort wasted by dedup), `TypeFilterMergeRule` (intersect record-type sets), `TypeFilterRedundantOverScanRule` (eliminate type-filter when scan ⊆ filter), `UnionMergeRule` (flatten nested UNION ALL), `IntersectionMergeRule` (flatten nested INTERSECTION with matching comparison keys), `NoOpFilterRule` (eliminate empty/all-TRUE filters), `ProjectionMergeRule` (collapse stacked projections — outer wins), `ProjectionElimRule` (eliminate identity SELECT *), `SortMergeRule` (Sort over Sort → outer Sort), `UnsortedSortElimRule` (eliminate Sort([])), `UnionSingletonElimRule` (Union([Q]) → Q), `IntersectionSingletonElimRule` (Intersection([Q]) → Q). All 15 in `DefaultExpressionRules()` and tested through FixpointApply end-to-end. **Remaining**: Java's PushXxxThroughYyy rules need TranslationMap port (gated on B0 follow-on) AND a memoization fix in the rule-engine driver — rules that yield expressions wrapping fresh References don't dedup against earlier yields, causing FixpointApply non-termination (diagnosed dayshift-58 with PushFilterThroughDistinct + DistinctOverUnionDedup; both reverted before push). The fix is sameChildReferences-equivalent dedup that walks SemanticEquals across References, not just pointer-equal Reference comparison. The terminating-rule pattern is "yield an expression that REUSES the input's existing Quantifiers" (used by all 15 default rules); rules that introduce new outer wrappers wrapping new sub-trees need this fix first. Index-pushdown rules are Batch A.
