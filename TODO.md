...
- [ ] **P2.3 Intersection cursors don't resume mid-stream (codebase-wide).** `executeIntersection` and `executeMultiIntersection` start every child cursor from a `nil` continuation rather than deserializing the parent-level `IntersectionContinuation` proto and splitting per-child bytes (documented at `executor.go` `executeIntersection`). So a `LIMIT`/transaction-split resume of an index-intersection or multi-aggregate `GROUP BY` restarts the children from the beginning. Pre-existing for the regular intersection; RFC-069 made multi-aggregate `GROUP BY` (count+sum) select the `MultiIntersectionOnValues` plan, so it now inherits the same limitation (the cursor emits a correct per-child continuation via the shared `buildIntersectionContinuation`, but the executor still ignores it). Benign for typical small-group aggregates (one transaction, no resume); fix = wire the executor-side per-child continuation decode for BOTH intersection executors. Surfaced by @claude + codex on PR #249.
