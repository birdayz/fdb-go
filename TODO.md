...
- [ ] **Qualified projection in CTE body** — `WITH x AS (SELECT d.id FROM t AS d) SELECT id FROM x` errors 42703 because the CTE materialises the projected column as `d.id` (dotted) rather than `id`. Surfaced by the `leaves` test in `recursive_cte.yaml` — worked around there with unqualified `id`. Not recursive-specific; applies to any CTE whose SELECT list uses `alias.col`. Minor but user-visible.
