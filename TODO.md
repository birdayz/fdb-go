...
- [~] **4.5 — Rules**
  ...
  - [~] RelationalExpression-shaped logical-rewrite rule set — **dayshift-58 shipped 14 rules** (8 initial + 6 follow-ons in the same shift): `FilterMergeRule` (collapse nested filters), `FilterDropTruePredicatesRule` (drop TRUE conjuncts), `DistinctMergeRule` (collapse nested DISTINCT), `TypeFilterMergeRule` (intersect record-type sets), `TypeFilterRedundantOverScanRule` (eliminate type-filter when scan ⊆ filter), `UnionMergeRule` (flatten nested UNION ALL), `IntersectionMergeRule` (flatten nested INTERSECTION with matching comparison keys), `NoOpFilterRule` (eliminate empty/all-TRUE filters), `ProjectionMergeRule` (collapse stacked projections — outer wins), `ProjectionElimRule` (eliminate identity SELECT *), `SortMergeRule` (Sort over Sort → outer Sort), `UnsortedSortElimRule` (eliminate Sort([])), `UnionSingletonElimRule` (Union([Q]) → Q), `IntersectionSingletonElimRule` (Intersection([Q]) → Q). All 14 in `DefaultExpressionRules()` and tested through FixpointApply end-to-end. **Remaining**: Java's PushXxxThroughYyy rules need TranslationMap port (gated on B0 follow-on) AND a memoization fix in the rule-engine driver — rules that yield expressions wrapping fresh References don't dedup against earlier yields, causing FixpointApply non-termination (diagnosed dayshift-58 with PushFilterThroughDistinct + DistinctOverUnionDedup; both reverted before push). The fix is sameChildReferences-equivalent dedup that walks SemanticEquals across References, not just pointer-equal Reference comparison; index-pushdown rules are Batch A.
