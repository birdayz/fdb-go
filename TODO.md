...
- [x] **RIGHT OUTER JOIN: unqualified column from unmatched right row.** ✅ nightshift-36 fix: when materialising the NULL-left row, sample a left row's keys and explicitly set any qualified left-side keys (e.g. `a.id`) to NULL in the combined map, skipping keys that also exist on the right (unqualified `'id'` legitimately takes the right's value). Fixes `SELECT a.id FROM a RIGHT OUTER JOIN b ON a.k = b.k` returning b.id instead of NULL for unmatched-right rows. Pinned by the third row in `outer_join.yaml`'s RIGHT OUTER JOIN test (a.id=NULL, b.label='gamma'). Remaining edge case: when `joined` is empty (no INNER matches / left table empty), left keys are not sampled so the bug returns — tracked for follow-up.
