...
- [ ] **CRITICAL** — Pure Go client crashes FDB server. Certain binding tester seeds (e.g. seed 6, `--num-ops 100 --api-version 730`) crash FDB 7.3.75 with SIGSEGV. 100% reproducible. CGo stacktester passes same seeds fine — confirmed our wire protocol is the cause. FDB trace: `FlowTransport.actor.cpp:749 "write buffer is full"` → crash. Round-trip marshal validation passes (structurally valid FlatBuffers) but we send something semantically wrong. **Next**: (1) in-process wire frame capture in `transport/conn.go` via `FDB_WIRE_LOG`, (2) tcpdump wrapper in binding test harness, (3) diff tool to compare our frames vs CGo frames byte-by-byte. See `pkg/fdbgo/client/CRASH_BUG.md`.
