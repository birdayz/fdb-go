...
- [ ] **vtprotobuf migration — remaining call sites** — Hot path done (store.go marshal/unmarshal, store_builder.go header). Remaining: `index_state.go` (MetaDataVersionStamp), `indexing_heartbeat.go` (heartbeat), `cursor_combinators.go` / `merge_cursor.go` / `dedup_cursor.go` (continuation tokens), `vector_index_maintainer.go` / `multidimensional_index_maintainer.go` (continuations), `time_window_leaderboard.go` (directory/window metadata), `key_value_cursor.go:39` (continuation unwrap). All low-frequency paths. Pool (`FromVTPool`/`ReturnToVTPool`) usable where the message doesn't escape (e.g. marshal-then-discard). **Cannot pool** `deserializeAndDiscover`/`deserializeRecord` — inner message escapes to caller.
