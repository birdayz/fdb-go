...
- [ ] **Integer overflow silent wrap (`+` / `-` / `*`).** Java uses `Math.addExact / subtractExact / multiplyExact` which throw `ArithmeticException`; our `applyMathOp` integer fast-path uses Go's native operators which silently wrap on overflow (e.g. `MAX_INT + 1 → MIN_INT`). SQLSTATE class 22 (data exception) — closest existing code is `22023 INVALID_PARAMETER` (used for ABS overflow). Needs checked arithmetic in the int64 fast-path. Flagged in `feedback_next_shift_arithmetic_overflow.md`.
