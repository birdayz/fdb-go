...
- [x] **Parenthesised boolean expressions in WHERE** ✅ nightshift-36: grammar parses `WHERE NOT (x = y)` as a `RecordConstructorExpressionAtom` with a single unnamed field. Added unwrap case in both proto (`evalExprAtom`) and map (`evalExprAtomOnMap`) paths: when the RecordConstructor has exactly one unnamed expression, treat it as the inner expression. For boolean inners, routes through `evalExprPredicateTri` so NULL-comparisons return tri-state NULL (encoded as `nil`) rather than collapsing to FALSE — which kept `NOT (b = NULL)` returning UNKNOWN (filtered) instead of wrongly flipping to TRUE. Multi-field / named constructors still error. Pinned by `boolean.yaml` (which now covers both the parenthesised and the precedence form).
