...
- [ ] **KeyExpressionFromProto depth limit** (pre-existing, flagged by reviewer in swingshift-35 round 7). Deeply nested `Then{Then{Then{...}}}` or `Nesting{...}` crafted proto still recurses through `KeyExpressionFromProto` without a depth bound. Go's large goroutine stack + Go fuzz engine's blob size cap make it unlikely to hit in practice, but the depth-paranoia fix (pass a depth counter, return error on >N) would match what other proto-parsing codebases do.
