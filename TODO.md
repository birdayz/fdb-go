...
- [x] **Derived-table + GROUP BY Java alignment** — addressed in swingshift-41. The extractFromSimpleTable derived-table branch previously returned early, dropping ORDER BY / LIMIT / OFFSET / GROUP BY / HAVING parse + GR1 validation. Refactored to fall through. Plus aggregateMapRows now seeds group-by values into the outExpr rowMap, validates aggregate args against source columns (42703), and detects ungrouped outExpr column refs (42803). `derived_table_group_by.yaml` pins 7 cases from Java's groupby-tests.yamsql (lines 136–185).
  - [x] Remaining sub-item — addressed in swingshift-41 (commit d7df086c). `GROUP BY col AS x, col2 AS x` now errors 42702 (AMBIGUOUS_COLUMN). A single AS alias is accepted (Java's syntactic extension) but duplicates are rejected. Outstanding: the alias itself is still not usable as a SELECT-list reference (Java's `SELECT x FROM t GROUP BY col1 AS x` works; ours errors 42703) — pinned as a narrower divergence.
...
- [~] **PK equality pushdown — SELECT / UPDATE / DELETE, single + composite, with extra predicates** (nightshift-42): `WHERE pk_col = literal` (single-col PK) and `WHERE pk1 = lit1 AND pk2 = lit2 AND …` (composite PK covering all PK columns) narrow from a full type-scan to a single-key `ScanRecordsInRange` lookup. AND chains with extra non-PK predicates (`WHERE pk = X AND status = 'active'`) also push down; the extras run as a post-filter via the scan loop's existing evalPredicate on the at-most-one-row cursor. Applied at all three cursor sites: SELECT, UPDATE, DELETE. Gated on: no aggregates/GROUP BY/HAVING/COUNT(*) (SELECT only), predicate is an AND-chain where equalities cover every PK column with type-matched literals. Partial PK matches, range predicates (`<`/`<=`/`>`/`>=`), OR/NOT/IS/IN/BETWEEN at top-level — all fall through to scan. Symmetric (`lit = col` / `col = lit`), qualified refs (`t.id = N`), AND order-insensitive, intermingled-table shape (bare FieldKeyExpression PK) bails to scan (no type filter). Type mismatch surfaces 22000 via the scan path. Pinned by `pk_pushdown.yaml` (41 cases across SELECT, UPDATE, DELETE + extras). **Follow-up**: range predicates (`pk > N`, `BETWEEN`) → range-bounded scan, secondary-index pushdown, intermingled-schema type-filter wrapper.
- [x] **Verified** (nightshift-39): SQL queries without PK pushdown do NOT use secondary indexes — every SELECT path without the pushdown above calls `store.ScanRecordsByType(...)`. UPDATE / DELETE still full-scan. Java cascades picks indexes; we only do PK equality. Secondary-index pushdown follow-up.
...
- [~] **Qualified projection in CTE body** (nightshift-42, partial): CTE column list now strips projection qualifiers at materialisation (`stripCTEColumnQualifiers` in both recursive-seed and non-recursive paths), so a CTE whose JOIN-path inner query projects `d.id` exposes bare `id` to downstream queries (matches Postgres). **Still broken**: a single-source CTE like `WITH x AS (SELECT d.id FROM t AS d) ...` errors 42703 at the inner SELECT itself — the proto-path column resolution in `execSelectQueryFull` doesn't consult `currentSourceAliases` the way the map-path (`scanTableToMaps`) does for qualified refs. Fix needed in the single-source qualifier resolution, separate from the CTE output naming.
