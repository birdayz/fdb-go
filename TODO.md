...
- [~] **PK equality pushdown — single-column MVP landed** (nightshift-42): `SELECT … WHERE pk_col = literal` on a single-column primary key now narrows from a full type-scan to a single-key `ScanRecordsInRange` lookup. Gated on: no aggregates, no GROUP BY, no COUNT(*), no HAVING, WHERE is exactly a `=` equality (no AND / OR / IS / IN / BETWEEN), literal type matches the PK column's proto kind (numeric / string / bytes). Composite PKs, AND chains, and range predicates (`<`, `>`, `<=`, `>=`) still take the scan path. Symmetric (`N = id` and `id = N` both push down). Qualified refs (`t.id = N`) push down via bare-last-segment match. Type mismatch surfaces 22000 via the scan path (pushdown falls through). Regression-pinned by `pk_pushdown.yaml` (16 cases). **Follow-up**: composite PK (`Concat(RecordTypeKey, Field(a), Field(b))` with `WHERE a = 1 AND b = 2`), AND chains with extra non-PK predicates as post-filter, range predicates, UPDATE / DELETE pushdown (same patterns apply to those scan sites). Indexes beyond the PK (secondary index pushdown) still pending.
