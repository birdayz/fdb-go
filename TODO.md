...
- [~] **A2 — Catalog wire format Go↔Java round-trip**: PARTIAL nightshift-53. The FDBMetaDataStore wire-format **functional** round-trip is DONE: cross-language metadata save/load that PROVES the loaded metadata is functionally usable to drive a record store. Seven new specs in `metadata_store_conformance_test.go` under "Cross-language functional round-trip (A2)":
  1. Java loads Go-written metadata + scans Go-written records (3 Orders)
  2. Java scans Go-built VALUE index (`Order$price`) using cross-language metadata
  3. Go scans Java-built VALUE index using cross-language metadata (reverse)
  4. Java reads Go-written split records (>100KB, `splitLongRecords=true`) using cross-language metadata
  5. Java scans multi-record-type store (Orders + Customers) using cross-language metadata — pins union-descriptor type-tag dispatch
  6. Java scans Go-built COUNT index (`Order$count_by_price`, BY_GROUP) using cross-language metadata — pins atomic-mutation maintainer + atomic-counter wire format
  7. Go loads Java-written metadata + scans Java-written records (4 Orders, reverse direction)

  Surfaced two new gotchas (`ExtensionRegistry` for `parseFrom` + DynamicMessage-vs-static `mergeFrom`) now documented in CLAUDE.md. **Remaining (next shifts)**: SchemaTemplateCatalog wire format (the relational/SQL layer's catalog) — blocked by Go sqldriver keyspace divergence (Go writes to `__SYS/__SYS/CATALOG` three-string subspace, Java to `(NULL, NULL, int64(0))` — see `pkg/relational/core/catalog/fdb_store_catalog.go:62-67`). That's the fuller A2 scope and needs the keyspace-alignment refactor first. SUM, MAX_EVER, etc. index types follow the same atomic-mutation pattern as COUNT — adding cross-language tests for them is mechanical once any non-VALUE maintainer has been wired (so unblocking the long tail of index-type cross-language pins is essentially "done in spirit" via the COUNT pattern).
