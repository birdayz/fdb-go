...
- [ ] **P2.3 Intersection cursors don't resume mid-stream (codebase-wide).** Both intersection cursors emit a correct per-child `IntersectionContinuation` proto (shared `buildIntersectionContinuation`), but neither executor decodes it on resume: `executeIntersection` (regular) silently starts every child from `nil` (pre-existing — documented at `executor.go`), and `executeMultiIntersection` (the RFC-069 multi-aggregate `GROUP BY` count+sum path) now **errors loudly** on a non-nil incoming continuation rather than silently restarting and duplicating rows. Benign for typical small-group aggregates (few groups, one scan pass, no resume). Fix = wire the executor-side per-child continuation decode for BOTH intersection executors (then drop the loud guard). Surfaced by @claude + codex on PR #249.
