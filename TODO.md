...
- [ ] **RIGHT OUTER JOIN double ON-evaluation (pre-existing, flagged dayshift-37 review).** `execSelectJoin`'s RIGHT JOIN block re-iterates every (left, right) pair to build `matchedRight[]`, re-evaluating the ON predicate that the main INNER/LEFT loop above already ran. O(n²) with a constant factor of 2 on the ON cost. Not measurable on small tables; worth fixing if anyone benchmarks RIGHT OUTER JOIN on a populated left side. Fix would memoise match results from the first pass.
