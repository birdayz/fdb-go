...
- [~] **Secondary-index pushdown MVP** (nightshift-42): single-column VALUE index equality pushdown on SELECT. `WHERE col = literal` where `col` is the sole key of a VALUE index now narrows the scan from `ScanRecordsByType` to `ScanIndexRecords` (index range + record fetch by PK). Extras in the AND chain are post-filtered via the scan loop's evalPredicate. PK pushdown still fires first for PK columns. **Follow-up**: composite index keys (`ON t(a, b)`), range predicates on indexed columns, UPDATE / DELETE pushdown sites, covering-index optimisation (skip the PK fetch when all projected columns are in the index entry).
