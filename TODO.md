...
**Goal: eliminate ALL `_custom.go` and `_helpers.go` files.** Everything should be in `_generated.go`. The 6 remaining hand-written pieces are all mechanically derivable from the C++ type system — no custom logic.

**`_helpers.go` is a smell** — it means the generator doesn't cover the type. Target: zero `_helpers.go` files.

- [ ] **HIGH — #1: MarshalInto for nested types** — Generate `MarshalInto(*ObjectWriter)` for: KeyRangeRef, MutationRef, ReplyPromise, TenantInfo, SpanContext, ReadOptions. Currently hand-written in `_custom.go` as standalone helper functions (`MarshalKeyRangeRef`, `WriteTenantInfo`, etc.). Purely mechanical: `WriteBytes`/`WriteUint8`/`WriteInt64` at vtable offsets. The extractor already knows the field types and offsets. Eliminates: `keyrangeref_custom.go`, `mutationref_custom.go`, `replypromise_custom.go`, `tenantinfo_custom.go`, `spancontext_custom.go`, `readoptions_custom.go`.
- [ ] **HIGH — #2: VecSerStrategy::String typed parser** — Generate `ParseKeyValueVector` + `KeyValue` struct for `VectorRef<KeyValueRef>`. Wire format: `[count(4)][keylen(4)][key][vallen(4)][val]...`. The extractor knows the field type is `VectorRef<KeyValueRef>` with `VecSerStrategy::String` from the C++ `dynamic_size_traits` specialization. Eliminates: `getkeyvaluesreply_helpers.go`.
- [ ] **HIGH — #3: Endpoint/NetworkAddress/IPAddress nested unmarshal chain** — Generate `ReadEndpointFromSlot`, `ReadEndpoint`, `ReadNetworkAddress`, `ReadIPAddress`, `EndpointInfo`. Currently hand-written with byte-offset heuristic to distinguish UID from address slot. Extractor should capture field names for Endpoint inner type (`token` vs `address`) and generate proper slot-based reads. Also: implement IPv6 (currently silently returns "0.0.0.0"). Eliminates: `networkaddress_custom.go`, `ipaddress_custom.go`.
- [ ] **HIGH — #4: CommitTransactionRequest full generation** — Generate `MarshalFDB` with nested WriteStruct for CommitTransactionRef (readVersion + mutation/conflict range vectors). Currently standalone `MarshalCommitTransactionRequest` function. The generated struct should have proper fields; MarshalFDB uses WriteMessagePacked + nested WriteStruct composition. Eliminates: `committransactionrequest_custom.go`.
- [ ] **HIGH — #5: Pair type decomposition** — Generate concrete Go struct per `pair<A,B>` instantiation with flattened fields. `ParseGetKeyServerLocationsResults` + `LocationResult` should be generated from the pair<KeyRangeRef, vector<SS>> type. Eliminates: `getkeyserverlocationsreply_helpers.go`.
- [ ] **HIGH — #6: Request type MarshalFDB generation** — All request types currently have hand-written `_custom.go` with MarshalFDB that writes nested structs (TenantInfo, SpanContext, ReplyPromise, KeySelectorRef). Once #1 generates `MarshalInto` for these nested types, the extractor can compose them into a generated MarshalFDB for the parent request. Eliminates: `getvaluerequest_custom.go`, `getkeyrequest_custom.go`, `getkeyvaluesrequest_custom.go`, `getreadversionrequest_custom.go`, `getkeyserverlocationsrequest_custom.go`, `opendatabasecoordrequest_custom.go`.

**Remaining custom stubs after all 6 are done:** `clientdbinfo_custom.go` (isDeserializing post-processing), `commitproxyinterface_custom.go` (endpoint derivation), `grvproxyinterface_custom.go` (endpoint derivation), `storageserverinterface_custom.go` (endpoint derivation) — these 4 have genuine post-deserialization logic (Pattern 2 from RFC 012). Keeping them as custom is correct.

- [ ] **MEDIUM — `Error` wire type** — `error_code` is `uint16_t` on wire. Will be correct when Error is generated via #1.
