...
- [ ] **Multi-CTE cross-join returns 1 row instead of |lo|*|hi|.** `WITH lo AS (...), hi AS (...) SELECT COUNT(*) FROM lo, hi` observed returning `[1]` when `|lo|=1` and `|hi|=2` (expected `[2]`). Harness-surfaced nightshift-36 while writing cte.yaml. Scalar subqueries in SELECT aren't supported by the grammar either, so we can't work around via `(SELECT COUNT(*) FROM lo) + (SELECT COUNT(*) FROM hi)`. Needs investigation of derived-table scope handling during cross-join materialisation.
- [~] **`SELECT tbl.*` qualified-star.** Partial fix nightshift-36: single-source FROM now accepts `SELECT tbl.*` as a SELECT * equivalent (correct when FROM has one source). Known gaps still open: (a) multi-source FROM (JOIN / comma-join) does NOT restrict to the aliased source's columns — it still projects everything, so `SELECT a.*, b.label FROM a, b` would return all of b's columns under `a.*` rather than just a's. Needs per-source column-list plumbing through the projector. (b) the qualifier is NOT validated against the actual table name/alias — `SELECT wrong_alias.*` silently succeeds. Pinned for the single-source case by `boolean.yaml`.
...
- [ ] **Outer-join conformance scenarios.** LEFT/RIGHT OUTER JOIN execution was landed in swingshift-33 (checked at lines 448-449 below); the gap is test coverage — no yamsql harness scenarios currently exercise outer joins. Add `outer_join.yaml` with NULL-padded right-side rows, NULL-vs-NULL in the join key, and OUTER + WHERE + GROUP BY combinations that swingshift-33 fixed.
