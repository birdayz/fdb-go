...
- [ ] **Per-column selectivity heuristics.** Infrastructure landed (nightshift-103): `IndexColumnSelectivity(N, isEquality, isUnique, hasRealStats)` with 1/N (unique), 1/√N (non-unique), 0.33 (range). `RangeSelectivity`, `HasRealStats` helpers. NOT activated in HintCost — applying during EXPLORE disrupts the winner cascade (tested, confirmed regression). Activation BLOCKED on BatchA→PLANNING migration (same dependency as advancePlannerStage).
