...
- [~] **A1 — Phase B SqlSteps**: foundation, runSql + runWithSetup, harness, and 19-entry corpus all landed swingshift-52 (PR #118). What's left is the Go-side runner, which is gated on Track C2 (QueryExecutor against an FDBRecordStore). Detail:
  ...
  - **Remaining for full A1 close**: Go-side runner (gated on Track C2). When Track C2 lands enough executor capability, every SeedRunCorpus entry becomes a real cross-engine equivalence assertion via `RunCorpusWithSetup` without harness changes — the Expected RowSet doubles as the Go-side reference.
...
- [~] **B0 — Phase 4.0 Type hierarchy**: seed landed nightshift-50, follow-ons landed swingshift-52. Now in `cascades/values/type.go` (1264 LOC):
  - Structured types: RecordType, ArrayType (with IsErased), EnumType, RelationType (always non-nullable, IsErased) ✅
  - Primitive singletons: NullableX/NotNullX for every primitive (incl. UUID + VERSION added swingshift-52), plus NullType, UnknownType, NoneType (always non-nullable, panics on nullable=true), AnyType (always nullable, panics on nullable=false) ✅
  - Promotion lattice: IsPromotable (from PROMOTION_MAP — INT→LONG/FLOAT/DOUBLE, LONG→FLOAT/DOUBLE, FLOAT→DOUBLE, NULL→any, NONE→ARRAY, STRING→ENUM, STRING→UUID), MaximumType (binary common-supertype for primitives), MaximumTypeOfMany (variadic fold) ✅
  - TypeRepository (named-type registry) ✅
  - Legacy bridge: FromValueType / ToValueType / ValueRichType ✅
  - Structured-type recursion in MaximumType: ARRAY element-by-element ✅, RECORD field-by-field ✅ (with Java's exact name-resolution rules), ENUM value-list equality ✅, RELATION inner-type ✅ — all landed swingshift-52.
  - Shape predicates: IsNull / IsNone / IsAny / IsUnresolved / IsArray / IsRecord / IsEnum / IsUuid / IsRelation as free functions (mirror Java's default methods) ✅
  - **Remaining**: type inference on `Value.Type()` so it returns rich Type instead of the legacy ValueType enum (largest remaining piece — every Value impl needs RichType()); plan-serialisation hooks (proto encoding for plan-cache). When `cascades/values/type.go` exceeds ~1500 LOC (currently 1422), split into `cascades/typing/` per RFC-025. Sized 1–2 shifts to "complete enough to retire ValueType".
...
- [~] **4.0 — Foundation types**
  - [~] `Type` / `TypeRepository` / `Typed` — seed extended nightshift-50 in `cascades/values/type.go`. `Type` interface (Code + IsNullable + Equals + String); `TypeCode` enum mirroring Java's well-known codes (NULL/BOOLEAN/INT/LONG/FLOAT/DOUBLE/STRING/BYTES/VERSION/ENUM/RECORD/ARRAY/RELATION/ANY/NONE/UUID); concrete impls now cover **PrimitiveType + RecordType (Field-based) + ArrayType + EnumType (Name+Number values)**; canonical singletons (NotNullInt, NullableString, …); `WithNullability(t, nullable)` helper; `Typed` interface (RichType()); legacy-bridge adapters `FromValueType` / `ToValueType` for incremental migration. **Remaining:** `UuidType`, `RelationType` structured impls; `TypeRepository` for type registration + named-type lookup; plan-serialisation hooks; full conversion / coercion lattice; type inference on ArithmeticValue / ScalarFunctionValue Type() so `Value.Type()` returns rich Type instead of legacy ValueType. Migration is incremental — old `ValueType` enum keeps working until call sites flip over piecewise.
...
- [~] **4.2 — Matching engine**
  - [~] `BindingMatcher` DSL — dayshift-46 seeded interface + `PlannerBindings` + `MergedWith` + generic `Get[T]` retrieval helper + `AnyValue` + `Instance` + `ArithmeticMatcher` + `AllOfMatcher` + `AnyOfMatcher`. swingshift-50 added `ListMatcher` (positional []any pairing, length-strict) + `AllElementsMatcher` (Java's MultiMatcher.AllMatcher — same downstream applied to every element). nightshift-50 added `SomeElementsMatcher` (Java's MultiMatcher.SomeMatcher), `AtLeastElementsMatcher` (Java's MultiMatcher.AtLeastMatcher), `EmptyCollectionMatcher` (Java's CollectionMatcher.empty), generic `TypedMatcher[T, U]` (Java's TypedMatcherWithExtractAndDownstream), and `CollectionMatcher` interface. Remaining (mostly Phase 4.3+ gated): `OptionalIfPresentMatcher` (no Go-native Optional shape), `PartialMatchMatchers` + `graph/` matchers (need Reference / DAG infra).
