...
- [~] **4.5 — Rules**
  ...
  - [~] RelationalExpression-shaped logical-rewrite rule set — **dayshift-58 shipped 12 rules** (8 initial + 4 follow-ons in the same shift): `FilterMergeRule` (collapse nested filters), `FilterDropTruePredicatesRule` (drop TRUE conjuncts), `DistinctMergeRule` (collapse nested DISTINCT), `TypeFilterMergeRule` (intersect record-type sets), `UnionMergeRule` (flatten nested UNION ALL), `IntersectionMergeRule` (flatten nested INTERSECTION with matching comparison keys), `NoOpFilterRule` (eliminate empty/all-TRUE filters), `ProjectionMergeRule` (collapse stacked projections — outer wins), `ProjectionElimRule` (eliminate identity SELECT *), `UnsortedSortElimRule` (eliminate Sort([])), `UnionSingletonElimRule` (Union([Q]) → Q), `IntersectionSingletonElimRule` (Intersection([Q]) → Q). All 12 in `DefaultExpressionRules()` and tested through FixpointApply end-to-end. **Remaining**: Java's PushXxxThroughYyy rules need TranslationMap port (gated on B0 follow-on); index-pushdown rules are Batch A.
