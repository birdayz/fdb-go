...
- [~] **Secondary-index pushdown** (nightshift-42, extended dayshift-43, covering + IN-list + LIKE prefix added swingshift-44): single-column AND composite VALUE index equality pushdown on SELECT, UPDATE, and DELETE. **Range predicates added in dayshift-43** — single-col VALUE index (\`>\`, \`>=\`, \`<\`, \`<=\`, bounded pairs) and composite VALUE index with leading equalities + any-column range (same relaxation as PK composite range). `BETWEEN lo AND hi` pushdown across both single-col and composite paths. `WHERE col = literal` (single-col) / `WHERE a = lit1 AND b = lit2 …` (composite equality) / `WHERE col > lit` (single-col range) / `WHERE a = lit1 AND b > lit2` (composite range) narrow the scan from `ScanRecordsByType` to `ScanIndexRecords` (index range + record fetch by PK). Extras in the AND chain are post-filtered. PK pushdown takes precedence. `IsIndexScannable` guard is applied inside each helper's index-iteration loop (dayshift-43 fix 5e28f487): a non-scannable first match no longer blocks a later scannable one. Partial matches fall through to scan. Pinned by `secondary_index_pushdown.yaml`. **Covering-index optimisation** (swingshift-44 commit `b2260177`): on the SELECT path, when every column referenced by projection / WHERE / ORDER BY is derivable from the index key + PK, the per-row LoadRecord fetch is skipped and the row is synthesised from the IndexEntry alone (dynamicpb message, downstream ProtoReflect().Has/Get still works uniformly). One FDB round-trip per row instead of two. Bails on SELECT * / qualifier.* / joins / aggregates / GROUP BY / HAVING / non-scalar kinds (enum / message / repeated). Pinned by `covering_index_pushdown.yaml` (23 cases). **IN-list pushdown** (swingshift-44 commit `2323bb93`): single-col PK + single-col VALUE index decompose `WHERE col IN (v1..vN)` into N sequential point scans. NOT IN / subquery IN / NULL-only lists bail. Pinned by `in_list_pushdown.yaml` (25 cases). **LIKE prefix pushdown**: `WHERE col LIKE 'foo%'` on STRING PK or single-col VALUE index decomposes to `[prefix, strinc(prefix))` (half-open). No interior wildcards, no underscore, no ESCAPE. Pinned by `like_prefix_pushdown.yaml` (25 cases).
