...
- [~] **4.5 — Rules**
  ...
  - [~] RelationalExpression-shaped logical-rewrite rule set — **dayshift-58 shipped 8 rules**: `FilterMergeRule` (collapse nested filters), `DistinctMergeRule` (collapse nested DISTINCT), `TypeFilterMergeRule` (intersect record-type sets), `UnionMergeRule` (flatten nested UNION ALL), `IntersectionMergeRule` (flatten nested INTERSECTION with matching comparison keys), `NoOpFilterRule` (eliminate empty/all-TRUE filters), `ProjectionElimRule` (eliminate identity SELECT *), `UnsortedSortElimRule` (eliminate Sort([])). All 8 in `DefaultExpressionRules()` and tested through FixpointApply end-to-end. **Remaining**: Java's PushXxxThroughYyy rules need TranslationMap port (gated on B0 follow-on); index-pushdown rules are Batch A.
