...
- [~] **PK equality pushdown — SELECT / UPDATE / DELETE, single + composite** (nightshift-42): `WHERE pk_col = literal` (single-col PK) and `WHERE pk1 = lit1 AND pk2 = lit2 AND …` (composite PK covering all PK columns) narrow from a full type-scan to a single-key `ScanRecordsInRange` lookup. Applied at all three cursor sites: SELECT, UPDATE, DELETE. Gated on: no aggregates/GROUP BY/HAVING/COUNT(*) (SELECT only), predicate is an AND-chain of pure equalities covering every PK column with no extras, literal Go types match PK column proto kinds. Partial PK matches, range predicates (`<`/`<=`/`>`/`>=`), OR/NOT/IS/IN/BETWEEN in WHERE, extra non-PK predicates — all fall through to scan. Symmetric (`lit = col` / `col = lit`), qualified refs (`t.id = N`), AND order-insensitive (tuple built in declared PK order). Type mismatch surfaces 22000 via the scan path. Pinned by `pk_pushdown.yaml` (38 cases across SELECT, UPDATE, DELETE). **Follow-up**: AND chains with extra non-PK predicates as post-filter (simple analysis), range predicates (`pk > N`) → range-bounded scan, secondary-index pushdown.
