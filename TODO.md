...
- [ ] **CRITICAL** — Pure Go FDB client wire protocol crashes FDB server (SIGSEGV). Root cause: our FlatBuffers serializer (`writer_direct.go` + codegen in `main.cpp`) produces a different byte layout than C++ `ObjectWriter` (`flat_buffers.h`). CGo stacktester passes same workloads. See `pkg/fdbgo/client/CRASH_BUG.md` for full analysis. Ground truth test vectors from C++ ObjectWriter in `pkg/fdbgo/wire/types/testdata.json`. Branch: `binding-tester-conformance`.
  ...
  **Remaining (CRITICAL — blocks all pure Go client use):**
  - [ ] Bug 5: **Vtable extraction: extra slot for Optional fields.** Our extractor produces root vtable with 11 entries (22 bytes), C++ has 10 entries (20 bytes). 2-byte vtable difference → 8-byte total size mismatch after alignment. Affects ALL types with Optional fields. Fix in `main.cpp` extractor's vtable extraction for `Optional<T>` (union_like in C++). See `gkslr_manual_test.go` for reproducer.
  - [ ] Bug 6: **`Optional<ReadOptions>` classified as bytes instead of nested struct.** `GetValueRequest`/`GetKeyRequest`/`GetKeyValuesRequest` have `Optional<ReadOptions>` which C++ serializes as a nested object (own vtable + alignment). Our extractor classifies it as `Optional + []byte`. Causes -16/-8/-24 size mismatches. Fix in `main.cpp` extractor field classification.
  - [ ] Bug 7: **`Vector<struct>` blob sizing too large.** `CommitTransactionRequest` with mutations is +40/+96 bytes vs C++. Our `blobSize()`/`writeBlob()` produces different padding. Needs step-by-step comparison of vector-of-struct serialization against C++ `SaveVisitorLambda`.
  - [ ] Bug 8: **C++ `emptyVector` re-use optimization missing.** C++ only allocates 4 bytes for the FIRST empty `dynamic_size` field; subsequent share the same offset. We always allocate 4 per empty field. Low priority — no test vector currently fails from this, but could over-allocate for types with multiple nil fields.
  - [ ] **Mechanical port completion**: `serializer.go` has `PrecomputeSize`/`WriteToBuffer`/`SaveWithVTables` ported from `flat_buffers.h`. Manual two-pass test in `gkslr_manual_test.go` validates the approach. Codegen needs to emit calls to new primitives instead of old `MeasureObject`/`WriteObject`/`PatchRelOff`. Validate all 10 ground truth test vectors byte-identical to C++.
