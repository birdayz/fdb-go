...
- [~] **PK pushdown — equality (single + composite) AND range (single-col)** (nightshift-42): equality pushdown covers `WHERE pk = lit` (single-col PK) and `WHERE pk1 = lit1 AND pk2 = lit2 AND …` (composite PK, every PK column equated). Range pushdown covers `WHERE pk OP lit` for `>`/`>=`/`<`/`<=` on single-col PKs, including bounded pairs like `pk > a AND pk < b` and equality-plus-range mixes; tuple construction uses `EndpointTypeRangeInclusive`/`RangeExclusive`. AND chains with extra non-PK predicates push down; extras run as post-filter via scan's evalPredicate. All three cursor sites (SELECT, UPDATE, DELETE) use the shared `tryPKEqualityFromWhere` → `tryPKRangeFromWhere` → scan fallback chain. Gated on: no aggregates/GROUP BY/HAVING/COUNT(*) (SELECT only), AND-only top level, type-matched literals, CompositeKeyExpression PK shape (intermingled-table bare-Field PK bails to scan for type-safety). Symmetric (`lit OP col` flips operator to treat col-on-left), qualified refs (`t.id = N`), type mismatch surfaces 22000 via scan. Pinned by `pk_pushdown.yaml` (48 cases across SELECT/UPDATE/DELETE × equality/range × single/composite PK). **Follow-up**: range pushdown on composite PK (range only on last component; earlier ones equated), secondary-index pushdown, intermingled-schema type-filter wrapper.
