...
- [ ] **MULTIDIMENSIONAL index scans still don't aggregate scan/byte/time limits across IN-join/IN-union legs — `ScanLimiterState` doesn't cover this index type.** `prefixSkipScanCursor` (`multidimensional_index_maintainer.go`) already had its OWN pre-existing aggregation, but only ACROSS THE PREFIXES of one MULTIDIMENSIONAL scan (`totalScanned`/`totalBytesScanned`/`startTime`, set up in 9.6 above, predating this item) — it never charges an outer shared `ScanLimiterState`. The cursor itself is constructed fresh, with fresh zero totals, once per IN-join/IN-union leg (exactly like every OTHER leaf cursor was before the item above), so a many-legged IN-list over a MULTIDIMENSIONAL index can still silently run past a configured scan/byte/time budget in one FDB transaction — the identical bug class the item above fixed for `keyValueCursor`/`indexCursor`/`recordKeyCursor`/`countKVCursor`/`bitmapKVCursor`/`rtreeScanCursor`(single-prefix)/`textCursor`, just not for this one. Not fixed as part of that item because `prefixSkipScanCursor`'s remaining-budget math (`remainingScan := scanLimit - c.totalScanned`, passed as each prefix's own `WithScannedRecordsLimit`) would need reworking to subtract from the OUTER shared total instead of (or in addition to) its own — a real, scoped change but with its own edge cases (what remains after another leg already spent the whole budget mid-prefix-enumeration) that deserved its own test pass rather than a rushed fold into an already-large change. Fix: either (a) have `prefixSkipScanCursor` ALSO charge an outer `resolveScanLimiterState(c.scanProperties.ExecuteProperties)` (if non-nil) on top of its own internal per-prefix accounting, using the outer state for the actual limit CHECK and its own internal totals only for the per-prefix remaining-budget math, or (b) simplify by dropping the cursor's own internal aggregation entirely and routing everything through the shared `ScanLimiterState` the same way every other leaf cursor now does (loses the "decrement remaining budget into each prefix's own limit" precision but matches the general pattern). See the code comments at `prefixSkipScanCursor`'s type doc and the `perPrefixProps.ExecuteProperties.ScanState = nil` line for the exact mechanics this needs to change.
