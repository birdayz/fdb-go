...
- [~] **A2 — Catalog wire format Go↔Java round-trip**: PARTIAL nightshift-53. The FDBMetaDataStore wire-format functional round-trip is DONE: cross-language metadata save/load that PROVES the loaded metadata is functionally usable to drive a record store + scan records + scan a VALUE index. Four new specs in `metadata_store_conformance_test.go` under "Cross-language functional round-trip (A2)" — Go saves metadata + records, Java loads + scans (records and index entries); reverse direction too. Surfaced two new gotchas (`ExtensionRegistry` + DynamicMessage-vs-static) now documented in CLAUDE.md. **Remaining (next shifts)**: SchemaTemplateCatalog wire format (the relational/SQL layer's catalog) — blocked by Go sqldriver keyspace divergence (Go writes to `__SYS/__SYS/CATALOG` three-string subspace, Java to `(NULL, NULL, int64(0))` — see `pkg/relational/core/catalog/fdb_store_catalog.go:62-67`). That's the fuller A2 scope and needs the keyspace-alignment refactor first.
