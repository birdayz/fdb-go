...
- [ ] **Bounded aggregate index scan (optimization).** Java's `AISCAN [EQUALS col1]` pushes WHERE predicates on group keys into the aggregate index scan range. Go's AggregateDataAccessRule fires on GroupByExpression but the WHERE predicate sits on a parent Filter, invisible to the rule. Fix requires either: (a) a predicate-pushdown rule that moves group-key predicates INTO the GroupBy's inner before AggregateDataAccessRule fires, or (b) a new rule that matches Filter(GroupBy) directly and produces bounded AggregateIndex plans. Attempted and reverted (swingshift-102): naive `findScanThroughFilter` fails because the Filter is a parent, not a child.
