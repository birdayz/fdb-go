...
- [x] **#84** **CRITICAL: Unified plan pipeline — eliminate naive generator.** Done: SELECT and DML (INSERT/UPDATE/DELETE) all route through Cascades. Naive generator retained only for DDL/SHOW/INFORMATION_SCHEMA (procedural, no optimization needed).
  ...
  **HN launch blockers (in priority order):**
  ...
  - [ ] **#98** Yamsql conformance: **~82/98 pass (83.7%)**, up from 77/97 (79.4%) at start of nightshift-80. nightshift-80 fixed: aggregate alias threading (AggregateSpec.Alias + executeAggregation dual-key storage), CTE column aliases (LogicalCTE.ColumnAliases + extractOutputColumns rename projection), EXISTS column leak (deriveColumnsFromJoin excludes inner for semi-joins), recursive CTE stack overflow (depth guard), derived table+join dropping (fall-through condition), post-agg projection aliases (visibleAliases from outName), buildOuterPlanOnDerived aggregation support (GROUP BY + qualifier stripping), buildDerivedTableSource aggregate scope synthesis. Remaining ~16 failures: derived table GROUP BY qualified scope (~7), recursive CTE post-order + error codes (~11), SELECT * expansion (~2), union star column mapping (~3), scalar subquery patterns (~3), datetime edge cases (~4), derived table cross-join alias + correlated subquery (~6). (~1-2 shifts for most; derived table scope needs architectural work)
