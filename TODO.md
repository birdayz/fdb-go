...
- [ ] **WHERE/HAVING on GROUP BY key with aggregate index.** `SELECT region, SUM(amount) FROM sales WHERE region = 'US' GROUP BY region` — planner picks IndexScan(agg_index, [=]) + StreamingAgg, which double-aggregates and returns 0 rows. Same bug for HAVING on group key. Root cause: ImplementIndexScanRule treats the aggregate index as a value index for the equality predicate; IndexScan + StreamingAgg is the wrong plan because it re-aggregates pre-computed values. Fix: either (a) AggregateDataAccessRule generates bounded-scan AggregateIndex plans with scan comparisons from the WHERE/HAVING predicate (Java's approach: `AISCAN [EQUALS col1]`), or (b) exclude aggregate indexes from ImplementIndexScanRule when a streaming aggregation would apply on top.
