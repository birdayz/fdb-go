...
- [ ] **Cascades package structure + deep unit tests (architectural prerequisite for Phase 4)** — before Phase 4.0 (Type hierarchy), 4.1 (RelationalExpressions), 4.5 (~69 rules) land more code, sub-package the cascades port to mirror Java's structure so each package can be unit-tested in isolation.
  - **Principles:**
    - **Mirror Java's structure**, not invent a Go-native layout. Java's `com.apple.foundationdb.record.query.plan.cascades` has well-bounded sub-packages — values, predicates, matching/{structure,graph}, rules, expressions, references, properties, typing, debug.
    - **No overly small packages.** Split only where Java suggests it AND there's a natural interface boundary. A package with one struct + two helpers stays in its parent.
    - **No leaky abstractions.** Cross-package deps must go through the package's exported interface. If a test or downstream caller has to import private types via `_test.go` tricks, the boundary is wrong.
    - **Strong unit-test coverage per package.** Take inspiration from Java's test suite (`fdb-record-layer-core/src/test/java/com/apple/foundationdb/record/query/plan/cascades/...`). Each Go sub-package gets its own deep tests for the surface it exports — parsing, matching, evaluating, simplifying — that don't require spinning up the conformance server or testcontainer FDB. Today's challenge is that poor isolation forces validation through `//conformance:conformance_test` + the yamsql harness; sub-packaging lets unit tests catch regressions cheaply.
  - **Today's package** `pkg/recordlayer/query/plan/cascades/` is one flat package: values, predicates, comparisons, matchers, combinators, rules, simplifiers, correlation. Acceptable for the Phase 4.0 seed; will be a maintenance burden once 4.5 ships ~69 rules.
  - **Audit deliverable**: list every cross-package leak (e.g. `pkg/relational/core/embedded/projection_fold.go` reaches into both `cascades` and `expr` to set up a `Resolver` for a fold pass — sign of an interface gap). Document in a sub-RFC.
  - **Layout deliverable**: target sub-package map mirroring Java, with file moves enumerated. Defer the actual moves to a follow-up shift unless it can be done safely in this one.
  - **Test-coverage deliverable**: for each existing/seed sub-package, list the Java test classes that cover the equivalent surface and what we'd port. Priority: values, predicates, matching, simplification — those are the most depended-on.
...
- [~] **4.-1 — Plan-equivalence harness (build FIRST, per RFC-022)**
  ...
  - [~] **Phase 2 — Java side:** swingshift-50 bumped fdb-record-layer-core / fdb-relational-api / fdb-relational-core to **4.11.1.0** in MODULE.bazel; the gitignored `fdb-record-layer/` submodule is on the matching tag. `SqlPlanSteps.java` lands the `planSql(clusterFile, schemaTemplate, sql)` step on `conformance_server.java` — registers fdb-relational's `EmbeddedRelationalDriver` lazily on first call, creates a uniquely-named schema template + database + schema per call, runs `EXPLAIN <sql>` via JDBC, returns the PLAN column. `pkg/relational/conformance/plandiff/`'s `javaEngine` is wired to POST to `/invoke` with the right shape (4 unit tests with httptest mock the server). Remaining: a real integration test that spins up the conformance server + FDB testcontainer + drives both engines end-to-end (significant infra; lives in `conformance/` next to existing testcontainer harness).
