...
- [~] **A2 — Catalog wire format Go↔Java round-trip**: PARTIAL nightshift-53. The FDBMetaDataStore wire-format **functional** round-trip is DONE: cross-language metadata save/load that PROVES the loaded metadata is functionally usable to drive a record store. Eleven new specs in `metadata_store_conformance_test.go` under "Cross-language functional round-trip (A2)":
  ...
  **dayshift-54 progress on SchemaTemplateCatalog**: confirmed via direct subspace byte inspection that Go's `catalog.OpenRecordLayerStoreCatalog()` and Java's `RecordLayerStoreCatalog` both target the SAME FDB subspace `(NULL, NULL, int64(0))` (3-byte prefix `0x000014`). The keyspace prerequisite is met — the Go sqldriver's three-string subspace is a separate concern (sqldriver writes elsewhere; the catalog package itself is Java-compat). However, attempting the round-trip surfaces a **catalog-metadata-version divergence**: Go's `BuildCatalogMetaData()` produces `RecordMetaData` at version 3, Java's `RecordLayerStoreCatalog` writes at version 4. Both have 3 record types + 3 indexes; somewhere in Java's `RecordLayerSchemaTemplate.toRecordMetadata()` the version is bumped one extra time. Result: `StaleMetaDataVersionError{Local:3, Stored:4}` blocks Go from opening the catalog Java initialized. Pinned by `conformance/schema_template_catalog_conformance_test.go`.

  **Remaining (next shifts)**: align the catalog metadata version. Likely 1-2 shifts of work to compare Go's `BuildCatalogMetaData()` shape against Java's `RecordLayerSchemaTemplate.toRecordMetadata()` shape and figure out the missing version bump. After the version alignment lands, the negative pin becomes a positive (template loads + columns match) — cross-language SchemaTemplateCatalog round-trip closes A2. MIN_EVER_LONG / MAX_EVER_TUPLE / MIN_EVER_TUPLE index types follow the same atomic-mutation pattern as COUNT/SUM/MAX_EVER — adding cross-language tests is mechanical follow-on with the harness pattern documented inline at the head of the "Cross-language functional round-trip (A2)" Describe block.
