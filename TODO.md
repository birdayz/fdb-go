...
- [~] **Secondary-index pushdown** (nightshift-42): single-column AND composite VALUE index equality pushdown on SELECT, UPDATE, and DELETE. `WHERE col = literal` (single-col index) and `WHERE a = lit1 AND b = lit2 AND …` (composite, every index column equated) narrow the scan from `ScanRecordsByType` to `ScanIndexRecords` (index range + record fetch by PK). Extras in the AND chain are post-filtered via the scan loop's evalPredicate. PK pushdown takes precedence at every call site. `IsIndexScannable` guard for WRITE_ONLY / DISABLED indexes. Partial composite matches (only some index columns equated) correctly fall through to scan. **Follow-up**: range predicates on indexed columns, covering-index optimisation (skip the PK fetch when all projected columns are in the index entry).
