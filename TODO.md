...
- [ ] **`walkExpressionTree` uses `firstPhysicalChild` on shared Memo References.** Java's `PlanningCostModel.FindExpressionVisitor.evaluateAtRef` asserts `memberResults.size() == 1` — each Reference should have exactly one member at cost-model time. Go's `firstPhysicalChild` picks the first physical member from multi-member References, so `maxDataAccessCardinality` (criterion #2) reports the same value for plans that selected different physical children from a shared inner Reference. This causes selective Fetch (good, 450K rows) to tie with full-range scan (bad, 900K rows) at criterion #2, falling through to the scalar cost tiebreaker. **Fix:** replace `firstPhysicalChild(ref)` with `findBestPhysicalExpr(ref, costModel)` in `walkExpressionTree`. Needs a `visited` set to break circularity.
- [ ] **SortCPU constant is ~7x too high.** Calibrated against real FDB: sequential scan ~3µs/row, in-memory sort ~4.5µs/row (amortized N*logN), random PK fetch ~50µs/row. Current SortCPU=1.0 makes sort appear 200x more expensive than scan; correct ratio is ~1.5x. Proposed: SortCPU=0.15 (×N×log₂N gives correct relative cost vs scan). FetchCPU=1.5 is approximately correct for the current single-axis model. Recalibrate after fixing `walkExpressionTree` since the scalar cost (criterion #16) will fire less often.
