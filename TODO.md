...
- [x] **GROUP BY/HAVING in correlated scalar subqueries.** Done in RFC-047 — a Go-only read-side extension (Java rejects correlated scalar subqueries at the grammar level entirely; zero wire impact). The stale "PredicatePushDownRule AliasMap.Compose conflict" blocker no longer applies: GroupByExpression is already a push-down barrier (no case in `pushPredicateToExpression`) and the panicking `AliasMap.Compose` has no production callers. `buildCorrelatedScalar` now builds GROUP BY (+ HAVING) into the inner plan and caps with `LIMIT 1`; the scalar contract is FirstOrDefault (first group + LEFT-OUTER NULL-on-empty), NOT a runtime cardinality assertion (Graefe). Empty input → 0 groups → NULL falls out naturally (vs no-GROUP-BY COUNT → 0). Group keys + aggregate operands resolve via the semantic scope (`ResolveIdentifier`), scalar column named with the bare operand to avoid an embedded-`.` qualifier mis-parse. 42803 enforced via `validateGroupByProjection`; multi-column + EXISTS-in-HAVING + unresolvable-expr-arg/key rejected. 23 FDB integration probes (incl. EXPLAIN-pins-StreamingAgg, empty→NULL contrast, expression group key, join+GROUP BY, determinism 10×).
  ...
  - [ ] **Follow-up: expression/constant-argument aggregate referenced in a *differing* HAVING clause of a correlated scalar subquery.** `buildCorrelatedScalar` synthesizes aggregate output names from the *bare* operand (an expression/constant arg collapses to `FN(*)`), but the HAVING-predicate rewrite (`rewriteAggregateValue`) names aggregates by the operand's *explain* (`COUNT(1)`, `SUM(A+B)`). Where the two schemes diverge, a HAVING reference to an expression/constant aggregate that differs from the projection's would mis-route (read the wrong slot / NULL). Caught by Codex; the interim fix **rejects** any such collision fail-safe (e.g. `COUNT(1)` + HAVING `COUNT(*)` both directions, `SUM(a*2)` + HAVING `SUM(a*3)`) — `COUNT(1)≡COUNT(*)` is valid SQL but indistinguishable here, so it is conservatively rejected. Bare-column / `COUNT(*)` aggregates name identically in both schemes and work (incl. `SUM(amount)` + HAVING `COUNT(*)`). **To close:** align the synthesized aggregate names with `rewriteAggregateValue` (operand-explain), keeping a dot-free alias for the single visible scalar output (the join case puts a qualifier dot in the explain). Then distinct expressions get distinct slots, HAVING resolves either direction, and no rejection is needed.
