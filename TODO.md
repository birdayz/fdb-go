...
- [x] **B5 Batch A — 5 of 6 rules + DML 2 of 3 rules**. New `pkg/recordlayer/query/plan/plans/` subpackage with `RecordQueryPlan` interface + 9 physical plans (Scan / Filter / Sort / Distinct / TypeFilter / Union / Insert / Delete / Update). Read-side rules: PrimaryScanRule, ImplementFilterRule, ImplementSortRule, ImplementDistinctRule, ImplementTypeFilterRule. DML rules: ImplementInsertRule, ImplementDeleteRule. All 7 have full 5-wrapper inner-recognition (asymmetry in initial port caught by reviewer + fixed). Bridges via 7 physical wrappers — RelationalExpression adapters with WithChildren + CostHinter (write-heavy WriteCPU for DML). `BatchAExpressionRules()` + `DMLImplementationRules()` composers. Remaining: covering-index + index-equality / range rules (gated on MatchCandidate / IndexAccessHint), ImplementUpdateRule (gated on UpdateTransform plumbing — UpdatePlan type itself is shipped as scaffolding).
