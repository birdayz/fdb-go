...
// This file keeps the CLI deps (cobra, fang, protoconfig, protoyaml) listed
// as DIRECT requires in the root module's go.mod. The `cmd/frl` module has
// its own go.mod but uses these too, and our Bazel build runs a single
// go_deps extension off the root go.mod — so they must live here too.
//
