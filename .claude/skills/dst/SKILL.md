...
**today** — and what each RFC-199 tier will add. Read [`rfcs/199-deterministic-simulation-testing.md`](../../../rfcs/199-deterministic-simulation-testing.md)
for the design; this skill is the *usage* manual.

## Honest status (read this first)

**SimFDB (RFC-199 Tiers 0–2) is built.** The record/relational stack now runs over a deterministic
...
The rule (from RFC-199 §3a): **intercept as high as you can while still covering the code you care
...
Brute-force, seed-driven bug hunter for the record layer (`pkg/simfdb/hunt/`, RFC-199 Tier 2). It drives
...
Concurrent-open-transaction driver (`pkg/simfdb/hunt/interleave/`, RFC-199 Tier 2, a `hunt.Workload`).
...
(`pkg/simfdb/hunt/rangeconflict/`, RFC-199 Tier 2, a `hunt.Workload`) interleaves point AND range ops
...
Continuation-under-fault replay driver (`pkg/simfdb/hunt/continuation/`, RFC-199 Tier 2, a
...
(`pkg/simfdb/hunt/sqlpage/`, RFC-199 Tier 2, a `hunt.Workload`), so the **executor's cursor tree** —
...
## atomicops (atomic-mutation differential)

Differentials FoundationDB's **ten atomic mutation types** (Add / And / Or / Xor / Max / Min /
ByteMax / ByteMin / AppendIfFits / CompareAndClear) end-to-end through the SimFDB backend against an
**independent Go reference model** (`pkg/simfdb/hunt/atomicops/`, RFC-199 Tier 2, a `hunt.Workload`).
The `interleave` driver only ever fires atomic-**add**; every other op — and every width / endianness /
missing-key edge — was otherwise unprobed by a hunt.

```sh
bazelisk test //pkg/simfdb/hunt/atomicops:atomicops_test --test_output=errors
# dst-hunt -profiles atomic-ops     (raw-KV, ~30k execs/sec/core under fuzz)
```

- **Two absolute oracles** (WRONG-absolute; the independent model is the reference): **STATE** — after
  each committed txn a fresh full-range read equals the model, *presence included*; **RYW** — a read
  inside the open txn after its pending atomic ops equals the model's in-txn projection (SimFDB folds RYW
  via `resolveKey`). STATE via `GetRange` is where key PRESENCE is observable; RYW compares value bytes
  only (nil==empty), so present-empty-vs-absent teeth live in STATE.
- **Why a THIRD model when `atomic.go` is already differential-tested against the client's `ryw.go`:**
  both of those are byte-carry-loop ports of C++ `Atomic.h`; a bug common to both survives that
  differential. This driver's numeric ops (Add/Max/Min) run a **`math/big` little-endian path** instead —
  a genuinely independent third leg. And it's the only atomic coverage that runs the ops *through the
  backend* (commit path, clear→delete wiring, RYW folding), not the pure function.
- **The load-bearing edge it targets:** ABSENT (no key) vs PRESENT-EMPTY (present, zero-length value).
  AndV2 / MinV2 / ByteMin / CompareAndClear branch on it (an absent key folds to param; a present-empty
  key runs the V1 body). The model tracks it via map membership; `TestAtomicOps_PresentEmptyVsAbsent`
  pins each case deterministically end-to-end. **A store that dropped a present-empty value on commit or
  GetRange would flip an atomic op onto its absent branch** — this is the exact divergence the driver
  hunts. (Probed the suspicious `buildView` "only insert keys where `valueAtEntry != nil`" line — it's a
  NON-bug: present-empty survives commit+GetRange. The driver *proves* it rather than assuming.)
- **Model-authoring rule (the differential's whole point):** write the model from the FDB/C++ *spec*, in
  a **different implementation style** than the code under test (big.Int vs byte-loop) — a transliteration
  of `atomic.go` would agree with its bugs and catch nothing. When the model and SimFDB disagree, read
  C++ `Atomic.h` to adjudicate which side is wrong; fix that side, pin a case in the focused test.
- **Result** so far: 3500 sweep seeds + 680k fuzz execs, **zero divergences** — expected, since `atomic.go`
  is a careful port, but now with an independent third leg and end-to-end (commit/RYW/clear) coverage
  confirming it. `Result.Counts` asserts all ten ops actually fire (NO-FAKE-CHECKBOX).

## golden (characterization gate)

Behavior lock for the SQL engine (`pkg/simfdb/hunt/golden/`, RFC-199 Tier 2 oracle). For a curated
...
- **A plan STRING can be invariant across a semantic fix — then the ROWS are the sentinel.** The #474
  MIN/MAX fix swapped the index type (`max_ever`→`permuted_max`), but both render `AggregateIndex(MAX, …)`
  — the golden captures the aggregate operator identity (`AggMax`), not the index type — so the plan line
  never diffed and the fix was UNPINNED by golden until the data forced it out. Fix: seed the exact case
  the change is *about* (an ALL-NULL group: permuted keeps `[g,NULL]`, ever dropped it) so the corrected
  behavior lands in the golden ROWS. Lesson: when a fix changes *semantics* an operator label hides, add
  the discriminating DATA, don't trust the plan diff to catch it.
...
- **Metamorphic corpus discipline (joins, nulls, rewrites):** every relation you add MUST be (a) a proven
  3VL identity — the join-key equality or `IS NOT NULL` guard has to exclude NULL/unmatched rows
  *identically on both sides*, or a false relation reads as a spurious bug; and (b) **verified
  non-vacuous before committing** — a throwaway in-package probe (`setup(scn)` + `runQuery`) that prints
  the actual row counts, because `empty == empty` passes the oracle with zero teeth. This session added
  `joins` (INNER-join commutativity / comma-vs-JOIN / ON-vs-WHERE / derived pushdown — all keyed on
  `e.dept=d.did`) and `nulls` (`a=a`≡`a IS NOT NULL`, De Morgan, `COUNT`/`SUM` ignore NULL); each was
  row-count-probed (`a=a`→5 not 7, proving the engine doesn't fold `x=x`→TRUE) before landing.
...
Status against RFC-199. Move rows from *Planned* to *Available* as tiers land, and add their commands
above.
...
7. **Dispatch a background hunter** (general-purpose subagent, `run_in_background`, **`isolation:
   ...
   harness if clean**. **`isolation: worktree` is necessary but NOT sufficient for SQL/metamorphic
   hunters.** The intent: a hunter that writes its gated `_test.go` / probe binary (`cmd/mmprobe`, a
   `scratch-mm/` dir) into the *shared* worktree makes your next commit's pre-commit `gazelle`/`gofumpt`/`lint`
   choke on files it's about to delete, and the commit fails until the hunter finishes. BUT a hunter that
   needs the DST packages (`pkg/simfdb/hunt/*`, `metamorphic`, `sqlpage`) often finds its fresh isolated
   worktree was forked off a branch that LACKS those packages, so it `EnterWorktree`s back into the shared
   `rfc179-dst` dir and pollutes it anyway (observed repeatedly). Defenses: (a) expect leaked scratch —
   before committing, `rm -rf cmd/mmprobe scratch-mm && git status` and clean any stray gated `_test.go`;
   (b) commit only your OWN staged files by explicit path, never `git add -A` while a hunter is live; (c) if
   you must run several, stagger them — two hunters bazel-building in the shared output base serialize and
   their scratch collides. And **kill hunters you no longer want** (`TaskStop <agentId>`) rather than
   letting them churn. A clean result is only meaningful if the machinery has teeth — have it report the
   live count of the bug class it fires (interleave/rangeconflict: `1020`s; continuation: pages), so "0
   findings" is backed by "N million conflicts actually resolved", not silence.
...
## From a DST finding to a fix (the repeatable recipe)
...
3. **Fix-detectors flip.** The DST driver's `TestKnownBug_X` asserts the bug is PRESENT (a fix-detector
   that turns red when someone fixes it). On the fix: rewrite it as a CORRECTNESS test (`TestX`:
   paged==unpaged / result==model), delete the "known bug" framing, and RE-ENABLE the query in the
   sweep's quarantine list. A flipped detector is the regression that proves the fix for good.
4. **Query-engine fixes: the full gate, every time.** Any planner/optimizer/executor change needs
   **Graefe + Torvalds + codex** ACK before merge (PR #201 shipped a 0-row bug by skipping Graefe).
   Run the two virtual reviewers as parallel subagents (`git diff <parent>`, "you are Goetz Graefe /
   Linus Torvalds", <350 words), and codex via `codex exec review --base <parent-branch>` (its scope
   flags don't take a custom prompt — it uses its own). Address every nit; re-request after changes.
5. **Stacked PRs for a batch of fixes.** Linear stack — base → fix1 → fix2 — so each PR's diff is only
   its own commits and shared files (TODO, the sweep) don't conflict. `gh pr create --base <parent>`.
   Read Java → implement (delegate to a fresh-context subagent with the exact path; resume the
   *investigation* subagent to implement, it already read the source) → verify (flip the detector + full
   gate) → review gate → open. The investigation is as valuable as the fix: it's what stops you shipping
   a divergence or rewriting a planner you didn't need to.
6. **Folding the fixes back onto the tooling PR (when the owner says "all on this PR").** Sometimes the
   directive is one comprehensive PR — framework + the bugs it caught + the fixes — not a stack. Merge
   the fix branch(es) into the tooling branch (`git merge <fixbranch> --no-ff --no-commit`; a branch
   stacked on another brings both with their reviewed SHAs preserved as ancestors). Two things to get
   right: (a) **the sweep's query list conflicts** — the fix *re-enables* the quarantined shape, your
   tooling commit *added* shapes; usually auto-merges, but READ the merged `queries()` and confirm both
   coexist and only the still-open shape stays quarantined; (b) **prove the merge added no new engine
   logic** — `git diff <fixbranch> HEAD -- <each engine file>` must report IDENTICAL for every
   planner/executor file, so the existing Graefe+Torvalds+codex ACKs still cover the logic and only the
   docs/harness reconciliation is new. Doc-only TODO conflicts: take the `[x]` FIXED side for the fixed
   findings, keep HEAD's side for any finding whose understanding advanced after the fix branch was cut
   (e.g. a defer-to-RFC decision). A no-master-merge freeze makes this consolidation safe — it's
   collecting unmerged work on one branch, not merging to master; note that a re-request applies before
   any eventual (frozen) merge. Then run the FULL matrix sweep post-merge (0 findings over ~10k+ seeds)
   to prove the fixes introduced no regression across the whole hunt space, not just the pinned tests.
...
3. **Verify before you write.** Every command here was run against the tree — a wrong command makes this
   skill worse than nothing. Re-run it; if a flag/path changed, fix it. When code + RFC-199 disagree with
