...
23. **An anonymous record through a derived row.**
    ...
    (round-trips with no record name; a record named RECORD keeps that name), and the
    inline-values pins expecting an anonymous retagged row.
