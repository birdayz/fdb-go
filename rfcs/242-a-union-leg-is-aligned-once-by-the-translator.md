...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   A **second** entry pins the same defect on a second target, and it is the one the engine
   fuzz nightly is red on now rather than the one that opened this RFC:
   ...
   that carries the folded one is a bare `Scan` — the table above's last row, which the first
   entry does not exercise. Measured both ways with the same command, on both trees:
   ...
   the SQL shapes above show, arrived at from random bytes with no SQL in the path. An
   unguided fuzz run at the merge-base rediscovered it inside its first second; the corpus
   entry makes it a deterministic sub-test that runs on every `go test` and every Bazel run
   of the target.
...
- **The third attempt to state one paragraph's scope, and the third that was too wide** (Graefe
  ...
  for sha in e1b4ed839 13ba74d63; do
  ...
  Over the WHOLE file at `13ba74d63` the same pattern says eleven, because the bounded comment
  now quotes two of the pointers it disclaims. Same tree, same pattern, two answers — which is
  why the region is part of the claim rather than an implementation detail of how it was taken.

  It is a LINE count. `grep -o | wc -l` counts OCCURRENCES and is a different measurement, and
  here it happens to answer nine as well, at both heads — which is worth writing down rather
  than leaving as agreement, because a pair that agrees by coincidence is the shape that
  ...
  stated unit stays checkable. A reviewer arrived at nine independently by a THIRD counting —
  distinct row-to-row references, which is eight, plus one for a line that says "above" twice.
  Three populations, two of them coinciding at nine: the figure means nothing without the
  command beside it, which is the whole reason the command is here and the figure is not in the
  sentences that discuss it.
...
- **The nightly's CURRENT red, pinned** (not a review finding — found while re-measuring the
  bookings this PR's final report was going to quote): the engine fuzz nightly is red on
  `FuzzPlanner_PlanFullPipeline`, a different target from the `FuzzPlanner_WithBatchA_NoPanic`
  that opened this RFC, and on the same defect. Six bytes, `Distinct(Union(UnsortedSort(Scan),
  Projection(Scan)))`, rediscovered at the merge-base within the first second of fuzzing (`go
  test -fuzz='^FuzzPlanner_PlanFullPipeline$'`, 24 workers, `fuzz: elapsed: 1s, minimizing`) and
  minimised from 43 bytes to six. Committed as corpus entry `05cb6c3c202ef15e` with both
  measurements in
  § Test plan item 1. It reaches the rule from the arm the first entry does not exercise — the
