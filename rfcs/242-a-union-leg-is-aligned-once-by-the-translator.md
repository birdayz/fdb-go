...
- **One declared name over two shapes fails loudly** (Torvalds; Graefe measured the same and
  ...
  row that names one field twice is never stamped, and flows as a map") and pinned at r21.
  Pinned over FDB
...
- **The r19 booking is RESTORED, and r20's withdrawal is retracted** (Graefe and codex each
  ...
  duplicate `ID`, and THREE constructors in that plan stay unstamped, each a map in which the
  second `ID` overwrites the first. The entry is restored with r19's attribution intact, and the
  reproducer is committed as the pin rather than a probe deleted afterwards
  (`TestFinalizePlanLeavesTheDuplicateNameJoinRowAMap`: the planned FULL OUTER JOIN still holds
  a duplicate-name row, every such row is unstamped, and it reddens when the closure lands).
