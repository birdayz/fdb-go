...
- **The r19 booking is RESTORED, and r20's withdrawal is retracted** (Graefe and codex each
  ...
  four constructors in that plan end up maps though only ONE repeats a name (r22 measures it). The entry is restored with r19's attribution intact, and the
...
- **The blast radius is the whole repository, and three durable sentences understated it**
  ...
  constructors end up maps though only ONE repeats a name, while the fourth, resolved before the
...
- **The SQL pin baked nothing, and its surviving arm was a tautology** (Graefe measured the first,
  ...
  mutation that appeared to confirm it only exercised the other arm. The pin now bakes, asserts
  the FULL population (all constructors unstamped, at least four), and carries a no-repeat control
  through the same route that must stamp — so a bake that stops running fails instead of passing
  as a finding.
