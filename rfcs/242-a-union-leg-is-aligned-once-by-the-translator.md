...
- **The artifact digest could not see an append.** It listed top-level NAMES, so a wrong-CWD
  ...
  the gate green. `du -ab`, recursive and byte-sized: an append moves the hash, and so does the
  pid file. Both verified directly.
...
11,443 entries — so the digest walked it and any concurrent agent touching that tree reddened the
arm for work this suite did not do. That is the exact failure the `find` form was chosen over
`git status` to avoid, reintroduced by the fix for a different blind spot, three lines under a
comment still claiming the scoping. `fdb-watch.*` covers `.log` and `.pid`, which is all the
broadening was for, and the named list is back. Verified: a Java-tree change leaves the digest
UNCHANGED, while the pid file and an append to the log both move it.
