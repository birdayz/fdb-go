...
r86 folds one codex finding: the bypass guard added the round before ran only on its success path.

- **The count decision was never driven through a failing state.** Every committed run reaches it
  with exactly one invocation, so weakening `= 1` to `-le 1` kept all 75 arms green and then
  ACCEPTED the bypass count of zero — the guard against a bypass, passing the bypass. The decision
  is a named function now and all three states are driven: one passes, zero and two are each
  required to report a failure. The weakening reddens the zero arm by name.

And a boundary worth recording, because two gates independently reached it and it should not be
rediscovered as a finding: the count arm guards the LIVE caller. Inlining the negative arm and then
weakening the predicate leaves everything green — but that is rewriting the assertion rather than
production drifting, and guarding a test against its own deletion is infinite regress. The gates
called this instrument sound at 75 arms and the same holds at 77.

r85 folds two NAKs, one codex finding, and one defect found by driving a mutation nobody asked for.

- **One decision point was still two.** The guard's predicate had been extracted into a helper and
  an arm drove the HELPER, while the live check called it separately — so reverting the live CALL
  to a path test left the helper untouched and every arm green. Reporting moved inside the
  function, so both the live check and the negative arm go through one decision; reverting the
  predicate reddens the negative arm by name.
- **And a caller could still leave.** One function is one decision point only while both callers
  use it: replacing the live line with an inline path test bypassed the guard entirely and nothing
  noticed. The guard counts its invocations and an arm requires exactly one — the negative arm's
  call runs in a subshell whose increment does not escape, which is why the expected value is ONE
  and why it is written down rather than inferred. The bypass now reports "called 0 times, want 1".
- **The mechanism given for the withdrawn two-red figure predicted the opposite of it.** The write-up
  said that figure came from pointing `md5sum` at a directory, "which changes both readings".
  Measured: changing BOTH readings empties both, they compare equal, and exactly one arm reddens —
  identical to `chmod 000`. Two reds require exactly ONE reading to move. The mechanism is the
  ASYMMETRY, not the unreadability, and the sentence named the property that cannot produce its own
  number. Two gates measured this independently.
- **And the round count in that same paragraph was off by one.** It said two rounds running; this is
  the third.

The shape is now stable enough to name once and stop restating: a claim about a test is a claim,
and the only thing that makes it true is running the mutation it names — not the mutation you
remember running, and not the one that sounds like it.

r84 folds one NAK and one codex finding, both on the guard added in r83, and both the same rule:
a proof that is not committed is not a proof, and a measurement written up from a different
mutation than the one it names is not a measurement.

- **The guard was exercised only on its success path.** The fixture is always readable, so the
  branch that distinguishes `[ -n "$reading" ]` from the `[ -f ]` it replaced never ran — revert
  the predicate and every arm stayed green. Manufacturing a genuinely unreadable file is not
  portable, since a root runner reads a `chmod 000` file happily, so the predicate is a NAMED
  function taking the reading as an argument and an arm drives it over the distinguishing state
  directly: a file that exists, with no reading for it, which a path test accepts and the guard
  must reject. Reverting `probe_has_reading` to a path test reddens exactly that arm.
- **And the write-up of the previous round's measurement named the wrong mutation.** It said an
  unreadable reading "reddens it AND the downstream arm". Under `chmod 000` — the shape the
  sentence describes — exactly ONE arm reddens; the downstream arm stays green comparing empty to
  empty, which is the vacuity being guarded against, not a second detection. The two-red reading
  came from pointing `md5sum` at a directory on ONE side only — an asymmetric change, and that is a different
  mutation. Corrected to what each shape actually produces.

r83 folds two NAKs and one codex finding, and all three are the previous round's fix being one
digit short.

- **The header assertion was an unanchored substring, so the number it pinned was not pinned.**
  `strings.Contains(msg, "2 arms ran, 3 pinned")` is satisfied by `"12 arms ran, 3 pinned"`: the
  left digit boundary is open. Inflating the reported count by ten, or deleting the message's
  leading prose, left all ten cases green — in the round whose subject was that those two numbers
  were asserted by nothing. The WHOLE first line is compared now, prefix and trailing period
  included, so the substring is bounded on both sides. Measured: +10 reddens five, deleting the
  prefix reddens five, deleting the header reddens five, swapping the two counts reddens three.
- **And "73 against 72" went stale in the comment written to stop things going stale.** The 72→73
  sweep updated the three shell sites and missed two Go ones, one of them the header comment added
  that same round, quoting the figure as "the whole argument". At 73 pinned entries the value is
...
- **And the probe guard pinned the wrong property.** It asserted `[ -f fdb-watch.log ]`, which
  proves the path exists and still leaves the reading empty if `md5sum` cannot produce one — and
  empty-against-empty is exactly the comparison the arm was added to stop. It asserts a non-empty
  READING now. Measured, as one mutation each: removing the file reddens the guard by name, and
  `chmod 000` on the file — the state that actually distinguishes the two predicates — reddens the
  guard and NOTHING ELSE. The downstream arm stays green there, comparing empty to empty, which is
  the vacuity the guard exists to catch rather than a second detection; an earlier version of this
  bullet claimed both arms reddened, having read that from a different mutation (`md5sum` pointed
  at a directory on the `pre_before` side ONLY) that leaves the two readings UNEQUAL, and is not the shape
  the sentence describes. The asymmetry is the mechanism, not the unreadability: pointing `md5sum` at a
  directory on BOTH sides empties both, they compare equal, and exactly one arm reddens — the same
  as `chmod 000`. Two reds require exactly one reading to move.
  Renaming the arm also made the label gate fire for real — 73 ran against 73 pinned, MISSING and
...
Three rounds running, the defect has been inside the sentence fixing its predecessor. That is the
whole finding: a fix is a claim, and it decays exactly like the claim it replaced.

r82 folds one NAK and two codex findings. The NAK and one finding are the same defect the round
before them was about, one level further in; the other is that defect appearing INSIDE the sentence
correcting it, which is now the third distinct occurrence and the reason it is written down here
rather than just fixed.

- **The two counts the message opens with were asserted by nothing.** `"%d arms ran, %d pinned"` is
  the evidence for every claim made about this gate — at 72 arms, the whole "73 against 72" argument was those
  two numbers — and swapping them, or deleting the header outright, left all ten cases green. Every
  erroring case pins its exact header now. Deleting the header reddens all five; SWAPPING the two
  counts reddens three, and the two it cannot reach are the ones whose counts are equal, where a
  swap is the identity. That is a property of the fixtures rather than a hole, and it is stated so
  the three is not later read as a shortfall.
- **"The state a count cannot see at all" was false, one line below the correction of the same
  overclaim.** A duplicate ALONGSIDE the full population is 4 lines against 3 distinct entries and
  any arithmetic catches it; only a duplicate REPLACING a missing arm is 3 against 3. Both homes
  say when a count is blind now, instead of asserting that it always is.
- **And the retracted claim survived a fourth time, in the status line.** "A copy-pasted arm would
  have read green at 73" is the sentence r81 retracts; the greps that were supposed to prove it gone
  looked for "reported green at 73 arms" and missed the paraphrase, forty words from a sentence in
  the same paragraph naming that exact trap. Corrected in place. A grep for the phrasing you
  remember writing is not a grep for the claim.

r81 folds one NAK, and it is the same defect a level deeper again: the assertions written to stop
the census message being undiscriminating were themselves undiscriminating.

- **Nothing bound a label to its section.** Each case matched a bag of substrings against the WHOLE
  message, so for a same-count substitution — the one state where two sections populate — the
  report naming `[c]` MISSING and `[d]` UNEXPECTED and the exactly inverted report both satisfy all
  four assertions. Swapping the payloads reddened three of ten, and the survivors were precisely
  the two-section cases; the single-section cases only passed because their empty sibling printed
  nothing, which is luck, not coverage. Assertions are per SECTION now: the message is split, the
  named heading located, and the labels required inside that line. Re-measured — swapping
  MISSING against UNEXPECTED reddens four (including the substitution case that survived before),
  swapping MISSING against DUPLICATED reddens four (including the duplicate-masking case that
  survived before), and deleting the DUPLICATED section reddens two.
- **The claim "the swap reddens four" was wrong when written.** It was three. The number is not
  restated here; the mutations and their reddened sets are, above, measured at this head.
- **And the reason given for removing the arithmetic was itself wrong.** "A full population plus a duplicate is not caught by any arithmetic" — it is: 73 lines against 72 distinct entries, and the removed clause did return an error for exactly that input. What it could not do was name the label. The claim overstated the change in the direction that justified it, which is the same failure as every scope claim on this branch, made while correcting one.
- **And an enumeration had outgrown itself.** The comment above the pinned labels said "Three states, three messages" and listed NONE, MISSING and UNEXPECTED — for the whole round after DUPLICATED was added as the fourth. It names four now, and says WHEN a count is blind: a duplicate alongside the full population is 4 lines against 3 distinct entries and arithmetic catches it, while a duplicate replacing a missing arm is 3 against 3 and nothing arithmetic can.
- **And the refuted sentence was still live one section away.** "Derives the count from them, so
  the two cannot disagree" was removed from the Go comment and left standing in the RFC's own r79
  bullet, forty lines from the r80 bullet calling it false. A correction is not done until the grep
  is zero across every file, and this is the third time on this branch that a fix reached the code
  and missed a durable home.

r80 folds two NAKs, both on the arm-population check added the round before, and both instances of
the failure that check exists to prevent — a gate whose own verdict cannot be told apart from a
different verdict.

- **The census message rendered every section unconditionally, so its assertions were vacuous.**
  `MISSING (…)` and `UNEXPECTED (…)` were both in the format string whatever the state, so a unit
  case asserting on the word MISSING also held for an unexpected arm. Swapping the two payloads
  left all nine cases green. Each section prints only when it has members now; every case asserts
  the sections that must be ABSENT as well as present.
- **A duplicate label with nothing missing was caught by arithmetic that nothing drove, and could
  not describe what it caught.** The only guard was `len(got) == len(expected)`, and no case
  exercised it, so deleting the clause changed nothing that any test could see. It was not blind —
  those lengths count different things, lines against distinct entries, so a full population plus
  one duplicate was 73 against 72 at that population — N+1 lines against N distinct entries at any
  population — and it did return an error. What it could not do is SAY anything:
  both label lists came back empty and the message named nothing. A repeated label is its own state
  now, with its own message and its own case, and deleting that section reddens two. The gain is
  diagnosis and coverage, not detection — an earlier version of this bullet claimed a copy-pasted
  label would have "reported green at 73 arms", which is false, and it was written that way because
  removing the arithmetic felt like it needed a stronger justification than it had.

Worth stating because five rounds have now ended the same way: **every finding since r75 has been
in the test instrument, not in RFC-242's change.** The rule the branch keeps rediscovering is that
a coverage claim is a claim — an arm is evidence only for the mutation someone actually ran against
it, and "it passes" is not that mutation.

r79 folds two NAKs and three codex findings. Every one is the same defect wearing a different
mask, and after four rounds of it the mask is worth stating outright: **a test instrument's
coverage claim is a claim like any other, and describing what you wrote is not measuring what it
catches.** Each arm below was added in good faith to close the previous round's gap, and each was
verified only by the fact that it passed.

- **The scope half of the digest was pinned by an arm that could not fail on it.** Removing the
  OUTER `-maxdepth 1` — the property restored two rounds earlier, after the gitignored Java
  checkout reddened this suite — left every arm green, because the reach case wrote
  `fdb-record-layer/core/build.out`, which matches no `-name` pattern and therefore answers `same`
  under both forms. Its verdict came from the name filter alone; the scoping it was named for was
  never exercised. It writes an `fdb-logs-*` name below the top level now, and removing the outer
  `-maxdepth 1` reddens exactly it.
- **The seeded directories were unasserted.** Move the `mkdir` for `sub/` back into the depth case
  and the suite stays green at 72 arms while the `-maxdepth 1` mutation falls from one red to zero
  — the case passing on the depth-1 entry it created, which is the r77 and r78 defect a third
  time, on the arm added to fix the second. Every seeded dependency is a stated PRECONDITION now,
  and that mutation fails by name.
- **The precondition mechanism was itself unpinned.** No passing case reaches its failure branch,
  so deleting the branch left all 70 arms green — a guard against silent degradation whose own
  first execution would be the moment it was needed. Two arms drive it: a deliberately false
  precondition must fail by name AND must run no mutation.
- **The name-class arms used spellings the workflow never writes.** Staging is
  `.fdb-logs-$c.$launch.new` and a retired generation is `.$prevgen.trash`; the container listing
  is `fdb-container-$c.log`, not `.txt`. Narrowing the selectors to the synthetic spellings left 69
  arms green while excluding the real artifacts. The fixtures are the production shapes now.
- **And the population check counted rather than identified.** Exact equality on the count still
  cannot see one arm skipped while another is added, or a duplicate standing in for a deletion —
  the cancellation it was introduced to prevent, surviving inside its own fix. It compares the arm
  LABELS now; renaming a single arm reddens it with an exact MISSING/UNEXPECTED diff at an
  unchanged 72-to-72 count.

r78 folds three NAKs and two codex findings. The same shape recurs a third time and is worth
naming plainly: **an arm that creates the thing it is supposed to detect passes without detecting
it.** It happened to the nested cases in r77, to the depth case inside r78's own first draft, and
it is why every case here now runs against state seeded before the snapshot.

- **The arms pinned one depth and two of seven name patterns.** All three nested cases wrote at
  depth 1, so `-maxdepth 1` on the inner `find` left 63 arms green; and only `fdb-watch.*` and
  `fdb-logs-*` were ever exercised, so five of the seven patterns could have been deleted with
  everything still green — a scope the arms asserted and did not cover. Six cases added: one at
  depth 2, and one per undriven pattern, each the only file matching its own. Measured at 69 arms:
  `-print` reddens the four cases that write below the top level; `-maxdepth 1` reddens exactly the
  depth-2 case; dropping any single one of the five patterns reddens exactly that pattern's case.
  The depth case's FIRST draft created its own intermediate directory and passed on the new
  depth-1 entry — the r77 defect one level down, caught by driving it rather than by reading it.
  `sub/` is seeded now too.
- **The arm-count guard was a floor, and a floor cannot see the failure it cites.** Add an arm and
  silently skip it and the total never moves — which is the 53-local-versus-52-in-CI case exactly;
  add one while another is deleted and they cancel. It is equality now, so the population must be
  RESTATED whenever it moves, which is this repository's prose rule enforced instead of remembered.
  Both directions are alarms and the message says which, including what to do when arms are
  deliberately retired. The decision moved out of the test body into `checkArms` and six unit cases
  drive it from explicit state — no output, output with no arms, one fewer, one more, exactly the
  pin, and prose containing "ok" that is not an arm — because a full run only ever exercises the
  equal branch and leaves the two that matter untested until the day they fire.
- **The equal-length arms degraded silently.** They rewrite a file with content of the same length,
  which is only a test while the length still matches — and the cases share a directory in order,
  so inserting one plausible case ahead of them changed what they started from: 64 arms, all green,
  the pin satisfied, and the `du -ab` mutation quietly falling from two red to one. `digest_case`
  takes a PRECONDITION now, and an insertion produces a named failure instead.
- **Two populations were stale inside the commit that existed to fix stale populations.** "All six
  share one directory" against nine cases, contradicting "these nine arms" thirty-nine lines below
  it; and the flat pid rewrite still called "the one" arm that reddens under `du -ab` when the
  nested rewrite added in the same commit made it two. Both are now fifteen and two.

r77 folds one codex P2 and three findings from the other gates. The P2 is the sharpest thing on
this branch: the arms written last round to stop the digest failing open had themselves failed
open, in the same dimension.

- **The digest arms did not pin recursion.** All six wrote a TOP-LEVEL name, so a digest that
  never descended passed every one of them. Replacing the inner `find {}` with `-print` left 60
  arms green while an append inside an existing `fdb-logs-*` directory — the exact wrong-CWD write
  the digest went recursive to catch — moved nothing. Three nested cases added, run against a
  directory seeded BEFORE the snapshot, since a case creating its own directory passes on the new
  top-level entry and says nothing about descending into it. Re-measured at 63 arms: `-print`
  reddens exactly the three nested cases; the prior `xargs -r du -ab` form reddens both
  EQUAL-LENGTH rewrites, flat and nested; dropping the directory line reddens the empty directory;
  a constant reddens all eight `moved` cases. Five content arms had looked like thorough coverage
  of a digest, and the dimension none of them varied was the one it existed for.
- **A shadowed variable.** A second `DWORK` was declared 200 lines below the live one. Nothing
  reads the first after that point, so there is no leak today and every arm passes — and the next
  arm added below it would have silently got the digest's scratch directory. Renamed, rather than
  left as a fact about where the file currently ends.
- **A comment that was false about the code beneath it.** "Each case runs in its own directory" —
  they share one, and they are deliberately ORDER-DEPENDENT: an append must follow the file it
  appends to, an equal-length rewrite must follow the length it matches. What the shared directory
  actually buys is that none of them touches the repo root. Stated that way now, with the ordering
  named as a constraint on inserting cases.
- **The gate asserted the wrong thing.** `TestRowdiffWatcherBehaviour` checked only for `ALL OK`,
  which is precisely what a suite whose arms stopped RUNNING prints — the 53-local-versus-52-in-CI
  failure this file already carries, undetectable by its own gate. An arm-count FLOOR now runs
  beside it, watching for COLLAPSE rather than growth so that adding arms stays free, with the
  retirement procedure written into the failure path. Driven red: delete one case and the suite
  still reports `ALL OK` at 62 arms while the gate fails.

r76 folds a Graefe NAK and four codex findings. None is an algorithm defect; every one is a claim
that outran what the code does, which is this branch's dominant failure and the reason the round
exists at all.

- **The RFC asserted the mechanism the previous commit had just deleted.** `du -ab` was described
  as "recursive and byte-sized", with an append and the pid file "both verified directly" — three
  sections away from the commit removing it precisely because it compares apparent SIZE. Both
  verifications were real and neither was evidence for the sentence: the append moved the hash by
  changing a length, the pid file by adding a path. Corrected in place, with what each reading
  actually showed.
- **The census counted marker occurrences, not verdicts.** 35 and 13 are occurrences of
  `**ACK for head` and `**NAK for head`; the comment at the id bound quotes both while discussing
  an earlier census, so it was counted as its own evidence. Classified one verdict per comment,
  anchored to a line start: **34 ACK, 12 NAK, 20 with neither, summing to 66.** The 13 had been
  contradicting the "twelve NAKs" further down the same entry from the moment it was written —
  a free internal check, never run. Fourth wrong version of this figure; the first counted a
  header string, the second was unbounded, the third counted the wrong population.
- **The RFC's copy of the census command could not run.** `gh api --paginate` emits one JSON array
  per page and the filter indexed it as an object, so a reader following the RFC got
  `Cannot index array with string "user"`. The working `jq -s '[.[]|.[]]'` form from `TODO.md` is
  in both homes now, and it was run verbatim from the RFC before this was written.
- **The digest's corrections were a scratch measurement.** Nothing committed drove
  `suite_artifacts`; its only callers bracket a suite expected to stay clean, so on the green path
  it compares an empty digest to an empty one and a constant would pass forever — the instrument
  used to check the suite, failing open. Six arms drive it now, each asserting an unscoped control
  digest moved first so a mutation that silently fails to land cannot read as a pass. Measured:
  the prior `xargs -r du -ab` form reddens exactly one, the equal-length pid rewrite; dropping the
  directory line reddens exactly one, the empty generation directory; a constant reddens all five
  `moved` cases. Suite at 60 arms then, local and Bazel agreeing.
- **And re-measuring the arm count caught a false symmetry that had never held.** A comment claimed
  deleting either copy of the tag guard reddens exactly one arm. Deleting the watcher's does.
  Deleting the forensics one aborts the extraction and the suite stops at 49 arms, zero red —
  because `awk` is still armed when it reaches the other step's `ver="`, so the first deletion
  silently borrows the second's guard while the second has nothing after it to borrow. Checked at
  the previous head too: false there as well, so this round did not break it, it found it. Both
  deletions are still caught, so the coverage claim holds and the description of it did not.

r75 folds codex's last three — one of them the only change on this branch that could have destroyed
data outside the repository — and corrects a count that a reviewer could not reproduce.

- **`rm -rf` on an unquoted expansion.** The cleanup accumulated paths in a STRING joined with
  `$*` and the trap expanded it unquoted, so a `$TMPDIR` containing a space split into separate
  arguments: `/tmp/a b/tmp.X` reaches `rm -rf` as `/tmp/a` and `b/tmp.X`. Reproduced before
  fixing — deleting something unrelated while leaking the real tree. An array, expanded quoted.
  Of everything on this branch this is the one line where a quoting slip was destructive rather
  than merely wrong, and it was introduced by the cleanup consolidation two rounds earlier.
- **The artifact digest could not see an append.** It listed top-level NAMES, so a wrong-CWD
  fragment that appended to an existing log, wrote inside an existing generation directory, or
  created `fdb-watch.pid` — which the watcher writes before it polls — left the hash unchanged and
  the gate green. It went recursive over `du -ab`, and an append and the pid file were both
  verified to move it. Both readings were true and neither showed what the sentence went on to
  claim, because `du -ab` reports apparent SIZE and path: the append moved the hash by changing a
  length and the pid file by adding a path, and neither one read a byte of content. The shape that
  separates the two is a rewrite of EQUAL length, and that one was invisible. r76 replaced it with
  a per-file `md5sum` plus a line per directory, and drives the digest from the suite instead of
  from a scratch shell — six arms, with the prior form measured red on exactly one of them.
- **The outage window was a deadline where the arm needed an observation.** A nominal two seconds
  against a cycle of `sleep 1` plus overhead can close before any call lands in it, so the
  sentinel arm could redden for a timing accident — a flake in the gate added to stop a vacuous
  pass. It is a countdown consumed BY the call now, so the observation holds by construction; the
  deadline form stays only for the backstop case, which needs elapsed seconds rather than a hit.
  Making that change re-broke the skip arm, because the count was consumed by `ps` while `cp` kept
  answering and skipping became unobservable again — so `cp` honours both knobs and consumes
  neither. Dropping the skip signal reddens three arms.

And the count. The gate-accounting entry said "51 comments"; a reviewer ran the API himself and got
**65**, which is right. The 51 was `grep -c 'Claude finished'` — comments containing one particular
header line, a narrower set than "comments by that author", reported as though it were the
population. The verdict figures did reproduce: 33 `**ACK for head`, 12 `**NAK for head`. Both the
`TODO.md` booking and this section now carry the exact command, because a count nobody else can
reproduce is the thing the booking is about.

Worth recording separately, because it bounds what every ACK on this branch is worth:
`gh pr view 770 --json reviewDecision,reviews` returns an empty decision and ZERO formal reviews.
The four-gate loop lives entirely in prose.

One more, caught after the ACKs and before the merge: broadening the artifact glob to `fdb-*` to
catch `fdb-watch.pid` also caught `./fdb-record-layer/` — the gitignored Java reference checkout,
11,443 entries — so the digest walked it and a concurrent agent working there reddened the arm for
work this suite did not do. The reach is narrower than "touching" it, and saying otherwise would
repeat the very size-for-content overclaim this round is correcting: the digest was `du -ab` at the
time, so only an ADD, a REMOVAL or a change of LENGTH under that tree could move it, and a
metadata-only touch or an equal-length rewrite could not. A Java build does all three, which is why
it fired. That is the exact failure the `find` form was chosen over `git status` to avoid,
reintroduced by the fix for a different blind spot, three lines under a comment still claiming the
scoping. `fdb-watch.*` covers `.log` and `.pid`, which is all the broadening was for, and the named
list is back. Driven from the suite now rather than asserted: a Java-tree change leaves the digest
UNCHANGED, while a new log, an append, the pid file, an equal-length rewrite of it and an empty
generation directory all move it.

One incident, not a code change: the three fixes above were written once, verified, and then
silently reverted in the worktree by another agent working in this checkout — the hazard CLAUDE.md
documents for `git add -A`, arriving instead as a plain overwrite. Caught because `git diff
--cached` showed two files where three were expected. Re-applied and re-verified from scratch.

r74 folds codex's four and records a process failure that is worse than any of them.

- **The arm added to catch a silent leak did not run in CI** (codex; Torvalds measured the same
  independently). It asked `git rev-parse --git-dir`, and under Bazel the suite runs from a
  runfiles tree with no `.git` — so the condition was false, the arm never emitted, and the suite
  reported 53 arms locally against **52** under the runner that gates merges. The fifteenth face,
  inside the arm added to close a neighbouring silence. It now compares a scoped `find` of the
  names a fragment can write, which needs no VCS and, as Torvalds noted, also stops `git status`
  reddening for a CONCURRENT agent's edit in a shared checkout. Local and Bazel now both report 54.
- **The outage arm could pass without the outage being observed** (codex). The window is nominal
  seconds and a cycle is a sleep plus overhead, so no call need land inside it — leaving
  `nfail=0` and the arm green whether the skip branch exists or not. A sentinel records which
  poller actually hit the window, and the arm requires it before reading the zero.
- **Five `trap … EXIT` lines, each replacing the last.** The final one omitted three directories
  the decision table had created. Measured with an isolated `TMPDIR`: the old shape leaks 3, the
  new one leaks 0. One accumulator, one trap set once — a trap rewritten as a file grows is a trap
  that forgets what came before the rewrite.
- **The census breakdown was wrong a fourth time in the same paragraph** (codex): eight source-like
  files and 49 fixtures, not seven and 41, and a superseded 48-file estimate survived four lines
  below the corrected figure.
...
For the length of this PR that gate was not a gate — it was a log nobody tailed. It had posted
**66 comments**, of which **34** carry an ACK verdict and **12** a NAK verdict, and **20** carry
neither — 34+12+20=66. The count is of VERDICTS, one per comment, anchored to the start of a line:

    gh api --paginate 'repos/birdayz/fdb-go/issues/770/comments' \
      | jq -s '[.[]|.[]] | map(select(.user.login|test("claude")) | select(.id <= 5561694850))' \
      | jq '[.[]|select(.body|test("(^|\n)\\*\\*ACK for head";""))]|length'

Counting OCCURRENCES of the marker string instead gives 35 and 13, and that is the wrong
population: the comment at the bound itself quotes both markers while discussing an earlier
census, so it is counted as its own evidence. The 13 was also visibly inconsistent with the "12
NAKs, each closed by that gate's own later ACK" two paragraphs down, and the inconsistency was
sitting in the text before anyone read it. Three earlier figures here were wrong: "51" counted a
header string rather than an author; the unbounded replacement was stale within the hour; and the
bounded one counted quoted markers as verdicts. Two of those NAKs were closed only because another
reviewer independently found the same defect. The first correction of this was itself unscoped —
"two NAKs" read off the tail of a list rather than counted — which is an unscoped count reported as
evidence, inside the correction of a reporting failure. Booked in `TODO.md` with the mechanism, because it is
this branch's own headline class applied to a gate instead of a test: a layer that cannot
distinguish *reviewed with no findings* from *never read* renders both as success.

One more thing the same reviewer measured, worth stating because it bounds what any of this is
worth: `gh pr view 770 --json reviewDecision,reviews` returns an EMPTY decision and ZERO formal
reviews. Every ACK on this branch — all four gates, every round — lives in prose, in PR comments
and agent transcripts. Nothing about it is enforceable by the platform, and a reader who wants to
check it has to read the comments, which is precisely the step whose omission is booked above.

Verified before merging, and this is what makes the four-gate claim checkable rather than asserted:
every one of those 12 NAKs is followed by that same gate's ACK on a later head, and the sequence
ends ACK `1a71d0a0d`, ACK `3886097a4`. The gate that raised each finding is the gate that closed it.

r73 clears the last NAK and folds three items both remaining gates asked for before merge.

- **The two "container is alive" exits were asymmetric** — found independently by Graefe and
  Torvalds, in the same shape and with the same one-word fix. The direct hit cleared the outage
  clock AND the skip signal; the re-issue's exit cleared only the clock. So a copier that rode out
  an outage and recovered THROUGH THE RE-ISSUE — the exact path the re-issue exists to serve —
  skipped one more cycle than it needed to, with the container present and the daemon answering.
  Bounded and lossless, but the decision table could not see it: `dec_case` asserted rc, reason
  and clock, three of the FOUR things the function writes, and the fourth was the variable whose
  deadness had been the previous round's second finding. It asserts `misses` now, and takes a
  starting state — one call cannot both enter an outage and come out of it, so without that the
  recovery paths were unreachable. Reverting the asymmetry reddens
  `recovery through the RE-ISSUE clears it too: got [0 none stopped 2], want [0 none stopped 0]`.
- **A comment stated the deleted design as current** (Graefe): "Three misses is the container
  being gone; one is the daemon being busy" sat directly above the copy loop, one round after
  misses stopped deciding removal at all — while a retraction of the same claim sat 190 lines
  above it in the same file. The same failure as the §Performance sentence, one file over, and the
  sweep for the superseded claim should have reached it.
- **The suite could write into the repository, and nothing said so** (Graefe). The
  decision-table arms left an untracked `fdb-watch.log` in the root, caught by reading
  `git status` rather than by any failure — no signal at all. Graefe's argument for an arm over a
  not-covered entry is the one that decided it: the list is for behaviours of the SUBJECT no arm
  drives, a harness writing outside its sandbox is a defect in the INSTRUMENT, and the cause is
  structural rather than incidental, because running extracted fragments with CWD at the repo root
  is the technique the whole suite is built on. So it recurs, and it is asserted: the tree is
  snapshotted before the first case and compared after the last. Making any fragment write
  relatively reddens it.

Also settled: the 39-vs-18 golden discrepancy reconciles exactly — 18 in `plan_shape.golden` plus
7 + 4 + 6 + 4 across the yamsql and hunt corpora is 39, and since the §Performance sentence names
the file, 18 is the right figure for it. Removed is 0 in both scopes, which is the load-bearing
half.
...
- **The trace measurements lived in a scratch directory** (Graefe conditional, Torvalds the same
  ...
  a committed suite (until 2026-09-22), `pkg/docscheck/rowdiff_watcher_suite.sh`, built the same way: it
  EXTRACTS `fdb-watch.sh` out of the workflow heredoc, so it cannot drift from what the nightly
  ran, and stubbed `docker` so it needed no daemon. It was removed on 2026-09-22 together
  with the watcher itself at the owner's request; see TODO.md "Nightly RowDiff FDB watcher
  and forensics steps removed".
