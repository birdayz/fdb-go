...
- **A stamped parent over an unstamped child fails the query** (Torvalds NAK): r33's comment said
  ...
  is open and is written as open: the shape is constructible over the values API and no query is
  known to produce it, and the booking says to settle it before closing rather than to assume it.
...
- **The reachability question was foreclosed, not open** (Graefe NAK; Torvalds asked what the
  ...
  Parent stamped implies child stamped, and the pair is reachable only by calling
  `SetMessageDescriptor` directly. `TestTheBakeStampsAParentAndItsChildTogetherOrNeither` drives
  that over a clean repository and a poisoned one and says, in its failure message, that the
  query-failure mode is armed if it ever goes red. The booking now says the route is closed and
  what closes it, instead of calling it open with no search behind the word.
...
- **The unreachability pin was vacuous, and its own mutation proved it** (Torvalds NAK): r35
  ...
  at all. Each arm measured red by a mutation aimed at it alone.
