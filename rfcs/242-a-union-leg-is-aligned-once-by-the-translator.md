...
- **A positional pointer into a growing table** (Torvalds): the head comment still said the
  ...
  Five have been appended since, and the row that actually kills the target-based title is now
  fifth from the end. Positions are not stable in a table that has grown every round, so the
...
- **Two more numbers in the comment being edited** (Torvalds NAK): it opened by saying THREE
  ...
  been complained about. The comment now quotes no count and no position at all, and says so,
  because this table has grown in every round of this span.
