...
- **The two counts the message opens with were asserted by nothing.** `"%d arms ran, %d pinned"` is
  the evidence for every claim made about this gate — the whole "73 against 72" argument is those
...
- **A duplicate label with nothing missing was caught by arithmetic that nothing drove, and could
  ...
  one duplicate is 73 against 72 and it did return an error. What it could not do is SAY anything:
