...
- **F2, a population no revision ever had.** The suite said "measured on this file at 18 arms".
  18 was an uncommitted intermediate: r60 has 6 arms and no `bazelrc` string at all, and the
  committed head had 24. A stated population that matches nothing is worse than none, because it
...
- **F5, the exit copy disagreed with the periodic one.** It still did `mv -f` into its
  ...
  same merge shape now, and its staging cleanup has an arm, which it did not: the periodic path's
  cleanup was covered and was vouching for a sibling that was not. Dropping the exit cleanup
  reddens that arm alone, at 27.
...
- **A trace copy that FAILED published a directory anyway** (codex P2, `nightly-rowdiff.yml:234`):
  ...
  once it succeeds. Reverting the fix reddens the first of those and only it, at 27 arms.
...
- **The tag guard was covered for one of its two steps.** Deleting the forensics copy reddens
  ...
  Re-measured at 27 arms, each deletion reddens its own step's arm and no other.
...
- **The staged promote introduced the failure it was closing, one step along.** As first written
  ...
  surviving the next publish. Restoring `rm -rf` + `mv` reddens that arm and only it, at 27 arms.
