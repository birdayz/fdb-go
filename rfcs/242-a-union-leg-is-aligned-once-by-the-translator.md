...
- **One list was serving two claims** (Graefe NAK): r32 enumerated the homes of the CONTROL claim
  ...
  `plan_finalize.go` and `values.go` — with `values/proto_type_test.go` stating the walk-order
  half without a number, so it cannot go stale with the count.
