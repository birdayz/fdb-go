...
- **One declared name over two shapes fails loudly** (Torvalds; Graefe measured the same and
  ...
  stays what it was: a type with no message form keeps its map, and the synthesised file
  failing to validate for another reason (a join row carrying one field name twice, which a
  nested FULL OUTER JOIN produces and answers today) is not turned loud here — making every
  compile failure a query failure broke that working query, measured
  (`TestNestedFullOuter_AncestorNullExtensionReachesLeg`). That row is booked: `TODO.md`, "A join row
  that carries one field name twice is never stamped, and flows as a map". Pinned over FDB
...
- **The swallow arm says what it covers** (Torvalds): its comment named only "a type with no
  ...
  call. Both sites say so now.
- **The r19 booking is WITHDRAWN, not corrected** (measured while writing the pin Torvalds asked
  for): r19 booked "a join row that carries one field name twice is never stamped, and flows as
  a map", attributing a `descriptor "__type__2.ID" already declared` seen once under r19's first
  cut to a nested FULL OUTER JOIN's null-extension row. The premise is false — a
  `RecordConstructorValue` disambiguates a repeated field name in `Type()` itself, so two `ID`
  fields become `ID` and `ID_2` and the descriptor validates — and the finding does not
  reproduce: a probe in the swallow arm over the whole `core/embedded` suite reports it entering
  ZERO times, under the new prefix, under the old one, and with the guard removed. The entry is
  removed and the pin that would have held it is not committed; what the r19 first cut tripped
  over is unidentified, and the honest record is that no shape is known to reach that arm.
  Graefe's correction to the entry's Java clause (`normalizeFields` disambiguates INDEXES, not
  names; two `ID`s throw in `computeFieldNameFieldMap`) is right and is why that row could never
  have been Java-legal either.
