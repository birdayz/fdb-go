...
- **One list was serving two claims** (Graefe NAK): r32 enumerated the homes of the CONTROL claim
  ...
  changes what a file claims.) Two further files state the walk-order half WITHOUT a number —
  `values/proto_type_test.go` and `record_constructor_message.go` — so they cannot go stale with
  the count; r33 named one of them and r34 adds the one r33 itself created.
