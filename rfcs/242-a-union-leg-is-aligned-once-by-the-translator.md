...
- **A trace copy that FAILED published a directory anyway** (codex P2, `nightly-rowdiff.yml:234`):
  ...
  once it succeeds. Reverting the fix reddens the first of those and only it, at 26 arms.
...
- **The dynamic-carrier test could not see the defect it named** (codex P2,
  ...
  reddens that test alone of the four, reporting `published correlation q$4, want the
  quantifier's own q$3`.
...
- **The watcher's tag guard was uncovered while the forensics one was pinned.** Deleting the
  forensics copy reddens exactly one arm; deleting the WATCHER copy reddened none, because the
  extraction named one step. It takes the step now and both are driven, so the r60 fold's "the
  tag guard is covered" was true of one of the two copies. Re-measured at 26 arms, each deletion
  reddens its own step's arm and no other.
...
- **The staged promote introduced the failure it was closing, one step along.** As first written
  it did `rm -rf` on the published directory and then `mv`, leaving an instant where NEITHER
  exists — and the stop step signals that loop's process group at an arbitrary point, so landing
  in that instant deletes a good snapshot and leaves the dump reporting no directory at all. The
  publish merges into the directory instead. The window is a timing race and no arm observes it;
  what is driven is the behaviour that separates the two shapes, a rotated-away trace file
  surviving the next publish. Restoring `rm -rf` + `mv` reddens that arm and only it, at 26 arms.
  The not-covered list says the race is closed by construction rather than pinned, because it is.
