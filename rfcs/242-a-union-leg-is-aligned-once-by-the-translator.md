...
- **The r19 booking is RESTORED, and r20's withdrawal is retracted** (Graefe and codex each
  ...
  (`TestFinalizePlanLeavesTheDuplicateNameJoinRowAMap`, as r22 re-cut it: the planned FULL OUTER
...
- **The absolute was inverted, not removed, and three copies said so** (Graefe and Torvalds,
  ...
  fourth, still calling the pin the tautology r22 re-cut. All four now state three-of-four with
  the survivor and the order dependence, found by grepping for the claim rather than by
  remembering where it was written.
- **The data loss was never there** (codex P2, measured over FDB): every home said the unstamped
  ...
  reads those slots by ORDINAL, so `SELECT a.id, c.id, d.foo` over the pinned shape returns
  `(1, 1, true)`: both `ID`s arrive. The map branch of `RecordConstructorValue.Evaluate` is not
  even entered for those constructors (probed: the only entries during that query are the
  INSERT path's). What the poisoning costs is descriptor IDENTITY — such a row cannot be handed
  out as an `api.Struct` — so the booking is LATENT, not a wrong answer, and its title says
  "leaves its plan's rows unstamped" now. The negative is pinned
  (`TestFDB_ADuplicateNameJoinRowStillReturnsEveryField`).
