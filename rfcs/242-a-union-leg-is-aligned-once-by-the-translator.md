...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   this round; and the `OrdinalLayoutDynamicCarrier` early return, which hands back
   `RequireFlowedObjectValue()` itself and satisfies the invariant trivially.
   `TestResultBaseForAnErasedRecordTakesTheCarrierlessExit` drives the third now, beside a
   concrete-record control that keeps the "no properties" assertion from being vacuous. A
   further route would have to break the invariant to break the argument, which is the point of
   stating one. So the closure is:
