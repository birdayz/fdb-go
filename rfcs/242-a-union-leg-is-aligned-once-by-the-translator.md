...
- **Two more numbers in the comment being edited** (Torvalds NAK): it opened by saying THREE
  ...
  rows and positions, which the row prose still broke nine times; r48 bounds it to the comment.)
...
- **The no-counts claim counted** (Graefe as his one condition, Torvalds NAK, codex P3): r46's
  ...
  by position nine times, including in the very string r47 edited, so even the narrowed claim was
  wider than the file. Three attempts to state one paragraph's scope, each wider than the text. And
...
- **The third attempt to state one paragraph's scope, and the third that was too wide** (Graefe
  ...
  positions, which the ROW PROSE breaks nine times — and one of them sits fourteen words after a
