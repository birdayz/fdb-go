...
- **The census message rendered every section unconditionally, so its assertions were vacuous.**
  ...
  the sections that must be ABSENT as well as present, and the swap reddens four of them.
- **A duplicate label with nothing missing was caught by arithmetic that nothing drove.** The only
  guard was `len(got) == len(expected)`, and no case exercised it, so deleting the clause changed
  nothing — while a copy-pasted case reusing a label would have reported green at 73 arms. The
  arithmetic is gone rather than tested: those two lengths count different things (lines against
  distinct entries), which is what made the comment "derives the count from them, so the two can
  never disagree" false. A repeated label is its own state with its own message, driven by its own
  case, and deleting that section reddens two.
...
- **And the population check counted rather than identified.** Exact equality on the count still
  ...
  LABELS now and derives the count from them, so the two cannot disagree; renaming a single arm
  reddens it with an exact MISSING/UNEXPECTED diff at an unchanged 72-to-72 count.
