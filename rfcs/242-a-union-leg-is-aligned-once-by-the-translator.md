...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   the provided layout's carrier — a substitution that branch guards with its own `Equals`. So
   the argument is closed rather than suggestive, with that one hop named rather than glossed:
