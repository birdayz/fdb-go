...
- **And the write-up of the previous round's measurement named the wrong mutation.** It said an
  ...
  came from pointing `md5sum` at a directory, which changes both readings and is a different
...
- **And the probe guard pinned the wrong property.** It asserted `[ -f fdb-watch.log ]`, which
  ...
  at a directory) that changes both readings and is not the shape the sentence describes.
