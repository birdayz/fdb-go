...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   merely analogous to the gate that later refuses the plan, it is the SAME PREDICATE ON THE
   SAME ACCESSOR: `plannerFuzzSameResultType` compares `GetFlowedObjectType()` with `Equals`,
   and `newPlanExprBaseForFirstQuantifier` refuses the union when `GetFlowedObjectType()` fails
   `Equals`. So the argument is closed rather than suggestive — the predicate holds before the
   rule, fails after it, and only the rule ran in between. So the second alignment is
...
- **The thing this RFC's own investigation was looking for, found by a gate reading `infra/`**
  ...
  budget is enforced by a test and this left 13 bytes of it. The STOP is withdrawn, and what
  remains is a deployment rather than a diagnosis.
