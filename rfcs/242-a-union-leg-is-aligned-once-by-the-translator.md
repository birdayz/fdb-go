...
- **The population was three and is four — and r31 made it five** (Torvalds NAK; Graefe the same,
  ...
  those five plus `plan_finalize.go`, which names the census pin in a comment without describing
  the control and so carries no claim to keep in step. (`values/proto_type_test.go` pins the
  mechanism under its own name and that grep does not reach it, which is the point of printing the
  command beside the list rather than a number: the list is only as wide as the pattern.)
