...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   Today they are the provided-layout branch (guarded by its own `Equals`), the identity layout
   (`NewOrdinalLayoutForCarrierType`, identical by construction), and the erased-record
   fallback, whose identity comes from `SnapshotExactType` and which had no test at all until
   this round; `TestResultBaseForAnErasedRecordTakesTheCarrierlessExit` drives it now, beside a
   concrete-record control that keeps the "no properties" assertion from being vacuous. A
   fourth route would have to break the invariant to break the argument. So the closure is:
