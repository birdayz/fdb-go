...
- **Two more numbers in the comment being edited** (Torvalds NAK): it opened by saying THREE
  ...
  been complained about. The comment now counts none of the table's rows and points into it by no
  position, and says why, because this table has grown in every round of this span. (r46 first
  wrote that as "no count of anything", which its own next sentence broke by counting rounds;
  scoped at r47, and the round count dropped rather than incremented.)
...
- **The no-counts claim counted** (Graefe as his one condition, Torvalds NAK, codex P3): r46's
  ...
  kind: it tells a later reader nothing needs updating while `FOUR` waits to go stale. Scoped now
  to the table's rows and to positional pointers, which is what was actually load-bearing. And
