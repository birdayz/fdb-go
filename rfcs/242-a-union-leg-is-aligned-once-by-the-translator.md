...
- **A superseded sentence survived in the homes the r28 delta did not edit** (Torvalds required
  ...
  standing rule asks: a grep of every superseded phrasing across the three homes returning zero,
  reported beside a positive control proving the greps were well-formed.
...
- **The set varies two factors; each pair isolates one** (Torvalds required fold; Graefe and codex
  ...
  fixed in turn shows one of them has no effect. Every home now says that, and names the two
  pairwise comparisons instead of asserting a one-variable design.
- **Two named lookups are not the whole map** (codex P2; Graefe raised it as optional, and noted
  ...
  which is neither the asserted two-field map nor the witness's result. Both computed reads now
  assert the map's exact size alongside its values, so the two arms stay symmetric and a
  shape regression in either cannot pass as a content match.
