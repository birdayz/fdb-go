...
1. **Fuzz corpus.** `testdata/fuzz/FuzzPlanner_WithBatchA_NoPanic/4a35dedaab03e663` (input
   ...
   that opened this RFC — is the sole failure of the engine fuzz nightly's most recent run
   before this was written: run 33955019772, 2026-09-05, "SQL Engine + Record Layer Fuzz",
   one failure across the 44 target/function pairs that rotation executed. Naming the run is
   the point: "the nightly is red on X" is a claim about a tree that moves nightly, and this
   one stops being true the moment this branch merges.
   ...
   a carrier for the flowed value by either of two routes — the provided-layout branch, which
   guards the substitution with its own `Equals`, and `newPlanExprBaseForType`'s identity
   layout, which is type-identical by construction. So the argument is closed rather than
   suggestive, with both hops named rather than glossed: the predicate holds before the rule,
   fails after it, and only the rule ran in between. So the second alignment is
...
- **A byte count that went stale inside the round that wrote it** (Graefe): `infra/README.md`
  ...
  point at `//infra:infra_test`, which prints the figure on every run, and say what does not
...
- **The thing this RFC's own investigation was looking for, found by a gate reading `infra/`**
  ...
  2026-08-01, the day the pool was provisioned. Fixed at the source with a job-in-flight guard,
  all three arms driven; the reasoning is in `infra/README.md` because the 32 KiB `user_data`
  budget is enforced by a test and the guard left a line or so of it — `//infra:infra_test`
  prints the exact figure and is where to read it, since it moved twice inside this round and
