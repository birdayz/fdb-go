...
- **Step 0:** DIVERGENCES.md §33-41/§376 no longer describe `ImplementIndexScanRule` as live, cite no
  non-existent test, invent no Java class, list two producers. Criterion is by PROPERTY over a defined
  set, not a bare name-grep. Partition every comment naming the rule into three buckets: (a) **accurate
  "retired" references** (e.g. `phase2_fuzz_test.go:12`) — correct, STAY untouched; (b) **standalone
  stale-as-live** comments (describe it as a current producer, not co-located with a later step's
  function) — Step 0 clears ALL of these; (c) **deferred stale-as-live** comments co-located with a
  function a later step edits — explicitly EXEMPT from Step 0, each swept in that step's PR. Step 0's
  pass condition: bucket (b) is empty. (b) and (c) together are the only stale-as-live wording; (a) is
  never touched. (Codex catches, folded: a bare `grep "ImplementIndexScanRule"` would delete correct
  history (a); and "no comment describes it as live" without the (c) exemption contradicts the
  deferral.)
