...
cases=[
 ...
 ('union-cardinality',(E/'logical_predicate.go').resolve(),'if matches[bareName] > 1 {','if matches[bareName] > 2 {'),
...
for name,path,old,new in cases:
 original=path.read_bytes(); text=original.decode(); assert text.count(old)==1,(name,text.count(old)); changed=text.replace(old,new); log=R/('alias-resolved-mutant3-'+name+'.log'); assert not log.exists()
 try:
  backup=R/('alias-resolved-original-'+name+'.bin'); backup.write_bytes(original)
  (R/'alias-resolved-mutation-active.json').write_text(json.dumps(dict(name=name,file=str(path),backup=str(backup),original_sha256=hashlib.sha256(original).hexdigest())))
  ...
  (R/'alias-resolved-mutation-active.json').unlink(missing_ok=True)
(R/'alias-resolved-mutations.json').write_text(json.dumps(results,indent=2)+'\n')
