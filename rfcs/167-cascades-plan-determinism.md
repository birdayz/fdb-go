...
2. **The `planHash` tie-break can't see the buried index.** Criterion #17 (`deepHashCode`, `planning_cost_model.go:841-857`) descends via `firstPhysicalChild` (first-member), and a shell's `HashCodeWithoutChildren` **omits the buried index name** — so the near-total order **collapses to an identical value** across idx_a/idx_b/idx_c and returns `0` (tie). The leaf `RecordQueryIndexPlan.HashCodeWithoutChildren` *does* include the index name (`index_scan.go:171-174`), but the shell hides it.
...
- Make the shell's structural hash **inner-aware** (walk `innerQuant` to the single pruned concrete member and hash *that*), so criterion #17 distinguishes idx_a/idx_b/idx_c **structurally — no cost-in-hash, no recursion into the comparator.** (Critique-driven: the synthesized "cost-aware `deepHashCode`" was rejected as circular and Java-unfaithful.)
- Generalize the Fetch-only guard to **all** nil-inner shell types at **every** selection site, and unify the two `OptimizeGroup` implementations (the legacy `planner.go` path is **dead** — `unified_tasks.go` schedules only `OptimizeGroupTask`; delete the legacy one rather than maintain parity).
...
**Phase 1 — Inner-aware shell hash + general guard + delete the dead path.** *This is the phase that actually fixes the headline query* (the completeness critic showed the guard alone is a no-op for all-shell groups).
- Make each shell wrapper's `HashCodeWithoutChildren` incorporate the buried inner's identity by walking `innerQuant` to its single pruned concrete member and folding that member's `concretePlanHash` (which already includes index name, reverse, covering, strictlySorted). Pure structural; no comparator re-entry.
...
- **Delete** the legacy `planner.go` `OptimizeGroup`/`OptimizeReferenceTask` path (dead — `unified_tasks.go` is the sole live selection site). One selection site = one place to keep deterministic.
