...
#### SLICE 2d LANDED (2026-07-09, Graefe design-ACK) — the 3+-link chain ORDINALIZES; the booked placement rework was UNNECESSARY. The deeper-nesting slice reduced from a `pushBuriedUnnestPredicateDown`/`rewriteUnnestPredicate` rework to a **~18-line clusterArity-gate lift**: `chainedBaseOrdinalizes(firstBase)` recurses the unnest-right spine — admit a SINGLE source (`clusterArity==1`) OR a CHAINED base whose OWN base ordinalizes — replacing the flat `clusterArity(firstBase) != 1` decline in `translateChainedUnnestOrdinal`. **A revertible de-risk SPIKE collapsed the booked scope: the TODO's "strand at `DEEP not resolvable, ordinal -1, row [ID SARR SCARR SUB X Y]`" was PRE-2c.** The 2c positional bake (`rebaseChainedOuterLegPredicate` baking outer refs over `ordinalLegType(join.Left)`) ALREADY lands a mixed-inner-ref clause as a `PredicatesFilter` at the INNERMOST Explode at depth-3 (`T4.ID=Z`→{T4.ID:11 Z:11}, `T4.ID=1 OR Z=20`→{11,12,13,20} — no strand); the seed machinery (`ordinalLegColumns`/`unnestOrdinalSeed`/`unnestBakedRootCollection`) was ALREADY recursion-ready — the `ordinalLegColumns` "DEPTH-2 ONLY" note was a gate CONSEQUENCE, not a structural limit (confirmed at 4-LINK: clean+outer-filter ordinalize, 4 nested FlatMaps; comment corrected to "ARBITRARY DEPTH"). So NO placement rework, NO `pushBuriedUnnestPredicateDown` change. Zero regressions: full sqldriver suite (dual-window oracle + cross-engine differential) + query pkg green under the spike AND the real impl. **Graefe design-ACK conditions, all met:** (Q1) scope to single-source∨chained EXACTLY — a MULTI-source BOX base (`FROM t, u, t.arr AS x, x.sub AS y, y.d AS z`, whose SARR link's base is the box `t⋈u`) stays DECLINED (c5b buried-box territory, the one place innermost-Explode placement is unproven), pinned by cert `boxbase_straddle_declines_namemodel` (NestedLoopJoin base kept, name-key rebase, straddle correct, NO strand); (Q2) recurse `clusterArity` structure ONLY — `unnestExistsSeedSafe(firstBase)` stays the SINGLE top-level guard because the existential arm is unreachable (EXISTS-over-chained is 0AF00 upstream — PROBED); (Q3) HONEST accounting — this NARROWS, not zeros, the chained name-model producer: the box-base chain + any `!ok` positional decline still reach `NewAnchoredJoinRecord`, so the full chained-zero is gated on c5b; `:1161` narrows (the chained clean-lift no longer needs the enclosure clear, but box/cluster still do — cluster_gate.go:315,356, w4b:565 — so it dies only at the atomic cap). **Certs:** the ordinal cert's "DECLINE: 3-link chain fails open to name-model" subtest FLIPPED → "3-link chain ORDINALIZES" (3-Explode nesting + `Scan(T4,[=])` outer-filter SARG) + a new "3-link outer filter SARGs the scan (positional bake ⇒ ordinal, not name-model)" subtest; the 2c `TestFDB_RFC173S4_ThreeLinkFilteredNameModel` cert RENAMED → `TestFDB_RFC173S4_ThreeLinkFilteredOrdinalizes` (all filtered 3-link shapes ORDINALIZE — innermost-Explode PredicatesFilter for the mixed straddle/OR, scan SARG for the lone outer conjunct — + the relocated box-base name-model regression guard = the codex/@claude 2c dimension, now on the still-name-model shape). **DISCRIMINATOR (rows + operator structure are identical for both models):** the outer conjunct SARGs `Scan(T4,[=])` in the ordinal path, which the name model CANNOT produce (it carries the outer ref by NAME in the AnchoredJoin record, out of the scan's reach). **ORTHOGONAL PRE-EXISTING gap discovered + booked (NOT folded — reproduces name-model, thrown at semantic resolution UPSTREAM of translation):** a WHERE ref to a DEEP chained alias 42703s — a 3-link AS-alias `Z` in WHERE resolves, but a 4+-link AS-alias `v`, a 3-link AT `o`, and a mid AT `yo` all `42703 column does not exist`. So filtered-deeper-than-3-link and any AT-in-WHERE aren't expressible in EITHER engine; the clean projections resolve at every depth (incl. AT). Reviews: Graefe design-ACK (scope-collapse a legitimate finding, not wishful — 4-link + oracles verified; "verify, don't assume the TODO" caught the pre-2c strand before building the rework). 4-gate on the impl HEAD.
