...
#### SLICE 2a LANDED (2026-07-08, Graefe design-ACK) — the NAME-AMBIGUOUS CROSS-LEG BARE-TWIN gathers via the positional wrap. `FROM A, B, A.arr AS x` where two SEPARATE legs share a bare column name (`k`) previously DECLINED the gathered ordinal path at `rfc173_w5_unnest_gather.go:126-135` (dup-name → name-model). It now GATHERS: the existing `wrapGatheredForGroupBy` positional named-projection (renamed `wrapGatheredPositionalKeys`; `gatheredGroupByWrapEligible`→`gatheredPositionalWrapEligible`) — already an aggregate-free `LogicalProjectionExpression` that `emitLeafKeys` binds each leg's columns to its `[Start,Width)` window as `ALIAS.COL` keys — is now applied to the NON-grouped return too, so a qualified `A.k`/`B.k` routes to its own leg (not last-leg-wins). The `:126-135` decline became `needsWrap`; `wantWrap = underAggregate || needsWrap`; a wrap-eligible dup wraps (grouped AND non-grouped), else raw seed (non-ambiguous byte-identical) or a name-model decline. THE DIMENSIONAL TRAP DISSOLVED: a BARE ambiguous reference (`SELECT k`) errors 42702 at semantic analysis BEFORE the translator (Java-matching standard SQL), so there is no first-match-vs-last-leg-wins user-facing divergence — only QUALIFIED reads reach the wrap. SCOPE (conservative, matching the de-risked shape): only CROSS-LEG dups (two separate legs) wrap; a WITHIN-ONE-LEG buried dup (a merge-opaque box's concat carrying two same-named columns, e.g. Order.PRICE + Customer.PRICE in a `(A FULL B)` box) stays name-model — its positional disambiguation is a later increment (2a'). Certificate `sqldriver/rfc173_s4_baretwin_gather_fdb_test.go`: qualified bare-twin resolves by window (wrap fires, red-first proven — reverting to decline drops the nested-Project plan); bare element X survives the wrap; WHERE on a qualified bare-twin resolves (the gathered path is NOT chainedUnnestUnderFilter-suppressed); grouped bare-twin COUNT; bare-ambiguous 42702; non-ambiguous byte-identical raw seed; within-box buried dup declines to name-model. Full suite 55/55. Narrows `:1151`'s bare-twin residency for the cross-leg case; the within-box dup + outer-column-predicate rebase (2b) + Q5 census remain. BOOKED non-blocking follow-ups for Slice 2 (both optimization/test-fidelity, not correctness): (a) clear `chainedUnnestUnderFilter` in `translateSubqueryRef` (next to the `inInnerCluster` clear) so a FILTER-LESS subquery under an outer filter keeps its ordinalization instead of being coarsely suppressed — fresh-scope discipline, correct either way; (b) a name-model-vs-ordinal plan-shape discriminator on the element-WHERE decline test (today the outer-column-WHERE malformed-without-suppress is the mechanism sentinel; the element-WHERE test guards rows only, since name-model and ordinal share the nested-FlatMap shape).
