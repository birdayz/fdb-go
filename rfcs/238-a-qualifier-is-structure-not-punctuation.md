...
`logical_predicate.go:7112` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6904`) resolved a DML target
...
(`logical_predicate.go:7003`) that folded case purely to find a descriptor, so
