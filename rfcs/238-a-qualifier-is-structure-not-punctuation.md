...
`logical_predicate.go:6964` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6756`) resolved a DML target
...
(`logical_predicate.go:6855`) that folded case purely to find a descriptor, so
