...
`logical_predicate.go:7118` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6910`) resolved a DML target
...
(`logical_predicate.go:7009`) that folded case purely to find a descriptor, so
