...
`logical_predicate.go:6854` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6646`) resolved a DML target
...
(`logical_predicate.go:6745`) that folded case purely to find a descriptor, so
