...
`logical_predicate.go:7062` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6854`) resolved a DML target
...
(`logical_predicate.go:6953`) that folded case purely to find a descriptor, so
