...
`logical_predicate.go:7020` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6812`) resolved a DML target
...
(`logical_predicate.go:6911`) that folded case purely to find a descriptor, so
