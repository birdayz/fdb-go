...
`logical_predicate.go:7113` where `buildSelectScope` takes the bare table name
...
`recordTypeCI` (`logical_predicate.go:6905`) resolved a DML target
...
(`logical_predicate.go:7004`) that folded case purely to find a descriptor, so
